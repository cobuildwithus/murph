...
export function buildJunctionClientUserId(secret, memberId, namespace = "") {
  ...
  const prefix = junctionClientUserIdPrefix(normalizedNamespace);
  return `${prefix}${base32(digest)}`.slice(0, 32);
}

export function inspectNamespacedJunctionUsers(raw, { expectedNamespace, expectedTeamId }) {
  assertUuid(expectedTeamId, "shared Junction team id");
  const normalizedNamespace = normalizeJunctionClientUserIdNamespace(expectedNamespace);
  if (!normalizedNamespace) {
    throw new Error("Junction E2E cleanup requires a non-empty client user namespace.");
  }
  const prefix = junctionClientUserIdPrefix(normalizedNamespace);
  assertRecord(raw, "shared Junction team user list");
  if (!Array.isArray(raw.users)
      ...
      || raw.limit < 1
      || raw.limit > JUNCTION_USER_PAGE_LIMIT) {
    throw new Error("Shared Junction team user list was invalid.");
  }
  if (raw.total !== raw.users.length) {
    throw new Error("Shared Junction team user enumeration was incomplete.");
  }

  const owned = [];
  for (const user of raw.users) {
    assertRecord(user, "shared Junction team user");
    const userTeamId = requiredString(user.team_id, "Junction user team id");
    assertUuid(userTeamId, "Junction user team id");
    ...
    const clientUserId = requiredString(user.client_user_id, "Junction client user id");
    if (clientUserId.startsWith(prefix)) {
      owned.push({
        clientUserId,
        userId: requiredString(user.user_id, "Junction E2E user id"),
      });
    }
  }
  if (owned.length > 1) {
    throw new Error("Shared Junction team contains more than one user in the E2E namespace; refusing cleanup.");
  }
  return owned[0] ?? null;
}
...
export async function cleanupE2e(junctionClientUserIdNamespace) {
  ...
  if (await listNamespacedJunctionUser(config)) {
    throw new Error("Junction E2E namespace is not empty after cleanup.");
  }
...
async function listNamespacedJunctionUser(config) {
  const url = new URL(`${junctionBaseUrl(config.junctionApiKey)}/v2/user/`);
  url.searchParams.set("offset", "0");
  url.searchParams.set("limit", String(JUNCTION_USER_PAGE_LIMIT));
  return inspectNamespacedJunctionUsers(await fetchJson(
    url,
    { headers: junctionHeaders(config.junctionApiKey) },
    "shared Junction team user enumeration",
  ), {
    expectedNamespace: config.junctionClientUserIdNamespace,
...
async function resolveJunctionUser(memberId, config) {
  const clientUserId = buildJunctionClientUserId(
    config.junctionClientUserIdSecret,
    memberId,
    config.junctionClientUserIdNamespace,
  );
...
export function normalizeJunctionClientUserIdNamespace(namespace) {
  ...
  if (!/^[a-z][a-z0-9]{0,7}$/u.test(normalizedNamespace)) {
    throw new Error(
      "Junction E2E client user namespace must be 1-8 lowercase letters or digits and start with a letter.",
    );
  }
  return normalizedNamespace;
}

function junctionClientUserIdPrefix(namespace) {
  const normalizedNamespace = normalizeJunctionClientUserIdNamespace(namespace);
  return normalizedNamespace ? `murph_${normalizedNamespace}_` : "murph_";
}
