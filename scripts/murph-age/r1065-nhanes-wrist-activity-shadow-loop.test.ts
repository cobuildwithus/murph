...
import {
  R1065_NHANES_WRIST_ACTIVITY_SHADOW_LOOP_SCHEMA_VERSION,
  runR1065NhanesWristActivityShadowLoop,
} from "./r1065-nhanes-wrist-activity-shadow-loop.ts";

describe("R1065 NHANES wrist activity shadow loop", () => {
  it("runs aggregate-only model comparisons from the private wrist cache", async () => {
    const tmp = await mkdtemp(path.join(os.tmpdir(), "murph-age-r1065-"));
    try {
      ...
      const { output, outputPath, r1034ReceiptPath } = await runR1065NhanesWristActivityShadowLoop({
      ...
      expect(output.schemaVersion).toBe(R1065_NHANES_WRIST_ACTIVITY_SHADOW_LOOP_SCHEMA_VERSION);
      ...
      expect(output.candidateRuns.length).toBe(9);
      expect(output.r1034CompatibleReceipt.candidateMetrics.length).toBe(8);
