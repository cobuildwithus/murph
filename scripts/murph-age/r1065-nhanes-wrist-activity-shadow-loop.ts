...
export const R1065_NHANES_WRIST_ACTIVITY_SHADOW_LOOP_SCHEMA_VERSION =
  "murph-age-r1065-nhanes-wrist-activity-shadow-loop.v1" as const;
...
const OUTPUT_FILE_NAME = "r1065-nhanes-wrist-activity-shadow-loop.latest.json";
const R1034_RECEIPT_FILE_NAME = "r1065-r1034-compatible-aggregate-receipt.latest.json";
...
export interface R1065NhanesWristActivityShadowLoopOptions {
...
export interface R1065NhanesWristActivityShadowLoopOutput {
  artifactBoundary: {
    aggregateOnly: true;
  ...
  schemaVersion: typeof R1065_NHANES_WRIST_ACTIVITY_SHADOW_LOOP_SCHEMA_VERSION;
  ...
  summary: {
    ...
    conclusion:
      | "wrist_activity_signal_ready_for_r1034_review"
      | "wrist_activity_signal_shadow_hold_for_calibration_or_external_validation";
    productDisplayAuthorized: false;
    rowValuesStored: false;
...
const CANDIDATES: Record<string, CandidateDefinition> = {
  ...
  C3_age_sex_wrist_activity: {
    comparatorId: "R0_age_sex_reference",
    featureKeys: ["age", "male", ...ACTIVITY_FEATURE_KEYS],
    role: "score_bearing_research_candidate",
  },
  C4_lab9_bp_body_wrist_activity_primary: {
  ...
  N1_coverage_quality_only_negative_control: {
    comparatorId: "C2_lab9_bp_body_primary",
    featureKeys: [...LAB9_BP_BODY_FEATURE_KEYS, "valid_day_count", "mean_daily_valid_minutes"],
    role: "negative_control",
  },
  N2_shuffled_wrist_activity_negative_control: {
    comparatorId: "C2_lab9_bp_body_primary",
    featureKeys: [...LAB9_BP_BODY_FEATURE_KEYS, ...ACTIVITY_FEATURE_KEYS.map((key) => `shuffled_${key}`)],
    role: "negative_control",
  },
...
export async function runR1065NhanesWristActivityShadowLoop(
  options: R1065NhanesWristActivityShadowLoopOptions = {},
): Promise<{ output: R1065NhanesWristActivityShadowLoopOutput; outputPath: string; r1034ReceiptPath: string }> {
  ...
  const c4Summary = c4ActivitySummary(r1034CompatibleReceipt.candidateMetrics);
  const conclusion = c4Summary?.negativeControlsBeaten === true
    && c4Summary.brierDelta !== null
    && c4Summary.brierDelta < 0
    && c4Summary.logLossDelta !== null
    && c4Summary.logLossDelta < 0
    && c4Summary.calibrationSlope !== null
    && c4Summary.calibrationSlope >= 0.9
    && c4Summary.calibrationSlope <= 1.1
    && c4Summary.eOverO !== null
    && c4Summary.eOverO >= 0.95
    && c4Summary.eOverO <= 1.05
    ? "wrist_activity_signal_ready_for_r1034_review"
    : "wrist_activity_signal_shadow_hold_for_calibration_or_external_validation";

  const output: R1065NhanesWristActivityShadowLoopOutput = {
    ...
    schemaVersion: R1065_NHANES_WRIST_ACTIVITY_SHADOW_LOOP_SCHEMA_VERSION,
    splitShape: splitShape(rows),
    status: "research-local-aggregate-only",
    summary: {
      c4WristActivityVsLab9: c4Summary,
      conclusion,
      productDisplayAuthorized: false,
      rowValuesStored: false,
      usableAsConsumerWearableValidation: false,
...
export function assertR1065Safe(output: R1065NhanesWristActivityShadowLoopOutput): void {
  const findings = [
    ...findForbiddenAggregateEgress(output),
    ...findR1065SpecificFindings(output),
  ];
  if (findings.length > 0) {
    throw new Error(`R1065 NHANES wrist activity shadow loop failed safety validation: ${findings.join("; ")}`);
  }
}
...
async function requireLocalCache(filePath: string): Promise<void> {
  try {
    ...
    throw new Error("R1065 requires the local ignored NHANES wrist analytic cache before scoring.");
  }
}
...
async function readAnalyticRows(filePath: string): Promise<ParsedRow[]> {
  ...
  if (rows.length === 0) {
    throw new Error("R1065 local NHANES wrist analytic cache produced no eligible aggregate rows.");
  }
...
function attachShuffledActivityValues(rows: ParsedRow[]): void {
  ...
  for (const row of rows) {
    ...
    for (const key of ACTIVITY_FEATURE_KEYS) {
...
function r1034MetricForRun(
  ...
  if (!comparator) {
    throw new Error(`R1065 candidate ${run.candidateId} requires comparator ${run.comparatorId}.`);
  }
  return {
...
function c4ActivitySummary(
  metrics: readonly R1034AggregateCandidateMetric[],
): R1065NhanesWristActivityShadowLoopOutput["summary"]["c4WristActivityVsLab9"] {
  const metric = metrics.find((candidateMetric) => candidateMetric.candidateId === "C4_lab9_bp_body_wrist_activity_primary");
...
function splitShape(rows: readonly ParsedRow[]): R1065NhanesWristActivityShadowLoopOutput["splitShape"] {
  ...
  })) as R1065NhanesWristActivityShadowLoopOutput["splitShape"];
}
...
function findR1065SpecificFindings(output: R1065NhanesWristActivityShadowLoopOutput): string[] {
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  runR1065NhanesWristActivityShadowLoop({
    analyticCachePath: process.env.MURPH_AGE_NHANES_WRIST_ANALYTIC_CACHE_PATH,
    outputDir: process.env.MURPH_AGE_RESEARCH_OUTPUT_DIR,
  }).then(({ output }) => {
    process.stdout.write(`${JSON.stringify({
      c4WristActivityVsLab9: output.summary.c4WristActivityVsLab9,
