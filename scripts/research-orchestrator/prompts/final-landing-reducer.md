...
- Before producing the final diff, check each protocol's expected biomarker signals against the associated Health Commons research. Pick primary and secondary markers from the evidence, prefer objective downstream markers people can reasonably check with wearables, sensors, home devices, or standard labs, and keep self-referential adherence or exposure markers out of primary outcomes.
- For promoted biomarkers, make `estimatedChange` and the mechanism description concrete and evidence-shaped. Explain what the protocol changes in the body or behavior, and why that could plausibly move the biomarker. Keep descriptions short enough for the protocol-page card UI.
- Provide a best-effort estimated-change range in the clearest marker-specific unit when defensible, such as bpm, mmHg, minutes, %, kg, mg/dL, mmol/L, score points, or similar. Use `kind: mixed_or_contextual` only when a numeric range would mislead.
- Verify every signal with a numeric `estimatedChange` has an `expectedDirection` that matches the range (both positive → up, both negative → down, mixed → mixed_or_contextual, zero boundary → up_or_stable/down_or_stable). A mismatch is a blocker — the UI will render the wrong graph shape.
