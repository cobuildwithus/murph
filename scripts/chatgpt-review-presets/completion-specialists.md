...
- Coverage: `agent-docs/prompts/coverage-review.md`. Apply its composed-proof
  rule and report only missing proof that could conceal a broken changed
  outcome or hard invariant at the highest stable boundary. For database
  collection paths, apply `docs/contracts/00-invariants.md` § Database Load And
  Collection Fanout.
