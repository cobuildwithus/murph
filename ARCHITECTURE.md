...
- `VaultCliError` keeps arbitrary diagnostic context private and carries model-repair guidance only through its explicit bounded repair contract. One pure, lazily loadable operator-config projection serves both setup and main CLI bridges, preserving native Incur parse/validation errors while carrying an explicitly supplied domain code, retryability, safe operation stage, hint, and value-free field errors through Incur's final envelope. This shared transport seam does not inspect generic validation context or synthesize domain repair details: individual domain owners populate them incrementally. Unmapped exceptions are classified once through the shared projection: expected filesystem failures receive stable categories, ordinary messages are bounded and path-redacted, and provider- or credential-shaped messages receive a fixed fallback. Failures before `serve()` use that same lazy projection when the caller requested machine output.
