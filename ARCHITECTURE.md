...
The iOS companion is the only owner of photo-library observation and on-device meal classification. A member explicitly enables the feature, and the companion considers only photos created after that opt-in; the hosted system never receives or scans the rest of the library. Foreground enrollment uses the member's Privy identity token, while background uploads use a dedicated renewable bearer that grants only meal-photo upload and self-revocation. `apps/web` stores only hashes of that bearer and installation UUID plus an encrypted idempotency secret, validates a bounded metadata-free JPEG, and resolves the member's established Linq or Telegram home thread before staging bytes through the internal Cloudflare control client. Each upload attempt owns a distinct staged object. Before the metadata-only mailbox append commits, web locks the hosted member and any active sponsorship membership/group rows, then rechecks the same enrollment, active member access, launch consent, and unchanged private route. The first accepted mailbox item chooses the canonical object for exact duplicate attempts; losing or failed attempts delete only their own unclaimed object, while ambiguous commit cleanup first reconciles against the mailbox. Postgres, Temporal, and the hosted mailbox receive metadata and the narrow direct route only, never image bytes.

`apps/cloudflare` encrypts each staged JPEG into a private per-user R2 object. Object deletion derives the user-namespaced R2 path directly and does not require the user's encryption context to remain available. The metadata-only `meal-photo.captured` mailbox item wakes `packages/assistant-runtime`, which verifies the object's length and digest, imports one idempotent photo-only meal through `packages/core`, and uses the Web-authorized direct route plus the hosted route-validation profile to idempotently install one private managed `dailyLocal` closeout for 21:00 in the vault timezone. Vaults with no successful automatic capture do not receive that automation; replays and later captures reuse the stable record, while paused or archived member state remains authoritative. Legacy in-flight wakes without route proof remain importable during coordinated rollout but cannot install the closeout. Routed import completion stays retryable until the automation is durable, and staged-object deletion is scheduled only after the workspace checkpoint succeeds. The R2 lifecycle rule makes staged meal-photo objects eligible for asynchronous deletion at 31 days, one day beyond mailbox recovery retention; successful imports still delete staging immediately after the checkpoint, and 31 days is not a guaranteed physical-deletion deadline.

The closeout runs through the ordinary managed-automation scheduler, assistant prompt/tool stack, current private route, and outbox. Scheduled execution reauthorizes that direct audience through the Web-owned home-route state before provider work and again before tool, commit, or delivery boundaries; a stale, unknown, or group route fails closed. It enriches the existing canonical automatic meals, summarizes supported current-day calories/macros without double-counting, and sweeps a bounded recent window for delayed captures. After each photo is inspected and supported structure is read back, the canonical `meal remove-photo` mutation appends an attachment-free meal revision and atomically replaces the verified raw JPEG bytes with a small non-image privacy tombstone plus updated manifest integrity proof. Historical append-only revisions therefore remain valid without retaining the meal image. Neither the enrollment row nor R2 is canonical meal truth; the member's encrypted hosted workspace remains the canonical structured meal owner.
