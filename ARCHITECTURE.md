...
- `packages/assistant-evals`: workspace-private terminal evaluation tooling. It owns provider-neutral scenario/run contracts and bounded local artifacts plus synthetic onboarding episodes that call the public `@murphai/assistant-engine` service in isolated temporary vaults. It does not own onboarding policy, product state, a second assistant lifecycle, or a model-grader runtime, and no production package may depend on it.
