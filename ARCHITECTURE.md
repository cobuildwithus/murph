...
- Wearable provider timeseries should not be retained as full provider sample arrays by default. Product, assistant, and CLI wearable summaries consume compact summary observations, derived facts, or display-grade metric samples; any timeseries-derived product fact must come from an explicit importer/projector step that reduces provider samples in memory and persists only compact evidence. Junction's contracts-owned policy admits only bounded daily aggregates, compact UTC-hour features, sparse readings/events, and note tags; optional resources stay disabled until exact opt-in, grouped provider arrays never become canonical evidence, and the importer does not invent coverage metadata. Junction `note` keeps dated tag names as completed intervention events while dropping free text, `fall` remains a sparse alert fact, and `electrocardiogram_voltage` plus `workout_stream` remain outside the generic timeseries seam. Dense raw retention remains a legacy/debug cleanup lane for already-written high-volume timeseries roles.
