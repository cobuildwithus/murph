...
The iOS companion is the only owner of photo-library observation and on-device meal classification. A member explicitly enables the feature, and the companion considers only photos created after that opt-in; the hosted system never receives or scans the rest of the library. Foreground enrollment uses the member's Privy identity token, while background uploads use a dedicated renewable bearer that grants only meal-photo upload and self-revocation. `apps/web` stores only hashes of that bearer and installation UUID plus an encrypted idempotency secret, validates a bounded metadata-free JPEG, and requires an established Linq or Telegram home thread before staging bytes through the internal Cloudflare control client. Each upload attempt owns a distinct staged object. Before the metadata-only mailbox append commits, web locks the hosted member and any active sponsorship membership/group rows, then rechecks the same enrollment, active member access, launch consent, and the continued presence of a private route; changing between valid private routes does not invalidate the capture. The first accepted mailbox item chooses the canonical object for exact duplicate attempts; losing or failed attempts delete only their own unclaimed object, while ambiguous commit cleanup first reconciles against the mailbox. Postgres, Temporal, and the hosted mailbox receive metadata only, never image bytes or copied recipient state.

`apps/cloudflare` encrypts each staged JPEG into a private per-user R2 object. Object deletion derives the user-namespaced R2 path directly and does not require the user's encryption context to remain available. The metadata-only `meal-photo.captured` mailbox item wakes `packages/assistant-runtime`, which verifies the object's length and digest, imports one idempotent photo-only meal through `packages/core`, resolves the current direct route from the Web member-routing owner, and uses the hosted route-validation profile to idempotently install one private managed `dailyLocal` closeout for 21:00 in the vault timezone. Vaults with no successful automatic capture do not receive that automation; replays and later captures reuse the stable record, while paused or archived member state remains authoritative. Import completion stays retryable until the automation and a current private route are durable, and staged-object deletion is scheduled only after the workspace checkpoint succeeds. The R2 lifecycle rule makes staged meal-photo objects eligible for asynchronous deletion at 31 days, one day beyond mailbox recovery retention; successful imports still delete staging immediately after the checkpoint, and 31 days is not a guaranteed physical-deletion deadline.

The closeout runs through the ordinary managed-automation scheduler, assistant prompt/tool stack, current private route, and outbox. The saved automation route is only a scheduler locator: execution resolves the current direct audience from Web-owned member routing before provider work and again before tool, commit, or delivery boundaries. Automatic-meal closeout delivery over direct Telegram repeats that same check at provider entry, so route or access changes after outbox persistence make zero calls to the stale target. A typed stale-target rejection terminalizes that obsolete intent without creating a replyable terminal-failure input, while preserving this closeout's existing pending occurrence and backoff policy. If dispatch reconciliation cannot clear the pending delivery, canonical cron status derives a stable repair deadline from the existing pending intent id and its exact terminal outbox timestamp, counts that repair as due at the deadline, and reprojects it on later passes while that exact terminal evidence remains retained. Post-checkpoint delivery rereads that status so the first wake is armed immediately; a bounded one-pass safety candidate accompanies the reread when aggregate status is unavailable or does not yet surface the exact repair deadline. Durable later liveness remains status-derived, and the ordinary cron repair owner performs the mutation. The next ordinary cron pass resolves the current route and creates the sole fresh intent. Other direct automations retain their existing saved-route and failure-note semantics. An unknown or group route fails closed. The closeout enriches the existing canonical automatic meals, summarizes supported current-day calories/macros without double-counting, and sweeps a bounded recent window for delayed captures. After each photo is inspected and supported structure is read back, the canonical `meal remove-photo` mutation appends an attachment-free meal revision and atomically replaces the verified raw JPEG bytes with a small non-image privacy tombstone plus updated manifest integrity proof. Historical append-only revisions therefore remain valid without retaining the meal image. Neither the enrollment row nor R2 is canonical meal truth; the member's encrypted hosted workspace remains the canonical structured meal owner.
