...
- Snapshot transport direction is explicit: the upload-side statement above that the Worker never receives the snapshot body remains true for presigned `PUT`. Current restore calls the fixed internal `workspace-snapshots.worker` host; Container outbound dispatch invokes the Worker handler, the phase-ordered R2 binding returns the encrypted object, and the Worker forwards its `ReadableStream` without buffering or cloning. The route exposes bytes only after exact write-fence, ref/AAD/user/key, size, checksum, and custom-metadata validation. The runner preserves streaming cancellation/backpressure and one complete replay-safe restore retry. During the three-release compatibility window, current Workers retain `/presign-get` for old runners and current runners use that path only after an unversioned non-OK response from an old Worker.
...
The snapshot-body statement in item 12 is upload-directional: the Worker never
accepts a v2 snapshot upload body. Restore response bytes use the versioned
internal-host/R2-binding stream described in the hosted execution-state section;
they are not buffered, cloned, decrypted, decompressed, or logged by the Worker.
