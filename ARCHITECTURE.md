...
The ordinary broad `murph.group` contract keeps only action names and a compact
schema-discovery envelope in fixed thread metadata. `action="schema"` returns
the exact request contract on demand, and complex actions are submitted under
`action="execute"`. Existing prompt-known simple actions may remain direct and
still pass the same strict parser. The separately planned scheduled and detached
read-only group contracts remain narrow and direct; they never discover or
advertise unrelated group mutations.
