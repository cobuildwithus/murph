...
- `VaultCliError.context` is the sole recovery metadata source: arbitrary diagnostic context remains private, and only allowlisted structured `context.issues` fields may produce value-free field guidance. One pure, lazily loadable operator-config projection serves both setup and main CLI bridges; escaped raw Zod errors use the same issue mapper, while native Incur parse/validation errors bypass it. Fixed filesystem failures receive stable safe categories, and every other unowned exception receives one value-free `UNKNOWN` message instead of arbitrary error prose. Failures before `serve()` use that same lazy projection when the caller requested machine output.
