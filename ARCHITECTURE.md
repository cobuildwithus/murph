...
The canonical account transaction also persists a foreign-key-free,
KMS-encrypted external-cleanup receipt before deleting the hosted member.
Cloudflare runtime/R2, Stripe-customer, and Privy-user progress advances
independently and monotonically; the existing hosted retention sweep retries
unfinished targets until confirmed deletion or absence, then removes the
receipt. Missing cleanup configuration remains pending rather than being
