...
The closeout runs through the ordinary managed-automation scheduler, assistant prompt/tool stack, current private route, and outbox. The saved automation route is only a scheduler locator: execution resolves the current direct audience from Web-owned member routing before provider work and again before tool, commit, or delivery boundaries. Automatic-meal closeout delivery over direct Telegram repeats that same check at provider entry, so route or access changes after outbox persistence make zero calls to the stale target. A typed stale-target rejection terminalizes that obsolete intent but preserves this closeout's existing pending occurrence and backoff; the next ordinary cron pass resolves the current route and creates a fresh intent. Other direct automations retain their existing saved-route semantics. An unknown or group route fails closed. The closeout enriches the existing canonical automatic meals, summarizes supported current-day calories/macros without double-counting, and sweeps a bounded recent window for delayed captures. After each photo is inspected and supported structure is read back, the canonical `meal remove-photo` mutation appends an attachment-free meal revision and atomically replaces the verified raw JPEG bytes with a small non-image privacy tombstone plus updated manifest integrity proof. Historical append-only revisions therefore remain valid without retaining the meal image. Neither the enrollment row nor R2 is canonical meal truth; the member's encrypted hosted workspace remains the canonical structured meal owner.
