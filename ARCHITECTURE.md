...
and snapshot restore use only that environment-specific canonical bucket. The
former OC buckets remain bound solely as retiring account-deletion targets while
they contain migrated duplicate ciphertext; runtime reads, writes, restores,
and presigns cannot select them. Account deletion synchronously deletes and
proves stable emptiness across the canonical and retiring buckets before it can
delete Durable Object state or report completion. Deploy preflight requires the
canonical buckets to report ENAM Standard and the retiring buckets to report OC
Standard. The runtime has no source-region fallback, dual write, migration
phase, or storage-specific admission gate. The retiring binding is removable
only after an independent inventory proves the OC buckets stably empty.
