...
- Hosted overnight assistant maintenance remains orchestration, not a second mutation owner: after memory consolidation it may invoke only explicitly named deterministic dry-run/apply CLI repairs, while core owns eligibility, canonical locking, receipt-guarded mutation, validation, and audit. The first such repair is `vault repair-junction-evidence-duplicates`; affected historical rows move to the explicit filtered v2 ingest shape, and replay derives id and evidence proof in one authoritative full-shard scan when the bounded tail scan misses. There is no generic maintenance registry or model-authored JSONL cleanup path.
