...
Reconciliation always drains and checkpoints visible deterministic system-mailbox lag through the existing `system_mailbox` processing mode before evaluating engagement or AI-usage authorization for a preserved or newly scheduled model wake. That mode imports only the system lane and returns before assistant execution; the next ordinary reconciliation applies the existing gates. It adds no queue, scheduler, cursor, or durable state owner.
