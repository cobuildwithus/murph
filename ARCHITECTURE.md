...
- `VaultCliError.context` is the sole recovery metadata source: arbitrary diagnostic context remains private, and an issue may produce value-free field guidance only from a finite `publicPath` written by the domain owner; raw validator paths are never inferred. One pure, lazily loadable operator-config projection serves both setup and main CLI bridges. Escaped raw Zod errors receive a fixed fieldless validation failure, while native Incur parse/validation errors bypass this projection. Known errors may preserve only the fixed stages `validation`, `read`, `write`, `configuration`, `authorization`, `transport`, `response`, `filesystem`, `render`, `persistence`, `conflict`, and `integrity`; issue presence never creates or overwrites a stage. Fixed filesystem failures receive stable safe categories, and every other unowned exception receives one value-free `UNKNOWN` message with the synthetic `command` stage instead of arbitrary error prose. Failures before `serve()` use that same lazy projection when the caller requested machine output.
