...
the current tool call returns immediately. After the existing canonical capture
save and hosted upload complete, the runtime stages one trusted system input on
the original accepted conversation route. The existing runtime wake interrupts
the dirty idle window before its routine checkpoint, or the foreground input
pass loop folds the completion into the next assistant-input batch when a turn is
already active. This adds no durable image job, mailbox kind, scheduler, retry
owner, reservation, allowance, or image-specific usage lifecycle; unfinished
work may be lost with the runner invocation.
