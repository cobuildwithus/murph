...
Scheduled model turns reuse the interactive assistant's static core, stable capability router, bounded committed transcript context, canonical-data guidance, personality, and channel-style layers without resuming either the interactive provider thread or its process. Ordinary attended turns retain the existing warm Codex App Server and explicitly select `dangerFullAccess` on every turn start. Each notification or maintenance occurrence instead starts a separately owned one-shot App Server in a private per-turn temporary directory. That exact process is stopped and its temporary directory is removed after success, failure, or cancellation, while the attended process and session reference remain unchanged.

The scheduled launch and thread request are non-model-controlled. They supply `environments: []`, `selectedCapabilityRoots: []`, a sterile temporary cwd that prevents project instruction discovery, and explicit gates for apps, plugins, native effects, web, permissions, memory, and collaboration. Shared Codex-home instructions may still influence prompt reasoning, but they cannot add an execution environment or widen a typed task capability. Because Codex selects tool mode and multi-agent implementation in a process-wide model manager, the parent resolves the ordinary model catalog, clones it at launch, and forces every model to native `tool_mode: "direct"` and `multi_agent_version: "disabled"`. On the pinned Codex build, the resulting provider graph contains only the task-selected `murph` namespace plus the effect-free in-turn `update_plan` utility. Unadvertised native execution and collaboration calls are rejected before their handlers can run.

Scheduled authority is an ephemeral discriminated task binding derived by the cron parent from canonical automation identity. Required canonical reads, bundled skill reads, writes, and exact external fetches run only through task-specific typed tools that call the existing query and effect owners against the real vault root; they accept no command, path, credential, arbitrary URL, or lifecycle target. Every scheduled read or effect re-reads the canonical automation source at its owner boundary and rejects a changed, inactive, expired, or mismatched revision. Except for the separately exact-owned group newsletter, every new non-direct Linq automation must persist one immutable `scheduledTask` identity at creation, use a time-driven schedule, and bind the explicit current non-direct Linq route; both the task and route stay immutable. `group_notification` permits an ordinary group message and the normal bounded generic scheduled reads but no `group_shared` read. `group_health_update` adds a selector-free projection of all current consented server-approved health shares while excluding group email and opaque member, record, share, event, route, and provider identifiers. Either task may use fresh or preserved conversation continuity. `group_challenge` instead fixes one challenge page and shared-data projection and requires both preserved continuity and a finite lifecycle. The challenge task alone may generate up to four bounded prompt-only images for its established comic flow, or one voice memo or song, never image and audio media together; no media argument accepts a model-selectable reference, URL, path, id, or voice. Experiment lifecycle cards are prepared by the lifecycle owner and attached by delivery rather than selected by the model.

The canonical Linq route on a group task is a bounded hint, not destination authority. Web must prove the same current non-direct group before the model, immediately before each group-shared read, and immediately before delivery. A local runtime cannot execute any group task. Historical untyped Linq automations without explicit direct-audience evidence are ambiguous because their free-text instructions cannot distinguish ordinary messages, health updates, and challenges or recover an exact challenge page, projection, and end. A temporary pre-claim cutover therefore atomically pauses every active ambiguous record from exact canonical snapshots; it never interprets instructions or grants authority. Claim and queued-delivery owners reject active or paused ambiguous records. For a linked authorityless pending intent, provider entry requires both its current canonical source and saved intent audience to prove a matching supported non-group route; missing, unknown, or group evidence fails closed. Legacy direct work may be salvaged only by explicitly retargeting it to a trusted current direct conversation; group use must be archived and recreated from the current group with a typed binding. Remove this denial bridge only after every rollback-eligible old runner is retired and a complete supported-vault inventory taken afterward proves zero active or paused ambiguous records and zero linked authorityless pending intents.

After scheduled App Server initialization and before `thread/start`, the parent reads effective configuration and fails closed if generic MCP configuration is enabled or malformed; ordinary attended MCP behavior is unchanged. Prompt overlays and advertised capability text describe this surface but are not themselves authority.
