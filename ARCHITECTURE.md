...
- `VaultCliError` keeps arbitrary diagnostic context private and carries model-repair guidance only through its explicit bounded repair contract. The CLI bridge preserves the domain code, retryability, safe operation stage, hint, and value-free field errors through Incur's final envelope. Unmapped exceptions are classified once at the CLI root: expected filesystem failures receive stable categories, ordinary messages are bounded and path-redacted, and provider- or credential-shaped messages receive a fixed fallback. Failures before `serve()` use that same projection when the caller requested machine output.
