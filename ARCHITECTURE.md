...
- Wearable provider timeseries should not be retained as full provider sample arrays by default. Product, assistant, and CLI wearable summaries consume compact summary observations, derived facts, or display-grade metric samples; any timeseries-derived product fact must come from an explicit importer/projector step that persists only compact evidence. Junction's sparse body resources (`weight`, `fat`, `body_mass_index`, `lean_body_mass`, and `waist_circumference`) retain one timestamped canonical observation and one compact evidence part per reading, never the provider response array or generic sample rows. Junction `note` likewise keeps dated tag names as completed intervention events and compact evidence while dropping free text. Dense streams reduce in memory to bounded aggregates when admitted. Dense raw retention remains a legacy/debug cleanup lane for already-written high-volume timeseries roles.
