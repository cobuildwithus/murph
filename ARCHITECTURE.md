...
- `packages/assistant-engine`: workspace-private headless assistant execution runtime that owns provider-turn execution, tool/runtime assembly, assistant state/outbox/status/store surfaces, assistant automation, the single assistant input spine, assistant-specific vault/inbox/knowledge tool surfaces, hosted computer-use dynamic tools, Murph-managed package skill assets under `skills/**`, attachment prompt-bundle audit support, and active-outbox reconciliation for assistant-owned one-time delivery staging under the exact flat assistant-runtime generated-delivery directory. It also owns the narrow derived export-pack retirement policy: serial delivery persists terminal `sent` without optional archive work, then one finite, interruptible quiescent sweep derives work from the trusted outbox inventory. That sweep reads each present sent archive once, rejects a manifest-declared `exports/packs/<packId>/` candidate as terminally ineligible when its central-directory aggregate exceeds the vault-file byte ceiling, and visits every archive and candidate once so an ineligible, deferred, or stale item cannot starve later independent work. It may remove only an unchanged pack whose bytes match that exact sent archive; present descriptor/path mismatches or unsafe refs fail closed. The final active-file ownership check, live snapshot revalidation, and removal share the assistant runtime write lock with canonical export-pack materialization, explicit prune, and outbox-intent creation; any active vault-file ref at or beneath the pack defers only that pack, using the selected vault filesystem's case policy so case-only refs cannot escape ownership. Outbox creation applies that same case policy when classifying direct pack files for final in-lock byte revalidation. Existing selected-vault and output directories are compared by physical identity, so a symlinked or case-only alias cannot bypass serialization. An exact stored snapshot is rematerialized within its original lock; a missing snapshot releases the lock before the full-vault rebuild, then the canonical write reacquires the lock, requires the manifest bytes to still equal the captured generation, and refuses to overwrite a pack that another operation completed under the same manifest. A physically distinct external output keeps payload reads and destination writes outside the lock. Its initial and final exact manifest, inventory, root-identity, and file-identity snapshots run under the shared lock so they cannot certify a manifest-first intermediate writer state; any conflict fails before destination writes, while an initially incomplete pack uses the existing lock-free rebuild. The staged terminal ZIP and its sent outbox intent remain continuation evidence across active ownership, cancellation, remaining packs, and ordinary terminal-outbox retention. Hosted idle cleanup invokes the sweep directly. Assistantd owns one ephemeral coalesced maintenance task: it begins only after the listener is accepting requests, aborts for authenticated foreground admission, and restarts after authenticated completion, including outbox-drain requests; health probes and unauthenticated traffic do not discard the owner. No cursor, second lifecycle service, or post-turn archive inspection is introduced. Broad low-frequency native tools keep their argument contracts and set Codex `deferLoading` at `thread/start`, leaving direct-model `tool_search` and code-mode `ALL_TOOLS` discovery to the pinned App Server rather than adding a Murph-owned discovery protocol. The stable assistant prompt may route to those package-owned skill files through `$MURPH_ASSISTANT_SKILLS_ROOT`; local and hosted runtime env setup stamps that var to the canonical package-owned skill root. Hosted native Codex skill rendering stays disabled because rendered runner-local paths can break hosted prompt-cache stability. It consumes neutral vault usecase services, runtime loaders, assistant vault path helpers, and the shared runtime-write-lock seam from `@murphai/vault-usecases`, and consumes provider-target normalization plus hosted provider-preset/config helpers from `@murphai/operator-config` instead of owning duplicate copies.
...
Paused signed Temporal requests return `retry_later`, direct web latency hints
schedule no work, existing invocations drain normally, and accepted mailbox
rows remain durable in Postgres. This adds no queue, dual-write layer, or
persisted pause owner.
