...
Before model or notification planning, the scheduler probes the existing meal closeout selector with `limit: 1` and the scheduled occurrence local date. The query projection filters automatic captures and same-occurrence removal revisions, orders retries before oldest retained photos, and applies the limit in SQL. Empty work consumes no model call. Read failures, invalid time context, and modified managed instructions retain the ordinary execution path. This adds no queue or persisted state and preserves the existing upstream AI-usage admission policy.
