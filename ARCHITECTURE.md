...
- Wearable provider timeseries should not be retained as full provider sample arrays by default. Product, assistant, and CLI wearable summaries consume compact summary observations, derived facts, or display-grade metric samples; any timeseries-derived product fact must come from an explicit importer/projector step that reduces provider samples in memory and persists only compact evidence. Junction's contracts-owned policy admits only bounded daily aggregates, compact UTC-hour features, sparse readings/events, and note tags; optional resources stay disabled until exact opt-in, grouped provider arrays never become canonical evidence, and the importer does not invent coverage metadata. Junction `note` keeps normalized dated tags on a kind-stable neutral note spine with compact evidence and drops free text before retention; its job payload freezes the admitted history generation so only a complete v2 chain certifies v2 coverage. Personal Patterns owns the narrower semantic projection and currently admits only a provenance-verified Oura `sauna` tag. `fall` remains a sparse alert fact, while `electrocardiogram_voltage` and `workout_stream` remain outside the generic timeseries seam. Dense raw retention remains a legacy/debug cleanup lane for already-written high-volume timeseries roles.
