...
- `packages/assistant-engine`: workspace-private headless assistant execution runtime that owns provider-turn execution, tool/runtime assembly, assistant state/outbox/status/store surfaces, assistant automation, the single assistant input spine, assistant-specific vault/inbox/knowledge tool surfaces, hosted computer-use dynamic tools, Murph-managed package skill assets under `skills/**`, attachment prompt-bundle audit support, and active-outbox reconciliation for assistant-owned one-time delivery staging under the exact flat assistant-runtime generated-delivery directory. It also owns the narrow derived export-pack retirement policy: serial delivery persists terminal `sent` without optional archive work, then the existing quiescent generated-delivery maintenance owner drains bounded, interruptible units while progress remains and foreground work has not requested cancellation. Each unit may remove only one unchanged manifest-declared `exports/packs/<packId>/` whose bytes match the exact sent archive; missing completed archive refs may be skipped, while present descriptor/path mismatches or unsafe refs fail closed. The final active-file ownership check, live snapshot revalidation, and removal share the assistant runtime write lock with outbox-intent creation; any active vault-file ref at or beneath the pack defers that pack while later eligible candidates may still progress. The staged terminal ZIP and its sent outbox intent remain continuation evidence across active ownership, cancellation, remaining packs, and ordinary terminal-outbox retention. Hosted idle cleanup and recurring local runtime maintenance re-derive work without a cursor or second lifecycle owner; assistantd admits requests before its initial pass and cancels that pass when foreground work arrives. Broad low-frequency native tools keep their argument contracts and set Codex `deferLoading` at `thread/start`, leaving direct-model `tool_search` and code-mode `ALL_TOOLS` discovery to the pinned App Server rather than adding a Murph-owned discovery protocol. The stable assistant prompt may route to those package-owned skill files through `$MURPH_ASSISTANT_SKILLS_ROOT`; local and hosted runtime env setup stamps that var to the canonical package-owned skill root. Hosted native Codex skill rendering stays disabled because rendered runner-local paths can break hosted prompt-cache stability. It consumes neutral vault usecase services, runtime loaders, and assistant vault path helpers from `@murphai/vault-usecases`, and consumes provider-target normalization plus hosted provider-preset/config helpers from `@murphai/operator-config` instead of owning duplicate copies.
