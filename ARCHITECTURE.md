...
- `apps/web`: hosted Next.js integration control plane for Vercel-style
  ...
  row containing the canonical allowlisted product-area/problem issue. The
  first three distinct member escalations per UTC day may send an immediate
  plain-text support alert that pairs readable labels derived from the parsed
  stored issue with the internal feedback and member ids. Web reads back and
  validates both rows before provider entry and formats from the first stored
  issue, so replay reuses one body and Resend idempotency key even if a later
  callback selects different codes. Missing, linked, free-form, legacy, or
  malformed detail fails closed. The alert never reads model-authored prose,
  raw conversation, health, contact, secret, provider, or other private context
  and adds no table, queue, cursor, approval state, or delivery owner.
...
The temporary OC-to-ENAM R2 cutover may pause hosted runtime admission only at
...
rows remain durable in Postgres. This adds no queue or persisted pause owner.
