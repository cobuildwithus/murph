...
- Assistant-generated one-time delivery files are the narrow non-canonical exception outside `.runtime`: `exports/assistant-deliveries/**` must stay in the normal encrypted workspace snapshot while its ref, filename/content type, size, and SHA-256 match an active outbox descriptor that needs restart-safe approval or delivery recovery. `packages/assistant-runtime` opts into reconciliation only after foreground and background assistant work is quiescent at the idle-snapshot boundary; `packages/assistant-engine` then retains exact active files and removes changed or unclaimed regular files before archive planning. The prefix must not be added to hosted snapshot exclusions, and generic vault paths never inherit its deletion authority.
