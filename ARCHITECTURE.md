...
consent, untouched-member Pulse trial enrollment, and companion-access assertion
...
Companion access admits canonical active or sponsored members plus a
non-suspended member whose own billing is `paused`. That paused exception is
limited to native account admission, Junction token repair, initial-onboarding
projection, sync status, and companion health-data ingress; ordinary assistant
runtime, meal-photo capture authority, browser billing, and every other paid
product boundary retain canonical active-access checks. Historical launch
consent and administrative suspension still fail closed on every applicable
companion route.
Meal-photo enrollment and activation keep that paid gate, but normalize an
active-access denial to a feature-scoped conflict so already-released native
clients do not reinterpret optional capture unavailability as whole-account
loss after companion admission succeeded.
