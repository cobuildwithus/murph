...
- `packages/contracts`: canonical Zod contracts, parse helpers, TypeScript types, generated JSON Schema artifacts, and shared assistant runtime event/cursor contracts
- `packages/runtime-state`: shared `.runtime` path resolution, low-level assistant-state path/queue helpers, and SQLite defaults for rebuildable local state used by query, inboxd, and CLI inbox flows
...
- `apps/web`: hosted Next.js integration control plane for Vercel-style deployments, backed by Postgres/Prisma for device OAuth sessions, encrypted token escrow, ownership mapping, webhook traces, sparse sync signals, Linq recipient bindings, queued Linq webhook events, and local-agent pairing while keeping canonical health and inbox data out of the hosted tier
...
8. Provider-backed assistant chat and outbound channel flows may persist local session metadata, local transcript files, explicit delivery bindings, typed conversational memory docs, auto-reply cursors/channel state, and scheduled assistant cron jobs outside the vault, including selected non-canonical health context for continuity, but they must not treat that state as canonical health truth or bypass canonical write boundaries for health data. Current outbound adapters include iMessage, Telegram, Linq, and AgentMail-backed email; email auto-reply is intentionally limited to direct threads and must preserve the inbound AgentMail inbox identity for replies, while Linq replies reuse the inbound chat id thread binding for the local webhook-driven conversation. Provider turns now bind the real current user prompt, session id, and turn id on the host side, expose `assistant memory search|get|upsert|forget` plus the `assistant cron` subtree through a bounded Codex MCP tool surface when available, serialize assistant-memory commits with vault-scoped locks plus minimal provenance metadata, and only execute due cron jobs while `vault-cli assistant run` is active for that vault.
...
11. The hosted `apps/web` control plane accepts only provider OAuth/webhook traffic plus authenticated browser/agent control traffic, keeps raw provider tokens away from browsers, stores only sparse Linq routing/event state instead of canonical captures, and should expose sparse sync hints/token export endpoints or queued Linq webhook events only to the paired local agent for the owning user. Browser-authenticated routes must rely on short-lived, request-bound signed assertions from a trusted auth edge or proxy, reject stale assertions, and consume browser-assertion nonces once so captured assertions cannot be replayed across hosted control-plane routes.
