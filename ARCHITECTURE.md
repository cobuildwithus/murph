...
- `VaultCliError.context` is the sole diagnostic metadata source. The shared operator-config projector exposes only the domain code, retryability, a bounded factual stage, and value-free fields derived from allowlisted Zod-like issues; setup and main CLI bridges consume that same projection. Journey-specific recovery actions stay with their assistant or command caller instead of shared domain mappers. Unmapped exceptions are classified once: expected filesystem failures receive stable categories, ordinary messages are bounded and path-redacted, and provider- or credential-shaped messages receive a fixed fallback. Failures before `serve()` use that same projection when the caller requested machine output.
