...
11. The hosted `apps/web` control plane accepts only provider OAuth/webhook traffic plus authenticated browser/agent control traffic, keeps raw provider tokens away from browsers, and should expose sparse sync hints/token export endpoints only to the paired local agent for the owning user. Browser-authenticated routes must rely on short-lived, request-bound signed assertions from a trusted auth edge or proxy, reject stale assertions, and consume browser-assertion nonces once so captured assertions cannot be replayed across hosted device-sync routes.
