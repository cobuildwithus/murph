...
- `packages/inboxd`: inbox capture ingestion/runtime package that persists canonical raw inbox evidence while keeping inbox-only cursors, source-specific checkpoints, capture indexes, and attachment job state in local SQLite state; current first-class connectors cover iMessage, Telegram, and AgentMail-backed email
...
- `packages/web`: local-only Next.js observability app that reads the selected vault on the server through the query package, preferring explicit `HEALTHYBOB_VAULT` selection and otherwise falling back to the saved CLI default vault, and may initiate device-auth control-plane actions against `packages/device-syncd`
...
3. Inbox capture persists raw evidence from iMessage, Telegram, and AgentMail email connectors, indexes attachments, and enqueues parse jobs in rebuildable local runtime state.
...
11. The hosted `apps/web` control plane accepts only provider OAuth/webhook traffic plus authenticated browser/agent control traffic, keeps raw provider tokens away from browsers, and should expose sparse sync hints/token export endpoints only to the paired local agent for the owning user.
