...
- Private member workspace archives may include inspected non-secret member runtime records and conversation history; support-bundle and hosted-checkpoint filters do not define this user-requested scope. The assistant reads eligible sources in place, stages only the newly generated archive, and uses the existing file-and-destination approval. Source-content exclusions are owned by `agent-docs/SECURITY.md`; this does not widen hidden attachment refs or promise a restorable service snapshot.
