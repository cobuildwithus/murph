...
- Wearable provider timeseries should not be retained as full provider sample arrays by default. Product, assistant, and CLI wearable summaries consume compact summary observations, derived facts, or display-grade metric samples; any timeseries-derived product fact must come from an explicit importer/projector step that reduces provider samples in memory and persists only compact evidence. Junction blood-oxygen and stress imports follow that boundary by attaching at most one versioned temporal-feature envelope to the existing resource/source/local-day artifact and emitting a fixed set of summary-grain observations; they retain no samples, hourly rows, or timestamp arrays. Only the closed date-by-date fetch path carries transient authority to publish those day-scoped features; precise resource windows and webhook-triggered imports retain the base daily observations but cannot derive temporal features from partial windows. Each authoritative source-day result reconciles the fixed feature-facet set through existing canonical event upserts and versioned retractions, so insufficient or capped replacement evidence removes stale derived facts without a second state owner. The reducer deterministically collapses equal instants by mean, keeps literal SpO2 sample burden separate from continuity claims, and admits run or successive-difference claims only for adjacent pairs inside both the cadence-derived boundary and a product-owned absolute gap cap; daypart comparison additionally requires a qualifying pair in each window. Queryable derived observations carry only bounded scalar evidence counts/method plus non-high confidence. Input remains capped at 5,000 samples per resource/source/day and 25,000 per resource/import, output at ten feature observations per local day and 1 KiB per envelope, and overflow suppresses only the temporal enhancement so established daily facts remain available. Junction `note` is a sparse product input: the importer keeps normalized dated tags on a kind-stable neutral `note` spine with compact evidence and drops the free-text value before raw snapshot retention. Personal Patterns owns the narrower semantic projection and currently admits only a provenance-verified Oura `sauna` tag as an action factor. The neutral external-reference facet is distinct from the legacy per-tag intervention facets, so existing event kinds are never rewritten in place. Note-history coverage version 2 performs one bounded semantic reimport for sources covered by the legacy intervention normalizer; query excludes the recognizable legacy rows while neutral replacements arrive. The existing resource-job payload freezes the admitted note-history generation across continuations and retries: unversioned persisted work remains v1 and only a complete admitted v2 chain can certify v2 coverage. Dense raw retention remains a legacy/debug cleanup lane for already-written high-volume timeseries roles; sparse or higher-sensitivity resources such as weight and glucose need a separate product/debug policy before any default ingestion or pruning.
