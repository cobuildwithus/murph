...
- Journal and Personal Patterns are query-owned Browser Vault projections over canonical events and metric points. Journal groups linked records for display without writing daily summaries or a second health-data store. Note timing and icon choices use existing canonical tags and a shared contracts catalog. Opening Journal requests updated data through the existing bounded Browser Vault runtime-refresh owner. Personal Patterns performs one bounded deterministic calculation and stores only the current derived report in Browser Vault; React and assistant prompts never recalculate it. Its query owner qualifies inferred absence from source-specific daytime evidence, assigns up to three unique comparable controls, weights independent episodes, screens retrospective stability, and selects one sleep-quality metric by availability. Runtime and Browser Vault share the full canonical source loader for this calculation; raw wearable observations stay internal to Patterns, while Journal and serialized entities use the existing default visibility filter. Browser Vault generation 17 invalidates older calculations without changing the report schema. Browser Vault generation 11 makes the Journal projection part of the core shard while keeping legacy replicas readable until the normal generation refresh replaces them.
