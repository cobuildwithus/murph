...
bucket. The Worker binding, direct-upload presign target, lifecycle automation,
snapshot restore, and account-deletion cleanup all resolve through that same
environment-specific bucket. Deploy preflight reads Cloudflare bucket metadata
and fails closed unless the selected bucket reports ENAM with the Standard
storage class. The runtime has no source-region fallback, dual write, migration
phase, or storage-specific admission gate.
