...
`apps/cloudflare` encrypts each staged JPEG into a private per-user R2 object. Object deletion derives the user-namespaced R2 path directly and does not require the user's encryption context to remain available. The metadata-only `meal-photo.captured` mailbox item wakes `packages/assistant-runtime`, which verifies the object's length and digest, imports one idempotent photo-only meal through `packages/core`, and uses the restored hosted operator home and hosted route-validation profile to idempotently install one private managed `dailyLocal` closeout for 21:00 in the vault timezone. Vaults with no successful automatic capture do not receive that automation; replays and later captures reuse the stable record, while paused or archived member state remains authoritative. Import completion stays retryable until the automation is durable, and staged-object deletion is scheduled only after the workspace checkpoint succeeds. The R2 lifecycle rule makes staged meal-photo objects eligible for asynchronous deletion at 31 days, one day beyond mailbox recovery retention; successful imports still delete staging immediately after the checkpoint, and 31 days is not a guaranteed physical-deletion deadline.

The closeout runs through the ordinary managed-automation scheduler, assistant prompt/tool stack, current private route, and outbox. It enriches the existing canonical automatic meals, summarizes supported current-day calories/macros without double-counting, and sweeps a bounded recent window for delayed captures. After each photo is inspected and supported structure is read back, the canonical `meal remove-photo` mutation appends an attachment-free meal revision and atomically replaces the verified raw JPEG bytes with a small non-image privacy tombstone plus updated manifest integrity proof. Historical append-only revisions therefore remain valid without retaining the meal image. Neither the enrollment row nor R2 is canonical meal truth; the member's encrypted hosted workspace remains the canonical structured meal owner.
