...
describe("hosted wake store", () => {
  ...
  it("rejects advancing the cursor when the recorded terminal receipt came from an older fetch position", async () => {
    const tx = createHostedWakeStoreHarness({
      cursor: {
        committedSeq: 1n,
        nextSeq: 3n,
  ...
  it("refreshes the terminal receipt fetch fence when the same wake is fetched again from the current cursor", async () => {
    const tx = createHostedWakeStoreHarness({
      cursor: {
        committedSeq: 1n,
        nextSeq: 3n,
  ...
  it("rejects stale cursor commits after a coalescing rewrite reuses the same wake id and seq", async () => {
    const tx = createHostedWakeStoreHarness({
      cursor: {
        committedSeq: 0n,
        nextSeq: 2n,
