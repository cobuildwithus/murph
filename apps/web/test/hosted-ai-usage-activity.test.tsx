...
describe("HostedAiUsageActivity", () => {
  ...
  it("renders a single ledger surface and prefilled Murph handoff", async () => {
    ...
    assert.doesNotMatch(markup, /Start a fresh group and get people talking\./);
    assert.doesNotMatch(markup, /Selected Jul 27, 2026/);
  ...
  it("keeps the enabled empty state compact and actionable", async () => {
    ...
    assert.match(markup, /<h3[^>]*>Referrals<\/h3>/);
    assert.match(markup, /Ask Murph/);
    assert.match(markup, /Choose a referral/);
    assert.match(markup, /Ask Murph what(?:&#x27;|')s available/);
    assert.match(markup, /Complete one before it closes/);
    assert.match(markup, /earn the usage reward/);
