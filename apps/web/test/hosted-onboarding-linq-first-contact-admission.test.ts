...
describe("Linq first-contact admission", () => {
  ...
  it("throws a typed 503 on non-success or malformed classifier responses", async () => {
    const rawProviderMessage = "RAW_PROVIDER_MESSAGE_WITH_SENTINEL";
    ...
    const classifierError = await classifyHostedLinqFirstContactAdmission({
      request: BASE_REQUEST,
    }).then(
      () => {
        throw new Error("Expected the classifier to reject.");
      },
      (error: unknown) => error,
    );

    await expect(Promise.reject(classifierError)).rejects.toMatchObject({
      code: "LINQ_FIRST_CONTACT_ADMISSION_CLASSIFIER_UNAVAILABLE",
      details: {
    ...
    expect(classifierError).toMatchObject({
      cause: {
    ...
    const response = hostedOnboardingHttp.jsonError(classifierError);
    ...
    expect(loggedPayload).toMatchObject({
      errorCauseMessage: "OpenAI Responses API error.",
    });
    expect(JSON.stringify(loggedPayload)).not.toContain(rawProviderMessage);
