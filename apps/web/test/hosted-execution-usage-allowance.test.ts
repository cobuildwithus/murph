...
describe("hosted AI usage allowance pricing", () => {
  ...
  it("lets a silent Sol automation exhaust near-limit allowance before Terra or Luna", async () => {
    const automationUsage = (
      model: "gpt-5.6-luna" | "gpt-5.6-terra" | "gpt-5.6-sol",
    ): AssistantUsageRecord => ({
      ...BASE_USAGE_RECORD,
      featureKey: "weekly_health_insight",
      providerRequestOutcome: "succeeded",
      requestedModel: model,
      servedModel: model,
      triggerKind: "automation",
    });
    // Usage accounting has no delivery-outcome exception: a completed scan
    // that finishes without a reply retains the same provider debit.
    const lunaCost = priceHostedAiUsageForAllowance(
      automationUsage("gpt-5.6-luna"),
    ).costUsdMicros;
    const terraCost = priceHostedAiUsageForAllowance(
      automationUsage("gpt-5.6-terra"),
    ).costUsdMicros;
    const solCost = priceHostedAiUsageForAllowance(
      automationUsage("gpt-5.6-sol"),
    ).costUsdMicros;

    expect({ lunaCost, solCost, terraCost }).toEqual({
      lunaCost: 77n,
      solCost: 1_896n,
      terraCost: 759n,
    });

    const spendBeforeAutomation = DIRECT_PULSE_ALLOWANCE_USD_MICROS - 1_000n;
    const lunaGate = await resolveHostedAiUsageGate({
      memberId: "member_123",
      now: "2026-03-29T12:00:05.000Z",
      prisma: createGatePrisma({
        spentUsdMicros: spendBeforeAutomation + lunaCost,
      }) as never,
    });
    const terraGate = await resolveHostedAiUsageGate({
      memberId: "member_123",
      now: "2026-03-29T12:00:05.000Z",
      prisma: createGatePrisma({
        spentUsdMicros: spendBeforeAutomation + terraCost,
      }) as never,
    });
    const solGate = await resolveHostedAiUsageGate({
      memberId: "member_123",
      now: "2026-03-29T12:00:05.000Z",
      prisma: createGatePrisma({
        spentUsdMicros: spendBeforeAutomation + solCost,
      }) as never,
    });

    expect(lunaGate).toMatchObject({
      allowed: true,
      remainingUsdMicros: 923n,
    });
    expect(terraGate).toMatchObject({
      allowed: true,
      remainingUsdMicros: 241n,
    });
    expect(solGate).toMatchObject({
      allowed: false,
