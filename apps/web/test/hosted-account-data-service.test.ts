...
describe("deleteHostedAccountData", () => {
  ...
  it.each([
    ...
    expect(serviceMocks.prepareHostedAccountDeletionCleanup).toHaveBeenCalledWith(
      expect.objectContaining({
        stripeCustomerIds: ["cus_delete_complete"],
      }),
    );
  ...
  it("cleans a completed superseded Checkout loser before deleting the accepted subscription", async () => {
    ...
    expect(serviceMocks.prepareHostedAccountDeletionCleanup).toHaveBeenCalledWith(
      expect.objectContaining({
        stripeCustomerIds: expect.arrayContaining([
  ...
  it("expires an exact open owned Family Checkout before deleting its group", async () => {
    ...
    expect(serviceMocks.prepareHostedAccountDeletionCleanup).toHaveBeenCalledWith(
      expect.objectContaining({
        stripeCustomerIds: ["cus_family_delete_open"],
      }),
    );
    expect(stripe.customers.del).not.toHaveBeenCalled();
  });
  ...
  it("recovers a Family Checkout that completes while expiration races, then refunds before group deletion", async () => {
    ...
    expect(serviceMocks.prepareHostedAccountDeletionCleanup).toHaveBeenCalledWith(
      expect.objectContaining({
        stripeCustomerIds: ["cus_family_delete_race"],
      }),
    );
    expect(stripe.customers.del).not.toHaveBeenCalled();
  });
  ...
  it("does not add a new balance gate for an already-known Stripe Customer", async () => {
    const deleteCalls: HostedAccountDeletionPrismaDeleteCall[] = [];
  ...
  it("fails closed when a recovered reservation Customer has a balance", async () => {
    const reservationId = "hbscr_balance_gate";
    const deleteCalls: HostedAccountDeletionPrismaDeleteCall[] = [];
    const billingRefRecord = {
      ...await makeCheckoutAttemptBillingRefRowForTest({
        memberId: "member_123",
    ...
    const stripe = {
      customers: {
        create: vi.fn(async () => ({ id: "cus_recovered_balance" })),
        del: vi.fn(),
        retrieve: vi.fn(async () => ({
          ...makeZeroBalanceStripeCustomerForAccountDeletionTest(
            "cus_recovered_balance",
            {
              customerReservationId: reservationId,
              memberId: "member_123",
              source: "hosted.auto_pulse_trial",
  ...
  it("fails closed when a recovered reservation Customer has live billing", async () => {
    const reservationId = "hbscr_live_billing_gate";
    const deleteCalls: HostedAccountDeletionPrismaDeleteCall[] = [];
    const billingRefRecord = {
      ...await makeCheckoutAttemptBillingRefRowForTest({
        memberId: "member_123",
    ...
    const stripe = {
      customers: {
        create: vi.fn(async () => ({ id: "cus_recovered_live" })),
        del: vi.fn(),
        retrieve: vi.fn(async () =>
          makeZeroBalanceStripeCustomerForAccountDeletionTest(
            "cus_recovered_live",
      ...
      subscriptions: {
        list: vi.fn(async () => ({
          data: [{
            customer: "cus_recovered_live",
            id: "sub_recovered_live",
            status: "active",
          }],
          has_more: false,
          object: "list",
  ...
  it("keeps known Customer deletion best effort when Stripe is not configured", async () => {
    serviceMocks.getHostedOnboardingStripe.mockReturnValue(null);
    serviceMocks.runHostedAccountDeletionCleanup.mockResolvedValue(
      makeCleanupRunResult({
        cleanupPending: true,
