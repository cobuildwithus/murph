...
describe("replaceUnconsumedHostedMailboxEnvelopePayloadTx", () => {
  it("re-encrypts one existing item without changing its ordering identity", async () => {
    installAadCheckingHostedSecureBoxTestCodec();

    try {
      const occurredAt = new Date();
      const memberId = "member_referral_repair";
      const mailboxItemId = "mailbox_referral_repair";
      const eventId =
        "assistant.notification.requested:usage-referral-reward:legacy-head";
      const notificationKey = "usage-referral-reward:legacy-head";
      const route = {
      ...
      const legacyWake = buildHostedExecutionAssistantNotificationRequestedWake({
        eventId,
        memberId,
        notification: {
      ...
      const upgradedWake =
        buildHostedExecutionAssistantNotificationRequestedWake({
          ...
          memberId,
          notification: {
            ...legacyWake.notification,
            externalThreadRouteAuthority: authority,
          },
          occurredAt: occurredAt.toISOString(),
        });
      const expectedPayloadHash = hashHostedMailboxStoredPayload({
        serialized: JSON.stringify(legacyWake),
        userId: memberId,
      });
      let row = {
        ...buildHostedMailboxItemRow({
          ...
          payloadInlineCiphertext: "legacy-ciphertext",
      ...
      const queryRaw = vi.fn(async (
        _query: { strings?: readonly string[] },
      ) => [{ id: mailboxItemId }]);
      const tx = {
        $queryRaw: queryRaw,
        hostedMailboxItem: {
      ...
      const result =
        await replaceUnconsumedHostedMailboxEnvelopePayloadTx({
      ...
      expect(updateMany).toHaveBeenCalledExactlyOnceWith({
        data: expect.objectContaining({
          payloadBytes: Buffer.byteLength(JSON.stringify(upgradedWake), "utf8"),
          payloadHash: expect.stringMatching(/^hmac-sha256:/),
          payloadInlineCiphertext: expect.any(String),
          payloadRef: null,
          payloadSchema: HOSTED_MAILBOX_ITEM_PAYLOAD_SCHEMA,
      ...
      expect(row.payloadHash).not.toBe(expectedPayloadHash);
      ...
      expect(lockQuery?.strings?.join("?")).not.toContain("INSERT");
      ...
      await expect(decodeHostedMailboxStoredPayload({
        ...
        userId: memberId,
      })).resolves.toEqual(upgradedWake);
    } finally {
      restoreDefaultHostedSecureBoxTestCodec();
    }
  });

  it("does not replace an item that was consumed before the repair lock", async () => {
    const occurredAt = new Date();
    const eventId =
      "assistant.notification.requested:usage-referral-reward:consumed-head";
    const wake = buildHostedExecutionAssistantNotificationRequestedWake({
      eventId,
      memberId: "member_consumed_repair",
      notification: {
        deliveryDedupeToken: "usage-referral-reward:consumed-head",
        deliveryDispatchMode: "queue-only",
        deliveryIdempotencyKey: "usage-referral-reward:consumed-head",
        externalThreadRouteAuthority: {
          channel: "linq",
          containerMemberId: "member_consumed_repair",
    ...
    const updateMany = vi.fn();
    const tx = {
      $queryRaw: vi.fn(async () => [{ id: "mailbox_consumed_repair" }]),
      hostedMailboxItem: {
        findUnique: vi.fn(async () => ({
          ...buildHostedMailboxItemRow({
            ...
            createdAt: occurredAt,
            dedupeKey: eventId,
            id: "mailbox_consumed_repair",
            kind: "assistant.notification.requested",
            lane: "system",
            occurredAt,
            payloadHash: "hmac-sha256:consumed",
            updatedAt: occurredAt,
            userId: "member_consumed_repair",
          }),
          contentRetiredAt: null,
        })),
        updateMany,
    ...
    await expect(replaceUnconsumedHostedMailboxEnvelopePayloadTx({
      envelope: wake,
      expectedPayloadHash: "hmac-sha256:consumed",
      mailboxItemId: "mailbox_consumed_repair",
      tx,
    })).resolves.toBeNull();
    expect(updateMany).not.toHaveBeenCalled();
  });
