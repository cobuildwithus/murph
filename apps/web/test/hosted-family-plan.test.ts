...
describe("hosted Family plan", () => {
  ...
  it("closes an undisclosed Family Checkout session when group ownership changes", async () => {
    const group = {
      billingStatus: HostedBillingStatus.not_started,
      id: "hbag_family",
      ownerMemberId: "member_owner",
    ...
    runtimeMocks.requireHostedStripeApi.mockReturnValueOnce({
      checkout: {
        sessions: {
          create: vi.fn().mockResolvedValue({
            id: "cs_test_familyOwnerChanged123",
            url: "https://checkout.stripe.com/c/pay/cs_test_familyOwnerChanged123",
          }),
          expire: checkoutExpire,
          retrieve: vi.fn().mockResolvedValue({
            customer: null,
    ...
    expect(checkoutExpire).toHaveBeenCalledWith("cs_test_familyOwnerChanged123");
    expect(tx.hostedAccountGroupBillingRef.updateMany).not.toHaveBeenCalled();
  });
