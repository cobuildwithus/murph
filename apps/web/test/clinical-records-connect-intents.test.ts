...
describe("Clinical Records connect intents", () => {
  ...
  it("reuses one live scheduled intent and never rotates its bearer on retry", async () => {
    const { createClinicalRecordConnectIntent } = await import(
      "@/src/lib/clinical-records/connect-intents"
    );
    const now = new Date("2026-07-10T12:00:00.000Z");
    ...
    const request = new Request(
      "https://join.example.test/api/internal/clinical-records/connect-link",
    );

    const first = await createClinicalRecordConnectIntent({
      memberId: "member_clinical_1",
      now,
      request,
      requestKey,
    });
    const claimHash = createHash("sha256").update(first.claim).digest("hex");
    ...
    const replay = await createClinicalRecordConnectIntent({
      memberId: "member_clinical_1",
    ...
    expect(mocks.oauthUpdateMany).toHaveBeenCalledOnce();
  });

  it("does not resurrect a scheduled intent after OAuth has started", async () => {
    const { createClinicalRecordConnectIntent } = await import(
      "@/src/lib/clinical-records/connect-intents"
    );
    const now = new Date("2026-07-10T12:02:00.000Z");
    mocks.findUnique.mockResolvedValue({
      claimHash: "scheduled-claim-hash",
      completedAt: null,
      createdAt: new Date("2026-07-10T12:00:00.000Z"),
      expiresAt: new Date("2026-07-10T12:15:00.000Z"),
      memberId: "member_clinical_1",
      providerDirectoryEntryId: null,
      startedAt: new Date("2026-07-10T12:01:00.000Z"),
    });
  ...
  it("scopes a scheduled bearer to the runtime-bound member", async () => {
    const { createClinicalRecordConnectIntent } = await import(
      "@/src/lib/clinical-records/connect-intents"
    );
    const requestKey = `scheduled_${"c".repeat(64)}`;
    const request = new Request(
      "https://join.example.test/api/internal/clinical-records/connect-link",
    );
    ...
    const second = await createClinicalRecordConnectIntent({
      memberId: "member_clinical_2",
      request,
      requestKey,
    });
