...
const config = readHostedLocalTemporalMailboxSignalFaultConfig(process.env);
const originalSignalWithStart = WorkflowClient.prototype.signalWithStart;
...
>(
  ...
  return await originalSignalWithStart.call<
    ...
    Promise<WorkflowHandleWithSignaledRunId<WorkflowFn>>
  >(this, workflowTypeOrFunc, options);
}

Object.defineProperty(WorkflowClient.prototype, "signalWithStart", {
  configurable: true,
...
function readHostedLocalTemporalMailboxSignalFaultConfig(
  ...
  if (
    source.MURPH_HOSTED_LOCAL_PROFILE?.startsWith("e2e:") !== true
...
async function consumeHostedLocalTemporalMailboxSignalFault(
  preloadConfig: HostedLocalTemporalMailboxSignalFaultConfig,
