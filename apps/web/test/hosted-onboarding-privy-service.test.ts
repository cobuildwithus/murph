...
describe("completeHostedPrivyVerification", () => {
  ...
  it("resolves a verified email against a pending Linq email contact without creating a duplicate member", async () => {
    const pendingEmailAddress = "linq-handle@example.com";
    const pendingEmailLookupKey = createHostedEmailLookupKey(pendingEmailAddress)!;
    const existingMember = makeMember({
      id: "member_pending_linq_email",
      maskedPhoneNumberHint: null,
      phoneLookupKey: null,
      phoneNumberVerifiedAt: null,
      privyUserId: null,
    ...
    const activeInvite = makeInvite(existingMember, {
      channel: "web",
      id: "invite_pending_linq_email",
      inviteCode: "invite-pending-linq-email",
      memberId: existingMember.id,
    });
    const prisma = asCompleteHostedPrivyVerificationPrisma({
      hostedInvite: {
      ...
      hostedMemberRouting: {
        findUnique: vi.fn(async ({ where }: { where?: Record<string, unknown> }) => {
          if (
            where?.pendingLinqParticipantContactLookupKey === pendingEmailLookupKey
    ...
    await expect(
      ...
      inviteCode: "invite-pending-linq-email",
      joinUrl: "https://join.example.test/join/invite-pending-linq-email",
      memberId: existingMember.id,
      stage: "checkout",
    });
  ...
  it("refuses public login-only completion when the verified identity has no hosted member", async () => {
    const prisma = asCompleteHostedPrivyVerificationPrisma({
      hostedInvite: {
        create: vi.fn(),
        findFirst: vi.fn(),
