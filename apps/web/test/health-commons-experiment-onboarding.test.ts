...
describe("ExperimentDetailPage onboarding projection", () => {
  it("projects Norwegian 4x4 onboarding details into the client protocol", async () => {
    ...
    expect(clientExperiment.onboarding).toEqual(expect.objectContaining({
      assistantPolicy: expect.objectContaining({
        askBeforeCreatingAutomations: true,
        missedLogFollowup: "opt_in_only",
        missedLogFollowupCopy:
          "Did you end up doing today's 4x4 session? Totally fine either way — I just want the experiment record to be accurate.",
      }),
