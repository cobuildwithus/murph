...
let labsPageModule: typeof import("../app/(dashboard)/labs/page");
let renderedPage: Awaited<ReturnType<typeof createPageHarness>>;
...
beforeEach(async () => {
  ...
  // Base UI chooses its browser layout effects at module evaluation time.
  renderedPage = await createPageHarness();
  cleanupRender = renderedPage.cleanup;
  labsPageModule = await import("../app/(dashboard)/labs/page");
});
...
describe("LabsPage", () => {
  it("keeps the hidden catalog out of search indexes", () => {
    const { metadata } = labsPageModule;
    expect(metadata.title).toBe("Labs — Murph");
    expect(metadata.robots).toEqual({ follow: false, index: false });
    expect(JSON.stringify(metadata)).not.toMatch(/junction/iu);
  });

  it("redirects an unauthenticated visitor before the catalog can request data", async () => {
    mocks.getHostedDashboardPageAuthSnapshot.mockResolvedValue({
      authenticated: false,
    ...
    await expect(renderLabsPage()).rejects.toThrow("NEXT_REDIRECT:/");
  ...
  it("uses the selected catalog type for the request and filtered empty-state recovery", async () => {
    fetchMock
      .mockResolvedValueOnce(jsonResponse(searchResponse([])))
      .mockResolvedValueOnce(jsonResponse(searchResponse([panelOffering])));
    const rendered = await renderLabsPage();

    await clickButtonElement(rendered, buttonByLabel(rendered, "Catalog type"));
    const panelOption = Array.from(rendered.window.document.querySelectorAll('[role="option"]'))
      .find((option) => option.textContent?.trim() === "Panels");
    assert.ok(panelOption);
    await act(async () => {
      panelOption.dispatchEvent(new rendered.window.Event("mousemove", { bubbles: true }));
    });
    await clickButtonElement(rendered, panelOption);
    await changeInput(rendered, "#labs-catalog-query", "  heart  ");
    ...
    expect(readRequestBody(0)).toEqual({
      action: "search", kind: "panel", limit: 20, query: "heart",
    });
    expect(rendered.container.textContent).toContain("No matching panels returned");
    expect(rendered.container.textContent).toContain("Try All catalog items or a shorter test name.");

    await clickButtonElement(rendered, buttonByLabel(rendered, "Catalog type"));
    const allOption = Array.from(rendered.window.document.querySelectorAll('[role="option"]'))
      .find((option) => option.textContent?.trim() === "All catalog items");
    assert.ok(allOption);
    await act(async () => {
      allOption.dispatchEvent(new rendered.window.Event("mousemove", { bubbles: true }));
    });
    await clickButtonElement(rendered, allOption);
    await submitForm(rendered, 0);

    expect(readRequestBody(1)).toEqual({
      action: "search", kind: "all", limit: 20, query: "heart",
    });
    expect(rendered.container.textContent).toContain("Broad Heart Panel");
    expect(rendered.container.textContent).not.toContain("No matching panels returned");
  });
...
async function createPageHarness() {
  ...
  // Linkedom has no layout engine; keep the popup behavior real while
  // supplying inline styles and avoiding browser scroll locking.
  rendered.window.document.documentElement.style.overflowY = "hidden";
  rendered.window.getComputedStyle = (element) => {
    assert.ok(element instanceof rendered.window.HTMLElement);
    return element.style;
  };
  vi.stubGlobal("getComputedStyle", rendered.window.getComputedStyle);
  return rendered;
}

async function renderLabsPage() {
  await renderedPage.rerender(await labsPageModule.default());
  return renderedPage;
}
