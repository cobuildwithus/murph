...
const mocks = vi.hoisted(() => {
  const state = {
    ...
    finalizeHostedLinqInstantFirstTurnReceipt: vi.fn(),
...
vi.mock("@/src/lib/hosted-onboarding/linq-instant-first-turn", () => ({
  ...
  finalizeHostedLinqInstantFirstTurnReceipt:
    mocks.finalizeHostedLinqInstantFirstTurnReceipt,
...
async function createDirectRootPreparationFailureFixture(input: {
  failureDomain?: "control" | "ingress";
} = {}) {
  ...
  unwrapRoot.mockImplementation(async (...args) => {
    const domain = args[0].domain;
    providerDomains.push(domain);
    if (transactionOpen) {
      providerDomainsAfterTransactionStart.push(domain);
    }
    if (!failureDelivered && domain === (input.failureDomain ?? "ingress")) {
...
describe("handleHostedOnboardingLinqWebhook", () => {
  beforeEach(() => {
    ...
    mocks.unwrapHostedDomainRootForWebByRootKeyId.mockResolvedValue({
      envelope: {
        domain: "control",
        rootKeyId: "root-control-historical",
        userId: "member_123",
      },
      rootKey: new Uint8Array(32),
    });
    mocks.unwrapHostedDomainRootsForWebByRootKeyIds.mockResolvedValue([]);
    ...
    mocks.finalizeHostedLinqInstantFirstTurnReceipt.mockResolvedValue(null);
  ...
  it("hands a terminal first-turn receipt back to the existing runtime wake path", async () => {
    const prisma = createHostedLinqDeliveryReceiptWebhookPrisma({
      sourceRef: "synthetic-first-turn-receipt",
      template: "invite_signup",
    });
    mocks.finalizeHostedLinqInstantFirstTurnReceipt.mockResolvedValueOnce({
      eventId: "evt_first_turn_inbound",
      linqChatId: "chat_first_turn",
      mailboxItemId: "mailbox_first_turn_inbound",
      source: "linq",
      userId: "member_first_turn",
    ...
    const event = buildHostedLinqProviderReceiptEvent({
      chatId: "chat_first_turn",
      eventId: "evt_first_turn_failed",
      eventType: "message.failed",
      messageId: "provider_msg_first_turn",
    });
    ...
    expect(mocks.finalizeHostedLinqInstantFirstTurnReceipt).toHaveBeenCalledWith({
      deliveryStatus: "failed",
      messageId: "provider_msg_first_turn",
      prisma,
    });
    expect(mocks.signalHostedMailboxAppendRuntime).toHaveBeenCalledWith(expect.objectContaining({
      abortSignal: expect.anything(),
      expectedUserId: "member_first_turn",
      mailboxItemId: "mailbox_first_turn_inbound",
    }));
  });
  ...
  it.each([
    {
      ...
      expectedValue: "Completed the check-in",
      label: "fallback text",
      part: {
        app: {
      ...
      privateSentinels: [
        "private-capability",
        "Private app name",
        "Private layout metadata",
      ],
    },
    {
      eventId: "evt_imessage_app_placeholder",
      expectedValue: "[iMessage app]",
      label: "fixed placeholder when fallback is absent",
      part: {
        type: "imessage_app",
      },
      privateSentinels: [],
    },
  ])("stages only iMessage app $label in the active-member canonical mailbox", async ({
    eventId,
    expectedValue,
    part,
    privateSentinels,
    ...
    for (const privateSentinel of privateSentinels) {
      expect(JSON.stringify(envelope)).not.toContain(privateSentinel);
    }
  ...
  it("keeps an unknown direct app card without fallback contentless when admission is off", async () => {
    ...
    const response = await handleHostedOnboardingLinqWebhook({
      ...
      rawBody: buildHostedLinqWebhookBody({
        ...
        eventId: "evt_contentless_app_direct",
        service: "iMessage",
      }),
      signature: null,
      timestamp: null,
  ...
  it("keeps an unknown group app card without fallback contentless when admission is off", async () => {
    mocks.hostedOnboardingEnvironment.linqFirstContactAdmissionMode = "off";
    const prisma = asPrismaTransactionClient({
      hostedInvite: {
        create: vi.fn(),
    ...
    const response = await handleHostedOnboardingLinqWebhook({
      ...
      rawBody: buildHostedLinqWebhookBody({
        data: {
          chat: {
            id: "chat_contentless_app_group",
            is_group: true,
            owner_handle: {
              handle: "+15550000000",
              id: "handle_owner_contentless_app_group",
              is_me: true,
              service: "iMessage",
            },
          },
        ...
        eventId: "evt_contentless_app_group",
        service: "iMessage",
      }),
      signature: null,
      timestamp: null,
    ...
    try {
      await expect(handleHostedOnboardingLinqWebhook({
        ...
        rawBody: buildHostedLinqWebhookBody({
          ...
          eventId: "evt_direct_consent_drift_root_failure",
        }),
        signature: null,
        timestamp: null,
      })).resolves.toMatchObject({
  ...
  it.each(["control", "ingress"] as const)(
    ...
    async (failureDomain) => {
      ...
      prisma.hostedLinqDelivery.findMany.mockResolvedValueOnce([{
        groupJoinOutreach: {
          id: "outreach_eligible_but_unread",
          offer: {
            group: {
              id: "group_eligible_but_unread",
              joinCode: "join_eligible_but_unread",
              runtimeMember: { suspendedAt: null },
              runtimeMemberId: "member_group_runtime",
            },
            revokedAt: null,
        ...
        groupJoinOutreachId: "outreach_eligible_but_unread",
        id: "delivery_eligible_but_unread",
        linqChatLookupKey: createHostedLinqChatLookupKey("chat_123"),
        phoneNumberLookupKey: createHostedPhoneLookupKey("+15550000000"),
      }]);
      ...
      try {
        await expect(handleHostedOnboardingLinqWebhook({
          ...
          rawBody: buildHostedLinqWebhookBody({
            ...
            eventId: `evt_direct_${failureDomain}_failure_owner_boundary`,
          }),
          signature: null,
          timestamp: null,
        })).rejects.toBe(preparationError);

        expect(providerDomains.sort()).toEqual(["control", "ingress"]);
        expect(providerDomainsAfterTransactionStart).toEqual([]);
        expect(prisma.$transaction).toHaveBeenCalledTimes(1);
  ...
  it("starts the Web reply beside shell prewarm and lets the runtime wait on its receipt", async () => {
    mocks.hostedOnboardingEnvironment.linqInstantStartPhonePrefixes = ["+1"];
    ...
    expect(callOrder).not.toContain("conversation-signal");
    expect(callOrder).not.toContain("activation-continuation");
  ...
  it("applies standalone opt-out blocking only to SMS-like or unknown phone first-contact services", () => {
    ...
    expect(resolveHostedLinqFirstContactContentDisposition({
  ...
  it("preserves media-only first-contact signup when admission is off", async () => {
    mocks.hostedOnboardingEnvironment.linqFirstContactAdmissionMode = "off";
    const invite = {
      channel: "linq",
      id: "invite_123",
  ...
  it("routes sparse webhook payloads when the saved home chat matches even if the incoming line is missing", async () => {
    ...
    unwrapRoot.mockImplementation(async (...args) => {
      expect(transactionOpen).toBe(false);
      providerPhases.push(args[0].domain);
      return defaultUnwrapRoot(...args);
    });
    mocks.lockAndReadActiveHostedDomainRootKeyIdTx.mockImplementation(
      async (...args: unknown[]) => {
    ...
    const hostedMemberRouting = createStatefulHostedMemberRoutingMock({
      linqChatIdEncrypted,
      linqChatLookupKey: createHostedLinqChatLookupKey("chat_home"),
      linqParticipantContactKind: "phone",
      linqParticipantContactLookupKey: createHostedPhoneLookupKey(
        "+15551234567",
      ),
      linqRecipientPhoneEncrypted,
      linqRecipientPhoneLookupKey: createHostedPhoneLookupKey(
        "+15550100001",
      ),
      memberId: "member_123",
      pendingLinqChatIdEncrypted: null,
    ...
    const prisma = asPrismaTransactionClient({
      ...
      hostedMember: {
        findUnique: vi.fn().mockResolvedValue({
          ...
          invites: [],
          phoneLookupKey: "+15551234567",
          routing: await hostedMemberRouting.findUnique({
    ...
    const executeRawImplementation = prisma.$executeRaw.getMockImplementation() as
      ...
      | undefined;
    if (!queryRawImplementation || !executeRawImplementation) {
      throw new Error("Expected the sparse direct lock-order Prisma mocks.");
    }
    prisma.$queryRaw.mockImplementation(async (
      query: readonly string[],
      ...values: unknown[]
