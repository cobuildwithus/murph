...
describe("hosted device-sync wakes", () => {
  beforeEach(() => {
    ...
    mocks.markConnectionSourceDataReceived.mockResolvedValue(1);
  ...
  it("reads current Apple registration outside transactions before phase-two activation and acceptance", async () => {
    ...
    mocks.listConnectionSources.mockResolvedValue([
      buildHostedConnectionSource("dsc_123", "apple_health_kit"),
    ]);
    const controlPlane = createHostedDeviceSyncPublicIngressService(
      new Request("https://control.example.test/api/device-sync/webhooks/junction", { method: "POST" }),
    );
  ...
  it("preserves the sole Junction source on webhook receipt signals", async () => {
    ...
    mocks.listConnectionSources.mockResolvedValue([
      buildHostedConnectionSource(connection.id, "health_connect"),
    ]);
  ...
  it("completes a verified Google Health migration without a browser session", async () => {
    const connection = buildHostedConnection({
      externalAccountId: "junction-user-established",
      id: "dsc_junction_fitbit_migration",
      provider: "junction",
      setupPhase: "source_confirmed",
    });
    const storedConnection = buildProviderConfigStoredConnection(connection);
    let sources = [
      buildHostedConnectionSource(connection.id, "fitbit", {
        resourceAvailabilitySummary: {
          ...
          canonicalCoverageFinalizedAt_sleep: "2026-08-11T10:06:00.000Z",
          canonicalCoverageFinalizedAt_steps: "2026-08-12T05:00:00.000Z",
          sleep: true,
          steps: true,
        },
      }),
  ...
  it("keeps legacy Fitbit active when durable Google Health verification is incomplete", async () => {
    const connection = buildHostedConnection({
      externalAccountId: "junction-user-established",
      id: "dsc_junction_fitbit_pending",
      provider: "junction",
      setupPhase: "source_confirmed",
    });
    const sources = [
  ...
  it("retries source-attributed webhook work when the cutover claim wins admission", async () => {
    ...
    const disconnectingFitbit = buildHostedConnectionSource("dsc_123", "fitbit", {
      lastErrorCode: "SOURCE_DISCONNECT_IN_PROGRESS",
      status: "connected",
    });
    mocks.listConnectionSourceAdmissionCandidates.mockResolvedValue([
      buildHostedConnectionSourceAdmissionCandidate(disconnectingFitbit),
    ]);
    const store = new PrismaDeviceSyncControlPlaneStore({
      prisma: getPrisma(),
    });
