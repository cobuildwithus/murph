...
import { fetchClinicalRetrievalDocument } from "@/src/lib/clinical-records/retrieval";
...
export const POST = withClinicalJsonError(async (request: Request) => {
  ...
  const parsed = hostedClinicalRecordsFetchDocumentRequestSchema.safeParse(
    await readJsonObject(request, { limitBytes: BODY_LIMIT_BYTES }),
  );
  if (!parsed.success) throw invalidRequestError();
  const result = await fetchClinicalRetrievalDocument({ memberId, request: parsed.data });
