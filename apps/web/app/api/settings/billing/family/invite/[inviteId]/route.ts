...
import { revokeHostedFamilyInviteTx } from "@/src/lib/hosted-onboarding/family-plan";
...
export const DELETE = withJsonError(async (
  ...
  const inviteId = await resolveDecodedRouteParam(context.params, "inviteId");
  ...
  const revoked = await prisma.$transaction(async (tx) => {
    ...
    return revokeHostedFamilyInviteTx({
      groupId: group.id,
      inviteId,
  ...
  if (!revoked) {
    throw hostedOnboardingError({
      ...
      httpStatus: 404,
      message: "That family invite is no longer pending.",
    });
  }
