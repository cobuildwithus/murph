...
// Companion (iOS) sign-in token exchange. Auth is a Privy identity token in
// `Authorization: Bearer ...`, verified through the existing server-side
...
export const POST = withJsonError(async (request: Request) => {
  ...
  const session = await controlPlane.createSdkSignInSession(
    auth.member.id,
    COMPANION_DEVICE_SYNC_PROVIDER,
  );
