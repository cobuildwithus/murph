...
export const POST = withJsonError(async (request: Request) => {
  enforceCompanionAuthDiagnosticThrottle(request);
...
function enforceCompanionAuthDiagnosticThrottle(request: Request): void {
  const now = Date.now();
  ...
  if (authDiagnosticThrottleTotal >= COMPANION_AUTH_DIAGNOSTIC_AGGREGATE_LIMIT) {
    throwRateLimited("Too many companion auth diagnostics.");
  }
  ...
  if (count > COMPANION_AUTH_DIAGNOSTIC_THROTTLE_LIMIT) {
    throwRateLimited("Too many companion auth diagnostics from this client.");
  }
...
function throwRateLimited(message: string): never {
  throw deviceSyncError({
