  ...
  try {
    result = await runWithHostedDeviceSyncPreparedWriteReplan(async () => {
      ...
      async (tx) => {
        ...
        return {
          shouldPrepareWake: status === "migration_pending"
            ? true
            : hasPayloadResources
              ? false
      ...
      if (initialAdmission.status === "completed") {
        return { retrySourceAfterCommit: false, wakeMailboxItemId: null };
      }

      const preparedDirty = initialAdmission.status === "ready" && hasPayloadResources
      ...
      return runWithHostedDomainRootProviderCallsDisabled(() =>
        ...
        async (tx) => {
          ...
          if (finalAdmission === "migration_pending") {
            if (!preparedMailbox) {
              throw createHostedDeviceSyncDirtyPreparationMismatchError();
            }
          ...
          return { retrySourceAfterCommit: false, wakeMailboxItemId };
        },
        {
          memberRowLockTimeoutMs: WEBHOOK_ADMISSION_MEMBER_ROW_LOCK_TIMEOUT_MS,
        },
...
async function inspectHostedDeviceSyncWebhookAdmissionTx(
...
): Promise<"completed" | "migration_pending" | "ready"> {
  ...
  if (!isHostedConnectionSourceAdmitted(dataSources, dataSourceProviderSlug)) {
    if (
      dataSource.status === "disconnected"
      && dataSource.lastErrorCode === null
      && new Date(dataSource.lastSeenAt).getTime() <= Date.parse(input.acceptedAt)
    ) {
      throw webhookSourceNotReadyError();
    }
    await completeHostedWebhookTraceTx(input, tx);
...
function webhookSourceNotReadyError(): ReturnType<typeof deviceSyncError> {
