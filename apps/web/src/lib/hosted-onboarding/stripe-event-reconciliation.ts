...
async function reconcileHostedFamilyTransitionUnderVerifiedOwnerLock(input: {
  ...
  return {
    ...
    preparedCryptoDomainRoots:
      input.processingContext.preparedCryptoDomainRoots,
    preparedFamilyCryptoDomainRoots:
      input.processingContext.preparedFamilyCryptoDomainRoots,
  };
}
  ...
  try {
    ...
    const preflightProcessingContext =
      ...
      && hostedStripeEventNeedsFamilyPreparationContext(stripeEvent)
        ? await prepareHostedStripeEventProcessingContext(stripeEvent, prisma)
        : undefined;
...
function hostedStripeEventMayActivateDirectMember(
  ...
  const checkoutOffer = parseHostedBillingCheckoutOffer(
    session.metadata?.checkoutOffer,
  );
  return checkoutOffer === HOSTED_PULSE_TRIAL_OFFER ||
