...
export async function readHostedLinqProductionCanaryOutcome(input: {
  ...
  try {
    return await readOutcome(input.prisma);
  } catch {
    // Decryption/control errors can include private refs. Keep this boundary metadata-only.
...
async function readOutcome(prisma: HostedOnboardingReadClient): Promise<LinqProductionCanaryOutcome> {
  ...
  const session = parseBrowserVaultSessionResponse(await control.createBrowserVaultSession({
