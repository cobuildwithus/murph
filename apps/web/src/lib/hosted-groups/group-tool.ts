...
import {
  resolveHostedLinqThreadRosterParticipants,
  selectHostedLinqThreadRosterHandles,
...
export async function handleHostedRuntimeGroupTool(input: {
  includeHostedGroupMembership?: boolean;
  memberId: string;
  ...
  if (input.request.action === "read_chat_participants") {
    return handleHostedRuntimeGroupReadChatParticipants({
      includeHostedGroupMembership: input.includeHostedGroupMembership ?? false,
      linqThread: input.request.linqThread ?? null,
      memberId: input.memberId,
    });
  }
...
async function authorizeHostedRuntimeGroupLinqThread(input: {
  ...
  try {
    const authorized = await assertHostedLinqRouteEgressAuthority({
      authority,
      ...(input.includeRosterSnapshot ? { includeRosterSnapshot: true } : {}),
      prisma: getPrisma(),
    });
    return {
      chatId,
      ownerMemberId: authorized.route.owner.id,
      rosterSnapshot: authorized.rosterSnapshot,
    };
  } catch (cause) {
    if (
      ...
      return { unavailableReason: "provider_unavailable" };
    }
    return { unavailableReason: "linq_thread_unauthorized" };
  }
}
...
async function handleHostedRuntimeGroupReadChatParticipants(input: {
  ...
  const handles = authorized.rosterSnapshot?.handles ?? [];
  ...
  const prisma = getPrisma();
  const rosterSelection = selectHostedLinqThreadRosterHandles(handles);
  if (rosterSelection.reason) {
    return unavailable("provider_unavailable");
  }
  const participantHandles = rosterSelection.handles;
  const classifiedParticipants: Array<{
    handle: string;
    hasOwnMurph: boolean;
  ...
  try {
    ...
    const participantMemberIdByHandle = new Map(
    ...
    for (const handle of participantHandles) {
      const participantMemberId = participantMemberIdByHandle.get(handle.handle) ?? null;
      classifiedParticipants.push({
    ...
    let hostedGroupMemberIds: Set<string> | null = null;
    let unknownHostedGroupMemberId: string | null = null;
    if (input.includeHostedGroupMembership) {
      const group = await prisma.hostedGroup.findUnique({
        ...
        select: {
          members: {
            where: {
              memberId: {
                in: resolvedParticipants.map((participant) => participant.participantMemberId),
              },
            },
      ...
      hostedGroupMemberIds = new Set(
        group?.members.map((member) => member.memberId) ?? [],
      );
      if (!group) {
        // Creating the first join offer also creates the hosted group and adds
        // the route owner. Until then, the owner's membership is unknown, not
        // evidence that someone in the chat needs an invitation.
        unknownHostedGroupMemberId = authorized.ownerMemberId;
      }
    }
    participants = classifiedParticipants.map(
      ({ participantMemberId, ...participant }) => ({
        ...participant,
        ...(hostedGroupMemberIds === null
          || (unknownHostedGroupMemberId !== null
            && participantMemberId === unknownHostedGroupMemberId)
          ? {}
          : {
            isHostedGroupMember: participantMemberId !== null
              && hostedGroupMemberIds.has(participantMemberId),
          }),
      }),
    );
  } catch {
    // A failed identity, access, or requested group-membership lookup must not
    // degrade into guessed participant state or an unstructured route error.
    return unavailable("membership_lookup_unavailable");
  }
