...
export const INITIAL_APPROVAL_ENROLLMENT_KIND = "approval.passkey.initial-enrollment";
...
export async function isInitialApprovalEnrollmentEligible(prisma: Client, session: HostedAppSession): Promise<boolean> {
  const [user, approval] = await Promise.all([
    prisma.hostedAuthRecord.findUnique({ where: { model_id: { model: "user", id: session.member.id } }, select: { memberId: true } }),
    prisma.hostedMemberApprovalCredentials.findUnique({ where: { memberId: session.member.id }, select: { memberId: true } }),
  ]);
  return Boolean(user?.memberId === session.member.id && approval === null);
}

function requireFreshPrimary(session: HostedAppSession, credentialsChangedAt?: unknown) {
  ...
  if (!session.authProof || !Number.isFinite(age) || age < 0 || age > 5 * 60_000
    || (credentialsChangedAt instanceof Date && credentialsChangedAt > session.primaryAuthenticatedAt!)) {
...
async function prepareRepair(input: Input) {
  ...
  if (!await isInitialApprovalEnrollmentEligible(input.prisma, input.session)) throw changed();
  requireFreshPrimary(input.session);
  const bound = await prepareHostedBoundLogin(input.prisma, input.session.member.id);
  requireFreshPrimary(input.session, bound.current.user.credentialsChangedAt);
  ...
  const bindingHash = digest(JSON.stringify([
    "murph-initial-approval-enrollment-v1", input.session.member.id, input.session.sessionId,
...
async function assertRepairCurrentTx(input: Input, prepared: Awaited<ReturnType<typeof prepareRepair>>, tx: Prisma.TransactionClient) {
  ...
  if (!await isInitialApprovalEnrollmentEligible(tx, input.session)) throw changed();
}

export async function createInitialApprovalEnrollmentOptions(input: Input) {
  return runWithFreshHostedDomainRootUnwrapCache(async () => {
    ...
    const material = createSensitiveActionChallengeMaterial({
      bindingHash: prepared.bindingHash, expiresAt, kind: INITIAL_APPROVAL_ENROLLMENT_KIND, origin: prepared.origin,
    });
    ...
    await input.prisma.$transaction((tx) => runWithHostedDomainRootProviderCallsDisabled(async () => {
      ...
      await tx.hostedSensitiveActionChallenge.deleteMany({ where: { memberId: input.session.member.id, kind: INITIAL_APPROVAL_ENROLLMENT_KIND } });
      await tx.hostedSensitiveActionChallenge.create({ data: {
        bindingHash: prepared.bindingHash, createdAt: now, expiresAt, kind: INITIAL_APPROVAL_ENROLLMENT_KIND,
...
export async function registerInitialApprovalEnrollment(input: Input & { token: unknown; response: RegistrationResponseJSON }): Promise<void> {
  return runWithFreshHostedDomainRootUnwrapCache(async () => {
    const prepared = await prepareRepair(input);
    ...
    if (!challenge || challenge.memberId !== input.session.member.id || challenge.kind !== INITIAL_APPROVAL_ENROLLMENT_KIND
      || challenge.bindingHash !== prepared.bindingHash || challenge.expiresAt <= new Date()) throw changed();
    const message = buildSensitiveActionMessage({ bindingHash: prepared.bindingHash, kind: INITIAL_APPROVAL_ENROLLMENT_KIND,
    ...
    await input.prisma.$transaction((tx) => runWithHostedDomainRootProviderCallsDisabled(async () => {
      ...
      await consumeSensitiveActionChallengeTx({ prisma: tx, challenge: {
        ...challenge, kind: INITIAL_APPROVAL_ENROLLMENT_KIND, credentialWrite, passkeys: [credential],
      } });
      // Keep this freshly proved session and revoke all other first-party
      // sessions using the same owners as saved-key recovery.
      const now = new Date();
