...
import { createLegacyApprovalRepairOptions, registerLegacyApprovalRepair } from "./legacy-passkey-repair";

const optionsSchema = z.object({}).strict();
const registerSchema = z.object({ legacyRepairToken: z.string().max(128), response: z.unknown() }).strict();

export async function legacyApprovalRepairRequest(request: Request, operation: "options" | "register",
  ...
  for (const [key, max] of [[`legacy-approval-repair:member:${session.member.id}`, 20],
    [`legacy-approval-repair:ip:${hostedAuthRequestIp(request)}`, 100]] as const) {
  ...
  if (operation === "options" && optionsSchema.safeParse(body).success) return jsonOk(await createLegacyApprovalRepairOptions(input));
  ...
  await registerLegacyApprovalRepair({ ...input, token: parsed.data.legacyRepairToken,
