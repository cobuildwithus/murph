  ...
  mergeHostedAccountDeletionOwnerCounts(
    ...
    await readHostedAccountDeletionOwnerCounts(
      ...
      Prisma.sql`
        ...
        deleted_device_provider_setups AS (
          DELETE FROM device_provider_setup AS setup
          WHERE setup.member_id IN (SELECT id FROM target_members)
          RETURNING 1
        ),
        ...
        SELECT
          ...
          (SELECT count(*) FROM deleted_device_provider_setups)
            AS "prisma.device_provider_setup",
    ...
    try {
      ...
      if (!revokeAccess) {
        if (cleanup.repairRequired) {
          // Permanently malformed application state is a repair condition,
          // not a blocking revocation failure.
          results.push({
            connectionId: connection.id,
            errorCode: null,
            providerLabel: resolveDeviceConnectionProviderLabel(connection),
            status: "warning",
            warningCode: cleanup.warning?.code
              ?? "DEVICE_PROVIDER_APPLICATION_REPAIR_REQUIRED",
          });
          continue;
        }
        results.push({
