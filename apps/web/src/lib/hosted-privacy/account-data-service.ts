...
export const HOSTED_ACCOUNT_DATA_STORE_COVERAGE = [
  ...
  {
    slug: "prisma.hosted_call_circle_participant",
    label: "Call Circle participants",
    deletion: "live-delete",
    note: "Deletes the member's Call Circle enrollment and every enrollment attached to a deleted group before relation cascades run.",
  },
  {
    slug: "prisma.hosted_call_circle_match",
    label: "Call Circle matches",
    deletion: "live-delete",
...
export async function deleteHostedAccountData(input: {
  ...
  const databaseDeletion: HostedAccountDeletionDatabaseResult = await input.prisma.$transaction(async (tx) => {
    ...
    const transactionFenceMemberIds = sortedUniqueStrings([
      ...
      ...familyBeneficiaryMemberIds,
    ]);

    for (const memberId of transactionFenceMemberIds) {
...
function buildHostedAccountPhoneCallWhere(
  memberIds: readonly string[],
  ...
  return {
    OR: [
      ...
      {
        callCircleMatch: {
          is: buildHostedAccountCallCircleMatchWhere(memberIds),
        },
      },
    ],
  };
...
function buildHostedAccountCallCircleParticipantWhere(
  memberIds: readonly string[],
): Prisma.HostedCallCircleParticipantWhereInput {
  return {
    OR: [
      { memberId: buildStringInFilter(memberIds) },
      { group: buildHostedAccountGroupWhere(memberIds) },
    ],
  };
}

function buildHostedAccountCallCircleMatchWhere(
  memberIds: readonly string[],
): Prisma.HostedCallCircleMatchWhereInput {
  const memberIdFilter = buildStringInFilter(memberIds);
  return {
    OR: [
      { memberAId: memberIdFilter },
      { memberBId: memberIdFilter },
      { group: buildHostedAccountGroupWhere(memberIds) },
    ],
  };
}

function buildHostedAccountGroupWhere(
  memberIds: readonly string[],
): Prisma.HostedGroupWhereInput {
  const memberIdFilter = buildStringInFilter(memberIds);
  return {
    OR: [
      { ownerMemberId: memberIdFilter },
