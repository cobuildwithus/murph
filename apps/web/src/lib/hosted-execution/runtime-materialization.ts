...
import { matchesHostedRuntimeMigrationRelease, type HostedRuntimeMigrationCompatibility, resolveHostedRuntimeMemberBackend, type HostedRuntimeBackend } from "@murphai/hosted-execution/runtime-migration";
import { lockHostedMemberRow } from "../hosted-onboarding/shared";

type Input = { prisma: PrismaClient; userId: string; objectId: string; workerVersion: string; compatibility?: HostedRuntimeMigrationCompatibility };
...
export async function resolveHostedLegacyMaterialization(input: Input) {
  return input.prisma.$transaction(async tx => {
    const gates = await tx.$queryRaw<Array<{ phase: string; worker_version: string | null; namespace_probe_id: string | null; creation_closed_at: Date | null }>>`
      SELECT phase, worker_version, namespace_probe_id, creation_closed_at FROM hosted_runtime_cutover WHERE id = 'runtime' FOR SHARE
    `;
    const gate = gates[0];
    if (!gate) throw new Error("Hosted runtime cutover state is missing.");
    if (gate.phase === "rolling" && !matchesHostedRuntimeMigrationRelease({ workerVersion: gate.worker_version, namespaceProbeId: gate.namespace_probe_id }, input)) return { cutover: "draining" as const, owner: null };
