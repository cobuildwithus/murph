...
export function useInitialApprovalEnrollment() {
  ...
  async function repair() {
    ...
    const getOptions = () => requestHostedOnboardingJson<{ options: PublicKeyCredentialCreationOptionsJSON; token: string }>({
      url: "/api/settings/approval-passkeys/initial-options", method: "POST", payload: {}, signal,
    });
    try {
      ...
      await requestHostedOnboardingJson({ url: "/api/settings/approval-passkeys/register", method: "POST",
        payload: { initialToken: registration.token, response }, signal });
      signal.throwIfAborted();
    } finally {
      active.current = null;
