...
- Web derives one read-only member plan-usage projection from that same allowance resolver and usage ledger for Settings and `murph.plan_usage`. It persists no forecast and performs no Stripe read. Eligible direct paid Pulse and Edge members may receive the thresholded `add_usage` action; the authenticated Settings surface exposes the fixed $5, $10, and $25 catalog. An opted-in `subscriptionActionQuote` remains separate current terms for an explicit subscription request, not a recommendation or consent.
