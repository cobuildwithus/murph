...
- `GET /blog` renders the public guide, field-note, and verified-case-study
  archive from the typed registry in `src/lib/blog.ts`.
- `GET /blog/:slug` renders one statically generated article.
- `GET /blog/rss.xml` and `GET /blog/sitemap.xml` publish article discovery
  metadata.
- Case-study entries require confirmed consent, a result summary, a verification
  date, at least one bounded evidence-basis label, and an explicit limitations
  note. The archive and RSS copy advertise case studies only after one is
  actually published.
...
### Manual Search Console topic intake

The blog has no reporting cron and does not auto-publish content. An operator
can run the twelve-month topic-gap intake on demand:

```bash
MURPH_GSC_CREDENTIALS_FILE=/absolute/path/outside/the/repository/search-console.json \
  pnpm --dir apps/web seo:opportunities
```

The Google service account must have read access to the Search Console property
`sc-domain:withmurph.ai`. Override the property only when necessary with
`MURPH_GSC_SITE_URL`. The command reads the credential file in memory, rejects
a credential path inside the repository, requests only the
`webmasters.readonly` scope, and writes a spreadsheet-safe CSV beneath the
ignored `.artifacts/seo/` directory. It ranks high-impression queries whose
landing page is not a dedicated blog article, or whose current article does not
meaningfully match the query. A `case-study-candidate` output is an editorial
prompt only; the registry still requires a verified result and confirmed
permission before publication. If the query export reaches its 100,000-row
safety limit, the command keeps the bounded CSV but reports that the intake is
partial instead of calling it complete.
