...
- Family conversion reuses an exact active direct paid or Trial Subscription in place under the owner lock; a Trial ends immediately and Trial-only metadata is cleared. Family invitation issue rejects an exact current owner/member contact before capacity purchase, and acceptance repeats that check transactionally. Suspended direct members retain exact-customer Portal management, inactive Family billing projects a Portal recovery action, and sponsorship payers retain cancellation-only management after beneficiary authority disappears. Late competing direct invoices are canceled and refunded only through the exact ordinary one-payment invoice owner; complex refund shapes remain support-required.
