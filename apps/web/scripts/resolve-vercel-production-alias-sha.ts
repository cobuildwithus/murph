...
export async function resolveVercelProductionDeployment(
  ...
  if (extractVercelDeploymentProjectId(deploymentResponse) !== configuredProjectId) {
    throw new Error(
      "Vercel production alias resolves to a different project than HOSTED_WEB_VERCEL_PROJECT_ID.",
    );
  }
