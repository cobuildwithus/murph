...
describe("runner bundle container-entrypoint esbuild step", () => {
  ...
  it("resolves the production budgets as the ratcheted baselines plus tolerance", () => {
    ...
    // Entry = measured CI Linux baseline (1,474,490B after the accepted
    // delivery continuation) + 48,000B noise band. Static closure = measured
    // CI Linux baseline (7,078,454B for the same PR head) + 96,000B noise
    // band.
    // Locking exact values makes any silent change to a ratchet a failing,
    // reviewed diff.
    expect(budgets).toEqual({
      entryBytes: 1_474_490 + 48_000,
      staticClosureBytes: 7_078_454 + 96_000,
