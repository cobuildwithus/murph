...
describe("runner bundle container-entrypoint esbuild step", () => {
  ...
  it("resolves the production budgets as the ratcheted baselines plus tolerance", () => {
    ...
    // Entry = measured CI Linux baseline (1,473,336B after PR #605 added
    // causal mailbox attribution) + 48,000B noise band. Static closure =
    // measured CI Linux baseline (7,060,222B after persisted provider-effect
    // authority) + 96,000B noise band.
    // Locking exact values makes any silent change to a ratchet a failing,
    // reviewed diff.
    expect(budgets).toEqual({
      entryBytes: 1_473_336 + 48_000,
      staticClosureBytes: 7_060_222 + 96_000,
