...
describe("hostedRunnerIntercept", () => {
  ...
  it("injects OpenAI authorization from versioned container identity when active runtime context is missing", async () => {
    const fetchMock = vi.fn<typeof fetch>(async () => new Response("ok"));
    vi.stubGlobal("fetch", fetchMock);
    const validateActiveRuntimeWriteFence = vi.fn(async (input: {
      userId: string;
  ...
  it("rejects container-identity OpenAI egress when the active write fence is missing", async () => {
    const fetchMock = vi.fn<typeof fetch>(async () => new Response("unexpected"));
    vi.stubGlobal("fetch", fetchMock);
    ...
    expect(mocks.emitHostedExecutionStructuredLog).toHaveBeenCalledWith(
      expect.objectContaining({
        details: expect.objectContaining({
          ...
          writeFenceValidationMode: "active_user_fence",
          writeFenceValidationRejectReason: "missing_write_fence",
        }),
        message: "Hosted runner provider egress completed.",
      }),
    );
  ...
  it("rejects container-identity OpenAI egress when a bound user header names another user", async () => {
    const fetchMock = vi.fn<typeof fetch>(async () => new Response("unexpected"));
    vi.stubGlobal("fetch", fetchMock);
    const validateActiveRuntimeWriteFence = vi.fn(async (input: {
      userId: string;
