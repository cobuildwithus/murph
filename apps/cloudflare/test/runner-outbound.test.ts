...
describe("handleRunnerOutboundRequest", () => {
  ...
  it.each([
    {
      expectedHeadCount: 1,
      foundBucketName: "bundles-test",
      label: "single source bucket",
      sourceActiveBridge: false,
    },
    {
      expectedHeadCount: 2,
      foundBucketName: "bundles-enam-test",
      label: "source-active bridge destination fallback",
      sourceActiveBridge: true,
    },
  ])("locates hosted-local direct-R2 snapshots through the local S3 control endpoint: $label", async ({
    expectedHeadCount,
    foundBucketName,
    sourceActiveBridge,
  }) => {
    ...
    const sourceBucket = sourceActiveBridge
      ? createRunnerOutboundEnv().BUNDLES
      : createWorkspaceSnapshotBucket(
    ...
    const fetchMock = vi.fn(async (
      ...
      if (url.origin === "http://127.0.0.1:39000" && method === "HEAD") {
        ...
        return new Response(null, {
          status: bucketName === foundBucketName ? 200 : 404,
        });
      }
    ...
    expect(new URL(requireTestString(body.getUrl, "hosted-local snapshot getUrl")).pathname)
      .toBe(`/${foundBucketName}/${objectKey}`);
    expect(bindingHead).not.toHaveBeenCalled();
    expect(fetchMock).toHaveBeenCalledTimes(expectedHeadCount);
  });
