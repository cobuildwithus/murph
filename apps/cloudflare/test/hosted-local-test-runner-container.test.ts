...
describe("hosted-local test RunnerContainer outbound composition", () => {
  ...
  it("observes an aborted canonical checkpoint before explicit reconciliation", async () => {
    const userId = "member_canonical_checkpoint_barrier_aborted";
    const realHandler = vi.fn(async () => new Response("checkpoint committed", { status: 200 }));
    const handler = wrapShutdownCheckpointPublicationBarrierForTest(realHandler);
    const abortController = new AbortController();
    const abortReason = new Error("Exact runtime-owner replacement aborted the checkpoint.");
    const checkpointRequest = new Request(createCanonicalCheckpointRequest(userId), {
      signal: abortController.signal,
    });
    armCanonicalCheckpointPublicationBarrier(userId);
    ...
    try {
      ...
      await expect(heldPublication).rejects.toBe(abortReason);
      expect(realHandler).not.toHaveBeenCalled();
      expect(readShutdownCheckpointPublicationBarrierState(userId)).toBe("aborted");
  ...
  it("observes a canonical checkpoint request that was already aborted", async () => {
    const userId = "member_canonical_checkpoint_barrier_pre_aborted";
    const realHandler = vi.fn(async () => new Response("checkpoint committed", { status: 200 }));
    const handler = wrapShutdownCheckpointPublicationBarrierForTest(realHandler);
    const abortController = new AbortController();
    const abortReason = new Error("The exact runtime owner was already cancelled.");
    abortController.abort(abortReason);
    armCanonicalCheckpointPublicationBarrier(userId);

    try {
      await expect(handler(
        new Request(createCanonicalCheckpointRequest(userId), {
          signal: abortController.signal,
        }),
        createOutboundEnv(),
        { containerId: "opaque-container-id" },
