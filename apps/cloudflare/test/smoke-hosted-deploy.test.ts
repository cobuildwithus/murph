...
describe("runSmokeHostedDeploy", () => {
  ...
  it("retries transient HTTP 5xx live model turn smoke responses", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "cloudflare-smoke-live-model-5xx-retry-"));
    const manifestPath = path.join(root, ".deploy", "runner-bundle", ".murph-runner-bundle-manifest.json");
    await mkdir(path.dirname(manifestPath), { recursive: true });
    await writeFile(
      manifestPath,
    ...
    const fetchImpl = async (url: RequestInfo | URL) => {
      ...
      if (
        ...
        if (liveSmokeAttempts === 1) {
          return new Response(JSON.stringify({
            detail: "container temporarily unavailable",
            error: "Deploy container smoke failed.",
            ok: false,
          }), { status: 503 });
        }
