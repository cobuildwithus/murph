...
Prepared restore and runner fallback snapshot `GET` URLs must carry `X-Amz-Expires=600` (exactly ten minutes). Snapshot `PUT` URLs remain at 600 seconds; this hardening does not change their lifetime. Existing valid v2 objects and required legacy refs remain restore-compatible, so no web/Cloudflare tandem deploy or snapshot-format migration is required. Deploy the Worker and runner bundle normally; during a gradual container rollout, old containers can consume the shorter Worker-minted capability while new containers add pre-extraction inventory validation. A rollback remains format-compatible but removes that validation from rolled-back containers. After deploy, confirm both prepared and fallback restore URLs report the ten-minute expiry and that archive failures emit only classified tar/zstd metadata without stderr detail fields.
