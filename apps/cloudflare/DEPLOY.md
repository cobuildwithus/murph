...
- `HOSTED_EXECUTION_IDLE_CHECKPOINT_DELAY_MS` defaults to `90000` and production rejects lower values. Deploy the code while retaining an explicit `180000` value, then canary `90000` and compare checkpoint attempts, snapshot bytes, foreground latency, owner-recheck cadence, and new-invocation volume before widening. Accepted start/wake owner rechecks move from 230 seconds to 140 seconds because orchestration recovery intentionally stays aligned with the earlier checkpoint horizon. The deploy variable is rendered through the ordinary optional Worker-variable path. This runtime-owned floor is independent of RunnerContainer lifecycle expiry.
