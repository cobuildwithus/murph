...
- if `HOSTED_EXECUTION_SMOKE_LIVE_MODEL_TURN=true`, the managed-container smoke runs one real `gpt-5.5` turn via `codex exec` inside the deployed container through the Worker OpenAI egress intercept
...
- `HOSTED_EXECUTION_SMOKE_LIVE_MODEL_TURN=true` to extend the managed-container smoke with one real `gpt-5.5` turn; requires `HOSTED_EXECUTION_SMOKE_RUNNER_CONTAINER=true`
