...
`CF_PUBLIC_BASE_URL` is required for the standard deploy-and-smoke flow because smoke targets the public Worker URL after deploy. Production preflight pins it to `https://murph-hosted.cobuildwithus.workers.dev`, the exact origin accepted by the private-media capability contract; change the shared contract and deploy invariant together before moving that origin. Runner internal-host requests use Cloudflare Container outbound interception instead of a public Worker callback route.
...
Hosted message images do not use Cloudflare Images. The runner stores generated bytes as canonical vault captures and final delivery uses Linq attachments or Telegram multipart upload. Linq group avatars remain available through the narrow `results.worker/private-image-urls` boundary: the Worker stores one deterministic, application-encrypted object in private R2 and returns an opaque one-day capability on the fixed `https://murph-hosted.cobuildwithus.workers.dev/private-media/v1/` origin for the immediate avatar mutation. The capability hides the member id, R2 key, storage namespace, and image hash; the public GET route decrypts and verifies the object and responds with `private, no-store`. Retry does not rewrite the object, and the 24-hour R2 lifecycle plus account deletion own cleanup without relying on Linq fetch timing. `HOSTED_PRIVATE_MEDIA_CAPABILITY_SECRET` is Worker-only, must contain at least 32 characters, and must not enter runner env. During this cutover, the workflow maps the existing GitHub environment secret named `CLOUDFLARE_IMAGES_SIGNING_KEY` into that new Worker variable so no secret value is copied or exposed; rename the GitHub secret in a later coordinated deploy. The legacy `results.worker/generated-images` route remains a `410 Gone` rolling-deploy tombstone.
...
`container_rollout=immediate`. Web temporarily accepts the previous signed
Images URL shape so an old Worker remains compatible during the short skew
window; the new Worker creates only encrypted R2 capabilities. Verify the
generated-image Linq attachment smoke, an R2-backed group-avatar mutation, the
private-media GET response headers, and the `410` tombstone. The Web deploy also switches the
...
brief window for signed legacy avatar URLs; an old generated-image runner
...
containers below it. Do not roll hosted Web back to the data-bearing card URL
implementation.
