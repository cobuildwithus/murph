...
- `HOSTED_EXECUTION_IDLE_CHECKPOINT_DELAY_MS` defaults to `60000` and production rejects lower values. Deploy the code while retaining an explicit `180000` value, then canary `60000` and compare checkpoint attempts, snapshot bytes, foreground latency, and new-invocation volume before widening. This runtime-owned floor is independent of RunnerContainer lifecycle expiry.
