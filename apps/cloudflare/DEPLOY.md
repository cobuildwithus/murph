...
The first shared preference-causal-sequence release uses a gated expand/switch rollout. Vercel predeploy first adds the nullable `causal_seq` column and unique index, then deploys the sequence-producing web build with `MURPH_ASSISTANT_PERSONALITY_CAUSAL_WRITES_ENABLED=0`; the old Cloudflare parser ignores the optional field. The normal post-deploy contract lane waits for old Vercel functions and requires sequences only after legacy unconsumed preference work has drained, failing closed for a later retry otherwise. Deploy this Cloudflare worker and runner bundle with the gate disabled and `container_rollout=immediate`, then prove the managed fleet has converged. Enable the Cloudflare gate before enabling the Vercel gate. Once either gate is enabled or positive preference watermarks exist, do not disable a gate or roll either plane back independently.
