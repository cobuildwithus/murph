...
- `HOSTED_ASSISTANT_MODEL`; worker deploy preflight requires an explicit allowance-priced launch model, currently `gpt-5.5` or `gpt-5.4-mini` for direct OpenAI. Production hot-reply deploys must use `gpt-5.4-mini` with `HOSTED_ASSISTANT_REASONING_EFFORT=low` to preserve the live reply latency SLO.
...
Run `pnpm --dir apps/cloudflare test:e2e:runner-control-token:local` when you specifically want the actual final hosted-runner app image proof for control-token recovery after supervisor memory loss. It assembles the runner bundle, builds the same `linux/amd64` app-layer Dockerfile used by the Cloudflare container, starts a surviving shell with an old `HOSTED_EXECUTION_RUNNER_CONTROL_TOKEN`, proves a newly minted token cannot control that shell, then destroys it and proves a fresh shell accepts only the new token.
...
The production deploy job uses Blacksmith only for that immediate break-glass shape (`deploy_worker=true`, `skip_predeploy_e2e=true`, and `container_rollout=immediate`). It then attaches the configured GitHub environment, downloads the immediate handoff, validates the runner-bundle tar entries before extraction, rejects unsafe archive entry types and symlink targets, loads the base image into Docker, validates the downloaded runner-bundle manifest against the protected-main checkout before any manifest refresh, renders env-specific deploy config and Worker secrets itself, refreshes the manifest timestamp for the newly rendered config, dry-runs the generated Wrangler deploy bundle, deploys directly with Wrangler, and runs the deployed endpoint smoke. This immediate path intentionally trusts protected-main Blacksmith runners for production deploy execution and secret-bearing Wrangler/smoke steps. Normal non-immediate deploys keep the production deploy job on GitHub-hosted Ubuntu after the Blacksmith gates pass, and render-only workflow runs skip the runner smoke/build-prep gates while still executing focused Cloudflare checks in the deploy job.
