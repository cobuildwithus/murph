...
Worker replacement is checkpoint-safe at the runtime fence rather than through rollout timing alone: if the exact outgoing attempt has started a durable snapshot-upload session within the last 15 seconds, `UserRunner` retains its fence and retries after one second. Other starts remain immediate, and an abandoned upload marker cannot defer replacement after that bounded handoff window.
