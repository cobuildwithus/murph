...
Personal-home Linq route-transition proof is also a producer/consumer contract change and requires consumer-first deployment. First deploy this Cloudflare/runner consumer with `container_rollout=immediate`, then require managed-container smoke to report the new runner-bundle fingerprint. Use audited production evidence to identify exact retained direct-Linq input IDs whose reply targets are each former personal route, and run `pnpm --dir packages/assistant-engine repair:legacy-personal-home-routes -- --vault-root <vault-root> --input-id <audited-input-id> --apply` for the affected vaults. The command is idempotent, uses the existing atomic canonical repair, does not scan input history, and exits nonzero on failure. Deploy Vercel/web only after the consumer fingerprint is verified; every admitted transition then binds the current home and carries exact former/current proof. Quota-rejected inputs remain ahead of both binding and mailbox append and therefore cannot consume a transition.
