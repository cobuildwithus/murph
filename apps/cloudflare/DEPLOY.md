...
Hosted message images do not use Cloudflare Images. The runner stores generated bytes as canonical vault captures and final delivery uses Linq attachments or Telegram multipart upload. Linq group avatars remain available through the narrow `results.worker/private-image-urls` boundary: the Worker uploads vault bytes through its `IMAGES` binding with signed delivery required and returns a one-day signed URL for the immediate avatar mutation. Configure the Worker-only `CLOUDFLARE_IMAGES_SIGNING_KEY` and optional `CLOUDFLARE_IMAGES_VARIANT`; neither may enter runner env. The selected variant must exist and must not enable Cloudflare's `Always allow public access` option. The deploy config provides the account-scoped Images binding, so no separate Images API token or account-id variable is required. The legacy `results.worker/generated-images` route remains a `410 Gone` rolling-deploy tombstone.

For the private-media cutover, enable a paid Images plan, configure the signing
key and optional variant, then
deploy Cloudflare/runner first with `container_rollout=immediate`. Verify the
generated-image Linq attachment smoke, a signed group-avatar mutation, and the
`410` tombstone, then deploy hosted Web immediately to switch the
...
brief window: an old runner against the new Worker receives `410` and falls
back to text, while a new runner no longer calls the image-upload route. Once a
