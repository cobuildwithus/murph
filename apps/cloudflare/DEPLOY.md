...
The first shared preference-causal-sequence release is a coordinated hard cut. Before applying its web migration, drain old `member.preferences.updated` work and prove the unconsumed row count is zero. The migration rejects any later unconsumed preference row without `causal_seq`, making an old producer fail visibly instead of losing an acknowledged save. Deploy this Cloudflare worker and runner bundle with `container_rollout=immediate`, prove the managed fleet has converged, and only then deploy the causal-sequence-producing Vercel build. After positive preference watermarks exist, do not roll either plane back independently.
