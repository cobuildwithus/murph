...
Personal-home Linq route-transition proof is also a producer/consumer contract change. First deploy this Cloudflare/runner consumer with `container_rollout=immediate` while Vercel keeps `HOSTED_LINQ_ROUTE_TRANSITION_PROOF_ENABLED=0` (the default), then require managed-container smoke to report the new runner-bundle fingerprint. Before enabling the producer, use audited production evidence to identify exact retained direct-Linq input IDs whose reply targets are each former personal route, and run `pnpm --dir packages/assistant-engine repair:legacy-personal-home-routes -- --vault-root <vault-root> --input-id <audited-input-id> --apply` for the affected vaults. The command is idempotent, uses the existing atomic canonical repair, does not scan input history, and exits nonzero on failure. Enable `HOSTED_LINQ_ROUTE_TRANSITION_PROOF_ENABLED=1` and redeploy Vercel only after every targeted repair succeeds. While the flag is off, admitted inbound messages still append and receive normal processing, but a home transition remains unbound so a later admitted inbound can reproduce proof; quota-rejected inputs likewise cannot consume the transition.

Once the producer flag has been enabled, or any transition-proof input/current-route snapshot has been persisted, this consumer commit is the hard Cloudflare/runner rollback floor. For an emergency web rollback, first restore the flag to `0`; do not roll Worker/runner below the floor while those records can exist. Post-deploy checks must prove a scheduled personal reminder resolves the current home route and a reply-anchored send remains on its matching inbound route.
