...
Deploy the backward-compatible billing/Family web producer before Cloudflare/runner. Cloudflare adds the current Family wire-contract version to new runner requests; unversioned warm legacy runners receive the exact legacy response without pricing or opaque member/invite ids. Gradual container rollout is safe for this surface. Verify the new runner-bundle fingerprint and signed billing/Family `read_status` before exercising a live mutation. On rollback, restore Cloudflare/runner first and keep the versioned web producer until old runners have drained.
