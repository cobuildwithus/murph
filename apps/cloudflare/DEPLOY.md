...
The final app-layer image generates a patched Codex model catalog from `codex debug models --bundled`, adds `gpt-5.5` flex service-tier support plus the `gpt-5.4-nano` deploy-smoke slug, validates those entries with `jq`, and exposes it through `MURPH_HOSTED_CODEX_MODEL_CATALOG_JSON` so hosted app-server turns can send OpenAI `service_tier: flex` and deploy smoke can run its bounded nano turn.
