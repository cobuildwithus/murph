...
Hosted generated images no longer require Cloudflare Images. The runner stores generated bytes as canonical vault captures and final delivery uses Linq attachments or Telegram multipart upload. The legacy `results.worker/generated-images` route is a `410 Gone` rolling-deploy tombstone: deploy Cloudflare with immediate container rollout, confirm old runners have drained, then remove the unreachable Cloudflare Images implementation and its `CLOUDFLARE_IMAGES_*` configuration in a follow-up.

For the private-media cutover, deploy Cloudflare/runner first with
`container_rollout=immediate`, verify the generated-image Linq attachment smoke
and the `410` tombstone, then deploy hosted Web immediately to switch the
...
# OPENAI_API_KEY.
# CLOUDFLARE_IMAGES_* is not required by private generated-image delivery.
