...
The expanded Family conversation-control response is a runner-parser hard cut: deploy its Cloudflare/runner consumer first with `container_rollout=immediate`, verify the new runner-bundle fingerprint, and only then deploy the web producer that emits pricing and opaque member/invite ids. Do not exercise the newly advertised mutations between those deploys. On rollback, restore web before runner.
