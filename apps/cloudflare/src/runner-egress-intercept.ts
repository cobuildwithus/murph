...
// ffmpeg-normalized audio. Remote-only parser passthrough may send much longer
// compressed originals, but the same byte cap keeps body + base64 + inference
// payload well inside the Worker isolate memory limit. Keep in sync with the
// parsers remote-transcription provider input cap.
