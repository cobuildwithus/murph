...
// The reviewed Telegram phone-call result callback extends the existing hosted
// runtime and Web-control graph without adding a forbidden boot input. Junction
// temporal authority, summary completeness, body composition, and bounded
// workout features extend the same lazy device-sync, importer, and query outputs
...
// platform tolerance.
// The merged bounded foreground-state work extends the existing Assistant
...
// Saved seven-day wearable views, the response-card tool, and bounded delivery
// receipts extend the existing lazy graph without adding a boot input. The
// combined macOS assembly measured 11,732,995B total on 2026-08-31; ratchet only
// the total baseline and retain the fixed allowance and startup-specific gates.
const RUNNER_ENTRYPOINT_BUNDLE_TOTAL_BYTES_BUDGET = 11_732_995 + 32_768;
