...
// existing Assistant Engine graph without adding a package. The combined
// reviewed Junction complete-summary and bounded workout-feature graph measured
// 9,188,582 B on Linux CI after merging the current stacked base on 2026-08-17;
// it grows the existing device-sync, importer, and query packages without
// adding a package. The static startup closure measured 24,950 B. Keep total
// output inside a narrow 32 KiB allowance and static startup inside an 8 KiB
// allowance. If a violation fires, investigate the listed largest inputs first;
// only raise the budget deliberately for understood, intended growth.
const VAULT_CLI_BUNDLE_TOTAL_BYTES_BUDGET = 9_221_350;
