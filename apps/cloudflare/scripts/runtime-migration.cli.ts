...
export function readRuntimeMigrationOperator(source: Source) {
  ...
  const { mode, maxSteps, activate } = readMigrationOptions(source);
  ...
  return { input, mode, maxSteps, activate };
}
...
function readMigrationOptions(source: Source) {
  ...
  const maxSteps = Number(source.MURPH_RUNTIME_MIGRATION_MAX_STEPS ?? "1");
  ...
  return { mode, maxSteps, activate: finalize === "true" };
}

export async function runRuntimeMigrationOperator(source: Source, fetchImpl?: typeof fetch) {
  const { input, mode, maxSteps, activate } = readRuntimeMigrationOperator(source);
  if (mode === "inventory") {
  ...
  return migrateHostedLegacyRuntime({ ...input, fetchImpl, maxSteps, activate });
}
