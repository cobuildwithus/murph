...
\echo 'Typing shell-prewarm lead buckets (milliseconds)'
WITH direct_candidates AS (
  SELECT
    ...
    trace.phase_breakdown_json #> '{boot}' AS boot,
    trace.phase_breakdown_json #> '{provider}' AS provider,
    EXTRACT(EPOCH FROM (trace.accepted_at - TIMESTAMP '1970-01-01')) * 1000 AS accepted_ms,
    count(*) OVER (
  ...
  WHERE trace.accepted_at >=
    ...
    AND trace.phase_breakdown_json #>> '{orchestration,shellPrewarmSource}' = 'linq-typing-started'
  ...
  SELECT
    ...
    provider,
    accepted_ms,
    accepted_ms - (orchestration ->> 'shellPrewarmFirstHintAtEpochMs')::double precision AS lead_ms,
    (orchestration ->> 'freshStartContainerReadyAtEpochMs')::double precision AS container_ready_ms
  FROM direct_candidates
  WHERE causal_candidate_count = 1
), labeled_samples AS (
  SELECT
    CASE
      WHEN lead_ms < 500 THEN '0-499ms'
      WHEN lead_ms < 1000 THEN '500-999ms'
      WHEN lead_ms < 2000 THEN '1000-1999ms'
      WHEN lead_ms < 3000 THEN '2000-2999ms'
      WHEN lead_ms < 4000 THEN '3000-3999ms'
      ELSE '4000ms+'
    END AS lead_bucket,
    'registered' AS prewarm_registration,
    CASE
      WHEN orchestration ->> 'shellPrewarmOutcome' = 'failed' THEN 'failed'
      WHEN (orchestration ->> 'shellPrewarmFinishedAtEpochMs')::double precision <= accepted_ms THEN 'healthy'
      ELSE 'uncertain'
    END AS prewarm_readiness,
    orchestration ->> 'shellPrewarmOutcome' AS prewarm_outcome,
    CASE (boot ->> 'restoreWasCold')
      WHEN 'true' THEN 'workspace_cold'
      WHEN 'false' THEN 'workspace_warm'
      ELSE 'workspace_unreported'
    END AS workspace_restore,
    CASE
      WHEN provider ? 'codexProcessPreparationMs'
        AND (provider ->> 'codexProcessPreparationMs')::double precision = 0 THEN 'codex_ready_or_resident'
      WHEN provider ? 'codexProcessPreparationMs'
        AND (provider ->> 'codexProcessPreparationMs')::double precision > 0 THEN 'codex_preparation_waited'
      ELSE 'codex_unreported'
    END AS codex_process,
    accepted_ms,
    container_ready_ms,
    EXTRACT(EPOCH FROM (runner_job_accepted_at - TIMESTAMP '1970-01-01')) * 1000 AS runner_job_accepted_ms,
    EXTRACT(EPOCH FROM (provider_start_at - TIMESTAMP '1970-01-01')) * 1000 AS provider_start_ms
  FROM attempt_samples
), metric_samples AS (
  SELECT
    lead_bucket,
    prewarm_registration,
    prewarm_readiness,
    prewarm_outcome,
    workspace_restore,
    codex_process,
  ...
  CROSS JOIN LATERAL (
    VALUES
      ('Accepted -> container ready', container_ready_ms - accepted_ms),
      ('Accepted -> runner job', runner_job_accepted_ms - accepted_ms),
      ('Accepted -> provider start', provider_start_ms - accepted_ms)
...
ORDER BY
  min(
    CASE lead_bucket
      WHEN '0-499ms' THEN 1
      WHEN '500-999ms' THEN 2
      WHEN '1000-1999ms' THEN 3
      WHEN '2000-2999ms' THEN 4
      WHEN '3000-3999ms' THEN 5
      ELSE 6
    END
  ),
  prewarm_registration,
