...
- The live v2 workspace snapshot is one encrypted zstd-compressed tar object under `users/<namespace>/workspace-snapshots/<snapshotId>.snapshot.enc`. The container still uploads it directly through a short-lived presigned `PUT`; restore now calls the fixed `workspace-snapshots.worker` internal host, where the existing Container outbound handler dispatches inside the Worker, reads through the phase-ordered R2 bindings, and forwards the object `ReadableStream` without buffering or cloning it. Cloudflare's isolate memory limit is not a streamed-response body limit; this path remains safe for snapshots larger than isolate memory only because the Worker returns the R2 stream directly. Never replace that handoff with `arrayBuffer()`, `bytes()`, `blob()`, `text()`, body cloning, or a buffering transform. The route validates the active write fence, complete v2 ref/AAD and bound-user namespace, exact key, byte size, SHA-256 checksum, and custom metadata before exposing the body. The runner retains one full replay-safe restore retry, propagates cancellation/backpressure, and temporarily falls back to the preserved presigned `GET` route only when an older Worker returns an unversioned error. Current Workers retain `/presign-get` for old runners during the documented three-release rollback window. This v2 format is a greenfield zstd hard cut: gzip v2 refs are not produced or restored.
