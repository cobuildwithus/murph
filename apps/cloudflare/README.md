...
- The OC-to-ENAM move uses Cloudflare-managed [Super Slurper](https://developers.cloudflare.com/r2/data-migration/super-slurper/) jobs instead of an application-owned copier. The source stays read-only, the destination is never promoted until the approved manifest has exact key-and-size parity, and the temporary two-bucket runtime bridge remains the cutover safety boundary. `HOSTED_R2_WRITE_ADMISSION=open|paused` is the temporary Worker-level runtime-start fence: paused requests retry through the existing Temporal/mailbox path without calling UserRunner. Keep `HOSTED_R2_PAUSED_CANARY_USER_ID_SHA256` unset until the destination Worker and every relevant Durable Object have converged; it then opens a brief callback-signed window for one quiescent operator-controlled member while every other request remains fenced. The configured value is a digest, never the raw member ID. Follow `R2_BUNDLES_ENAM_MIGRATION.md`.
