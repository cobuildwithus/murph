...
- Hosted R2 uses one canonical production bucket in ENAM and one isolated preview bucket for all runtime reads, writes, presigns, lifecycle rules, and restores. The former OC bucket is bound only for account-deletion erasure while migrated duplicate ciphertext remains; no runtime path reads, writes, restores, or presigns it. Deploy preflight requires canonical buckets to be ENAM Standard and retiring buckets to be OC Standard. The retiring binding can be removed only after an independent inventory proves the OC bucket stably empty.
...
- `RUNNER_CONTAINER`
- `BUNDLES`
- `BUNDLES_RETIRING_OC`, deletion-only until the former OC bucket is stably empty
