...
boundary. Deploy preflight and runtime egress reject other mappings so Web's
provider-aware allowance rates remain bound to the actual upstream model. Specialized
...
- The OC-to-ENAM online copier treats approved immutable keys as immutable identities, not permanent-retention promises. A CopyObject `404` is accepted only when a strongly consistent source HEAD proves that the planned object was deleted, and the skipped key must remain absent from the final destination inventory. A successful or precondition-failed copy followed by a missing source is an ambiguous copy/delete ordering and fails closed. The initial CopyObject allows one additional attempt only when the first built-in-fetch rejection is a `TypeError` whose direct cause code is `UND_ERR_CONNECT_TIMEOUT`, proving that the failed connection attempt never reached the server. Exactly one fully drained HTTP `500` may instead use R2's [strongly consistent](https://developers.cloudflare.com/r2/reference/consistency/) HEADs to reconcile exact destination and source ETag plus size. An already-exact destination requires no new PUT; an absent destination with an exact source permits exactly one raw create-only recovery PUT because Cloudflare documents [`500 InternalError`](https://developers.cloudflare.com/r2/api/error-codes/) as retryable. That recovery does not reset the pre-connect budget, and only success or `412` reaches the ordinary post-result identity validation. A repeated or otherwise unresolved HTTP, transport, redirect, socket, HEAD, or body failure is terminal and quarantines that destination without another recovery PUT. All other first-attempt outcomes preserve their existing terminal behavior. The source and destination copy conditions remain separately checked because Cloudflare documents that they are [not atomic with one another](https://developers.cloudflare.com/r2/api/s3/extensions/). Apply requires the runbook's explicit single-copier assertion, must never overlap, and is prohibited after destination activation. One source-active apply process retains the source keys it observes, brackets R2 inventories with equal active-owner snapshots, and performs internal convergence cycles. Production cutover holds that exact process at temporary convergence until the operator supplies the existing source-PUT-drained assertion; timing-only rehearsals omit the hold. A fresh process has no provenance and strictly rejects every destination-only eligible object. Every object present in the final OC inventory must still exist identically in ENAM. The online copier never overwrites or deletes an object in either bucket.
- The one raw HTTP `500` recovery observes R2's [one-write-per-second same-key limit](https://developers.cloudflare.com/r2/platform/limits/). Its one-second not-before interval begins only after the first `500` body drains; the copier performs reconciliation HEADs during that interval, then waits the remainder before issuing the unchanged create-only recovery PUT. A paced `429` is still terminal and never receives a third attempt.
...
- Optional Venice core inference is configured only as the all-or-none group
  ...
  model vars must respectively be `openai-gpt-56-luna`,
  `openai-gpt-56-terra`, and `openai-gpt-56-sol`. The fleet default remains
