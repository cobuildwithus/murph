...
- The container bridge now ties each internal `/__internal/run` request to an abort signal, so a worker timeout or client disconnect kills the full isolated child process group instead of leaving compute running inside a warm container.
...
- The checked-in deploy surface now documents and forwards only the canonical runtime vars that the container actually consumes, such as `AGENTMAIL_BASE_URL`, `FFMPEG_COMMAND`, `PADDLEOCR_COMMAND`, `WHISPER_COMMAND`, `WHISPER_MODEL_PATH`, and the explicit runner web-control host allowlist override.
