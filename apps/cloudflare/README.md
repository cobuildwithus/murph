...
- Once container shutdown starts, its wake endpoint explicitly distinguishes a
  retiring-container rejection from an unknown active-child rejection. The
  UserRunner preserves the shutdown checkpoint fence and returns
  `retry_later` for one second later; the existing caller-owned continuation
  performs the retry, so replacement startup follows checkpoint completion
  without a generic fifteen-second delay. Old warm containers that omit the
  additive header retain the prior retry behavior.
