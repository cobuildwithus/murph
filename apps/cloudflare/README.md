...
- Hosted R2 uses one canonical production bucket in ENAM and one isolated preview bucket. The Worker binding, presign target, lifecycle helper, cold restore, and account-deletion cleanup all use the selected environment bucket. Deploy preflight fails closed unless Cloudflare reports ENAM with the Standard storage class.
