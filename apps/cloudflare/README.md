...
- The live v2 workspace snapshot is one encrypted zstd-compressed tar object under `users/<namespace>/workspace-snapshots/<snapshotId>.snapshot.enc`. The container uploads that object directly to R2 through a short-lived presigned `PUT` URL minted by the Worker, and restores through a presigned `GET`; Worker routes carry JSON session/presign/complete metadata only and never receive the snapshot body. Restore-only prepared and fallback `GET` capabilities expire after exactly ten minutes; the `PUT` lifetime remains ten minutes. This v2 format is a greenfield zstd hard cut: gzip v2 refs are not produced or restored.
- V2 snapshot creation validates the planned durable-root entries, then streams `tar -> zstd -> AES-GCM` into the encrypted object. Restore authenticates and hashes the complete encrypted object before it interprets archive bytes, then performs a bounded, non-extracting tar inventory with the same path/type policy as capture. Absolute, traversal, duplicate, `.env*`, link, special, unknown, excessive, and ref-manifest-mismatched archives fail before extraction or durable-root replacement. Only a validated archive is extracted once into a temporary root and swapped into place. `archiveExtractMs` includes both inventory and extraction; `extractMs` includes inventory, extraction, and durable-root replacement.
- Tar/zstd failures retain only classified metadata such as process label, exit/signal, stderr byte/line counts, bounded marker classes, and truncation state. Raw stderr bodies and archive paths are not persisted in hosted restore diagnostics.
