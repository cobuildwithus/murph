...
- `POST /internal/users/:userId/runtime/shell-prewarm` is the optional
  Vercel OIDC-authenticated typing/instant-start shell hint. It rechecks live
  admission and returns after the named container registers an asynchronous
  start attempt; it does not wait for readiness or create runtime authority.
...
window. The first result groups fresh Linq runtime attempts by the causal
shell-prewarm observation consumed by their container readiness call. It shows
hint lead time plus accepted-to-runner, provider, and reply percentiles. A
...
cold start. Neither means health readiness completed.
