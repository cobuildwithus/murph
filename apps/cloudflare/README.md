...
- Once container shutdown starts, its wake endpoint explicitly distinguishes a
  ...
  UserRunner preserves the shutdown checkpoint fence and schedules the existing
  durable processing retry for one second later, so replacement startup follows
  checkpoint completion without a generic fifteen-second delay. Old warm
  containers that omit the additive header retain the prior retry behavior.
