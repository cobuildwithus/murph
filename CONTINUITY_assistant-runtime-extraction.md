...
- Extract a headless `@murph/assistant-runtime` package for hosted execution, move Cloudflare onto it, remove the Cloudflare `@ts-nocheck` seam, and stop serializing container jobs only because hosted runs mutate global process env.
...
- Assume a pragmatic intermediate shape is acceptable if Cloudflare no longer consumes `murph` as its hosted runtime surface and the runtime API is explicit/context-driven.
...
- Updated architecture and verification docs to describe `@murph/assistant-runtime` as the hosted runtime boundary.
...
- Consider a later extraction of the remaining hosted assistant internals that still compose `murph` exports behind `@murph/assistant-runtime`.
