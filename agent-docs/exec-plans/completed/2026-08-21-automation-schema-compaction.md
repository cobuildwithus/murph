...
- Trigger: round 6 exposed the same replay-classification mechanism as rounds 4 and 5. The `sourceOutboxIntentId` correction distinguished one producer family rather than the semantic difference between an ordinary answer that depends on omitted member intent and independently initiated assistant output that remains the visible subject of a follow-up.
- Root-cause proof: scheduled notification turns intentionally persist assistant output without a member transcript entry. A supported incompatible turn-scoped automation target clears native resume and makes the next ordinary reply depend on committed history. After older member text expires, the round-5 classifier removed the just-delivered reminder because it had no private-completion outbox id.
- Ownership decision: extend the existing transcript entry with one optional true-only `standaloneAssistantContext` fact. Notification turns, Assistant Ask continuations, and private-completion imports write the same semantic fact. Replay reads only that fact for current entries; existing durable outbox provenance remains a narrow compatibility proof for private completions persisted before the additive field.
- Rejected alternatives: `persistUserPromptToTranscript: false` is not semantic because ordinary turns also use it after persisting the current member prompt earlier; transcript ordering cannot distinguish the two cases after retention; reordering notification delivery persistence would change a reliability-sensitive boundary and still would not give every standalone producer a uniform outbox id; a producer-specific exception table would repeat the defect mechanism.
- Reset concept budget: one optional field inside the existing transcript schema/input/persistence owner, one true-only finalizer input, the existing replay classifier, and current call-site annotations. No service, queue, state machine, lifecycle, reconciliation pass, migration, registry, dependency, or new state owner. The existing `sourceOutboxIntentId` keeps only its original replay-safe import/deduplication role plus bounded legacy read compatibility.
- Required proof: real retention plus ordinary dependent output yields marker-only replay; real private-completion reconciliation writes the generic fact and legacy outbox-only entries remain readable; an incompatible scheduled automation turn persists standalone context, clears native resume, and gives an immediate short reply the exact reminder without the older ordinary answer; existing count/byte marker reservation and following-turn resume regressions remain green.
