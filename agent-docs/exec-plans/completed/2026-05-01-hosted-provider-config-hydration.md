...
- Agent token export/refresh APIs now keep returning `OAUTH_TOKENS_REQUIRED` for hydrated provider-config/none accounts.
...
- Close the plan with a scoped follow-up commit.
