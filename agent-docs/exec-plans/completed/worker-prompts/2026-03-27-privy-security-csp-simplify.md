You are Codex Audit Worker PS operating in the current shared worktree. Do not create a commit.

Before any edits:
- Read `agent-docs/exec-plans/active/COORDINATION_LEDGER.md`.
- Add a row as `Codex Audit Worker PS` only if you need to edit files; review-only is preferred.
- Preserve unrelated in-flight edits and do not revert anything.

Task:
- Run the required completion-workflow simplify audit for the Privy security/CSP change in `apps/web`.
...
- Any directly affected code/tests needed to validate behavior or spot duplication.
