Run the required completion-workflow `test-coverage-audit` pass for the current greenfield compatibility cleanup.
...
Scope:
- Inspect the current diff and directly affected behavior in:
...
Existing proof already added or updated:
...
Goal:
- Find any meaningful coverage gaps introduced by the cleanup.
- Implement only the highest-impact additional tests if needed.
- Prefer the highest stable public boundary available.

Verification:
- Run the narrowest relevant tests for any changes you make.

Output:
- Return copy/paste-ready prompts per `agent-docs/prompts/test-coverage-audit.md`.
