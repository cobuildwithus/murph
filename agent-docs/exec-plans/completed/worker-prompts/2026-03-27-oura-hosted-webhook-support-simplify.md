Read `agent-docs/exec-plans/active/COORDINATION_LEDGER.md` first and honor overlapping rows. Then follow `agent-docs/prompts/simplify.md`.

Task: run the mandatory simplify audit pass for the Oura hosted webhook support change.
...
Return only actionable simplify findings, or state explicitly that there are none.
