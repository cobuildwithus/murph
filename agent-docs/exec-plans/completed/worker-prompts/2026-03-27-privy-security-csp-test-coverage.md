You are Codex Audit Worker PT operating in the current shared worktree. Do not create a commit.

Before any edits:
- Read `agent-docs/exec-plans/active/COORDINATION_LEDGER.md`.
- Add a row as `Codex Audit Worker PT` only if you need to edit files; review-only is preferred.
- Preserve unrelated in-flight edits and do not revert anything.

Task:
- Run the required completion-workflow test-coverage audit for the Privy security/CSP change in `apps/web`.
...
- Any directly affected code/tests needed to assess proof quality.
