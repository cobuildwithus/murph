...
- Success means restore performs one decompression/extraction pass, creation-time planned-entry validation remains covered, and the changed Cloudflare path passes scoped verification.
...
- Preserve legacy restore compatibility outside this local v2 archive helper; do not add a custom tar extractor, env flag, sampling mode, or optional postflight path.
...
- Keep snapshot creation validation for unsafe paths, symlinks, hardlinks, special files, environment files, and selected archive-entry aliases.
...
Now:
- Rebased draft PR ready for final verification and push.

Next:
- Push the branch and check PR CI/review state.
