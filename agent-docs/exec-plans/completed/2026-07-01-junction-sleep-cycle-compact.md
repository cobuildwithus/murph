...
Implemented. Review follow-ups addressed: parentless, incomplete, no-window direct, and overlapping stage timelines remain raw-only; complete parented cycles emit clipped, zero-inclusive compact fallback facts; and sleep summary plus sleep-cycle stage metrics share one canonical sleep-stage identity for the same provider/source/window/day/stage.

## Verification

- `pnpm --filter @murphai/importers test -- device-providers-junction` passed: 13 files, 266 tests.
