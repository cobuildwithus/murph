...
- Exact candidate CI passed package coverage, build/typecheck, runner-bundle,
  ...
  pull-request-evidence checks. Its required release aggregate remained red only
  because the candidate's then-current base added
  ...
  the same missing-mock-export error. The missing mock was fixed independently
  on `main` before PR #2119 squash-merged.
...
- The reviewed implementation branch's current-base merge-tree was
  conflict-free. PR #2119 then squash-merged; the follow-up carries only this
  completed-plan archive and still requires the normal pushed check observation
  before handoff.
