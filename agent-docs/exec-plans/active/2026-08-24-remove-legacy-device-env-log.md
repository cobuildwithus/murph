...
- The retired event code and its dedicated tests are deleted from both runtime
  owners.
...
- In scope: hosted runtime logging call/function, event-code registry, and the
  focused assertions that existed only for this diagnostic.
...
2. Risk: the retired event code remains accepted or asserted elsewhere.
   Mitigation: search the authored tree after the edit and retain no references;
   ignore deploy bundle outputs that are regenerated by their canonical owner.
...
- Passed: authored-tree stale-reference search and `git diff --check`.
