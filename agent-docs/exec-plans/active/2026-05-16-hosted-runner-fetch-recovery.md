...
Stop hosted runner attempts from failing an entire invocation on a single transient, replay-safe artifact read failure, and stop device-sync dirty recovery from appending a new durable system mailbox wake for the same unresolved dirty revision on every sweep.
...
- Keep the fix small and composable; no scheduling rewrites.
...
- Stable dirty wake dedupe must still create runner demand after the first mailbox pointer is checkpointed; dirty recovery now uses explicit workflow intent to bypass mailbox-progress suppression.
