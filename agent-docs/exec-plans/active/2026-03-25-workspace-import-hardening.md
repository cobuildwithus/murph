...
- Repo verification runs a guard that blocks sibling-`dist` tsconfig aliases, relative cross-package imports, undeclared `@healthybob/*` subpath imports, and sibling-prebuild typecheck scripts.
...
- Replaced the brittle relative cross-package import in `packages/importers/test/importers.test.ts` with `@healthybob/core`.
