...
- Nullable rows created during mixed-version rollout are classified outside the
  reconnect transaction in bounded batches, and reconnect retries at most once
  when deploy skew races the cleanup boundary.
...
4. Classify legacy nullable rows outside the reconnect transaction.
...
- Mixed old/new Web processes may coexist: new writers populate the bit; old
  writers leave it null. A reconnect classifies bounded null rows before
  replacement and refuses to commit if another old writer wins the serialized
