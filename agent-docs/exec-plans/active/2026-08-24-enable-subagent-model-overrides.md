...
- Hosted Codex exposes its native per-spawn model selector for the existing supported Luna, Terra, and Sol catalog.
...
2. Risk: exposing the selector accidentally creates unrestricted provider/model input.
   Mitigation: rely on the native tool enum/catalog generated from the already admitted hosted models; add no free-form Murph-owned routing layer.
...
- Explicit supported path: an ordinary hosted conversation reaches Codex's native `spawn_agent` schema; the generated config exposes the optional model selector, and the image-owned catalog supplies the admitted choices. Effective child metadata remains the billing source of truth.
...
- Difference from plan: none. Verdict: `Ready`.

## Deployment

- No migration or persisted-state change is required. Roll out through the normal hosted runtime/container release.
- During gradual rollout, existing warm runner processes can retain the old config while new processes expose the selector; this is compatible skew and only affects feature availability.
- Post-deploy, verify a newly started hosted run exposes the selector and that an explicit Luna child is attributed to Luna.
