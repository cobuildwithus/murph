...
- The latest `main` is integrated through two base-only merges. The first merge's
  sole textual conflict was the adjacent reliability contract; the resolution
  preserves both this activation-owned enrollment contract and `main`'s updated
  memory-maintenance contract. The second merge was conflict-free and touched
  none of the onboarding follow-up's production owners. On the integrated tree,
  focused verification passes 45 assistant-engine tests, 430 assistant-runtime
  tests, 94 hosted-execution tests, 180 Web activation and routing tests, both
