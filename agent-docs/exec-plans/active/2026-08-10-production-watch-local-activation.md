...
- The first real post-merge preflight exposed three Codex feature names that are not supported by the pinned `0.144.4` executable and incomplete synthetic MCP disable tables that `codex exec` validates even when those tables are disabled. The launch contract now omits unsupported feature names, runs with a private config home linked only to the private authenticated session file, defines only the Cloudflare Observability MCP, and verifies that it is the sole effective server before execution. Cloudflare-specific setup failures also remain scoped to Cloudflare instead of replacing successful deterministic Vercel and Stripe evidence.
- A real scheduler preflight now passes. A following aggregate-only four-source collection returned no collector failures or anomaly candidates: PostgreSQL health and hosted-issue aggregates were healthy, Cloudflare authenticated and proved a zero-activity window, and the Vercel/Stripe status probes succeeded. An independent read-only PostgreSQL aggregate and separate Vercel/Stripe probes agreed; the scheduler remains unloaded until the new exact head completes ReviewGPT and CI.
