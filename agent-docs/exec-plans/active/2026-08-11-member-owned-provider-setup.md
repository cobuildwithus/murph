...
- Shared setup, secret sealing, OAuth-state binding, recovery, progress, and
  connection activation stay provider-neutral for the bounded lifecycle where
  a browser-created developer application immediately exposes a client id and
  secret, uses OAuth, and supports browser-managed deletion. Provider dashboard
  selectors, marker rules, OAuth details, and polling behavior remain in a
  finite adapter.
...
- In scope: reusable setup contract and adapter seam, Strava dashboard
  ...
  design-catalog component and section studies, polling activation,
  deterministic provider-neutral contract and production-shaped adapter/service
  integration coverage, docs, and changelog.
- Out of scope: additional provider adapters, generic browser scripting,
  device-code, certificate, delayed-approval, rotating-registration, or other
  materially different application and credential lifecycles;
...
- Deterministic provider-neutral registry/view proof plus production-shaped
  service and Strava adapter integration coverage. Real-provider proof is
  operator-owned and may use only a disposable approved account with no
