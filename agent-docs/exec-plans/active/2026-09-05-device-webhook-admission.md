...
- Product UX patch: restore bounded history fetching for legacy Fitbit and unrelated connected sources. Retain Google Health completion until legacy Fitbit is terminal, preserving the original history window without claiming that a fetch notification proves new data arrived. Prove durable handoff and unchanged unsafe-input rejection with synthetic tests, then separately verify retained Queue admission.
- ReviewGPT round 1: accepted the finding that early Google Health admission could consume history without fetching it, because the existing executor skips migration-fenced imports. The user authorized remediation. Narrow the existing predicate, without changing execution, migration, persistence, or retry owners; do not enqueue work that would block cutover's dirty-work drain.
...
- Fitbit and unrelated-source historical-fetch cases reproduce the original provenance error. Google Health retention fails before the review correction and passes after it, including replay of the unchanged old window after legacy termination. The full hosted wake suite passes 200 tests, including mixed-job negative cases using an executable unrelated source. Three provider-owned parsing/fetch tests pass, including actual Google Health execution before and after cutover with history older than the reconciliation window and no Fitbit coverage. Nine changelog rendering tests and Web typecheck pass. Complexity debt drops from 112 to 110 and maximum complexity from 59 to 57; remaining transaction hotspots are unchanged outside the extracted predicate.
