...
- Hosted Linq control-plane `occurredAt` derivation matches the shared hosted/onboarding path for canonical `message.received` payloads.
...
- focused `apps/web/test/linq-control-plane.test.ts`
...
- Preserve unrelated dirty-tree work in the overlapping runner and control-plane files.
...
3. Risk: touching `apps/web/src/lib/linq/control-plane.ts` could conflict with the active sparse-recipient fallback row.
   Mitigation: limit the control-plane change to timestamp derivation only and avoid reopening recipient resolution.
...
4. Align shared Linq canonicalization plus hosted control-plane `occurredAt` derivation with the hosted/onboarding `received_at ?? created_at` contract.
...
- Align the hosted control-plane timestamp choice to the shared canonical helper behavior instead of creating another hosted-only precedence rule.
...
- shared/hosted Linq `occurredAt` now follows canonical `received_at ?? created_at`
