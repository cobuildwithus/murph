...
- First-reviewed versus current shape: both use one member-plus-provider preference row, the existing `murph.device` operation, the existing signed Web callback, and the existing mailbox/transcript/delivery owner. Review remediation added no owner or lifecycle; it clarified copy and tool scope, strengthened account-deletion proof, and now tightens email sender authority at the existing input gate.
- Churn: the current authored-source shape is 517 additions and 9 deletions (526 lines total), below the 2,000-line trigger. Since the first-reviewed head, review remediation changed three authored-source files by 16 additions and 2 deletions, far below the 500-line and 25% growth trigger.
- Repeated mechanism: none. The round-two finding is a missed reuse of the existing email authorization signal, not evidence that the preference or delivery architecture needs another mechanism.
- Decision: continue with the one-predicate authorization correction and focused regressions. Splitting or redesigning would add coordination without removing a source of truth or owner.
