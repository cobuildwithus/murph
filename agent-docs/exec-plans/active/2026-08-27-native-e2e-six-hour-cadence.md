...
- A scheduled controller skips paid native work when its latest successful
  scheduled run already covered the current protected-`main` SHA.
- A failed scheduled canary retries in the next six-hour slot, while an
  explicit rerun can retry the same SHA immediately.
- Production canaries retain protected-main ancestry proof, current-production
  alias proof, immutable native source binding, bounded timeouts, environment
  separation, and non-canceling single-platform concurrency.
...
- Out of scope:
  ...
  - Reworking the existing native dispatch, source-attestation, production
    alias, or device postcondition code.
  - Deleting the dormant PR lifecycle implementation shared by the dispatcher;
    that cross-repository contract is independent of admission cadence.
...
- Technical constraints:
  ...
  - The current production alias must still match the admitted protected-main
    SHA before private dispatch.
...
1. Risk: A schedule lands before the newest `main` deployment reaches the
   production alias.
   Mitigation: Retain the alias proof; a failure remains unsatisfied and retries
   at the next six-hour slot.
2. Risk: A no-change gate accidentally suppresses an untested revision.
   Mitigation: Compare against only the most recent successful scheduled run;
   missing/malformed GitHub history fails the gate instead of guessing.
...
3. Delete obsolete PR retry ownership and update focused workflow assertions.
...
- Treat the latest successful scheduled run's `head_sha` as the no-change
  checkpoint. This reuses GitHub's existing durable run history and adds no new
  state owner.
...
- Preserve production-only `non_destructive_existing_identity` dispatch and
  leave dormant PR-mode dispatcher code intact to avoid an unrelated private
...
- Commands to run:
  ...
  - Required exact-head GitHub Actions plus preliminary specialist and final
    ReviewGPT gates.
- Expected outcomes:
  - Deterministic tests prove six-hour schedules, fixed concurrency, successful
    SHA checkpointing, rerun behavior, absence of PR/deployment admission, and
    unchanged production/source/privacy boundaries.
