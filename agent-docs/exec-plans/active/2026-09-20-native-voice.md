...
- An active development API credential is needed to prove the intended hosted authentication path. The authorized local and development credential is inactive; local subscription success is not hosted API-key compatibility.
- Native session cleanup during connection setup and trusted final usage forwarding remain unresolved. Prefer native fixes over a second Murph sideband or billing authority.
- ReviewGPT's initial native-first recommendation is captured. Its source-grounded implementation-design follow-up is still running in the same consultation, with the original waited invocation retaining capture ownership.
