...
Fix hosted device-sync webhook storm handling at the correct boundary: coalesce rehydratable dirty hints while preserving Junction direct payload webhooks.
...
- Payload-bearing webhooks are never accepted and dropped; they are durably persisted or returned retryable without completing the claimed trace.
...
- Keep the architecture simple: dirty flags are level-triggered, direct payloads are event/job-triggered.

## Plan

1. Inspect current public-ingress, hosted wake, dirty-store, trace, and Junction payload behavior.
2. Add a provider/job-level classifier for coalescible hint webhooks versus durable payload webhooks.
