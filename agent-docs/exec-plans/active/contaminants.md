...
Implementation note, 2026-06-20: threshold comparisons use imported `normalized_*` threshold triplets as the direct concentration-comparability gate. Body-weight/day guidance, such as BPA TDI rows, is stored as raw source guidance and scored at read time only when the exact linked product label has `serving_grams`. There is no product-specific threshold application table. Bulk legal or commodity threshold snapshots are source material until transformed into broad Murph screening rows; the API never falls back to raw threshold units/bases, categories, brands, or names.
