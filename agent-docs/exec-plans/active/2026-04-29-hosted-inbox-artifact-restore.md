...
State:
- Investigating with focused E2E coverage and parallel codepath review.
...
Now:
- Check assistant prompt/turn materialization and subagent findings to isolate likely production-only failure.

Next:
- Decide whether to keep the passing E2E as regression coverage and whether a targeted fix belongs in download diagnostics, parser failure handling, or assistant prompt fallback.
