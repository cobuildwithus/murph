...
- Private deployment companion is draft PR 116. Local source/typecheck and actual cross-repository forwarding proof pass; broader verification still has a host-load-sensitive CLI timeout and pending build evidence.
...
- Existing conversation-readiness changelog item updated for concurrent ready starts; add the public PR number to its provenance before final candidate review.
