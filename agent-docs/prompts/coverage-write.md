...
- When the parent agent is Codex and local subagent tooling is available, this pass should run as a spawned Codex worker subagent, with high reasoning by default, or xhigh reasoning when the parent workflow classifies the change as large or complex.
- When the parent agent is Claude or another surface that cannot spawn Codex subagents directly, this pass may run on Codex `gpt-5.5` through the local Codex CLI as described in the completion workflow.
