...
| `P0-08` | Isolated runtime-log recording in `hosted-runtime-log/store.ts`. | Append rejects inactive members before isolated checkout, then takes the subject advisory lock, rechecks reversible member authority, reads the permanent deletion fence, and inserts. Account cleanup creates that fence and deletes rows under the same lock. | The early primary read holds no isolated client. One final primary read remains under the isolated subject lock because billing suspension is reversible and cannot create the permanent account-deletion fence. | **Separate isolated runtime-log pool**, 5 default/10 maximum; primary lookup uses the primary pool. | **P0, mitigated with explicit residual** | Keep the pre-check for the common inactive/stalled path and the final recheck for reversible suspension. The monotonic digest-only fence closes the permanent deletion race without replicating mutable billing authority or adding a service/state owner. Deploy the additive fence compatibility phase and drain old writers before moving the pre-check. | `agent/runtime-log-cross-db-critical-section` |
