...
- `vault-cli assistant chat` prefers an Ink-based stderr UI when `react`, `ink`, and `ink-text-input` are installed in the CLI workspace, then falls back to the readline loop when those optional UI dependencies are unavailable.
