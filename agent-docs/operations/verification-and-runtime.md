...
candidate tree in `run.txt`, and prints that stable repository-relative bundle
location before returning the provider status. The next paid run replaces that
same checkout-local bundle, and a successful run removes it so stale failure
evidence cannot be mistaken for the current result. Do not add `--keep` merely
to inspect a failure that this bundle already preserves.
