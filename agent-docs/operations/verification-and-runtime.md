...
- The local web entrypoint lives under `packages/web`; `pnpm web:dev` starts the app in webpack mode through a wrapper that binds to `127.0.0.1`, blocks framework `.env*` reads, preserves the original `pnpm` launch cwd for relative `HEALTHYBOB_VAULT` values, requires `HEALTHYBOB_VAULT` at runtime, ensures the built `@healthybob/contracts`, `@healthybob/runtime-state`, and `@healthybob/query` entrypoints exist before Next starts, pins webpack resolution to `packages/query/dist/index.js` so workspace source aliases do not leak into the app bundle, and renders only a read-only localhost surface with safe-field search over the query layer.
