...
- `vault-cli assistant ask|chat|session` persist only minimal local session metadata under `assistant-state/`; provider transcript history stays external when the adapter supports it.
...
- The local web entrypoint lives under `packages/web`; `pnpm web:dev` starts the app in webpack mode through a wrapper that binds to `127.0.0.1`, blocks framework `.env*` reads, requires `HEALTHYBOB_VAULT` at runtime, and renders only a read-only localhost surface with safe-field search over the query layer.
