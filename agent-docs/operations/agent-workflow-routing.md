...
| Standard repo change | Ordinary repo code/test/config change | `agent-docs/operations/completion-workflow.md`, `agent-docs/operations/verification-and-runtime.md` | Yes | Yes for multi-file or high-risk work | Mandatory before handoff: for prompt-primary changes, run only `prompt-review` unless non-prompt code independently triggers another pass; otherwise run `security-privacy-review` when the change materially touches auth/session behavior, secrets, payments, external ingress/egress, public APIs/routes, trust boundaries, or persisted/uploaded/user-facing data exposure; run `coverage-write` under the completion workflow audit worker routing whenever the verification lane includes owner coverage; add `deep-review` when the completion workflow's complex-or-sensitive conditions apply; finish with the parent's explicit local final review, and treat the PR-lane ReviewGPT loop (`pnpm review:gpt pr-review` on the repo-configured ReviewGPT browser lane, randomized by default across usable lanes, fired on push in parallel with PR CI) as the required final review gate for PR-lane work | Follow verification doc; prefer `pnpm test:diff <path ...>` when it is truthful, otherwise run the touched owner coverage command(s) and add direct scenario proof when required | `scripts/finish-task` if plan-bearing, otherwise `scripts/committer` |
| High-risk or cross-cutting change | Auth, secrets, trust boundaries, runtime entrypoints, schema/storage, billing, deploy surfaces, or broad refactors | `agent-docs/SECURITY.md`, `agent-docs/RELIABILITY.md`, `agent-docs/operations/completion-workflow.md`, `agent-docs/operations/verification-and-runtime.md` | Yes | Yes | Mandatory before handoff: run `security-privacy-review`; run `coverage-write` under the completion workflow audit worker routing whenever the verification lane includes owner coverage; add `deep-review` when the completion workflow's complex-or-sensitive conditions apply; finish with the parent's explicit local final review, and treat the PR-lane ReviewGPT loop (`pnpm review:gpt pr-review` on the repo-configured ReviewGPT browser lane, randomized by default across usable lanes, fired on push in parallel with PR CI) as the required final review gate for PR-lane work | Full verification baseline unless the user explicitly says otherwise; when the change is scoped enough for owner-level verification, prefer truthful `test:diff` coverage or the edited owner coverage command(s) | `scripts/finish-task` |
...
- Treat a dedicated task worktree as temporary local state. Preserve it while its PR is open or its task, review, CI, plan, coordination-ledger row, or follow-up work remains active. Once the PR is confirmed merged or closed and the checkout is no longer needed, retire it from another checkout: stop only processes started for that task, confirm the target is neither the primary nor current worktree, verify it is unlocked and has no dirty or untracked files, then run `git worktree remove <path>` without `--force`. Preserve the task branch by default. Never remove a dirty, locked, process-used, open-PR, or active-task checkout. Never raw-delete the checkout or run `git worktree prune` as part of retirement; if any gate or `git worktree remove` fails, preserve it and report the blocker.
...
- For non-trivial PR-lane work, the PR-lane ReviewGPT loop in `agent-docs/operations/pr-reviewgpt-loop.md` is the required final review stage and merge-readiness gate after the completion workflow. It runs `pnpm review:gpt pr-review` on the repo-configured managed ReviewGPT browser lane, randomized by default across usable Eragon, Phlebas, and Mountain lanes, against guarded artifacts for a clean worktree at the pushed PR head. Fire each round as soon as the head it reviews is pushed, in parallel with PR CI — do not wait for green CI to start a round. Do not report a PR as good to merge until that loop reaches zero accepted findings and PR CI is green on the final head, unless the current user task explicitly opts out. After a zero-finding round, a later base-branch-only PR update does not require rerunning the loop; wait for CI on the updated head and continue the merge path. Docs/process-only PRs and trivial copy-only changes stay exempt unless the user asks for the loop.
...
- Frontend implementation is Fable-only. Codex-native and other non-Claude parents do not implement user-facing `apps/web` frontend/UI changes themselves — including the frontend portion of mixed changes and review-driven UI fixes; they delegate that implementation to the Claude Code CLI on the Fable model via non-interactive `claude --model claude-fable-5 --dangerously-skip-permissions -p` with the concrete implementation plan as the prompt, using one of the two local Claude Code homes: normal home resolution (`~/.claude`, the `cc` launcher) or `CLAUDE_CONFIG_DIR=~/.claude-2` (the `cc2` launcher) when the primary home is busy. An operator launcher is acceptable only when it selects the same model and one of those homes. Run the delegated worker in the parent's current checkout or task worktree and instruct it not to create branches or worktrees, commit, or push; the delegating parent keeps planning, review, verification, audit, and commit duties. Delegated frontend work still runs the full completion workflow for its task class, including `frontend-review` when that workflow requires it. If the Claude CLI or its auth is unavailable, report the limitation and escalate to the user instead of implementing the frontend change on a non-Fable model.
