...
- During a merge, the pre-commit hook skips CLI schema regeneration only when
  the staged `packages/cli` tree exactly matches `MERGE_HEAD`; any task-authored
  CLI difference keeps generation enabled. Outside that base-only case, CLI
  artifact generation fails closed when tracked or untracked unstaged
  `packages/cli` inputs could make working-tree output differ from the commit
  tree. A successful run stages `config.schema.json`, `incur.generated.ts`, and
  `vault-cli-skill-hash.generated.ts` together.
