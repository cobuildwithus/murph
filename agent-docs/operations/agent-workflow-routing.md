...
- For changes that reasonably touch user data/state, auth/session behavior, secrets, payments, health data, contact identifiers, observability/logging, external surfaces, or trust boundaries, the completion workflow's `security-privacy-review` pass is a mandatory audit alongside any other required passes for the task class.
- Exception: really low-impact copy-only `apps/web` edits may skip `security-privacy-review`, `frontend-review`, `coverage-write`, and `task-finish-review` subagents when they only change static text and do not affect layout, UI state, auth, pricing logic, schemas, runtime behavior, or security/privacy claims. Still run local readback and the narrowest focused checks that prove the copy surface.
...
- For package/app changes, prefer `pnpm test:diff <path ...>` when it truthfully covers the task; otherwise run the edited owner's scoped coverage command before the required `gpt-5.4-mini` `coverage-write` pass and final review.
