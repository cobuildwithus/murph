...
- For non-trivial PR-lane work, `agent-docs/operations/pr-reviewgpt-loop.md` owns the final ReviewGPT gate: review each pushed PR-specific head in parallel with CI and require zero accepted findings plus green CI. That doc owns evidence, browser lanes, reruns, and base-only updates. Docs/process-only PRs and trivial copy-only changes stay exempt unless the user asks for the loop.
