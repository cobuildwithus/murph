...
| Standard repo change | Ordinary repo code/test/config change | `agent-docs/operations/completion-workflow.md`, `agent-docs/operations/verification-and-runtime.md` | Yes | Yes for multi-file or high-risk work | Mandatory before handoff: for prompt-primary changes, run only `prompt-review` unless non-prompt code independently triggers another pass; otherwise run `security-privacy-review` when the change materially touches auth/session behavior, secrets, payments, external ingress/egress, public APIs/routes, trust boundaries, or persisted/uploaded/user-facing data exposure; run `coverage-write` on Codex `gpt-5.5` with the workflow audit reasoning default whenever the verification lane includes owner coverage; add `deep-review` when the completion workflow's complex-or-sensitive conditions apply; finish with the parent's explicit local final review, and treat the external `review:gpt pr-review` loop (fired on push, in parallel with PR CI) as the required final review gate for PR-lane work | Follow verification doc; prefer `pnpm test:diff <path ...>` when it is truthful, otherwise run the touched owner coverage command(s) and add direct scenario proof when required | `scripts/finish-task` if plan-bearing, otherwise `scripts/committer` |
| High-risk or cross-cutting change | Auth, secrets, trust boundaries, runtime entrypoints, schema/storage, billing, deploy surfaces, or broad refactors | `agent-docs/SECURITY.md`, `agent-docs/RELIABILITY.md`, `agent-docs/operations/completion-workflow.md`, `agent-docs/operations/verification-and-runtime.md` | Yes | Yes | Mandatory before handoff: run `security-privacy-review`; run `coverage-write` on Codex `gpt-5.5` with the workflow audit reasoning default whenever the verification lane includes owner coverage; add `deep-review` when the completion workflow's complex-or-sensitive conditions apply; finish with the parent's explicit local final review, and treat the external `review:gpt pr-review` loop (fired on push, in parallel with PR CI) as the required final review gate for PR-lane work | Full verification baseline unless the user explicitly says otherwise; when the change is scoped enough for owner-level verification, prefer truthful `test:diff` coverage or the edited owner coverage command(s) | `scripts/finish-task` |
...
- Audit worker routing depends on the parent agent. If the parent agent is Codex and native subagent tooling is available, run required audit passes as native Codex subagents; do not shell out to `codex exec` from Codex just to create another Codex worker. If the parent agent is not Codex, for example Claude, or cannot create native Codex subagents, run Codex-pinned passes (`security-privacy-review` and `coverage-write`) on Codex `gpt-5.5` through the local Codex CLI using non-interactive `codex exec` with stdin closed. Use `MURPH_AUDIT_CODEX_HOME` as `CODEX_HOME` when set; otherwise use the CLI's normal home resolution. Do not use `codex exec review` for these prompt-based completion passes. The remaining required audit subagents (`prompt-review`, `frontend-review`, `deep-review`) run on whatever model the parent agent is currently running on unless the pass is Codex-pinned above. All audit subagents use high reasoning by default. Use xhigh reasoning for large or complex changes, high-risk/cross-cutting changes, or audits spanning multiple owners, architecture decisions, or trust-boundary decisions. If a required Codex model/effort override, CLI, or auth is unavailable, report the limitation and run the pass on the parent agent's current model instead of skipping it.
...
- For package/app changes, prefer `pnpm test:diff <path ...>` when it truthfully covers the task; otherwise run the edited owner's scoped coverage command before the required Codex `gpt-5.5` `coverage-write` pass using the workflow audit reasoning default and final review.
