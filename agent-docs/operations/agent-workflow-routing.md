...
- For package/app changes, prefer `pnpm test:diff <path ...>` when it truthfully covers the task; otherwise run the edited owner's scoped coverage command before the required Codex `gpt-5.5` `coverage-write` pass using the workflow audit reasoning policy and final review.
