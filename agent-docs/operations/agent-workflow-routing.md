...
- When creating or updating a PR, follow `agent-docs/operations/completion-workflow.md` § PR Description. Keep the body short: why and outcome, Product UX result, direct evidence, and changelog. Add only the risk details that the changed path needs. Frontend evidence names the changed states and viewports; it does not require a catalog edit or screenshot count.
