...
- Prefer Fable for user-facing `apps/web` frontend/UI implementation, including the frontend portion of mixed changes and review-driven UI fixes. Before falling back, inspect every discoverable local Claude Code home plus its safe session/background-agent index, check relevant live-session working directories without signaling or commandeering another process, and try the available authenticated homes with the Fable model. When a usable lane exists, delegate through non-interactive `claude --model claude-fable-5 --dangerously-skip-permissions -p` with the concrete implementation plan, in the parent's current checkout or task worktree, and instruct it not to create branches/worktrees, commit, or push. If that sweep proves Fable unusable because the CLI, auth, model, credits, or every safely reusable session is unavailable, the parent agent may implement the frontend change directly instead of blocking; record the reason in the turn or active plan. The parent always retains planning, review, verification, audit, and commit duties, and both delegated and fallback implementations run the full completion workflow for their task class, including `frontend-review` when required.
