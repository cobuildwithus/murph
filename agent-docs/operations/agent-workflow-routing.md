...
- For new or materially changed user-facing product behavior, run the completion workflow's local `product-experience-review` pass across the full conversational, runtime, and/or frontend journey. Material frontend workflow changes also activate the frontend lens in the preliminary ReviewGPT pass; trivial static copy, presentation-only polish, internal-only changes, and docs/process work do not trigger product-experience review.
...
- Exception: really low-impact copy-only `apps/web` edits may skip local `product-experience-review` and the preliminary specialist ReviewGPT pass when they only change static text and do not affect layout, UI state, auth, pricing logic, schemas, runtime behavior, or security claims. Still run local readback and the narrowest focused checks that prove the copy surface.
