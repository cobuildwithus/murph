...
| Tiny low-risk repo change | Narrow, single-purpose repo code/test/config change in one subsystem | `agent-docs/operations/completion-workflow.md`, `agent-docs/operations/verification-and-runtime.md` | Usually optional unless multi-file/high-risk | Mandatory before handoff: use the worktree/PR lane and run exactly one review-only local specialist subagent with every applicable Product UX, prompt, frontend, and coverage lens. If a cross-cutting trigger unexpectedly applies, run the separate final ReviewGPT gate concurrently after focused proof and candidate review, or use local `deep-review`, never both. Report evidence-backed dispositions after the specialist result and continue accepted remediation without a turn-ending pause; final ReviewGPT `FINDINGS` still pause all candidate mutation unless a narrow `Complexity Collapse` or non-production-only remediation exception applies. A validated final `PASS` does not pause, and evidence-backed rejections are terminal. Exception: qualifying meaning-preserving tiny copy corrections skip the specialist subagent and use local readback plus focused checks. | For a PR, run focused local tests/direct checks and require green exact-head CI; local umbrella commands are optional diagnostics. A direct default-branch push requires `pnpm verify:acceptance`. | `scripts/finish-task` if plan-bearing, otherwise `scripts/committer` |
| Standard repo change | Ordinary repo code/test/config change | `agent-docs/operations/completion-workflow.md`, `agent-docs/operations/verification-and-runtime.md` | Yes for multi-file or high-risk work | Mandatory before handoff: run exactly one review-only local specialist subagent with every applicable Product UX, prompt, frontend, and coverage lens. When the cross-cutting trigger applies, start the separate final ReviewGPT gate concurrently after focused proof and candidate review, or use local `deep-review` when that final gate will not run; never both. Report evidence-backed dispositions after the specialist result and continue accepted remediation without a turn-ending pause; final ReviewGPT `FINDINGS` still pause all candidate mutation unless a narrow `Complexity Collapse` or non-production-only remediation exception applies. A validated final `PASS` does not pause, and evidence-backed rejections are terminal. | For a PR, run focused local tests/direct checks and require green exact-head CI; use broader local commands only when the evidence or a CI failure requires them. A direct default-branch push requires `pnpm verify:acceptance`. | `scripts/finish-task` if plan-bearing, otherwise `scripts/committer` |
| High-risk or cross-cutting change | Auth, secrets, trust boundaries, runtime entrypoints, schema/storage, billing, deploy surfaces, or broad refactors | `agent-docs/SECURITY.md`, `agent-docs/RELIABILITY.md`, `agent-docs/operations/completion-workflow.md`, `agent-docs/operations/verification-and-runtime.md` | Yes | Mandatory before handoff: run exactly one review-only local specialist subagent with every applicable Product UX, prompt, frontend, and coverage lens. Start the separate final ReviewGPT gate concurrently after focused proof and candidate review, or use local `deep-review` only when the final gate will not run; never both. Report evidence-backed dispositions after the specialist result and continue accepted remediation without a turn-ending pause; final ReviewGPT `FINDINGS` still pause all candidate mutation unless a narrow `Complexity Collapse` or non-production-only remediation exception applies. A validated final `PASS` does not pause, and evidence-backed rejections are terminal. | For a PR, run high-signal focused local proof and direct scenarios, then require the full exact-head CI surface. A direct default-branch push requires `pnpm verify:acceptance`. | `scripts/finish-task` |
...
- Narrow merge-commit exception: when an ordinary base-reconciliation merge is already active and `scripts/committer` rejects `MERGE_HEAD`, resolve the intended conflicts, stage the complete merge resolution, and require `git diff --name-only --diff-filter=U` to print nothing. Then run ordinary `git commit` without path arguments so Git records both parents and the normal hooks run. Do not pass `--no-verify`, use `git commit-tree`/`git update-ref`, stage unrelated changes, or invoke this exception for an ordinary task commit. This exception finishes an existing merge only; it does not authorize starting a merge or bypassing worktree, plan, review, verification, or moving-base rules.
...
- Product UX, prompt, frontend, and coverage audits run together in exactly one review-only local specialist subagent. Treat this workflow doc plus `AGENTS.md` as standing permission to spawn that combined pass when any lens applies. The fallback local `deep-review` is a distinct cross-cutting subagent pass used only when its trigger applies and the final ReviewGPT gate will not run.
- Run the specialist subagent against the stable isolated candidate before the parent's final review. When the final ReviewGPT gate also applies, its full-patch round 1 may start concurrently against the exact pushed version of that candidate after focused local proof and the parent's candidate review. The stages stay independent: the specialist result does not establish or advance the final baseline, and accepted findings from either stage must be resolved before completion.
...
- For final-ReviewGPT-eligible PR-lane work, `agent-docs/operations/pr-reviewgpt-loop.md` owns the cross-cutting gate. Preserve the immutable round-one baseline even when a parallel specialist finding causes remediation, and use correction-delta rounds for all later behavior-bearing fixes. After every substantive specialist result, the parent presents evidence-backed accept/reject dispositions as a progress update and may continue accepted remediation without a turn-ending pause. Final `FINDINGS` still pause all candidate mutation, including pending specialist remediation, except that narrowly proven behavior-preserving `Complexity Collapse` and non-production-only remediation exceptions may proceed after the same update; a validated final `ROUND_OUTCOME: PASS` proceeds without a user-resume pause. Agents may reject speculative, unproven, or disproportionate fixes, and only accepted findings require remediation. Require each applicable stage to have a resolved result with zero unresolved accepted findings, every required disposition boundary complete, and green CI. Never also run local `deep-review` for the same completed change. That doc owns exact-head packaging, browser lanes, anomaly retrospectives, reruns, invalid-run retry counting, and base-only updates.
- After a resolved final round with zero accepted findings, green required CI on the PR-authored head plus a clean current-base `git merge-tree --write-tree` proof is sufficient preparation. At an authorized merge boundary, wait only for routed review gates and required GitHub checks. If strict-current enforcement blocks the merge, prefer the merge queue; otherwise the unchanged reviewed patch gets at most one normal base update, affected-surface proof, and required CI without another ReviewGPT round. A later base advance never resets that budget or restarts CI: rerun the merge-tree, use an already-authorized non-refresh merge path when it is clean, or report `moving-base race` and stop with the PR and worktree active. Use the ordinary next round when the base-only classification is uncertain or false; `agent-docs/operations/pr-reviewgpt-loop.md` owns the exact terminal path.
- Prompt-primary PRs run the specialist subagent with the prompt lens. They still skip the separate final ReviewGPT gate unless non-prompt scope independently triggers it or the current user explicitly requests it.
...
- Exception: an `apps/web` copy-only edit may skip the specialist subagent only for a meaning-preserving typo, punctuation, grammar, or equivalent localization correction that meets the completion workflow fast path. Still run local readback and the narrowest focused checks that prove the copy surface.
