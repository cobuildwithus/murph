...
- Required workflow audit subagents run on whatever model the parent agent is currently running on (e.g., a Fable parent spawns Fable audit subagents). They use high reasoning by default. Use xhigh reasoning for large or complex changes, high-risk/cross-cutting changes, or audits spanning multiple owners, architecture decisions, or trust-boundary decisions.
...
- For package/app changes, prefer `pnpm test:diff <path ...>` when it truthfully covers the task; otherwise run the edited owner's scoped coverage command before the required `coverage-write` pass on the model in use using the workflow audit reasoning default and final review.
