...
- After a zero-finding final round, a base-only merge or rebase does not rerun ReviewGPT when each bounded manual conflict is inspected and mechanically preserves the reviewed PR plus current-base behavior without newly authored behavior or contract changes. Run affected-surface proof and exact-head CI; use the ordinary next round when that classification is uncertain or false.
