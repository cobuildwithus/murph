...
| Docs/process-only | Repo docs, process docs, plans, agent workflow docs | `agent-docs/operations/verification-and-runtime.md` | Yes | For multi-file or likely multi-turn work | No by default | Repo baseline by default; scoped verification allowed only per verification doc | `scripts/finish-task` if plan-bearing, otherwise `scripts/committer` |
...
- Read `agent-docs/FRONTEND.md` and `packages/web/AGENTS.md` for UI or `packages/web` work.
