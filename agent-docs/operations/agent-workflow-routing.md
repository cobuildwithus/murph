...
- For non-trivial PR-lane work, the post-CI external `review:gpt pr-review` loop in `agent-docs/operations/pr-deep-review-loop.md` is a required merge-readiness gate after completion workflow and green PR CI. Do not report a PR as good to merge until that loop reaches zero accepted findings, unless the current user task explicitly opts out. Docs/process-only PRs and trivial copy-only changes stay exempt unless the user asks for the loop.
