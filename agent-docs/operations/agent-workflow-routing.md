...
- For new or materially changed user-facing product behavior, run the completion workflow's `product-experience-review` pass across the full conversational, runtime, and/or frontend journey. Material frontend workflow changes run both this pass and `frontend-review`; trivial static copy, presentation-only polish, internal-only changes, and docs/process work do not trigger it.
...
- Exception: really low-impact copy-only `apps/web` edits may skip `product-experience-review`, `frontend-review`, and `coverage-write` subagents when they only change static text and do not affect layout, UI state, auth, pricing logic, schemas, runtime behavior, or security claims. Still run local readback and the narrowest focused checks that prove the copy surface.
