...
- For non-trivial PR-lane work, the post-CI external `review:gpt pr-review` loop in `agent-docs/operations/pr-deep-review-loop.md` is a required merge-readiness gate after completion workflow and green PR CI. Do not report a PR as good to merge until that loop reaches zero accepted findings, unless the current user task explicitly opts out. After a zero-finding round, a later base-branch-only PR update does not require rerunning `review:gpt`; wait for CI on the updated head and continue the merge path. Docs/process-only PRs and trivial copy-only changes stay exempt unless the user asks for the loop.
- The `simplify`, `security-privacy-review`, and `coverage-write` audit subagents run on Codex `gpt-5.5` through the local Codex CLI (`codex exec`, with `CODEX_HOME` taken from the local `MURPH_AUDIT_CODEX_HOME` override when set, otherwise the Codex CLI's normal home resolution), regardless of the parent agent's model, to keep audit cost on the Codex subscription. The remaining required audit subagents run on whatever model the parent agent is currently running on (e.g., a Fable parent spawns Fable `task-finish-review` subagents). All audit subagents use high reasoning by default. Use xhigh reasoning for large or complex changes, high-risk/cross-cutting changes, or audits spanning multiple owners, architecture decisions, or trust-boundary decisions. If the Codex CLI or its auth is unavailable, report the limitation and run the pass on the parent agent's current model instead of skipping it.
