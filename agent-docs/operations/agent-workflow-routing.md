...
- When creating or updating a PR, follow `agent-docs/operations/completion-workflow.md` § PR Description. Keep the body concise: why and outcome, Product UX result, direct evidence, deployment concerns, changelog, and the required compact added/deleted LOC breakdown by source, tests/fixtures, docs, config/tooling, and generated/other. Add only the risk details that the changed path needs. Frontend evidence names the changed states and viewports; it does not require a catalog edit or screenshot count.
