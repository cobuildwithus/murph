...
- Codex-native agents always satisfy required completion audit passes with spawned local subagents, not `codex exec`. Claude (and other non-Codex) parent agents always route every required completion audit pass (`prompt-review`, `security-privacy-review`, `coverage-write`, `frontend-review`, `deep-review`) through the local Codex CLI by calling `codex` (non-interactive `codex exec`) or the `c1` operator alias, with `CODEX_HOME` taken from the local `MURPH_AUDIT_CODEX_HOME` override when set, otherwise the Codex CLI's normal home resolution; do not run a required pass on the parent's own model or another model family while the Codex CLI is available. All audit subagents use high reasoning by default. Use xhigh reasoning for large or complex changes, high-risk/cross-cutting changes, or audits spanning multiple owners, architecture decisions, or trust-boundary decisions. If subagent tooling or Codex CLI auth needed for a non-Codex parent is unavailable, report the limitation and run the pass on the parent agent's current model instead of skipping it.
