...
| Tiny low-risk repo change | Narrow, single-purpose repo code/test/config change in one subsystem | `agent-docs/operations/completion-workflow.md`, `agent-docs/operations/verification-and-runtime.md` | Usually optional unless multi-file/high-risk | No specialist audit. Run the final ReviewGPT gate only if a cross-cutting trigger unexpectedly applies or the user requests it. | For a PR, run focused local tests/direct checks and require green exact-head CI; local umbrella commands are optional diagnostics. A direct default-branch push requires `pnpm verify:acceptance`. | `scripts/finish-task` if plan-bearing, otherwise `scripts/committer` |
| Standard repo change | Ordinary repo code/test/config change | `agent-docs/operations/completion-workflow.md`, `agent-docs/operations/verification-and-runtime.md` | Yes for multi-file or high-risk work | No specialist audit. Run the final ReviewGPT gate when a cross-cutting trigger applies or the user requests it. | For a PR, run focused local tests/direct checks and require green exact-head CI; use broader local commands only when the evidence or a CI failure requires them. A direct default-branch push requires `pnpm verify:acceptance`. | `scripts/finish-task` if plan-bearing, otherwise `scripts/committer` |
| High-risk or cross-cutting change | Auth, secrets, trust boundaries, runtime entrypoints, schema/storage, billing, deploy surfaces, or broad refactors | `agent-docs/SECURITY.md`, `agent-docs/RELIABILITY.md`, `agent-docs/operations/completion-workflow.md`, `agent-docs/operations/verification-and-runtime.md` | Yes | Run the final ReviewGPT gate unless the user explicitly opts out. No local subagent is a fallback requirement. | For a PR, run high-signal focused local proof and direct scenarios, then require the full exact-head CI surface. A direct default-branch push requires `pnpm verify:acceptance`. | `scripts/finish-task` |
...
- The parent challenges the plan and walkthrough during candidate and final
  review. Do not add a separate mandatory Product UX audit.
...
- Product UX, prompt, frontend, and coverage proof are parent-owned completion responsibilities. They do not require a specialist audit, preliminary ReviewGPT pass, or local subagent.
- Local `deep-review` remains available when the user explicitly requests it; it is not a fallback completion requirement.
...
- Frontend-only PRs keep Product UX, rendered, accessibility, and focused proof, but skip the final cross-cutting ReviewGPT gate unless backend, authority, persisted-state, provider, deploy, high-risk-refactor, or another independent cross-cutting scope triggers it.
- For final-ReviewGPT-eligible PR-lane work, `agent-docs/operations/pr-reviewgpt-loop.md` owns the cross-cutting gate. Preserve the immutable round-one baseline and use correction-delta rounds for later behavior-bearing fixes. Final `FINDINGS` pause candidate mutation except for the narrowly proven behavior-preserving `Complexity Collapse` and non-production-only remediation exceptions; a validated final `ROUND_OUTCOME: PASS` proceeds without a user-resume pause. Agents may reject speculative, unproven, or disproportionate findings, and only accepted findings require remediation. Require the gate to have a resolved result with zero unresolved accepted findings, every required disposition boundary complete, and green CI. That doc owns exact-head packaging, browser lanes, anomaly retrospectives, reruns, invalid-run retry counting, and base-only updates.
...
- Prompt-primary PRs require parent prompt inspection and focused regression proof. They skip the final ReviewGPT gate unless non-prompt scope independently triggers it or the current user explicitly requests it.
...
- For prompt-primary changes, the parent reads the current official OpenAI prompt guidance and inspects the complete assembled prompt before handoff.
- Apply `agent-docs/operations/completion-workflow.md` § Product and Rendered Evidence before final-gate exemptions. Any change to semantic user-facing copy; user-visible action purpose, count, or priority; required interaction steps; UI state selection and visible feedback or progress; user-visible element or screen existence; asynchronous continuation or wake ownership; or journey timing, delivery, permission, or recovery requires Product UX planning and walkthrough across the full conversational, runtime, and/or frontend journey. Meaning-preserving tiny copy corrections, implementation-only presentation, internal-only changes, and docs/process work use their narrower parent-owned evidence route.
- For user-facing `apps/web` UI changes outside the tiny copy-only fast path, include enough redacted rendered evidence to judge every material visual or responsive claim. Inspect both phone and desktop when responsive behavior can change; do not require a second viewport only to meet a quota.
...
- For particularly complex or sensitive changes, run the final ReviewGPT gate on the eligible PR lane unless the user explicitly opts out. Do not substitute a mandatory local subagent.
- An `apps/web` copy-only edit may use the lightweight fast path only for a meaning-preserving typo, punctuation, grammar, or equivalent localization correction. Still run local readback and the narrowest focused checks that prove the copy surface.
