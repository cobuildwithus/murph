...
- Beyond required audits, non-Codex parents (Fable) prefer the local Codex CLI for the token-heavy, low-judgment legwork — bulk code exploration/investigation and implementation — to conserve parent-model context, per the Fable delegation charter in `CLAUDE.md`. Read code inline for small targeted lookups, when an architecture decision needs firsthand understanding, or when a genuinely hairy/tricky problem is worth Fable chasing down directly; delegation is the default, not a hard rule that forces shallow understanding. Fan explorations, implementations, and audits across the independent Codex homes (`c1`/`c3`/`c6`/`c7`) to run them in parallel and spread rate limits. This is a Fable operating posture, not a change to which audit passes are required.
