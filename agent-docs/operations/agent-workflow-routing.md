...
- When the parent agent is Codex and local subagent tooling is available, run required audit passes as spawned local subagents; do not use `codex exec` for those Codex-run completion audits. When the parent agent is Claude or another surface that cannot spawn Codex subagents directly, the `security-privacy-review` and `coverage-write` passes may run on Codex `gpt-5.5` through the local Codex CLI (`codex exec`, with `CODEX_HOME` taken from the local `MURPH_AUDIT_CODEX_HOME` override when set, otherwise the Codex CLI's normal home resolution). The remaining required audit subagents (`prompt-review`, `frontend-review`, `deep-review`) run on whatever model the parent agent is currently running on. All audit subagents use high reasoning by default. Use xhigh reasoning for large or complex changes, high-risk/cross-cutting changes, or audits spanning multiple owners, architecture decisions, or trust-boundary decisions. If required subagent tooling or Codex CLI auth is unavailable for the chosen route, report the limitation and run the pass on the parent agent's current model instead of skipping it.
