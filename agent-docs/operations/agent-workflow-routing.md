...
- Prompt-primary PRs use the required local `prompt-review` pass as the only required completion audit unless non-prompt code independently triggers another audit pass. The prompt-review pass must read the current OpenAI prompt guidance every time.
