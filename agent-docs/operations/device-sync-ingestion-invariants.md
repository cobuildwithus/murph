...
- Data-less historical completion is not a data-arrival fact. Its prepared resource fetch may be admitted while legacy Fitbit remains active only when every job has the same explicit admitted source, a bounded window, and no inline data carrier. Keep data-arrival attribution absent; the existing provider read and canonical import boundaries still prove provenance and migration authority.
