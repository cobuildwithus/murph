...
- Google Health migration freshness is a first-admission fact, not a retry-attempt timestamp. Web resolves the existing trace-keyed successor-arrival mailbox identity before advancing `lastDataAt`, advances it only when that trace is newly inserted and its original acceptance time is strictly after the current exact source `firstSeenAt`, and resolves the trace item even when the same delivery also appends a source-confirmation wake. A duplicate or pre-epoch waiter cannot certify a later authorization.
