...
Required workflow audit subagents use risk-scaled GPT-5.5 reasoning effort. Use medium reasoning for ordinary completion audits, low only for tiny low-risk audits, high for high-risk/cross-cutting or multi-owner audits, and xhigh for large, complex, or trust-boundary-heavy audits. If the current subagent tooling cannot honor the selected reasoning effort, report that limitation explicitly instead of silently changing the pass.
...
9. When step 8 uses an owner-coverage or truthful diff-coverage lane, run the required `coverage-write` pass as a Codex `gpt-5.5` audit worker (per the audit worker rules below) using the workflow audit reasoning policy after any security-privacy pass. Hand that worker `agent-docs/prompts/coverage-write.md` plus the audit handoff packet below, and keep its write scope limited to tests or direct-proof scaffolding for already-landed behavior.
...
- Parent-agent routing is explicit:
  - If the parent agent is Codex and native subagent tooling is available, run required audit passes as native Codex subagents. Do not shell out to `codex exec` from Codex just to create another Codex worker. For Codex-pinned passes such as `security-privacy-review` and `coverage-write`, request Codex `gpt-5.5` and the workflow reasoning effort when the subagent tool supports model/effort overrides; if it cannot honor the override, report that limitation instead of silently downgrading.
  - If the parent agent is not Codex, for example Claude, or the parent cannot create native Codex subagents, run Codex-pinned passes through the local Codex CLI: non-interactive `codex exec` with stdin closed, e.g. `</dev/null`. With an open non-TTY stdin it waits for piped input and hangs. Use `MURPH_AUDIT_CODEX_HOME` as `CODEX_HOME` when set; otherwise let the CLI use normal home resolution. If the Codex CLI or its auth is unavailable, report the limitation and run the pass on the parent agent's current model instead of skipping it.
  - Do not use `codex exec review` for these prompt-based completion passes. Use a normal `codex exec` invocation with the audit prompt and handoff packet, or a native Codex subagent when the parent is Codex.
- The remaining required passes (`prompt-review`, `frontend-review`, `deep-review`) run on whatever model the parent agent is currently running on unless the pass is Codex-pinned above; do not route them to a different model family.
- Required audit subagents use medium reasoning for ordinary completion audits. Use low only for tiny low-risk audits, high for high-risk/cross-cutting or multi-owner audits, and xhigh for large, complex, or trust-boundary-heavy audits.
...
- `coverage-write` is the default write-capable audit pass, must run on Codex `gpt-5.5` with the workflow audit reasoning policy, and should stay narrowly scoped to tests or direct-proof scaffolding.
...
- Do not satisfy required audit passes with `pnpm review:gpt`, `review:gpt` presets, external ChatGPT threads, `cobuild-review-gpt`, or any `thread wake`/autosend flow. Those run in a different stage: the external review loop for non-trivial PR-lane work is `agent-docs/operations/pr-deep-review-loop.md`; it runs after this workflow completes (in parallel with PR CI, per that doc), never instead of it.
...
- The required model choice, Codex `gpt-5.5`, plus the selected workflow audit reasoning effort; do not silently substitute a mini model, a different model family, or a different reasoning effort for this pass.
