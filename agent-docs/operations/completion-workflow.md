...
- Do not satisfy required local audit passes with `pnpm review:gpt`, `review:gpt` presets, external ChatGPT threads, `cobuild-review-gpt`, or any managed-browser/`thread wake`/autosend flow. The PR-lane skip above is the explicit exception: for PR-lane patch implementation, `agent-docs/operations/pr-reviewgpt-loop.md` serves as the audit gate in lieu of spawning the individual local required subagent passes. That loop uses `pnpm review:gpt pr-review` on the repo-configured managed ReviewGPT browser lane, fires in parallel with PR CI per its own doc, and must reach zero accepted findings before merge.
