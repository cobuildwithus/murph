...
For every user-facing `apps/web` UI change, a Codex-native parent must also run the separate Claude Code UI double-check below. That second-model review is not a local audit subagent pass and does not replace `frontend-review` when it applies, the browser proof required by the routed workflow, verification, parent final review, or ReviewGPT.
...
specialist audits, any required Claude Code UI double-check, parent final review,
...
4. Decide the audit path required by the routed task class:
   ...
   - user-facing `apps/web` UI changes add the dedicated `frontend-review` pass and, for a Codex-native parent, the Claude Code UI double-check below
...
6. When `frontend-review` applies, spawn a dedicated audit subagent, hand it `agent-docs/prompts/frontend-review.md` plus the audit handoff packet below, and run it before final review. Keep it review-only and scope it to user-facing `apps/web` surfaces plus the frontend guidance in `agent-docs/FRONTEND.md`. The separate Claude Code UI double-check remains required and runs at the end of step 9.
...
17. Final handoff must report required-check results, direct scenario evidence, and audit findings accepted, fixed, or rejected with reasons. For user-facing `apps/web` UI work completed by a Codex-native parent, also report whether the Claude Code double-check used Fable or the Opus fallback, why fallback was needed when applicable, and how its findings were resolved. Green required checks remain the default completion bar; if a required check failed for a credibly unrelated pre-existing reason, handoff must name the failing command, failing target, and why the current diff did not cause it.
...
The tiny copy-only fast path does not waive the Claude Code UI double-check for a Codex-native parent. It applies to every user-facing `apps/web` UI change, including copy-only and minor presentation changes.

## Claude Code UI Double-Check

After the final user-facing `apps/web` UI and its rendered evidence are stable, a Codex-native parent must ask Claude Code for one fresh, review-only second-model check from the task checkout:
...
2. If that invocation cannot run or cannot return a usable review because the Fable model, authentication, credits, or invocation is unavailable, run the same packet once with `claude --model opus --permission-mode plan --no-session-persistence -p`. Fable unavailability is not a blocker when the Opus fallback completes.
3. Do not use a shell alias, sweep alternate profile homes, reuse or signal a live Claude session, or commandeer another process. If Claude Code itself cannot run or neither model route can return a usable review, report the exact gap and do not claim this double-check passed.
...
The parent verifies every finding against the real UI, resolves accepted findings through the normal review-resolution loop, and reruns the same selected Fable or Opus route only when a meaningful UI fix changes the reviewed surface.
