...
4. Decide the audit path required by the routed task class:
   ...
   - user-facing `apps/web` UI changes add the dedicated `frontend-review` pass and, for a Codex-native parent with available Claude credits, the Claude Code UI double-check below; explicit Claude credit or quota exhaustion uses the ordinary `frontend-review` pass as the non-blocking substitute
