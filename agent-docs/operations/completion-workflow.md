...
When the routed task class requires local audit passes such as `prompt-review`, `security-privacy-review`, `coverage-write`, `frontend-review`, or `deep-review`, treat them as mandatory completion steps before handoff, not optional close-out checks after code, tests, or commit. The PR lane replaces only the default local `deep-review` pass as described below; it does not replace specialist evidence or write-capable proof.
...
**ReviewGPT certifies the PR-specific patch, not the base snapshot.** Once a round reaches zero accepted findings, a later advance of `main` or the configured base does not invalidate it. If the branch is updated by a normal merge or rebase that changes only base history—with no manual conflict resolution and no PR-specific code, test, config, or durable-doc edit—preserve the existing ReviewGPT evidence, rerun CI on the updated head, and continue toward merge. Do not spend another browser/model round merely because the PR is behind or because unrelated work landed on the base. A new ReviewGPT round is required only when the PR-specific patch changes; manual conflict resolution counts as a patch change unless the agent proves it introduced no PR-specific diff change.
...
10. For user-visible, persisted-state, operational, or trust-boundary changes, capture at least one direct scenario check in addition to scripted tests and record the exact evidence. On the current-checkout lane, when `deep-review` applies, spawn it after the specialized review and coverage/proof work is stable: a dedicated review-only audit subagent handed the `murph-deep-review` workflow plus the audit handoff packet below, asked the exact question "What final bugs or edge cases could still break this change in production?" On the PR lane, the ReviewGPT gate owns this default cross-cutting pass unless the user explicitly asks for a local `deep-review` too.
...
This section governs the local `deep-review` trigger. On a PR lane, the ReviewGPT loop in `agent-docs/operations/pr-reviewgpt-loop.md` satisfies this default cross-cutting pass; it does not satisfy the specialist passes above.
