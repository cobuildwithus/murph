...
9. When step 8 uses an owner-coverage or truthful diff-coverage lane, run the required `coverage-write` pass as a Codex `gpt-5.5` audit worker (per the audit worker rules below) using the workflow audit reasoning default after any security-privacy pass. Hand that worker `agent-docs/prompts/coverage-write.md` plus the audit handoff packet below, and keep its write scope limited to tests or direct-proof scaffolding for already-landed behavior.
