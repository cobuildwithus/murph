...
When the routed task class requires audit passes such as `prompt-review`, `security-privacy-review`, `coverage-write`, `frontend-review`, or `deep-review`, treat them as mandatory completion steps before handoff, not optional close-out checks after code, tests, or commit — unless the worktree/PR-lane skip below applies.
...
This completion workflow is standing user approval to spawn the required local audit subagents and review:gpt for routed repo tasks. Do not skip or downgrade a required audit pass because generic agent instructions say subagents need an explicit user request; this document and `AGENTS.md` are that explicit repo-level request for the required completion passes.

**Worktree/PR-lane skip for patch implementation.** When the assistant is implementing a code/test/config patch in a separate git worktree on a task branch with a PR (or that will open one), it may skip the required local audit subagent passes (`prompt-review`, `security-privacy-review`, `coverage-write`, `frontend-review`, `deep-review`) and rely on the external PR-lane ReviewGPT loop in `agent-docs/operations/pr-deep-review-loop.md` to cover that audit surface. The ReviewGPT loop reviews the pushed PR head through the required ReviewGPT file attachments for that pushed head: the guarded source snapshot ZIP, including `review-gpt-pr-context/pr.diff` and `review-gpt-pr-context/changed-files.txt`, plus the matching repomix artifact. It reaches zero accepted findings before merge, which already exercises the cross-cutting bug, security, frontend, coverage, and prompt surfaces those local passes cover. Skipping the local subagent passes on the PR lane is the default; do not spawn them in addition unless the user explicitly asks for a specific local pass, or the change still needs a local proof (for example a `coverage-write` truthful-coverage gap that must be filled before the PR is even reviewable). Step 11's parent-owned local final review still runs, and the PR-lane ReviewGPT loop remains a hard merge-readiness gate. This skip does not apply to current-checkout (non-PR-lane) work: those changes still require the routed local audit passes per the rest of this document.
...
Final review is not a spawned subagent pass. The parent agent runs an explicit local final review (step 11), and for PR-lane work the external loop in `agent-docs/operations/pr-deep-review-loop.md` is the required final review gate before merge-readiness (rounds fire on push, in parallel with PR CI).
Removed 2026-06-12: the `simplify` and `task-finish-review` subagent passes. June 2026 transcript mining across Codex/Claude sessions and `audit-packages/` artifacts showed `simplify` produced no accepted findings, and `task-finish-review` produced mostly low-severity polish while the specialized passes caught the real local bugs and the external review:gpt loop caught what the entire local stack missed. The parent-owned scope-and-shape check (step 2) owns simplification; `/simplify` remains available on demand.
...
15. For non-trivial PR-lane work, run the external loop in `agent-docs/operations/pr-deep-review-loop.md` to zero accepted findings before calling the PR merge-ready, firing each round as soon as its head is pushed (in parallel with PR CI, per that doc — green CI on the final head stays a separate merge-readiness requirement). Do not run the PR-lane ReviewGPT loop against unpushed local changes, local ZIPs, or repomix context that does not represent the pushed PR head; commit and push the reviewed head first, then review the PR URL with ReviewGPT's required file attachments for that pushed head. If a zero-accepted-findings ReviewGPT round already passed and the only later PR update is a normal merge or rebase of `main` or the PR base branch, with no manual conflict resolution or other file edits, do not run ReviewGPT again; wait for PR CI on the updated head and continue the merge path. Expect the Pro review wait/recovery window documented there; slow rounds are not a reason to downgrade models, switch to connector-based context, or start duplicate threads. It is the required final review gate for that lane and never substitutes for the local passes above.
...
When opening or updating a PR for worktree/PR-lane work, the PR body must state the PR's intent so reviewers (human and ReviewGPT) judge the diff against the requirement, not against its current runtime state. Keep it tight — a few short sections, not a wall of text.
...
- Non-Codex parent agents such as Claude run only the Codex-billed `security-privacy-review` and `coverage-write` passes on Codex `gpt-5.5` through the local Codex CLI (non-interactive `codex exec` with stdin closed, e.g. `</dev/null`; with an open non-TTY stdin it waits for piped input and hangs), to keep audit cost on the Codex subscription instead of per-token parent-model spend. When the `MURPH_AUDIT_CODEX_HOME` environment variable is set, pass it as `CODEX_HOME` so operators can point audits at a specific local Codex home; otherwise let the Codex CLI use its normal home resolution (existing `CODEX_HOME` or `~/.codex`). If the Codex CLI or that auth is unavailable in the current environment, report the limitation and run the pass on the parent agent's current model instead of skipping it.
- The remaining required passes (`prompt-review`, `frontend-review`, `deep-review`) run on whatever model the parent agent is currently running on; do not route them to a different model family.
...
- For current-checkout (non-PR-lane) work, do not satisfy required audit passes with `pnpm review:gpt`, `review:gpt` presets, external ChatGPT threads, `cobuild-review-gpt`, or any `thread wake`/autosend flow. Those run in a different stage. For PR-lane patch implementation, the worktree/PR-lane skip above explicitly authorizes the external PR-lane ReviewGPT loop in `agent-docs/operations/pr-deep-review-loop.md` to serve as the audit gate in lieu of spawning the local required subagent passes; that loop fires in parallel with PR CI per its own doc and must reach zero accepted findings before merge.
