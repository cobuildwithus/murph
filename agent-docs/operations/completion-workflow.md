...
**Worktree/PR-lane skip for patch implementation.** When the assistant is implementing a code/test/config patch in a separate git worktree on a task branch with a PR (or that will open one), it may skip the individual required local audit subagent passes (`prompt-review`, `security-privacy-review`, `coverage-write`, `frontend-review`) and rely on the PR-lane Codex deep-review loop in `agent-docs/operations/pr-deep-review-loop.md` to cover that audit surface. That loop runs the `deep-review` pass on the local Codex CLI against a clean worktree at the pushed PR head, reviewing the same commit CI and merge target refer to; Codex reads the worktree and computes the diff directly, so there is no ZIP/repomix packaging. It reaches zero accepted findings before merge, and as the consolidated PR gate its whole-diff sweep exercises the cross-cutting bug, security, frontend, coverage, and prompt surfaces those individual local passes cover. Skipping the individual local subagent passes on the PR lane is the default; do not spawn them in addition unless the user explicitly asks for a specific local pass, or the change still needs a local proof (for example a `coverage-write` truthful-coverage gap that must be filled before the PR is even reviewable). Step 11's parent-owned local final review still runs, and the PR-lane Codex deep-review loop remains a hard merge-readiness gate. This skip does not apply to current-checkout (non-PR-lane) work: those changes still require the routed local audit passes per the rest of this document.
...
Final review is not a spawned subagent pass. The parent agent runs an explicit local final review (step 11), and for PR-lane work the Codex deep-review loop in `agent-docs/operations/pr-deep-review-loop.md` is the required final review gate before merge-readiness (rounds fire on push, in parallel with PR CI).
Removed 2026-06-12: the `simplify` and `task-finish-review` subagent passes. June 2026 transcript mining across Codex/Claude sessions and `audit-packages/` artifacts showed `simplify` produced no accepted findings, and `task-finish-review` produced mostly low-severity polish while the specialized passes caught the real local bugs and the post-completion PR deep-review loop caught what the entire local stack missed. The parent-owned scope-and-shape check (step 2) owns simplification; `/simplify` remains available on demand.
...
15. For non-trivial PR-lane work, run the Codex deep-review loop in `agent-docs/operations/pr-deep-review-loop.md` to zero accepted findings before calling the PR merge-ready, firing each round as soon as its head is pushed (in parallel with PR CI, per that doc — green CI on the final head stays a separate merge-readiness requirement). Do not run the loop against unpushed local changes or a dirty worktree that does not represent the pushed PR head; commit and push the reviewed head first, then run the loop on the local Codex CLI against a clean worktree at that pushed head. If a zero-accepted-findings round already passed and the only later PR update is a normal merge or rebase of `main` or the PR base branch, with no manual conflict resolution or other file edits, do not run the loop again; wait for PR CI on the updated head and continue the merge path. It is the required final review gate for that lane and never substitutes for the local passes above.
...
When opening or updating a PR for worktree/PR-lane work, the PR body must state the PR's intent so reviewers (human and the Codex deep-review loop) judge the diff against the requirement, not against its current runtime state. Keep it tight — a few short sections, not a wall of text.
...
This block is the load-bearing input for the PR-lane Codex deep-review loop and any human reviewer; without it, the reviewer may misread a deliberately-disabled wiring state as evidence the functionality should be deleted, or rank a Critical correctness gap as a complexity-collapse opportunity.
...
- Do not satisfy required audit passes with `pnpm review:gpt`, `review:gpt` presets, external ChatGPT threads, `cobuild-review-gpt`, or any managed-browser/`thread wake`/autosend flow — ReviewGPT is retired for completion review as of 2026-07-06 and all review runs on the local Codex CLI. For PR-lane patch implementation, the worktree/PR-lane skip above explicitly authorizes the PR-lane Codex deep-review loop in `agent-docs/operations/pr-deep-review-loop.md` to serve as the audit gate in lieu of spawning the individual local required subagent passes; that loop fires in parallel with PR CI per its own doc and must reach zero accepted findings before merge.
