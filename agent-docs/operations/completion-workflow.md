...
For every user-facing `apps/web` UI change, a Codex-native parent must also attempt the separate Claude Code UI double-check below while Claude credits are available. That second-model review is not a local audit subagent pass and does not replace `frontend-review` when it applies, the browser proof required by the routed workflow, verification, parent final review, or ReviewGPT. Explicit Claude credit or quota exhaustion routes to the ordinary Codex `frontend-review` pass instead and does not block completion.
...
17. Final handoff must report required-check results, direct scenario evidence, the `product-experience-review` purpose verdict when that pass applies, and audit findings accepted, fixed, or rejected with reasons. For user-facing `apps/web` UI work completed by a Codex-native parent, also report whether the review used Fable, the Opus fallback, or the credit-exhaustion Codex `frontend-review` substitute; state why fallback was needed and how findings were resolved. Green required checks remain the default completion bar; if a required check failed for a credibly unrelated pre-existing reason, handoff must name the failing command, failing target, and why the current diff did not cause it.
...
5. Rerun the affected audit pass when the fix materially changes that pass's risk surface:
   ...
   - rerun the selected Claude Code Fable or Opus UI reviewer when meaningful UI fixes land after its review; when credit exhaustion selected the Codex substitute, follow the normal `frontend-review` rerun rule instead
...
If ReviewGPT is selected, an explicit request for deep review or a final bug hunt is satisfied by that PR gate and does not add a local `deep-review`. If ReviewGPT is opted out or unavailable and a condition above applies, use local `deep-review` instead and report the route change. Do not use either cross-cutting gate as a replacement for `product-experience-review`, `frontend-review`, or `coverage-write`. If none of the conditions above apply, skip cross-cutting review and proceed through the normal completion workflow.
...
3. Explicit Claude credit or quota exhaustion is the only non-blocking Claude Code gap. If either attempted route reports it, stop making Claude requests. An already-completed task-scoped `frontend-review` satisfies the substitute; if that pass was skipped under the tiny copy-only fast path, run the required `frontend-review` pass now with its normal audit handoff packet. Record the credit-exhaustion route without claiming that the Claude Code double-check passed.
...
- `product-experience-review` is a review-only pass for new or materially changed user-facing product behavior across conversation, runtime, and web UI. It states the irreducible purpose, traces immediate feedback through every asynchronous handoff to final delivery or recovery, tests the ordinary timing class, and challenges every avoidable word, action, choice, screen, concept, and delay. It requires rendered evidence when frontend craft is in scope and does not replace `frontend-review`'s accessibility, responsive, and design-system ownership.
...
- Do not satisfy `prompt-review`, `coverage-write`, `product-experience-review`, or `frontend-review` with `pnpm review:gpt`, ReviewGPT presets, external ChatGPT threads, or managed-browser/`thread wake`/autosend flows. ReviewGPT and local `deep-review` are mutually exclusive cross-cutting gates. Prompt-primary PRs use `prompt-review` and do not run ReviewGPT unless non-prompt scope independently requires the loop; eligible PRs must still reach zero accepted findings before merge.
