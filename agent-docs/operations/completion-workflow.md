...
1. Finish the functional implementation first.
   ...
   Use those canonical commands unchanged. Automatic execution stays local; use explicit `MURPH_VERIFY_EXECUTOR=crabbox` only for the documented admission fallback. Use the `:local` aliases only for executor diagnosis or an explicitly environment-bound check.
