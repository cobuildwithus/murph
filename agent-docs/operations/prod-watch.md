...
2. The supervisor acquires a non-waiting run lock, gathers deterministic database/repository aggregates, and runs status-only Vercel and Stripe availability/authentication probes.
...
The scheduler creates up to 288 fresh Cloudflare collection sessions per day. Vercel and Stripe stay deterministic and availability-only: their fixed CLI probes discard stdout/stderr at the process boundary, and they never fetch or parse row/event payloads. Add detailed metrics only when a provider-supported aggregate or field-projection API can preserve that boundary.
...
1. `launchd` invokes `/bin/zsh -f -c` so user shell startup files are disabled, rebuilds the child environment from an empty base, then starts the verified Node executable directly with an exact-head-pinned owner-only runtime's `tsx` entrypoint and `prod-watch.ts run --scheduled --provider-child`. It exports the dedicated Codex home for authentication only, the installer-approved absolute Codex executable and SHA-256, the shared private state root, the approved head derived from the clean installer checkout, and one fixed bounded PATH containing `$HOME/.local/bin` plus standard Homebrew/system directories. Before Node starts, the command uses the absolute system Git executable with global/system configuration disabled and fsmonitor forced off to verify the pinned head and clean tracked tree. It clears named test controls and never enables worker dispatch.
...
5. Deterministic adapters invoke one fixed Vercel project-inspection command and one fixed Stripe live-balance command only to prove CLI availability and authentication. Those children inherit no writable authority from the watcher, use ignored stdio, and return only exit status; the watcher never buffers, parses, or persists their free-form output or provider objects. Their source coverage therefore remains `partial` and contributes no production metrics. A new `codex exec --ephemeral --ignore-user-config --strict-config` process receives only bounded aggregate context and a JSON output schema, uses read-only sandboxing, pins the reviewed model and effort, disables web/apps/hooks/multi-agent/shell and other nonessential built-ins, defines only the lockfile-pinned Cloudflare Observability MCP transport, writes one temporary provider envelope, and exits. The dedicated Codex home supplies authentication material only; arbitrary instructions and tool configuration are not inherited. The file is validated locally and deleted after merge. `--provider-shadow` exercises the same path but does not merge evidence or advance provider health.
...
# Vercel/Stripe availability probes plus the isolated Cloudflare child.
...
Unknown fields, overlong arrays, arbitrary dimensions, free-form text, invalid timestamps, and malformed tokens fail closed. The local parser also rejects absolute/local paths, URLs, UUIDs, common provider/direct-ID shapes, credential-shaped values, JWTs, and long numeric identifiers before evidence can enter state or a projection. The production source universe is always database, Vercel, Cloudflare, and Stripe; callers cannot narrow it. Provider status may be `ok` with `auth: ok` and no metric facts to represent an availability-only probe, but that source remains partial and non-scorable. When any provider rate facts are present, complete coverage requires a provider-wide request/error/timeout triple whose exact dimensions are only `{source}`. Surface-specific counters are supplementary, and every emitted exact-dimension triple must still be complete. Measured zero numerators are valid, but missing numerators are unknown. Provider producers cannot supply `sampleCount` fields: the matching exact-dimension request counter is the only rate denominator and the local scorer owns that relation. A source failure never becomes a zero counter. Evidence that is availability-only, degraded, partial, stale, failed, or unauthenticated is excluded from production scoring and provider release correlation; its health/failure metadata still drives monitor incidents.
...
| Vercel | Deterministic status-only CLI probe | Provider-supported aggregate/field-projection API when available | One fixed project-inspection command proves CLI availability and authentication. Stdio is ignored by the parent, so no request-log row, free-form message, URL, request ID, or provider payload enters watcher memory or state. Coverage stays partial and contributes no counters, latency, fingerprints, or release context. |
...
| Stripe | Deterministic status-only CLI probe | Provider-supported aggregate/field-projection API when available | One fixed live-balance command proves CLI availability and authentication. Stdio is ignored by the parent, so no event object, resource snapshot, object ID, or provider payload enters watcher memory or state. Coverage stays partial and contributes no counters, latency, fingerprints, or release context. |
...
| Provider/deployment error-rate regression | 50 requests/deployments and 10 errors | Current rate >=2%, at least 3x prior rate, and +1 percentage point; or fixed >=5% when no baseline exists | 2 runs | Diagnosis only unless deployment-correlated and nonsensitive. Availability-only Vercel/Stripe probes cannot produce this candidate; Stripe remains alert-only if a future aggregate adapter supplies rate evidence. |
...
| Provider/ingress timeout-rate regression | 50 requests/accepted ingress items and 5 timeouts/incomplete items | Current rate >=1%, at least 3x prior rate, and +0.5 percentage point; or fixed >=2% when no baseline exists | 2 runs | Diagnosis only unless deployment-correlated and nonsensitive. Availability-only Vercel/Stripe probes cannot produce this candidate; Stripe remains alert-only if a future aggregate adapter supplies rate evidence. |
...
| Private production data leakage | Aggregate-only fixed SQL; strict allowlists; hashed fingerprints; unknown-field rejection; no raw text fields; status-only Vercel/Stripe probes with ignored stdio; latest snapshot only. |
...
| Provider evidence | Availability-only evidence is accepted as authenticated but partial and non-scorable. If rate facts are present, a source-wide `{source}` request/error/timeout triple is the sole source-completeness proof; consistent surface-only subsets remain partial. Exact-dimension rate denominators remain mandatory supplementary proof. Fake Vercel/Stripe CLIs emit hostile free-form text, URLs, request/object IDs, and nested payloads on both output streams; exact invocation, ignored stdio, snapshot/state absence, and peak three-way fanout are asserted. A hostile source Codex config cannot override the reviewed model/effort or enable web, apps, hooks, multi-agent, remote plugins, or shell; the fake child captures the real stdin and proves each aggregate-only prompt clause plus the dynamic worker/source/window request. |
...
| Scheduler | Template proof requires a 300-second interval, `KeepAlive=false`, `/bin/zsh -f -c`, an empty-base child environment, exact-head/runtime placeholders, an absolute system Git executable, disabled global/system Git configuration and fsmonitor, a fixed helper PATH, the dedicated Codex home/profile, installer-pinned Codex path/digest, production test-control clearing, provider collection, no worker dispatch, and no concrete machine path. Rendered output retains literal `$HOME` paths and rejects unsafe repository/runtime paths. A macOS-only command smoke places a detectable `.zshenv`, hostile PATH-first Git, and blocking global fsmonitor in the scheduler HOME; starts from a minimal launchd environment; proves none execute; runs the rendered Node/tsx chain; and proves collection. The same smoke proves wrong-head and dirty-tree gates block Node. Separate regressions prove that the production entrypoint rejects test controls and an alternate Codex executable. A clean disposable checkout plus disposable-home/fake-`launchctl` lifecycle test proves unmanaged-file refusal, final preflight execution from the pinned runtime, preflight before managed replacement, pinned runtime setup, exact prior-plist/job restoration after bootstrap/enable/final-confirmation failure, truthful first-install cleanup failure, and uninstall behavior; an operator smoke remains required before activation. |
...
Scheduler replacement is transactional at the managed-job boundary: the candidate is prepared and preflighted from its pinned runtime before cutover, and any bootstrap, enable, or final-state-confirmation failure restores the exact prior managed plist and loaded/absent state. If restoration itself cannot be confirmed, install fails with an explicit restore-failure code and does not claim success.
