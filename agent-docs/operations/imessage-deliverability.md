...
- Keep `HOSTED_ONBOARDING_LINQ_ROUTE_TRANSITION_PROOF_ENABLED=0` until the proof-consuming runner fingerprint and exact-input migration are verified. The disabled state still admits replacement-chat inbound and preserves reply-anchored delivery, but retains the former home binding. Enable the flag only after consumer verification, and disable it before rolling the runner below the proof-consuming version.
