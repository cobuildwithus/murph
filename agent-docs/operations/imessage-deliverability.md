...
- A recognized member's iMessage group inbound on their exact assigned `AT_RISK` Linq line stays on the ordinary canonical group route because the member initiated that group. If a managed incoming group line is hard-blocked, Web must not send from it, mutate the group roster, or create route-transfer state. Instead, the triggering active member gets one private, link-free recovery message from a healthy proactive line selected through the existing line policy and daily capacity counter. The recovery delivery intent pins the healthy sender and deterministic copy variant to the original inbound event; retries preserve both. Every reviewed variant must show the same selected Murph number and tell the member to add it inside the existing group chat, then retry the intro.
