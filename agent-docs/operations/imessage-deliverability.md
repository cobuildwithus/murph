...
- Legacy bare personal routes are upgraded only after a fresh direct-home route confirms personal runtime scope. That fresh route proves its own target; an immutable accepted Linq input event with direct-conversation metadata and an authorized Linq reply target is required to prove a former target. Managed automation rows never create route authority. The canonical registry derives the complete exact-target set under its owner lock and commits every route plus one audit in a single write batch. Archived routes, explicit group routes, unrelated targets, and targets without direct provider evidence stay unchanged.
