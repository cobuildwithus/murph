...
to persist a strict-v1 outbox intent containing a wearable card. The first such
intent makes the V7-capable runner the rollback floor: stop new V7 production
and drain, resolve, or compatibly rewrite every persisted V7 intent before
reverting, because an older runner rejects V7 rather than converting it to text.

Saved-view persistence is a reader-first compatibility rollout. Deploy a
preferences reader that accepts both an omitted `savedHealthViews` field and its
bounded typed collection before enabling save/edit commands or assistant reads.
Once any writer persists a non-empty collection, that compatible reader is the
rollback floor: disable the writer and remove the field with compatible code
before reverting to a strict pre-field reader. Card delivery must treat an
unreadable or deleted saved view as missing; it must not recreate the view or
fall back to a name-shaped mutation.
...
For the managed goal-aware daily-nutrition workflow, nullable goal snapshots remain replay/rendering compatibility only. New cards require the complete five-metric active bundle and the prompt-owned known-context numeric-suitability rule. Before an interactive suitability question or a numeric target proposal or change, the assistant inspects the current conversation and performs one canonical-memory read; it does not repeat categories those sources already establish. It does not run a universal medical-history or measurement preflight, and reads another clinical record family only when the focused evidence raises a concrete concern. Safety applies only to the target or advice the known fact materially affects, so an allergy, intolerance, dietary restriction, or clinician-directed diet is not by itself a reason to suppress benign totals or an accepted compatible bundle. Known context that makes self-directed numeric guidance unsuitable suppresses the card with no Goal or measurement mutation, while a missing or failed unrelated read does not block a routine card. An interactive requested card whose fresh totals reveal incomplete meal coverage first repairs the exact existing meal from accepted conversation context and bounded saved-meal evidence; when identity or amount remains insufficient, it asks one focused question and stops without presenting the partial card as the completed day. Partial cards remain compatible only for an explicit request to see available partial data after the limitation is clear. This generic meal-recovery question does not widen scheduled messaging authority. After explicit interactive card or target-setting intent, a genuinely missing bundle creates or updates one paused canonical proposal and explains its five provisional values in ordinary text; only a later unambiguous acceptance may reapply the same known-context rule, activate and read back the proposal, re-read same-date totals, and attach the pending card. Scheduled closeout authority never permits screening questions, target setup, or proposal mutation.
...
strict V1-V7 presentation envelopes and otherwise fails closed without weakening
