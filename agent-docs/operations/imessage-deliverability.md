...
Response cards are optional outbox-owned presentation siblings of response media, not a direct-send surface or a separate delivery owner. The same attachment tool serves explicit current private-direct requests and the managed meal closeout; it remains unavailable in groups and unrelated scheduled turns. A card replaces the whole final response, so it is eligible only when that card alone completely satisfies the current request. The outbox continues to own the semantic message, target, status, receipt, retry, and idempotency lifecycle, and a card cannot coexist with media.

Card values are immutable message content owned by the existing outbox effect. V1 remains a supported calorie/protein/carbs/fat compatibility input. V2 adds the exact canonical fiber total and nullable per-metric goal snapshots. Each non-null goal snapshot carries only an exact target copied after a complete bounded active-goal read proves exactly one qualifying record for that daily metric and unit, plus Murph's frozen semantic assessment. A saturated result or ambiguous match leaves the target null. Missing or partial totals can carry only an `unavailable` status, and an assessed status must not point opposite the frozen total and target. The snapshot is presentation for that message, not persisted goal progress or a new source of truth. A missing or untrusted target stays null and neutral. There is no persisted remote card state, fetch API, cleanup lifecycle, or card-specific queue. Linq sends a noninteractive static layout containing the date, meal count, available totals, and an explicit partial marker instead of delegating the transcript balloon to the installed extension. The first non-null V2 goal in canonical metric order contributes its exact target and frozen status, so the static replacement does not silently discard all goal context. Nutrition keeps a fixed, non-sensitive HTTPS product URL that does not encode health state. V3 compact tables instead use a bounded Base64URL fragment for the same immutable presentation values visible in the private-direct message. That fragment may contain health-related values, but it omits tracking metadata and must never contain member identity, a canonical record reference, a credential, or other authority; the offline Messages extension decodes it locally and the Web origin never receives it through an HTTPS request. Hosted inbound routing keeps the opaque conversation locator used for continuity separate from the trusted provider reply thread. An ordinary auto-reply sends through that thread binding and does not duplicate it as an explicit target, so Linq appends the card to the existing direct chat. The fallback body stays free of dates and numbers, truthfully asks the member to request a text summary, and avoids Apple's card-downgrading data detectors. Linq capability failure or an ineligible route falls back to the same deterministic ordinary text through the existing channel delivery path. Before that text enters Linq, the current outbox intent is atomically frozen as text-only under its existing delivery key. After capability succeeds, only a definitive pre-acceptance app-card rejection (HTTP 400, 415, or 422, plus a structured stale-chat 404) may use that text fallback; an arbitrary or malformed 404 remains terminal. The outbox first replaces the card and promotes its durable delivery key to the distinct stable fallback key. Before the fallback enters Linq, the existing Web transaction terminalizes the exact original provider-dispatch fence and claims that stable fallback key under the same intent, chat, source, and thread target. A process interrupted after local fallback persistence repeats the same idempotent fence transfer on retry, so a lost control response cannot leave an obsolete card dispatch blocking later group routing. The committed text identity replaces the in-flight card authority for provider entry, receipts, failure handling, and authorized stale-direct-thread materialization, so the original card request cannot veto recovery. A process interrupted after provider acceptance therefore replays only the frozen text and same key. A timeout, transport error, rate limit, or server failure remains ambiguous. A card-bearing Linq attempt marked as possibly accepted instead enters the existing delivery-confirmation-pending state with its card and provider-idempotent key retained. A later drain bypasses capability selection and replays only the identical card body and key without requiring a rehydrated raw recipient phone; it cannot replace the original card effect with text. If a resumed non-replay card finds its Web provider-dispatch claim already started, it becomes confirmation-pending before another capability or provider message request. Provider acceptance or deduplication closes the original Web dispatch fence. Text retry is permitted only after a definitive card rejection and the text-only transition has committed with the card cleared and the stable fallback identity frozen. A future interactive transcript renderer must complete production-device presentation proof before changing this static default.

The exact-dispatch preflight is lifecycle-independent: the same original-card
or fallback row remains recovery authority after provider start, acceptance,
delivery, or failure. It must match the runtime source and source reference,
target kind, thread, and source line; any mismatch stops before local card
mutation or provider entry.

The optional capability read gets one complete 2.5-second attempt, including
successful or error response-body consumption, with no transport, server, or
rate-limit retry. Its timeout, any other capability failure, or an ineligible
route selects the same persisted deterministic text path before message
dispatch. Only an exact classified HTTP 404 `chat_not_found` joins 400, 415,
and 422 in the definitive app-card rejection set; generic 404 remains outside
fallback.

For a structured stale-chat rejection during detached exact replay, the
same-chat fallback description above does not require reuse of the rejected
chat. Web identifies the exact predecessor by its stable delivery identity,
runtime intent, source, and thread kind, then authorizes a current direct chat
before local card-to-text persistence. The transaction terminalizes the
predecessor on its old chat and claims the fallback on that authorized current
chat. If no current chat is available, the card and original key remain intact
and confirmation-pending. Delivery-control errors retain their type through the
Linq HTTP wrapper; post-admission 429 exhaustion is ambiguous, while a
capability-specific 429 may select deterministic text before message dispatch.
...
an error. A hosted delivery-control exception is rethrown for reconciliation
instead of being logged as selected fallback. Diagnostics identify only the bounded failure class and safe provider
