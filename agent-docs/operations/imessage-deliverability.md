...
- Before hosted cron runs, a bounded, time-rotated legacy migration may add that marker to a known-direct pre-marker route only when the signed web control plane confirms the exact stored target against the member's canonical current or pending home route. The assertion bypasses group-container authority; unknown, copied, group, and mismatched targets stay unmarked and fail closed. Unconfirmable compatibility records do not defer unrelated cron work.
- Provider admission for a hosted Linq cron requires that marked route on its canonical automation or authenticated device-activity parent. Generic legacy local rows have no parent-route authority, so they stay failed with retry ownership before any provider call. A device occurrence queued immediately before marker repair keeps authority only across that exact marker-strengthening change; every user-editable route change still makes it stale.
- The resulting direct-home-only assertion is per-delivery authority, not route ownership. It stays on the exact outbox intent through retries, and Web rejects both the initial assertion and final provider claim if the target has become any durable thread route, including one owned by the same member.
- Outbox writers persist an explicit direct-home-only `true` or `false` classification. Pre-classification direct Linq intents without a reply anchor fail closed as direct-home-only at provider entry; legacy reply-anchored intents retain ordinary current-inbound authority. Once written, `true` is monotonic across dedupe reuse. This schema establishes a runner rollback floor: deploy Web first, then converge every runner before new outbox writes, and do not roll a runner below the boolean-aware schema.
- Background marker repair yields or aborts before another control-plane assertion and never commits a partial confirmation batch after foreground input arrives.
...
- Legacy bare personal routes are upgraded only from immutable direct Linq mailbox inputs. Web routing binds every admitted direct-home transition and carries a narrow former/current home-chat proof on that exact mailbox item. Foreground processing defers cron and completes the current reply first; the raw pending-input index then retains the immutable proof until a background pass repairs successfully before cron. A failed canonical write therefore preserves the existing immediate wake/retry path without delaying the accepted reply. Quota rejection remains ahead of binding and mailbox append, so a later admitted inbound can still produce proof instead of losing it.
- Routes that became stale before transition proofs existed require an audited, exact retained input whose reply target is the former personal route. Run `pnpm --dir packages/assistant-engine repair:legacy-personal-home-routes -- --vault-root <vault-root> --input-id <audited-input-id> --apply` for those inputs during the consumer-first rollout. This bounded operator path is idempotent and does not scan generic retained history. Managed automation rows and generic retained input history never create route authority. The canonical registry derives the complete exact-target set under its owner lock and commits every route plus one audit in a single write batch. Archived routes, explicit group routes, unrelated targets, and targets without direct routing evidence stay unchanged.
- Keep `HOSTED_ONBOARDING_LINQ_ROUTE_TRANSITION_PROOF_ENABLED=0` until the proof-consuming runner fingerprint and exact-input migration are verified. The disabled state still admits replacement-chat inbound, commits that chat as the durable home, and preserves reply-anchored delivery, but omits former-home proof from the mailbox event for old-consumer compatibility. Migrate legacy routes on the former chat from an audited exact retained input before enabling the flag, and disable the flag before rolling the runner below the proof-consuming version.
