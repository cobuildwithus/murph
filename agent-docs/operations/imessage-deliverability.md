...
- A terminal HTTPS URL may be sent as Linq's native link-only rich-preview part. An existing-chat link-only message, including a payment URL, remains one provider message with no added text. When caller-supplied text or media precedes the URL, the existing local or hosted egress owner accepts that primary message first and then sends the link-only preview under the same delivery's deterministic `:link` idempotency-key suffix. The primary message id remains the logical delivery result. Hosted two-request sends divide the existing ten-second provider budget evenly so they still fit the group-aware provider fence; retry replays both stable identities and lets Linq deduplicate the accepted half. New-chat creation may use that two-step flow only when the caller supplied URL-free text or media for the required first message; link-only, multi-URL, or other URL-bearing first-message input must fail before provider entry instead of inventing or leaking opener copy. Reaction-bound messages keep their URL nonterminal and stay one provider message. In existing chats, embedded, other nonterminal, non-HTTPS, credentialed, and oversized URLs remain ordinary text.
