...
- Legacy bare personal routes are upgraded only from an immutable direct Linq mailbox input. The web routing transaction carries a narrow former/current home-chat transition proof on that mailbox item; the accepted input event preserves it until the runtime attempts repair. The runtime repairs before checkpoint, so a failed canonical write leaves the existing mailbox item pending for its normal wake/retry path. Managed automation rows and generic retained input history never create route authority. The canonical registry derives the complete exact-target set under its owner lock and commits every route plus one audit in a single write batch. Archived routes, explicit group routes, unrelated targets, and targets without direct routing evidence stay unchanged.
