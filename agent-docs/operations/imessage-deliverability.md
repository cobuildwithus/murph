...
- A model-admitted instant-start invite is pending continuation proof, not a send target. While it remains attached, only the exact pending direct chat, participant, and contacted line may continue enrollment; ordinary signup dispatch fails closed. Stripe provisioning and activation serialize on the existing member row, and signup delivery rechecks both the cleared continuation and inactive member state immediately before provider dispatch.
