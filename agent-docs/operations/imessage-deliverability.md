...
- A recognized member's iMessage group inbound on their exact assigned `AT_RISK` Linq line stays on the ordinary canonical group route because the member initiated that group. If the exact assigned managed incoming group line is hard-blocked, Web must not send from it, mutate the group roster, or create route-transfer state. Instead, the triggering active member gets one private, link-free recovery message from a healthy proactive line selected through the existing line policy and daily capacity counter at transport time. The recovery id is stable for the member, hard-blocked incoming line, and group thread, while a safe source ref distinguishes exact provider events. Any live or successful attempt remains the tuple-wide convergence point, so repeated source events do not repeatedly prompt. A provider-correlated terminal failure may advance to one of five bounded attempt keys only for a different source event; replay of the exact failed event cannot send again. Retries preserve a pinned healthy sender only while it remains healthy and different from the failed incoming line; otherwise provider entry fails closed. Every reviewed variant must show the same selected Murph number and tell the member to add it inside the existing group chat, then retry the intro.
