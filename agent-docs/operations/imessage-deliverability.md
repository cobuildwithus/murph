...
Response cards are optional outbox-owned presentation siblings of response media, not a direct-send surface or a separate delivery owner. The same attachment tool serves explicit current private-direct requests, exact private-direct scheduled turns whose saved instructions explicitly request a card, and the managed meal closeout; it remains unavailable in groups. Occurrence authority alone is not card intent. A card replaces the whole final response, so it is eligible only when that card alone completely satisfies the current request. The outbox continues to own the semantic message, target, status, receipt, retry, and idempotency lifecycle, and a card cannot coexist with media.

Card values are immutable message content owned by the existing outbox effect. V1 remains a supported calorie/protein/carbs/fat compatibility input with unchanged text rendering. V2 adds the exact canonical fiber total and nullable per-metric goal snapshots. Before every daily-nutrition attachment, the prompt applies the dedicated card-time safety gate against already-known current context, even when a complete active bundle exists and during a scheduled closeout; prior acceptance is not current safety proof, and scheduled authority does not permit new screening questions or target setup. The managed workflow then requires one unambiguous scalar target for each of the five metrics, resolved across active canonical Goals. After an explicit interactive card or target-setting request, absent goals cause it to reuse one paused canonical Goal proposal and explain all five effective values and their material reasoning in ordinary text. Any write that adds or changes a derived managed target atomically supplies the complete intended array with paused status, even when that Goal was active, so revised values cannot appear before renewed acceptance; overlap removal alone may retain active status when the explicit owner supplies the metric. If an explicit card request caused the proposal, the member's next unambiguous acceptance may activate and read back that record, reapply safety, re-read same-date totals, and attach the first goal-aware card in the same response. Target-setting-only requests, corrections, declines, ambiguous replies, and compound requests remain text-only. A scheduled meal closeout may use an already accepted complete bundle, but it never asks for target inputs or creates, changes, or explains a proposal; without the bundle it retains ordinary closeout text and no card. An explicit member- or clinician-owned target wins metric by metric. A saturated or ambiguous goal read, a scalar-incompatible range, unsafe numeric context, or a missing responsible calorie input suppresses the card instead of producing a goal-less one. Each attached snapshot carries only an exact target copied from the resulting complete active bundle plus Murph's frozen semantic assessment. Missing or partial totals can carry only an `unavailable` status, and an assessed status must not point opposite the frozen total and target. The snapshot is presentation for that message, not persisted goal progress or a new source of truth. The nullable schema remains for compatibility; it is not permission for this managed workflow to attach an incomplete card. There is no persisted remote card state, fetch API, cleanup lifecycle, or card-specific queue. Deterministic V2 semantic text includes every metric's exact target and frozen status, including an explicit unavailable target/status where applicable, so email and Linq's ordinary-text fallback preserve the same five-goal meaning. Linq explicitly enables interactive presentation so recipients with the shipping Messages extension see its SwiftUI balloon; recipients without it retain the provider static layout. That static layout contains the date, meal count, available totals, an explicit partial marker, and the first non-null V2 goal's exact target and frozen status in canonical metric order. New V1, V2, and V3 cards use the same bounded HTTPS Base64URL fragment family for the immutable presentation values visible in the private-direct message; the extension also retains legacy data-URL reads for already-sent nutrition cards. Encoding is not encryption. Every fragment must omit member identity, canonical record references, credentials, and other authority; V3 additionally strips its tracking reference before encoding. The offline Messages extension decodes the fragment locally, and the Web origin never receives it through an HTTPS request. Hosted inbound routing keeps the opaque conversation locator used for continuity separate from the trusted provider reply thread. An ordinary auto-reply sends through that thread binding and does not duplicate it as an explicit target, so Linq appends the card to the existing direct chat. The fallback body stays free of dates and numbers, truthfully asks the member to request a text summary, and avoids Apple's card-downgrading data detectors. The optional capability read gets one complete 2.5-second attempt, including successful or error response-body consumption, with no transport, server, or rate-limit retry; timeout, any other capability failure, or an ineligible route immediately falls back to the same deterministic ordinary text through the existing channel delivery path. Before that text enters Linq, the current outbox intent is atomically frozen as text-only under its existing delivery key. A caught capability-check failure writes one bounded warn entry to the durable hosted runtime log before that same fallback; a successful `available: false` result remains expected and silent. After capability succeeds, only a definitive pre-acceptance app-card rejection (HTTP 400, 415, or 422) may use that text fallback; the outbox writes the same bounded entry with the rejection class, then replaces the card and promotes its durable delivery key to the distinct stable fallback key. These entries are fire-and-forget and allowlist-projected: they carry only the fallback reason and safe error classification—never error messages, card values, request or response bodies, recipient or thread values, delivery keys, credentials, or provider prose—and a failed or stalled log write does not alter control flow. That committed text identity also replaces the in-flight card authority for provider entry, receipts, failure handling, and authorized stale-direct-thread materialization, so the original card request cannot veto recovery. A process interrupted after provider acceptance therefore replays only the frozen text and same key. A timeout, transport error, rate limit, or server failure from the message mutation remains ambiguous and cannot trigger a second send. Production-device proof of visible nutrition and compact-table balloons is required before interactive rollout because provider acceptance and delivery receipts do not prove extension rendering.
