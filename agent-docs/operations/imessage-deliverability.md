...
- On the Linq instant-start path, web fires one best-effort typing start on the inbound chat right after the planner transaction creating the member commits, so a first-ever message is not met with a silent chat while the cold runtime boots. It is a one-shot feedback hint with a short timeout, no retries, no egress assertion, and no effect on webhook handling; on the success path the runtime's own typing session and reply supersede it. If the webhook instead fails after the hint started, web chains one best-effort typing stop behind the in-flight start so the hint cannot promise a reply that no surviving continuation owns; the later webhook retry starts fresh.
