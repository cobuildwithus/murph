...
`packages/contracts/src/health-entities.ts` should remain the shared owner of health taxonomy and registry metadata.
...
- per-family registry directory, id/title/status keys, sort behavior, and transform helpers reused by query
- Goal's shared frontmatter schema, upsert payload schema, relation metadata, and CLI/query mechanical command metadata
...
- CLI can reuse the same kinds, prefixes, aliases, scaffold payloads, and landed Goal command/runtime metadata instead of carrying a second taxonomy table.
...
- CLI operator-facing help text beyond the landed shared mechanical metadata
...
- port additional registry families onto the richer shared definition shape proven by Goal
