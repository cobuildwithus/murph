...
| `pnpm --dir packages/inboxd exec vitest run test/inbox-media-retention.test.ts` plus the focused assistant-runtime and Cloudflare snapshot tests | Hosted inbound-video transience proof. | Canonical video classification fails snapshot exclusion closed on invalid capture records; ordinary video paths are excluded even before local deletion; explicitly durable raw refs remain portable; zero-window hosted cleanup tombstones and deletes only unprotected video; image/audio deadlines and pending-input behavior remain unchanged. |
...
| `node --test scripts/native-ios-hosted-e2e.test.mjs` | Focused behavior proof for the protected native iOS hosted E2E seam: contract v3 and `orchestrator_owned_reset`, exact Vercel project/custom-environment/ref/SHA binding, anonymous candidate reachability, lane-only alias-before-deployment retirement fencing, immutable private-iOS tag/run binding, exact Web-head revalidation immediately before private dispatch, isolated-database and PostgreSQL timeout fencing, bounded child-command results, fresh Privy timing, shared-sandbox Junction namespace cleanup including orphan recovery without touching unrelated users, default-compatible production client-user derivation, and connected Apple Health postcondition. Repo Hygiene runs this test. Narrow workflow-source assertions bind PR-number latest-pending controller concurrency, direct-rerun rejection, pre-setup head revalidation, the unchanged globally serialized live slot, explicit Web-token/PR-number dispatch inputs, truthful status descriptions, enumerated hosted/auth/device-connect selection including renamed paths, unrelated Web-path rejection, and the shared infrastructure-only exact-head Repo Hygiene retry owner. The cross-lane retry contract extracts the iOS and Android status commands, runs each through the real parser, and proves one eligible exact-head rerun POST. | `.github/workflows/native-ios-hosted-e2e.yml`, `.github/workflows/native-android-hosted-e2e.yml`, `scripts/native-ios-hosted-e2e*.mjs`, `apps/web/src/lib/hosted-web/public-url.ts`, and `apps/web/vercel.json` |
| `node --test scripts/ci-markdown-docs-scope.test.mjs scripts/pull-request-ci-policy.test.mjs` | Source, mutation, API-fixture, Git-inventory, shell-receipt, and fake-`gh` execution proof for the draft-first, ready-only exact-head pull-request lifecycle and production Web ignored-build boundary. The base-trusted PR classifier revalidates the event against the current PR before and after traversing and count-checking at most 3,000 files, checks both sides of renames, admits only flat dated records matching `docs/release-notes/YYYY-MM-DD-<lowercase-slug>.md`, and defaults every incomplete, unsafe, stale, raced, or mixed inventory to full CI. Host Support then binds one documentation job to the event base and exact synthetic merge candidate, `git diff --check`, frozen tooling, docs drift/reference enforcement, and doc gardening; a moving-base fixture proves that base-only changes cannot enter the candidate inventory in reverse. Release aggregation accepts docs mode only when that job succeeds and every runtime shard skips. The Vercel mode binds `production`/`main`, current HEAD, the current and last-successful deployment SHAs, ancestry, and a complete no-rename Git inventory to the same policy; any unavailable or ambiguous state retains the build, and custom environments always build. Workflow proof requires each existing required owner to execute the exact-base PR classifier independently, keeps heavy jobs on the full main-push/fail-closed PR path, and exercises the host docs/full plus Stripe skipped/full receipt modes. Existing coverage preserves intentionally lightweight synchronize owners, rejects synchronize admission in expensive workflows, requires fail-closed draft admission, locks the event-time-ready read-only receipt and exact-SHA trusted draft reset, rejects missing or ambiguous candidates before mutation, and proves repository-owned Frog PR creation stays draft by default. | The five Markdown-fast-path workflows under `.github/workflows/**`, `apps/web/vercel.json`, `scripts/ci-markdown-docs-scope*.mjs`, `scripts/frog-autofix.ts`, and the living CI/operator documentation |
| `node --test scripts/native-android-hosted-e2e.test.mjs` | Focused behavior proof for the protected native Android hosted E2E seam: contract v1, exact Web and Android SHA binding, immutable private-Android lightweight tag proof, explicit reset versus non-destructive identity lifecycle, short dispatch lease, exact private run receipt, cancellation and hard execution fencing, shared identity/deployment cleanup ownership, credential scrubbing, production-alias proof, a truthful explicit shared-helper retry reason, and a connected Health Connect postcondition. Repo Hygiene runs this test alongside the unchanged native iOS contract. | `.github/workflows/native-android-hosted-e2e.yml`, `scripts/native-android-hosted-e2e*.mjs`, shared owners under `scripts/native-ios-hosted-e2e*.mjs`, and `agent-docs/operations/native-android-hosted-e2e.md` |
| `node --test scripts/hosted-orchestration-compatibility.test.mjs` | Focused behavior proof for the protected public-to-private Temporal compatibility controller: conservative changed-file selection with rename and pagination coverage, same-repository human authority, exact public-head and default-base revalidation, default-branch-only SHA-global status publication, run/head-bound bounded producer artifacts, committed exact-SHA private policy, immutable private lightweight-tag/workflow/run identity, first-attempt binding, closed dispatch inputs, bounded exact-id `404` visibility recovery after accepted dispatch, supported-reader completeness, producer/reader proof-digest binding, unique successful reader jobs, deterministic attestation, and exact accepted-run cancellation. Repo Hygiene runs this test. | `.github/temporal-compatibility-controller.json`, `.github/workflows/temporal-compatibility.yml`, `.github/workflows/repo-hygiene.yml`, and `scripts/hosted-orchestration-compatibility*.mjs` |
...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_RUNTIME_LOG_POSTGRES=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-runtime-log-postgres-concurrency.test.ts` | Opt-in real-PostgreSQL proof for the isolated runtime-log database, monotonic digest deletion fence, reversible member authority, and bounded account cleanup. The suite rejects non-loopback database URLs, creates and migrates a temporary logical database, and uses the production store SQL. | Three delayed initial primary-authority reads hold zero isolated clients with pool max two; append-first cleanup deletes the committed row; the exact deletion-first pre-check gap drops the stale active append after cleanup commits and releases the subject lock; reversible suspension after the pre-check is rejected by the final authority read without creating a permanent fence; timed-out cleanup rolls back without a partial fence, and repeated cleanup converges idempotently to one fence and zero rows. |
...
| `pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-onboarding-linq-thread-route.test.ts apps/web/test/hosted-group-store.test.ts apps/web/test/hosted-group-assistant-ask.test.ts apps/web/test/hosted-group-materialization-backfill.test.ts` | Focused routed-group materialization and historical-repair composition proof. | The canonical route service invokes the same structural group-store primitive for new Linq and Telegram routes; that primitive creates one unnamed ordinary group and route-owner membership without a vault share while preserving existing group configuration and the user active-access gate. Discovery sees the owner membership, multiple unnamed memberships remain fail-closed, and the aggregate-only backfill command is bounded, serial, resumable, idempotent, and has an exact zero-pending readiness check. |
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-onboarding-linq-participant-addition-concurrency.test.ts` | Opt-in real-PostgreSQL proof for routed-thread creation, historical materialization, and participant-addition ordering. The suite rejects non-loopback database URLs and runs after migrations. | The mixed-privacy-version Telegram creation race commits exactly one route, synthetic member, activation wake, ordinary unnamed group, and discoverable route-owner membership even when the owner has 25 active profile-name grants; no automatic twenty-sixth grant is created and the loser leaves no partial state. The production backfill command likewise converges a historical route through the private membership reader without granting a vault share. Existing participant-addition and route-rekey lock-order cases remain covered, and the shared route owner makes the committed group-state proof apply to future Linq and Telegram creation. |
...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-usage-credit-account-deletion-postgres.test.ts` | Opt-in real-PostgreSQL proof for statement-bounded historical purchase detachment during account deletion. The suite rejects non-loopback database URLs and runs after migrations. | The production preflight scans a 512-row already-safe payer history once with zero per-row transactions, then the terminal owner performs exactly one scoped read and one set update; every target advances its reconciliation version exactly once, while a target-beneficiary row and an unrelated foreign-owner row remain unchanged. |
...
five-edit cap, missing-original retry, and side-effect isolation. The mailbox
preparation suite proves one max-seven item-plus-sidecar query over at most two
privacy versions, exact snapshot revalidation after sorted locks, one prepared
decrypt batch with no per-row rereads, and no append-root preparation for the
seventh sentinel row. The edit-service composition suite proves both the
six-historical-root limit shape and the appendable five-history shape prepare a
distinct active root before the sole transaction, use one historical metadata
batch plus one active-root read, keep KMS concurrency at four or fewer, repeat
one snapshot read after sorted locks, run live authority inside the transaction,
and reuse the prepared active root for an accepted append. Crypto-owner tests
prove cache-only missing/binding/mixed-failure behavior, sibling draining, and
master/caller-copy zeroization without new metadata or KMS work. Webhook tests
prove the
six-attempt bound succeeds after five consecutive mismatches, fails closed on a
sixth, preserves a required preparation failure, and retains blind-only direct
authority; the real-PostgreSQL suite proves three contenders survive two stale
snapshots and commit in event order. Hosted
...
- The same workflow also preserves the Ubuntu `pnpm release:check` surface without running it as one long job: release metadata/build/typecheck, package coverage shards, app verification, fixture coverage, and the production runner-bundle byte budget run as parallel jobs, then a final `Release checks (ubuntu)` aggregator preserves the required-check name. The aggregator accepts exactly two proof modes: every full shard succeeds while the docs-only job remains skipped, or an exact-base classifier proves the current PR inventory contains only eligible Markdown, the exact-candidate documentation job passes `git diff --check`, `pnpm docs:drift`, and `pnpm docs:gardening`, and every runtime shard remains skipped. Any incomplete classifier or documentation result fails closed. The app-verification shard provisions an isolated loopback PostgreSQL 17 service and sets the dedicated supplement-search test database variable, so its rollback-only 100+ query PostgreSQL corpus runs on full pull-request and `main` lanes while the ordinary hosted-web build database remains the unreachable CI placeholder. The bundle lane is the byte-budget authority: it runs on native `ubuntu-24.04` x86_64, installs the frozen dependency graph, and executes the full `pnpm --dir apps/cloudflare runner:bundle` production assembly. For full pull requests it checks out the current pull merge ref with both parents, proves the second parent is the event's exact PR head, resolves the base branch directly from `origin`, proves the first parent is that live base, and re-reads the base after assembly so a moving or stale `main` comparison cannot certify the artifact. macOS bundle measurements remain useful diagnostics but do not ratchet the deployment budget.
...
- The `retell-call-result-roundtrip` scenario selects a typed, fixed Web-only
  ...
  `apps/web/.test-dist` and injects only that emitted JavaScript with
  `NODE_OPTIONS=--require`; neither a TypeScript loader nor the production Next
  instrumentation graph owns the hook. Next externalizes
  `@temporalio/client` in every server build so the preload and request route
  share the Web process's physical module in source `next dev` and prepared
  `next start`. The preload requires an exact E2E profile, hosted-local test
  routes, and the exact scenario member. Focused Web coverage proves the
  compiled preload changes that physical prototype, executes the real source
  instrumentation `register()` without changing it, and conditionally executes
  a current completed smoke artifact's emitted instrumentation bundle under
...
- `.github/workflows/release.yml` uses GitHub-hosted `ubuntu-24.04` jobs and derives both release matrices from the immutable tag checkout. Package discovery fails closed unless every `packages/*` workspace owner is assigned to one non-empty coverage shard or carries an explicit reasoned exclusion; the current plan admits Health Commons and Hosted Local Harness rather than silently omitting their suites. Release metadata, the clean-build/typecheck/doc preflight, six isolated package shards, scenario-integrity coverage, the memory-measured hosted-web build/lint/smoke lane, four native Vitest hosted-web file shards, and Cloudflare verification all remain required. The hosted-web test shards start only after the reusable typecheck passes, while the build lane reruns the Health Commons trace and prepared-smoke instrumentation assertions in the checkout that produced their outputs. The final job keeps the required `build` name, runs with `always()`, rejects failed, canceled, or skipped prerequisites, verifies `HEAD == GITHUB_SHA`, rebuilds publishable outputs in that clean checkout, and only then packs. The deterministic hosted-web placeholders still keep build and smoke on their truthful paths without injecting production secrets. Packing scans the final tarballs before writing their manifest, retains the upload/download handoff for one day, reruns the scan after download before GitHub Release upload, and scans again inside the npm publisher before its first provider request.
...
- Assistant Ask has focused contract, parser, Web authority/idempotency,
  ...
  wording, unsupported subject-led or unseparated private directives, and
  unclear or context-dependent text are rejected before enqueue; a missing direct route
...
- `.github/workflows/native-android-hosted-e2e.yml` is the parallel trusted
  ...
  same structural Web and workspace boundaries as iOS, plus shared and
  Android-specific controller owners. New owner files and renamed paths select
  automatically; nested hosted Web UI, content, and tests plus unrelated
  workspace packages do not consume the live slot. It retains the
...
- PR mode is real and hosted. Trusted default-branch orchestration first enumerates active deployments in the dedicated E2E Vercel project, rejects any production/foreign deployment, and retires only lane-marked deployments. For every validated deployment it enumerates aliases from that exact deployment, validates the complete alias response, deletes those aliases first, and only then deletes the deployment. It then completely enumerates the configured Junction sandbox team, validates every returned team id, ignores unrelated client-user-id namespaces, rejects more than one user in the explicit E2E namespace, deletes only the sole namespace-owned user, and proves that namespace empty without treating the shared team as a cleanup target. It resets the explicitly E2E-named isolated Postgres database through Prisma and deletes the fixed user in the dedicated Privy app. The exact PR SHA deploys to Vercel custom environment `native-ios-e2e`; that target applies the PR's real Prisma migrations before the normal minified Web build and consumes its runtime configuration directly from the dedicated Vercel custom environment. Before native dispatch, a bounded request to the exact generated origin must succeed anonymously without redirects or a Vercel protection bypass.
...
- `Live hosted-local Stripe browser matrix` runs only on trusted `main` pushes, uses the protected `hosted-stripe-billing-sandbox` GitHub Environment, and is serialized with `cancel-in-progress: false`. Pull-request code never enters a secret-bearing job, and this workflow must never be converted to `pull_request_target`.
- The always-present `Required hosted Stripe billing boundary` accepts exactly three event-scoped results: a Markdown-only PR with both runtime jobs skipped, a full PR with hermetic success and the live job skipped, or a `main` push with both hermetic and live success. The dedicated secret, sandbox account, price IDs, public Privy app id, and active default Portal configuration with plan updates and immediate invoicing stay outside the repository. The browser remains the authoritative proof that Stripe exposes both dedicated individual products.
