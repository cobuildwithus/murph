...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-group-join-outreach-reply-recovery-postgres.test.ts` | Opt-in real-PostgreSQL proof for the group-join reply recovery boundary. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | A temporarily unassignable reply leaves its group context available; a later reply produces the intended group-aware invite URL; provider acceptance consumes that exact context; terminal failure reopens it; a stale earlier failure cannot undo a newer dispatch or accepted daily signup fact; and a later delivered receipt restores consumption. Separately, a crashed pre-provider claim remains bound to its exact context while a distinct pending group reply sends independently; both exact group deliveries consume their own outreach and set one shared daily fact; then a fresh ordinary same-day inbound is suppressed without another provider call |
