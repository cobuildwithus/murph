...
| `pnpm provider-requests:guard` | Babel-based static guard, included in root typecheck and diff preflight. It scans production `.cjs`, `.cts`, `.js`, `.jsx`, `.mjs`, `.mts`, `.ts`, and `.tsx` under apps, packages, and scripts while excluding declarations, generated output, fixtures, and tests. At explicit registered-provider boundaries it preserves the official-SDK object-spread, `Object.assign`, untyped request-object, and typed-builder checks; independently follows bounded local/field/parameter/config URL dataflow into global fetch, imported Node HTTP/HTTPS and Undici transports, explicit aliases, injected fetch-call signatures, and strict fetch-shaped wrappers at their nearest lexical binding; and rejects concrete low-level handwritten provider wire contracts without treating provider-domain helpers or SDK-backed business adapters as transports. Provider symbols, resolved bindings, arguments, and hosts accumulate so an allowed-provider label cannot erase simultaneous required-provider evidence; file hints are fallback-only, ambiguous dataflow names every matching provider, and local wrapper callers are not duplicated. Reports are deterministic and deduplicated. The explicit narrow exceptions are opaque presigned GET/HEAD transfers or PUTs with provable byte/stream bodies, no credentials or static `credentials: "omit"`, and no computed or credential-bearing headers; unambiguous single-slash internal/same-origin traffic; hosted-runner incoming-`Request` pass-through; dynamic SMART/FHIR endpoints with no explicit registered-provider identifier or host; registered providers without a verified provider-owned TypeScript SDK; and named xAI OpenAI-compatible extensions. It does not claim detection of unregistered providers or provider dataflow with neither a registered host nor provider identifier. | `apps/**`, `packages/**`, `scripts/**`, `scripts/check-provider-request-boundaries.ts`, and `scripts/check-provider-request-boundaries.test.ts` |
