...
- Automatic meal-photo capture is covered by hosted-web enrollment/upload and current-direct-route tests, hosted-execution wake/route parsing tests, Cloudflare private-object and control-proxy tests, assistant-runtime canonical import/idempotency/automation-postcondition tests, managed-automation tests, and canonical meal photo-retirement/CLI tests. Routine CI does not grant a real iPhone Photos permission or upload to the production R2 bucket, so deployed product proof still requires an explicit physical-device capture.
