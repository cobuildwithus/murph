...
- Cloudflare storage coverage locks one canonical `BUNDLES` binding and presign
  target, direct-upload drain fencing, single-bucket restore and deletion, and
  deploy preflight rejection for any bucket that is not ENAM Standard. Routine
  checks do not prove a live 100-percent Worker rollout or fresh production
  bucket inventory; deployment smoke and operator checks own that proof.
