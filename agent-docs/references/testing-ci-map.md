...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-execution-usage-postgres-concurrency.test.ts` | Opt-in real-PostgreSQL proof for hosted usage accounting and deterministic replay. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | A real priority first-turn usage record persists its pricing basis and version, settles twice the standard normalized spend into the member period, and a first writer holding an uncommitted deterministic row still makes an exact concurrent replay wait before both converge on one immutable row. |
