...
- Automatic meal-photo capture is covered by hosted-web enrollment/upload, schema-v2 authority ordering and tombstone cases, lost-response prepared-state denial, exact bodyless activation and replay, activation/deletion ordering, schema-v1 compatibility, strict request bounds, static migration shape, and an opt-in local-PostgreSQL test that executes the exact expand/contract files with a legacy-window write and validated row constraints; companion bearer-consent status/acceptance, verified-email route fallback and current-recipient resolution, accepted-capture member-wide engagement, and model-gate-with-system-lag tests; hosted-execution wake/route parsing tests; Cloudflare private-object, processing-mode, and signed control-proxy tests; Temporal blocked-system and foreground-fairness tests; assistant-runtime system-only cron projection/post-checkpoint cleanup, canonical import/idempotency/automation-postcondition, and fail-closed email-authority tests; managed-automation tests; oldest-first closeout-work CLI tests; and canonical meal photo-retirement tests. Routine CI does not grant native photo-library permission or upload to the production R2 bucket, so deployed product proof still requires an explicit physical-device capture.
- The temporary R2 cutover bridge has focused Cloudflare coverage for live
  source/destination version skew: late old-version writes stay readable through
  destination-miss fallback, new current-version writes stay in ENAM, listing
  remains on the active bucket, and deploy preflight accepts
  `destination_active+open` with no canary. The emergency write-admission gate
  retains route coverage for both callback-signed Temporal ensures and Vercel
  OIDC direct hints, including proof that paused requests do not call UserRunner
  or schedule `waitUntil`. Parser, deploy-config, preflight, direct-upload bucket
  affinity, and hosted-local tests cover the status/config shape. Routine checks
  do not prove a live 100-percent Worker rollout, production runner and OC-ticket
  drain, or fresh bucket inventories; the cutover runbook owns that deployed
  proof while normal runtime admission remains open.
