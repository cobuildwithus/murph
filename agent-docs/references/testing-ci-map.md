...
- Cloudflare storage coverage locks one canonical `BUNDLES` binding and presign
  target, direct-upload drain fencing, canonical-only restore, dual-bucket
  deletion with retained retry ownership on partial failure, and deploy
  preflight rejection unless canonical buckets are ENAM Standard and retiring
  deletion buckets are OC Standard. Routine checks do not prove a live
