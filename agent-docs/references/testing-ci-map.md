...
- The hosted-local active-turn latency scenario proves same-chat late-input folding, forces a 20-second provider-cleanup stall and requires the second reply to preempt it, and checks that a projected wake does not trigger immediate full idle-shutdown work under the 90-second floor.
- The dedicated `foreground-reply-priority` hosted-local scenario keeps the
  production 90-second idle floor, seeds every registered system wake kind,
