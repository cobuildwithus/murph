...
| `node --test scripts/check-frontend-evidence.test.mjs scripts/check-pr-changelog.test.mjs scripts/check-pr-deployment-concerns.test.mjs` | Focused Node tests for the pull-request evidence guards. Every PR must include one deployment-concerns disposition: applicable changes provide the complete skew, order, rollback, exposure, reversibility, convergence, and post-deploy contract; other changes provide a concrete not-applicable reason. The frontend guard separately requires literal `Direct:` and `Coverage:` list items matched to each user-facing hosted-web UI diff, without a catalog edit or screenshot quota; it exempts only isolated static object-literal route metadata when base/head source comparison proves the remaining page or layout source unchanged. The guards validate GitHub-rendered GFM so comments, code blocks, and raw HTML cannot be mistaken for visible proof. | `scripts/check-{frontend-evidence,pr-changelog,pr-deployment-concerns}.mjs`, their focused tests, `.github/workflows/pr-evidence.yml`, and `.github/pull_request_template.md` |
...
- `.github/workflows/pr-evidence.yml` checks every pull request for changelog and deployment-concerns dispositions. Applicable deployment changes must provide the complete skew, order, rollback, exposure, reversibility, convergence, and post-deploy contract; all other changes need a concrete not-applicable reason. The workflow also requires literal `Direct:` and `Coverage:` evidence matched to the changed risk whenever the base-to-head diff changes user-facing hosted-web UI. It does not require a catalog edit or screenshot count, and it skips only an isolated static object-literal `metadata` export change when the existing route's remaining runtime source is exactly unchanged and neither rendered code nor viewport/theme metadata shares that declaration.
