...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/device-sync-start-account-deletion-concurrency.test.ts apps/web/test/hosted-account-data-service.test.ts` | Opt-in real-PostgreSQL proof for hosted device connection start and direct-OAuth callback completion against account deletion. Both suites reject non-loopback database URLs and run in the hosted E2E PostgreSQL job after migrations. | Link/OAuth authorization finalization commits the seed and retains the exact callback marker; a sibling start preserves every other live marker; suspension after staging retains seeded, seedless, and SDK markers without a local connection or token; suspension before staging rejects provider work; SDK finalization retains its marker through token delivery; direct OAuth rejects deletion after state consumption before token exchange and after exchange before durable connection commitment, then clears only the exact marker after success |
...
removal. Its opt-in PostgreSQL cases also prove live and consumed device-start
markers reject deletion before suspension while expired abandoned starts are
resolved only after the suspension fence commits. `hosted-account-deletion-cleanup.test.ts` proves receipt-bound
