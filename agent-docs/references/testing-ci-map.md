...
| `pnpm --filter @murphai/assistant-runtime exec vitest run test/hosted-runtime-workspace-entrypoint.test.ts -t "keeps image work detached"` | Focused restored-workspace proof for hosted image-generation concurrency and completion authority. | The origin turn's exact usage persists before allowance reservation and dispatch; later conversation turns run while origin usage, provider work, capture continuation, and upload are pending; image usage settles before checkpoint; upload completion only stages a fresh ordinary Murph turn with optional media and never sends or attaches automatically. |
...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-ai-usage-reservation-postgres-concurrency.test.ts` | Opt-in real-PostgreSQL proof for hosted AI usage reservation admission and lifecycle invariants. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | Same-member claims serialize on the member row, one image fits exactly once, replay remains singular, active claims reduce the ordinary usage gate, abandoned pre-dispatch claims restore capacity after the bounded TTL while dispatched claims retain it, a pre-dispatch refund reversal invalidates dispatch, the composite foreign key plus identity check permit settlement only by the exact reserved image usage ID, credit-backed reserve and settlement retain member-before-period lock order, and completed usage settles the captured admitted period after the current entitlement period advances |
