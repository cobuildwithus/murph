...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/device-sync-start-account-deletion-concurrency.test.ts` | Opt-in real-PostgreSQL proof for hosted device connection start against account deletion. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | Link/OAuth finalization commits the seed and callback state atomically; a successful sibling start preserves every other live marker; suspension after staging retains seeded, seedless, and SDK markers without a local connection or token; suspension before staging rejects provider work; SDK finalization atomically consumes only its marker before waiting deletion observes the committed connection |
