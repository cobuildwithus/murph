...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-onboarding-linq-activation-postgres.test.ts` | Opt-in real-PostgreSQL proof for the prepared direct-Linq control-root/member lock graph and activation/preflight authority transitions. The suite rejects non-loopback database URLs, uses independent one-connection clients, and leaves the production control-root advisory lock unmocked. | A member-first lifecycle owner can hold the member row while prepared direct ingress takes control-root authority; the nonblocking member attempt releases that root without deadlock and a replay appends exactly one mailbox item. After group eligibility changes, successful preparation still gives that committed item canonical duplicate-wake authority while a distinct event follows current group semantics. A stable inactive member preflights and commits only its missing control root before private group/routing policy, without ingress or mailbox mutation; after activation commits the remaining roots, repeated null planning fails closed before group, daily, invite, route, or mailbox mutation, while a freshly prepared attempt completes once. |
