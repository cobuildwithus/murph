...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-group-join-offer-replay-postgres.test.ts` | Opt-in real-PostgreSQL proof that a hosted Linq group-offer receipt can authorize membership and sharing only once. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | A leave racing an exact redelivery serializes on the group owner and provider-event row, the original membership stays revoked, and no replay can restore membership or shares |
