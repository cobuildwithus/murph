...
| --- | --- | --- |
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm --dir apps/web test -- hosted-mailbox-engagement-postgres.test.ts` | Opt-in local PostgreSQL proof using transaction-local temporary tables and the production mailbox engagement lookup. | Accepted Telegram conversations and meal captures qualify within 28 days; consumed/content-retired Telegram metadata remains eligible. Old rows, unrelated members, email, and system-only work do not qualify. Pair with `hosted-orchestration-reconciliation-facts.test.ts` for inactivity and usage authorization. |
