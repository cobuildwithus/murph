...
| --- | --- | --- |
| `pnpm --dir apps/web test:prepared -- member-owned-provider-setup-service.test.ts member-owned-provider-setup-primitive.test.ts member-owned-provider-setup-account-deletion.test.ts member-owned-provider-setup-ui.test.tsx member-owned-provider-setup-migration.test.ts hosted-execution-computer-use.test.ts hosted-orchestration-signal-runtime.test.ts device-connect-intent-route.test.ts hosted-connect-start.test.ts device-reconnect-link-tool.test.ts hosted-account-data-service.test.ts hosted-health-data-consent-withdrawal.test.ts device-provider-application-store.test.ts device-provider-application-ingress-store.test.ts device-provider-application-cleanup.test.ts prisma-store-oauth-connection.test.ts prisma-store-oauth-sessions.test.ts device-sync-callback-route.test.ts connect-page.test.ts device-sync-settings-routes.test.ts device-sync-internal-connect-route.test.ts production-migration-guard.test.ts hosted-onboarding-privacy-foundation-migration.test.ts changelog.test.ts changelog-page.test.tsx` | Focused Web proof for the reusable model-driven member-owned provider setup owner and Strava's declarative registry entry. | Provider-indexed registry and projection behavior; pre-Kernel exact-run admission, setup-owned persistent-profile acquisition, mismatch rejection, and browser-provisioning cleanup; ordinary Playwright and OS actions on the exact setup-owned run; full model-owned application-form submission with no application-name authority; blind registered-selector credential capture, optional reveal, direct encrypted sealing, page scrubbing, malformed-result scrubbing, and no credential-bearing tool result; retryable capture after an interruption; setup-version cancellation versus capture; exact continuation and handoff binding; serialized OAuth/deletion; terminal run release before deleted-slot reuse; client-ID-exact deletion, mismatch and non-unique fail-closed behavior, clean already-absent convergence, and lost-result retry; account-deletion, consent-withdrawal, `/connect`, migration, changelog, and deployment-skew coverage. Fixture-DOM tests exercise capture happy path, non-unique ID/secret selectors, reveal, credentials-path mismatch, deletion client-ID mismatch, non-unique deletion authority, successful deletion, and already absence. No test in this lane contacts live Strava. |
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/member-owned-provider-setup-oauth-deletion-postgres-concurrency.test.ts` | Opt-in real-PostgreSQL proof for member-owned provider cancellation, deletion, and reconnect ownership. | Run admission and Cancel cover six production orderings: cancellation before the setup CAS retires the unbound reservation without Kernel creation; binding during profile provisioning retains `canceling` until the worker rejects; cancellation during an in-flight browser create retains deterministic cleanup until the late exact browser is deleted; a lost create response converges through the existing stale-provisioning boundary; a no-handle client timeout retains visible cancellation until that boundary; The same timeout without Cancel rejects an exact-owner retry before the existing browser-provisioning stale boundary; afterward the computer owner deletes and terminalizes only the deterministic stale browser, preserves the simple `browser_setup` phase, and admits at most one exact same-setup successor. Provider form submission and application identity are not inferred from browser provisioning cleanup. OAuth callback persistence and deletion admission serialize on the existing hosted-member row. Callback-first leaves one active connection and moves setup to `disconnect_first`; deletion-first persists `deletion_pending` and rejects the stale callback without creating a connection. A deleted terminal setup deactivates before the partial-unique member/provider slot admits a fresh pending successor. |
| `pnpm exec vitest run --config apps/cloudflare/vitest.config.ts apps/cloudflare/test/runner-outbound.test.ts -t "provider setup|allowlisted hosted web-control" --no-coverage` | Production Cloudflare outbound proof for provider-setup validation and tool callbacks. | Both shared exact POST paths traverse `handleRunnerOutboundRequest`, retain the active runtime write fence, bind and sign the forwarded request for the runtime member, strip child authority headers, preserve Web responses, and reject GET plus neighboring paths. |
| `pnpm --dir packages/device-syncd test -- strava-provider.test.ts` | Focused Strava provider proof used by the exact member-owned application configuration. | Read-only OAuth exchange, initial backfill job creation, scheduled polling/reconciliation, refresh, and `POST /oauth/revoke` with exact Basic application authentication plus a form-body token and secret-safe errors. Existing webhook parsing remains covered, but member-owned Strava webhook administration remains disabled. |
...
| `pnpm provider-requests:guard` | Babel-based static guard, included in root typecheck and diff preflight, that rejects object spreads, `Object.assign`, and untyped object-literal request variables at registered official provider SDK boundaries. It covers Composio, Stripe, Kernel, Linq, Retell, Temporal, OpenAI, Resend, Junction, ElevenLabs, Exa, Lob, and Google Cloud client calls and typed builders across production apps, packages, and JavaScript/TypeScript scripts so optional provider keys remain visible to the SDK declarations. It also rejects `fetch`/`Request` destinations whose registered provider origin is statically visible through local literals, templates, URL objects, or variables. Inline exceptions are limited to a matching imported SDK transport adapter and Linq SDK-owned `uploadUrl`/`downloadUrl` presigned bytes; imported or otherwise runtime-computed destinations and non-Fetch HTTP libraries remain outside the proof. | `apps/**`, `packages/**`, `scripts/**`, `scripts/check-provider-request-boundaries.ts`, and `scripts/check-provider-request-boundaries.test.ts` |
...
| `node --test scripts/native-ios-hosted-e2e.test.mjs` | Focused behavior proof for the protected native iOS hosted E2E seam: contract v3 and `orchestrator_owned_reset`, exact Vercel project/custom-environment/ref/SHA binding, anonymous candidate reachability, lane-only retirement fencing, immutable private-iOS tag/run binding, isolated-database and PostgreSQL timeout fencing, bounded child-command results, fresh Privy timing, lane-exclusive Junction team cleanup including orphan recovery, production client-user derivation, and connected Apple Health postcondition. Repo Hygiene runs this test; it intentionally does not parse workflow source text. | `.github/workflows/native-ios-hosted-e2e.yml`, `scripts/native-ios-hosted-e2e*.mjs`, `apps/web/src/lib/hosted-web/public-url.ts`, and `apps/web/vercel.json` |
...
- `.github/workflows/web-viewport-overflow.yml` runs the `pnpm --dir apps/web test:viewport-overflow` Playwright gate on GitHub-hosted `ubuntu-24.04` for every pull request and `main` push. It installs only Chromium (`playwright install --with-deps chromium`), first proves `headless:false` can launch inside `xvfb-run`, and then lets Playwright's `webServer` boot the hosted-web dev server with the placeholder smoke env. The job needs no Postgres service or real secrets. On failure it uploads the Playwright HTML report as an artifact.
...
- The required native companion auth/control/device-sync acceptance lane is `.github/workflows/native-ios-hosted-e2e.yml`. It runs from trusted default-branch `workflow_run` code after Repo Hygiene, conservatively selects every `apps/web/**` change plus the current hosted-Web source-package closure and directly consumed root build/deploy/workflow owners, and publishes the stable `Native iOS hosted E2E` commit status. A selected same-repository human-authored PR enters the protected `native-ios-hosted-e2e` environment; fork/bot or missing-environment cases fail closed rather than receiving live authority.
- PR mode is real and hosted. Trusted default-branch orchestration first enumerates active deployments in the dedicated E2E Vercel project, rejects any production/foreign deployment, retires only lane-marked deployments, then enumerates the lane-exclusive Junction sandbox team. It rejects more than one or any wrong-team/client user, deletes the production-derived sole user when the isolated member exists or the sole orphan when it does not, proves the team empty, resets the explicitly E2E-named isolated Postgres database through Prisma, and deletes the fixed user in the dedicated Privy app. It then deploys the exact PR SHA to Vercel custom environment `native-ios-e2e`; that target applies the PR's real Prisma migrations before the normal minified Web build and consumes its runtime configuration directly from the dedicated Vercel custom environment. Before native dispatch, a bounded request to the exact generated origin must succeed anonymously without redirects or a Vercel protection bypass.
