...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-ops-app-review-member-postgres.test.ts` | Opt-in real-PostgreSQL proof for the Ops App Review member's split provider/database resolution owner. The suite rejects non-loopback database URLs and runs after migrations. | Deletion after the initial principal lookup is rejected by a later exact-ID Privy authority read; an existing deletion receipt plus member deletion committed after that exact read and before transaction checkout cannot recreate the identity from stale prepared authority; and an existing member whose private identity remains sealed under a decrypt-only historical control root succeeds only after that projection is preloaded before the provider-disabled transaction. |
