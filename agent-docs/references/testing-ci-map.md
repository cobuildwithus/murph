...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-preference-handoff-sweeper-postgres.test.ts` | Opt-in real-PostgreSQL proof for the bounded preference, vault-refresh, and Clinical Records handoff selector. The suite rejects non-loopback database URLs and executes the production sweeper query plus canonical access recheck after migrations. | Two expired participant leases cannot consume the outer bounded page ahead of a current synthetic-room candidate; two retention-old Clinical Records mailbox rows cannot consume it ahead of a live current-generation wake; and canonical active-access revalidation completes before the selected handoff request. |
