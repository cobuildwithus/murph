...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_RUNTIME_LOG_POSTGRES=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-runtime-log-postgres-concurrency.test.ts` | Opt-in real-PostgreSQL proof for the isolated runtime-log database, digest-only monotonic deletion fence, and bounded account cleanup. The suite rejects non-loopback database URLs, creates and migrates a temporary logical database, and uses the production store SQL. | Append-first cleanup deletes the committed row; deletion-first cleanup persists one fence that blocks a primary-authorized append and remains idempotent on repeated cleanup; an injected failure after the real diagnostic delete rolls back both the row deletion and fence before retry converges to zero rows and one fence; overlapping multi-subject cleanup avoids deadlock; dedicated reads and bounded retention run against the migrated schema. |
