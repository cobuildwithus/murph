...
| `pnpm provider-requests:guard` | Babel-based static guard, included in root typecheck and diff preflight. It scans production `.cjs`, `.cts`, `.js`, `.jsx`, `.mjs`, `.mts`, `.ts`, and `.tsx` under apps, packages, and scripts while excluding declarations, generated output, fixtures, and tests. At explicit registered-provider boundaries it preserves the official-SDK object-spread, `Object.assign`, untyped request-object, and typed-builder checks; independently follows bounded local/field/parameter/config URL dataflow into global fetch, imported, CommonJS-loaded, dynamic-imported, or TypeScript import-equals Node HTTP/HTTPS and Undici transports, direct invocations and exact namespace/destructuring aliases, fetch `.call`, closed `.apply` tuples, pre-bound transports in closed local object/array members, injected fetch-call signatures, imported named fetch-contract boundaries, defaulted fetch parameters, chronological simple assignments, and exact one-hop wrappers that forward every argument in place. Destructured declaration/assignment paths and defaults enter the same chronological binding census. Same-file function declarations, arrows, and function expressions use one cross-kind lexical decision with mapped parameters and chronological writes plus one nested object/array parameter projection; each selected value retains its source reference and call-site observation position through that projection, and an opaque or spread-produced value retains a reachable default until presence and non-`undefined` are proven. Supported same-file calls establish their direct-return values in that same authority before projection, so call-produced objects, arrays, and member references compose with root, exact-property, and wildcard-property defaults. Those defaults share one call-time transition that gives the default its own lexical position and references while preserving call-time binding chronology and property observation. Closed member spreads, conditional roots, direct writes, and `Object.assign` mutations retain every possible provider transport or URL value across all chronologically possible local alias roots. One mutation-target expression authority follows identifier/static-member and conditional/logical/sequence alternatives for direct writes and `Object.assign`, retains computed segments as conservative wildcard matches, and makes parameter, `this`, immediate/awaited-call, and every unsupported target fail closed when it receives a provider-bound transport. The guard rejects concrete low-level handwritten provider wire contracts without treating provider-domain helpers or SDK-backed business adapters as transports. Its lexical census includes destructured parameter shadows, property paths, defaults, and types; assigned transports retain their declaration scope, and assigned namespaces require matching declaration identity and fail after a nearer reassignment. Provider symbols, resolved bindings, bound and invocation arguments, exact properties in closed object/array origin maps, statically recoverable fetch tuples, exact fetch-compatible provider-bound targets, object properties, and hosts accumulate in one fact result consumed by classification and every exception, while loose `Request` handlers do not become provider transports. An allowed-provider label cannot erase simultaneous required-provider evidence; file hints are fallback-only, ambiguous dataflow names every matching provider, and local wrapper callers are not duplicated. Reports are deterministic and deduplicated. The explicit narrow exceptions are opaque presigned GET/HEAD transfers or PUTs with AST- or type-proven byte/stream bodies, no spread call init, no credentials or static `credentials: "omit"`, a unique digest-pinned transfer owner, either allowlisted literal headers or a path-scoped audited transfer-header factory, and—when the URL name itself carries provider identity—the URL normalizer registered on that exact owner; the one streamed direct-R2 smoke also requires an exact request binding and pipe target. Other exceptions are statically known single-slash application traffic or `new URL` composition against exact `location.origin`; registered providers without a verified provider-owned TypeScript SDK; the exact path-scoped xAI Responses request carrying one `x_search` extension with storage disabled, a direct static URL literal, and direct closed request/payload/tool objects that cannot be replaced by later mutation or alternate expression shapes; exact path/provider/import/raw-target SDK transport bridges whose unique adapter and SDK-construction function source spans are SHA-256 pinned; and the separately proved Resend and Exa overrides with literal provider operations and direct closed request init. Duplicate adapters or effects, decoy or changed constructor wiring, aliases, helper mutation, `Object.assign`, spreads, computed properties, and duplicate properties fail those proofs. Comments never suppress findings. Generic runner forwarding and dynamic SMART/FHIR traffic require no exception because expressions with no registered-provider facts never become candidates; provider-bearing lookalikes fail normally. It does not claim detection of unregistered providers or provider dataflow with neither a registered host nor provider identifier. | `apps/**`, `packages/**`, `scripts/**`, `scripts/check-provider-request-boundaries.ts`, and `scripts/check-provider-request-boundaries.test.ts` |
