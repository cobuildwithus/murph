...
- Automatic meal-photo capture is covered by hosted-web enrollment/upload, verified-email route fallback and current-recipient resolution, and recent-capture engagement tests; hosted-execution wake/route parsing tests; Cloudflare private-object and signed control-proxy tests; assistant-runtime canonical import/idempotency/automation-postcondition and fail-closed email-authority tests; managed-automation tests; oldest-first closeout-work CLI tests; and canonical meal photo-retirement tests. Routine CI does not grant a real iPhone Photos permission or upload to the production R2 bucket, so deployed product proof still requires an explicit physical-device capture.
