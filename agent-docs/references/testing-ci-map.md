...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-group-join-offer-replay-postgres.test.ts` | Opt-in real-PostgreSQL proof that a hosted Linq group-offer receipt can authorize membership and sharing only while its recorded membership identity and sharing-decision revision remain current. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. The in-memory group-store and reaction suites separately prove the temporary rollout flag rejects all four mutation entry points and rolls matched reaction admission back before receipt commit. | A receipt whose acceptance rolls back retries once when unchanged; a newer no-op denial, event membership, revocation, leave, or changed member terminally supersedes the old event without mutation; a fresh post-leave event may rejoin; legacy pending versions cannot be rebound; an applied replay racing leave remains revoked; and rollout maintenance returns retryable 503 before mutation or current receipt admission |
...
- After hosted scenarios initialize the schema, the Linq route-authority matrix leg runs the focused real-PostgreSQL proofs for deterministic hosted usage replay; atomic group-offer receipt claims; acceptance rollback followed by unchanged retry, newer membership, changed member, an explicit no-op permission denial, selected-share revocation, or leave; exact applied replay versus leave; genuinely new-event rejoin; legacy null/bare-pending/version-one-pending fail-closed behavior; both participant-addition route-row orderings; the canonical chat-ownership-before-route-row order shared by usage-limit dispatch and route-key convergence; and device-sync exact-payload plus companion-receipt lock order against concurrent account deletion.
