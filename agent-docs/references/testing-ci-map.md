...
| `pnpm provider-requests:guard` | Babel-based static guard, included in root typecheck and diff preflight. It scans production `.cjs`, `.cts`, `.js`, `.jsx`, `.mjs`, `.mts`, `.ts`, and `.tsx` under apps, packages, and scripts while excluding declarations, generated output, fixtures, and tests. At explicit registered-provider boundaries it preserves the official-SDK object-spread, `Object.assign`, untyped request-object, and typed-builder checks; independently follows bounded local/field/parameter/config URL dataflow into global fetch, imported, CommonJS-loaded, or TypeScript import-equals Node HTTP/HTTPS and Undici transports, direct invocations and aliases, injected fetch-call signatures, and strict fetch-shaped wrappers at their nearest lexical binding; and rejects concrete low-level handwritten provider wire contracts without treating provider-domain helpers or SDK-backed business adapters as transports. Its lexical census includes destructured parameter shadows. Provider symbols, resolved bindings, arguments, computed origin maps, explicitly provider-bound HTTP transport targets, object properties, and hosts accumulate in one fact result consumed by classification and every exception, so an allowed-provider label cannot erase simultaneous required-provider evidence; file hints are fallback-only, ambiguous dataflow names every matching provider, and local wrapper callers are not duplicated. Reports are deterministic and deduplicated. The explicit narrow exceptions are opaque presigned GET/HEAD transfers or PUTs with AST-proven byte/stream bodies, no credentials or static `credentials: "omit"`, and either allowlisted literal headers or a path-scoped audited transfer-header factory; statically known single-slash application traffic or `new URL` composition against exact `location.origin`; hosted-runner incoming-`Request` pass-through; dynamic SMART/FHIR endpoints with no explicit registered-provider identifier or host; registered providers without a verified provider-owned TypeScript SDK; and the exact path-scoped xAI Responses request carrying one `x_search` extension with storage disabled and closed request/payload/tool objects that cannot override those effective values. It does not claim detection of unregistered providers or provider dataflow with neither a registered host nor provider identifier. | `apps/**`, `packages/**`, `scripts/**`, `scripts/check-provider-request-boundaries.ts`, and `scripts/check-provider-request-boundaries.test.ts` |
