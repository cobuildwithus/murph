...
| `node --test scripts/hosted-orchestration-compatibility.test.mjs` | Focused behavior proof for the protected public-to-private Temporal compatibility controller: conservative changed-file selection with rename and pagination coverage, same-repository human authority, exact public-head revalidation, immutable private lightweight-tag/workflow/run identity, first-attempt binding, closed dispatch inputs, supported-reader digest completeness, unique successful reader jobs, deterministic attestation, and exact accepted-run cancellation. Repo Hygiene runs this test. | `.github/workflows/temporal-compatibility.yml`, `.github/workflows/repo-hygiene.yml`, and `scripts/hosted-orchestration-compatibility*.mjs` |
...
- Private `cobuildwithus/murph-cloud`'s `Public Murph Integration` workflow runs focused hosted-local E2E jobs on Blacksmith for every private pull request and `main` push, and manual dispatch targets an exact public ref. Its compatibility-only dispatch mode is the private owner of the immutable supported-reader manifest: it adds the pinned controller revision, runs the exact public producer SHA against every reader, and returns unique reader jobs plus one digest-bound attestation. Reader jobs use exact name `Temporal compatibility reader [sha=<40-lowercase-hex>]`. The sole attestation uses `Temporal compatibility attestation [public=<public-sha>;request=<request-id>;readers=<digest>]`, where `<digest>` is SHA-256 over sorted lowercase reader SHAs joined by newline with a trailing newline. Public `.github/workflows/temporal-compatibility.yml` is the stable required-status owner. Trusted default-branch `workflow_run` code classifies the complete renamed-file set, passes irrelevant commits without private authority, and admits relevant same-repository human heads to the protected `temporal-compatibility` environment. The controller checks the current public head, immutable private tag and reviewed SHA, exact private workflow metadata, returned run id, private run event/path/SHA/attempt/repository, complete job pagination, unique successful reader jobs including the pinned controller revision, and the exact public-SHA/request-id/supported-reader digest attestation. It never checks out PR code with private credentials or reads private logs/artifacts. Timeout or uncertain polling cancels and, when needed, force-cancels only the accepted private run. Configure environment variable `TEMPORAL_COMPATIBILITY_GITHUB_APP_ID`, `TEMPORAL_COMPATIBILITY_PRIVATE_REF`, and `TEMPORAL_COMPATIBILITY_PRIVATE_EXPECTED_SHA`, plus secret `TEMPORAL_COMPATIBILITY_GITHUB_APP_PRIVATE_KEY`; the App installation is limited to private Murph Cloud with Actions write and Contents read. Land and pin the private compatibility workflow before enabling the public status, then make only public `Temporal compatibility` required.
