...
| `pnpm exec vitest run --config apps/web/vitest.workspace.ts --project hosted-web-sync-settings --no-coverage apps/web/test/hosted-device-sync-closed-loop-quiescence.test.ts` | Focused deterministic closed-loop device-sync work-conservation and quiescence proof. The existing `hosted-device-sync-*.test.ts` project pattern wires it into hosted-web app verification. | One canonical provider obligation advances once; local retry, generic timers, two checkpoint faults, and one cold restore do not mint global work; three later sweep buckets remain idle; and an incident-shaped local-cadence projection fails the conservation assertion. |
