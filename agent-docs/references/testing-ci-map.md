...
| `node --test scripts/pull-request-ci-policy.test.mjs` | Source, mutation, and fake-`gh` execution proof for the draft-first, ready-only exact-head pull-request lifecycle. It inventories every `pull_request` workflow, preserves intentionally lightweight synchronize owners, rejects synchronize admission in expensive workflows, requires fail-closed draft admission without skipped required-check success, locks the event-time-ready read-only synchronize receipt and exact-SHA trusted draft reset, resolves same-repository plus fork feature/default branches through the base repository's `head=owner:branch` open-PR query and exact workflow-run head identity without trusting the optional PR association array, rejects missing or ambiguous candidates before mutation, proves draft-time synchronize cannot undo newer Ready intent, and proves repository-owned Frog PR creation stays draft by default. | Expensive and lightweight workflows under `.github/workflows/**`, `scripts/frog-autofix.ts`, and the living CI/operator documentation |
