...
seventh sentinel row. Crypto-owner tests prove seven distinct roots use one
metadata batch, at most four concurrent KMS calls, sibling draining and key
zeroization, while prepared opens perform no metadata or KMS work. Webhook
tests prove one mismatch retry, preservation of a required preparation failure,
and blind-only direct authority; the real-PostgreSQL suite proves a stale
prepared snapshot loses to the serialized correction and succeeds only after a
fresh preparation. Hosted
