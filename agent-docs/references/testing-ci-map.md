...
| `node --test scripts/native-ios-hosted-e2e.test.mjs` | Focused behavior proof for the native iOS production canary and retained dispatcher contract. | Six-hour staggered schedule, successful-SHA checkpoint admission, same-SHA skip and explicit-rerun recovery, fixed non-canceling concurrency, absence of PR/deployment/manual admission and destructive PR credentials, protected-main and current-production alias binding, contract v3, immutable private-iOS tag/run binding, bounded cancellation/child commands, privacy-safe output, and retained dormant PR-lifecycle helper invariants. Repo Hygiene runs this test. | `.github/workflows/native-ios-hosted-e2e.yml`, `.github/workflows/native-android-hosted-e2e.yml`, `scripts/native-ios-hosted-e2e*.mjs`, `apps/web/src/lib/hosted-web/public-url.ts`, and `apps/web/vercel.json` |
...
| `node --test scripts/native-android-hosted-e2e.test.mjs` | Focused behavior proof for the native Android production canary and dispatcher contract. | Six-hour staggered schedule, successful-SHA checkpoint admission, same-SHA skip and explicit-rerun recovery, fixed non-canceling concurrency, absence of PR/deployment/manual admission and destructive shared-lifecycle credentials, exact Web/Android SHA and immutable lightweight-tag binding, short dispatch lease, exact private-run receipt, cancellation/execution fencing, credential scrubbing, current-production alias proof, and connected Health Connect postcondition. Repo Hygiene runs this test. | `.github/workflows/native-android-hosted-e2e.yml`, `scripts/native-android-hosted-e2e*.mjs`, shared owners under `scripts/native-ios-hosted-e2e*.mjs`, and `agent-docs/operations/native-android-hosted-e2e.md` |
...
- `.github/workflows/native-ios-hosted-e2e.yml` and
  ...
  47 every six hours. Neither workflow admits pull-request, deployment-status,
  or manual events, and neither publishes a required commit status.
- Each controller starts with a read-only selection job. On the first attempt,
  it compares the current default-branch SHA with the latest successful
  scheduled run of that exact workflow. An equal SHA skips the paid native
  journey; no prior success or a changed SHA runs it. A failed journey leaves
  no successful checkpoint, so the next schedule retries. An explicit rerun of
  the same trusted scheduled run bypasses the skip. Fixed, non-canceling
  per-platform concurrency prevents overlapping native journeys without
  accumulating per-commit waiters.
- Both production canaries prove the checked-out SHA belongs to current `main`
  and that the current production alias resolves to that SHA. They dispatch
  only `production_canary` with `non_destructive_existing_identity`. The
  workflows do not enter the historical shared PR environment or receive its
  database, Privy, Junction, candidate-deployment, or reset authority.
...
- The dispatcher modules retain deterministic PR-mode code and focused tests,
  but no public workflow currently admits that mode. Those dormant helpers are
  not an operator runbook. Reintroducing candidate deployment or destructive
  shared-identity orchestration requires a separate trust, cost, and product
  decision.
- Dormant PR mode remains test-covered but has no public workflow admission.
  Its retained dispatcher path enumerates active deployments in the dedicated
  E2E Vercel project, rejects production or foreign deployments, retires only
  lane-marked deployments, resets only the isolated identity, and dispatches
  only after the exact candidate origin is anonymously reachable. This is
  regression coverage for dormant code, not a current operator runbook.
...
- In dormant PR mode, native success is necessary but not sufficient: the
  retained code verifies a fresh Privy principal, the exact hosted member, and
  the platform-specific Junction connection before its fail-closed cleanup.
  The current production canaries do not run this destructive lifecycle.
- The dormant PR path's Vercel custom environment remains the sole owner of its
  isolated runtime configuration and explicit E2E Junction namespace. The
  current production canaries neither read those values nor create candidate
  deployments.
