...
| `pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-onboarding-stripe-alert-email.test.ts apps/web/test/hosted-onboarding-logging.test.ts apps/web/test/hosted-onboarding-stripe-webhook-service.test.ts apps/web/test/hosted-onboarding-stripe-event-reconciliation.test.ts` | Focused Stripe failure-alert proof for the shared website/Assistant provider-call boundary, canonical verified payment-failure webhooks, first-attempt reconciliation reporting, replay identity, private-data exclusion, and unchanged billing control flow. | `apps/web/src/lib/hosted-onboarding/{stripe-alert-email,stripe-error-log,webhook-service-stripe,stripe-event-reconciliation}.ts` and their focused hosted-web tests |
