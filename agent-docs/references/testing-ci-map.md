...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/device-sync-webhook-trace-batch-postgres.test.ts` | Opt-in real-PostgreSQL proof for bounded device-webhook trace admission batching. The suite rejects non-loopback database URLs and requires the current migrations. | Eight fresh traces are claimed through one set admission; the same owner then classifies processed and active rows, conditionally takes over an expired lease, and prevents a same-input duplicate from sharing the new claim token. |
