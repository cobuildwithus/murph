...
| `pnpm stripe-requests:guard` | Babel-based static guard, included in root typecheck and diff preflight, that rejects object spreads in official Stripe SDK request arguments and Stripe-typed parameter builders. This keeps every optional provider key visible to the SDK's generated TypeScript declarations. | `apps/web/src/**`, `apps/web/scripts/**`, `scripts/check-stripe-request-spreads.ts`, and `scripts/check-stripe-request-spreads.test.ts` |
