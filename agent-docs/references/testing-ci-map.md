...
- `apps/cloudflare/test/codex-openai-egress-conformance.test.ts` binds the
  ...
  image, and workspace Codex pins. It scans the installed native artifact
  offline and fails on every unclassified route candidate; asserts allowed,
  websocket-only, and blocked dispositions through the production Worker; and
  drives a real pinned App Server `web.run` turn through synthetic
  `/v1/responses` and `/v1/alpha/search` upstreams. The fixture is review
  evidence only and never generates the production allowlist. Linux CI scans
  the same platform family shipped in the hosted runner; local macOS and Windows
  runs remain useful platform diagnostics.
