...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/prisma-database-retry-postgres.test.ts` | Opt-in real-PostgreSQL proof that the shared Prisma client retries only failures that did no work. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | A contended transaction-start timeout is retried and its callback runs exactly once, a retried write persists exactly one row, a real pool-checkout timeout on an ordinary non-transaction write is retried through the query extension and also persists exactly one row, and a transaction that opened and then expired raises the same `P2028` code without being replayed |
...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-group-join-outreach-reply-recovery-postgres.test.ts` | Opt-in real-PostgreSQL proof for the group-join reply recovery boundary. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | A temporarily unassignable reply leaves its group context available; a later reply produces the intended group-aware invite URL; provider acceptance consumes the context; terminal failure reopens it for a group-aware retry; a stale earlier failure leaves both the newer provider-dispatch-started attempt's daily-send gate and eventual accepted group context closed; and a later winning delivered receipt restores consumption |
