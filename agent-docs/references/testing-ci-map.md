...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/device-sync-webhook-trace-batch-postgres.test.ts` | Opt-in real-PostgreSQL proof for bounded device-webhook trace admission batching and the composed maximum-callback scheduler path. The suite rejects non-loopback database URLs and requires the current migrations. | Eight fresh traces are claimed through one set admission; the same owner then classifies processed and active rows, conditionally takes over an expired lease, and prevents a same-input duplicate from sharing the new claim token. A 100-entry, 65%-hot-account replay composes the actual Web scheduler, shared ingress, hosted control-plane store, and migrated PostgreSQL trace owner against a scalar control; it preserves hot-account claim/completion order, advances independent accounts with no more than four live database operations, reduces the changed slice from 200 to 116 transactions and 131 statements, accounts for every accepted result, and locks the three-account 26,750-event model to 30,096 transactions and 33,442 statements instead of 53,500 scalar operations. |
