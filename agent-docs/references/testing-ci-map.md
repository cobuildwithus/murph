...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-group-join-outreach-reply-recovery-postgres.test.ts` | Opt-in real-PostgreSQL proof for the group-join reply recovery boundary. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | Eight cases prove that a temporarily unassignable reply retains its exact group context; crashed and distinct replies remain independent; generic/group terminal receipts converge in both orders; production group-owner account deletion preserves suppression while a sibling delivery is live, clears it after the final correlation is removed, and permits an ordinary retry after late receipts; the suspension/provider fence handles both account-deletion/reply orderings with a root Prisma client and a provider barrier outside preparation; a real HTTP response-body stall aborts before transaction expiry while deletion remains behind the drain and completes durable cleanup once; drain contention times out before provider entry without a durable attempt; and buffered terminal failure leaves the exact outreach open and daily marker clear so a fresh same-day generic delivery can proceed |
