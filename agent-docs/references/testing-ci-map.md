...
`packages/assistant-evals` is root-wired into `pnpm typecheck`, `pnpm test`, and
package coverage. Those automated lanes prove provider-free contracts,
cancellation cleanup, artifact safety, CLI behavior, and onboarding scenario
composition. `pnpm eval:onboarding` remains an explicit live-model evaluation;
its ignored synthetic evidence is graded separately through ReviewGPT.
