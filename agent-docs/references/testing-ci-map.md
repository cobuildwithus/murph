...
- `.github/workflows/frontend-design-proof.yml` checks every pull request for the required architecture-and-reuse summary. It also requires the `/design` catalog update, an Evidence item, and a Coverage explanation matched to the changed risk whenever the base-to-head diff changes user-facing hosted-web UI.
