...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-group-join-offer-replay-postgres.test.ts` | Opt-in real-PostgreSQL proof that a hosted Linq group-offer receipt can authorize membership and sharing only while its recorded authority remains current. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | A receipt whose acceptance rolls back retries once when unchanged; a newer event's membership, selected-share revocation, leave, or changed member terminally supersedes the old event without mutation; a fresh post-leave event may rejoin; legacy bare pending cannot be rebound; and an applied replay racing leave remains revoked |
