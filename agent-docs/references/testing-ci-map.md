...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-onboarding-linq-chat-health-inventory-postgres.test.ts` | Opt-in real-PostgreSQL proof for bounded Hosted Linq chat-health inventory convergence. The suite rejects non-loopback database URLs and runs after migrations. | Stale, equal, newer, legacy-key, ambiguous-line, and duplicate projections converge while preserving line freshness; replaying an earlier independently committed duplicate chunk cannot replace the globally selected later winner; and opposite current privacy-key versions contend on the same complete advisory lock set so a forced legacy-row migration cannot lose the newer writer. |
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-vault-share-grant-limit-postgres-concurrency.test.ts` | Opt-in real-PostgreSQL proof for the atomic vault-share grantor/scope limit and delivery corruption boundary. The suite rejects non-loopback database URLs and runs after migrations. | Two concurrent grants against a 24-row cohort admit exactly one 25th active share through the production grant owner; a directly injected 26th row makes the bounded delivery read fail closed instead of truncating the invalid cohort. |
