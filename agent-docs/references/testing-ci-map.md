...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-pending-group-setup-postgres-concurrency.test.ts` | Opt-in real-PostgreSQL proof for one-use next-Linq-group ownership intent. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | Two simultaneous group claims against one prepared owner yield exactly one claimed setup, and deleting that owner cascades a subsequently re-armed intent |
...
Hosted-execution parser tests lock the closed prepare/read/cancel wire
contracts; assistant-engine tests lock the private fresh-text gate and skill
guidance; Hosted Web group-tool tests lock current-line preparation and status
handling. Selection and prepared-route tests prove lone-candidate ownership,
