...
| `node --test scripts/hosted-orchestration-compatibility.test.mjs` | Focused behavior proof for the protected public-to-private Temporal compatibility controller: conservative changed-file selection with rename and pagination coverage, same-repository human authority, exact public-head and default-base revalidation, default-branch-only SHA-global status publication, bounded unprivileged fixture artifacts, repository-scoped App-token resolution of private `main` before dispatch, fixed private workflow identity, `main` dispatch with returned run details, exact pre-resolved private-head and first-attempt run binding, final public- and private-head movement checks, closed dispatch inputs, bounded exact-id `404` visibility recovery, private-owned supported readers plus the exact dispatched private candidate, producer/reader proof-digest binding, unique successful reader jobs, deterministic attestation, and bounded exact accepted-run cancellation. Repo Hygiene runs this test. | `.github/workflows/temporal-compatibility.yml`, `.github/workflows/repo-hygiene.yml`, and `scripts/hosted-orchestration-compatibility*.mjs` |
...
stay serial inside each GitHub-hosted two-core runner; only the independent jobs
fan out. The build/typecheck job remains the sole owner of the prepared Messaging
