...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/member-owned-provider-setup-oauth-deletion-postgres-concurrency.test.ts` | Opt-in real-PostgreSQL proof for member-owned provider cancellation, deletion, and reconnect ownership. | Run admission and Cancel cover four production orderings: cancellation before the setup CAS retires the unbound reservation without Kernel creation; binding during profile provisioning retains `canceling` until the worker rejects; cancellation during an in-flight browser create retains deterministic cleanup until the late exact browser is deleted; and a lost create response converges through the existing stale-provisioning boundary. None leaves a live browser, active run, or provider navigation. OAuth callback persistence and deletion admission serialize on the existing hosted-member row. Callback-first leaves one active connection and moves setup to `disconnect_first`; deletion-first persists `deletion_pending` and rejects the stale callback without creating a connection. A deleted terminal setup deactivates before the partial-unique member/provider slot admits a fresh pending successor. |
