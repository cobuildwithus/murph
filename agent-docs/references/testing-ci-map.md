...
- Both production canaries prove the checked-out SHA belongs to current `main`.
  ...
  same protected-main checkpoint. The iOS controller selects the actual production
  alias SHA, proves its protected-main ancestry and exact deployment, and repeats
  exact deployment verification after the native journey. Its controller revision
  may be newer than production; the dispatch-time alias must still match the
  selected deployment. Android requires the production alias to equal the selected
  main revision and retries mismatches at the next slot. They dispatch only `production_canary` with
