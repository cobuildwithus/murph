...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/device-sync-scheduled-wake-retention-postgres.test.ts` | Opt-in real-PostgreSQL proof for scheduled-v3 mailbox duplicate recovery after payload retirement. The suite rejects non-loopback database URLs and runs after migrations. | Generic append remains strict. Scheduled recovery accepts only the structurally exact row named by the runtime's first-pending projection when the imported watermark and allocated high-water cover it, including after the handled frontier advances past a retained retry. Missing or different ownership, an unhandled gap, legacy unsequenced ambiguity, malformed or impossible watermarks, remaining payload surfaces, changed occurrence metadata, and legacy schedule identities fail closed. |
