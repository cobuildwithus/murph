...
| `pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-pending-group-setup-fanout.test.ts apps/web/test/hosted-onboarding-linq-thread-route.test.ts apps/web/test/hosted-onboarding-linq-line-store.test.ts apps/web/test/hosted-member-routing-pending-group.test.ts apps/web/test/hosted-onboarding-linq-observability-store.test.ts apps/web/test/hosted-crypto-domain-root-store.test.ts apps/web/test/hosted-onboarding-linq-mailbox-root-prewarm.test.ts` | Deterministic public-Linq pending-setup fanout and preparation-boundary proof. | Exact 32-member no-sender and selected-race shapes keep owner statement counts constant in roster size; managed lines, narrow routing, and all five recovery attempts per candidate use set reads; only eligible private routing values are opened; root metadata is set-based; KMS concurrency never exceeds four; an uncached selected root completes before `BEGIN`; permanent invalid-root state is consumed only after exact lock while transient KMS state is retained; no provider/KMS work occurs under the transaction or setup lock; and a production-shaped 32-participant webhook retry preserves the exact replacement-line owner with sequential transactions. |
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-pending-group-setup-postgres-concurrency.test.ts` | Opt-in real-PostgreSQL proof for the encrypted one-use next-Linq-group transfer envelope prepared before `BEGIN`. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | A 32-candidate replay completes through a one-connection pool with 8 SQL statements for the ambiguous no-sender shape and 13 for the sender-selected locked shape; two simultaneous claims still converge on one setup and route; corrupt-selected consumption, exact replacement-line accepted/in-flight authority, rollback, replay, one-use consumption, restore fencing, and owner-delete cascade remain covered without transaction-held KMS. |
