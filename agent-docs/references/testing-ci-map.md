...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-usage-referral-recovery-postgres.test.ts` | Opt-in real-PostgreSQL proof for the bounded usage-referral recovery lane-head selector. The suite rejects non-loopback database URLs and runs after migrations. | A live generic system predecessor is selected once for a lane containing a later referral notification, while a retention-old prefix advances the effective cursor so the live referral itself becomes the selected head. |
