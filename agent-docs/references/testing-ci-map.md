...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/device-sync-prepared-webhook-authority-postgres.test.ts` | Opt-in real-PostgreSQL proof for delayed prepared webhook authority revalidation through the production-composed hosted ingress service. The suite rejects non-loopback database URLs and requires the current migrations. | A real Svix-signed Apple Health source-registration event is verified and prepared once, then consumed through the actual Junction registry and hosted Prisma store. A replacement start is held after capturing its start instant but before persistence; the old signed envelope is prepared while the expired prior setup is still durable, retries while the persisted replacement setup is live, and terminally completes only its trace when the exact same envelope is delivered at replacement expiry, preserving the setup on replay. A signed Strava deauthorization prepared for established connection A is consumed after same-account reconnect B; because B's `connectedAt` is later than the frozen receipt, hosted admission completes only the trace and preserves B's active status, credentials, receipt/freshness, dirty state, signals, mailbox/wake, and job surfaces. Consent revocation during source cleanup terminally completes only the claimed trace without provider I/O, receipt/source mutation, dirty state, signals, or mailbox work. A source epoch superseding the first admission phase during the one provider read is retained by the second locked revalidation with the same trace-only terminal result. |
