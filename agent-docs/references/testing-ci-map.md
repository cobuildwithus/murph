...
| `node --test scripts/linq-production-canary-ci.test.mjs` and `.github/workflows/linq-production-canary.yml` | Focused source proof plus the protected hourly production iMessage journey. | One staggered hourly automatic admission with manual recovery, protected-main ancestry, current-alias and exact-deployment verification, fixed non-canceling concurrency, least-privilege checkout, live-step-only canary credentials, a fixed-target input-free reset, the ordinary member runtime model default, listener-before-send behavior, exact canonical welcome and bundled identity question, a synthetic identity answer that reaches the container on turn three, and content-free per-turn timings even on budget failure. The twenty-second reply budget remains strict; a ninety-second observation window exposes slow replies, while unavailable replies identify their turn and stage. The production GitHub Environment owns the Photon project credentials, Murph target, reset credential, existing Vercel verifier values, and production base URL. Vercel separately owns the same reset credential and fixed Photon sender under `HOSTED_ONBOARDING_LINQ_PRODUCTION_CANARY_RESET_SECRET` and `HOSTED_ONBOARDING_LINQ_PRODUCTION_CANARY_PHONE_NUMBER`. |
