...
| `node --test scripts/check-frontend-evidence.test.mjs scripts/check-pr-changelog.test.mjs scripts/check-pr-deployment-concerns.test.mjs` | Focused Node tests for the pull-request evidence guards. Every PR must include one deployment-concerns disposition: applicable changes provide the complete skew, order, rollback, exposure, reversibility, convergence, and post-deploy contract; other changes provide a concrete not-applicable reason. The frontend guard separately requires direct proof plus a Coverage explanation matched to each user-facing hosted-web UI diff, without a catalog edit or screenshot quota. The guards validate GitHub-rendered GFM so comments, code blocks, and raw HTML cannot be mistaken for visible proof. | `scripts/check-{frontend-evidence,pr-changelog,pr-deployment-concerns}.mjs`, their focused tests, `.github/workflows/pr-evidence.yml`, and `.github/pull_request_template.md` |
...
- `.github/workflows/pr-evidence.yml` checks every pull request for changelog and deployment-concerns dispositions. Applicable deployment changes must provide the complete skew, order, rollback, exposure, reversibility, convergence, and post-deploy contract; all other changes need a concrete not-applicable reason. The workflow also requires direct evidence and a Coverage explanation matched to the changed risk whenever the base-to-head diff changes user-facing hosted-web UI. It does not require a catalog edit or screenshot count.
...
- The required native companion auth/control/device-sync acceptance lane is `.github/workflows/native-ios-hosted-e2e.yml`. It runs from trusted default-branch `workflow_run` code after Repo Hygiene, conservatively selects every `apps/web/**` change plus the current hosted-Web source-package closure and directly consumed root build/deploy/workflow owners, and publishes the stable `Native iOS hosted E2E` commit status. A selected same-repository human-authored PR enters the protected `native-ios-hosted-e2e` environment; fork/bot or missing-environment cases fail closed rather than receiving live authority. The destructive live job retains valid waiters in FIFO order up to GitHub's bounded `queue: max` capacity and never replaces an existing pending waiter. Do not rerun a canceled native workflow directly: a rerun keeps its old run identity and is rejected before the live group. From an authenticated operator checkout, run `node scripts/native-ios-hosted-e2e-retry.mjs --pr <number>` instead; the helper proves the PR is still open, same-repository, human-authored, and at the same exact head, then reruns the newest successful exact-head Repo Hygiene owner so its completion emits a fresh native workflow waiter.
