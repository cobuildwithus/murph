...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-linq-app-card-fallback-postgres.test.ts` | Opt-in real-PostgreSQL proof for native-card ambiguity and definitive-rejection delivery-fence ownership. The suite rejects non-loopback database URLs and runs after migrations. | Acceptance of an exact card replay closes its original provider-dispatch fence; definitive card rejection atomically terminalizes that predecessor and claims the stable fallback as the only active fence; transition replay is idempotent; fallback acceptance leaves the chat with no unresolved dispatch |
