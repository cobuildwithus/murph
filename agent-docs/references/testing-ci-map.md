...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_POSTGRES_CONCURRENCY=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/device-sync-start-account-deletion-concurrency.test.ts` | Opt-in real-PostgreSQL proof for hosted device connection start against account deletion. The suite rejects non-loopback database URLs and runs in the hosted E2E PostgreSQL job after migrations. | Finalization-first ordering replaces the staged marker with the optional connection seed and callback state atomically before deletion observes and removes both; suspension after staging leaves seeded and seedless lifecycle markers visible for provider cleanup; suspension before staging rejects both forms without starting local provider state |
