...
- Automatic meal-photo capture is covered by hosted-web enrollment/upload route and JPEG-boundary tests, hosted-execution wake parsing tests, Cloudflare private-object route/encryption/lifecycle tests, assistant-runtime canonical import/idempotency/managed-closeout/post-checkpoint cleanup tests, assistant-engine opt-in-only 9pm managed-seed and skill tests, and core/CLI receipt-guarded photo-tombstone tests that validate the vault after removal. Routine CI does not grant a real iPhone Photos permission, execute a production scheduled model closeout, or upload to the production R2 bucket, so final rollout proof still requires an explicit physical-device opt-in/upload and later 9pm delivery/cleanup observation against the deployed heads.
