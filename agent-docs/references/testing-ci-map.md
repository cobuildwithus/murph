...
- `.github/workflows/frontend-design-proof.yml` checks every pull request for the required architecture-and-reuse summary. It also requires the `/design` catalog update and hosted desktop/mobile screenshot links whenever the base-to-head diff changes user-facing hosted-web UI. The guard recognizes only exact add/remove diffs for the known null-rendering `VercelTelemetry` boundary as non-visual; any other changed line in the same UI file restores the ordinary proof requirement.
