...
| `pnpm test:frontend-evidence` | Focused Node tests for the pull-request frontend-evidence guard. The guard requires direct proof plus a Coverage explanation matched to each user-facing hosted-web UI diff. It does not require a catalog edit or screenshot count. It validates GitHub-rendered GFM so comments, code blocks, and raw HTML cannot be mistaken for visible proof. | `scripts/check-frontend-evidence.mjs`, `scripts/check-frontend-evidence.test.mjs`, `.github/workflows/pr-evidence.yml`, and `.github/pull_request_template.md` |
...
- `.github/workflows/pr-evidence.yml` checks every pull request for a changelog decision. It also requires direct evidence and a Coverage explanation matched to the changed risk whenever the base-to-head diff changes user-facing hosted-web UI. It does not require a catalog edit or screenshot count.
