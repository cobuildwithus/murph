...
| `DATABASE_URL="$LOCAL_POSTGRES_URL" MURPH_TEST_RUNTIME_LOG_POSTGRES=1 pnpm exec vitest run --config apps/web/vitest.workspace.ts --no-coverage apps/web/test/hosted-runtime-log-postgres-concurrency.test.ts` | Opt-in real-PostgreSQL proof for the isolated runtime-log database, monotonic digest deletion fence, and bounded account cleanup. The suite rejects non-loopback database URLs, creates and migrates a temporary logical database, and uses the production store SQL. | Three delayed primary-authority reads hold zero isolated clients with pool max two; append-first cleanup deletes the committed row; the exact deletion-first pre-check gap drops the stale active append after cleanup commits and releases the subject lock; timed-out cleanup rolls back without a partial fence, and repeated cleanup converges idempotently to one fence and zero rows. |
