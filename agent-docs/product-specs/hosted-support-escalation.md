...
- The support email contains a fixed escalation sentence, Murph's stored sanitized product issue, the internal feedback id, and the internal member id. Missing, member-linked, unsanitized, or still-prefixed stored detail fails closed before the provider boundary.
...
- The model-facing contract forbids raw conversation or voice text, names, handles, contact details, health facts or values, diagnoses, medications, precise locations, relationships, secrets, provider payloads, and unrelated context in the written issue. The email path has no direct raw-message, transcript, or provider-payload field and accepts only the stored model-authored issue after deterministic sanitization. That sanitizer does not semantically prove free-form prose: the explicit verified-private request accepts the documented residual risk that unrecognized private meaning or copied wording can remain when the issue is disclosed to the dedicated internal support recipient.
