...
3. The Settings customization section reopens the same picker so an existing
   member can save a fresh card with a different portrait.
...
- Implemented: "Add Murph to Contacts" opens `GET /api/murph-contact-card?avatar=<id>` (`apps/web/app/api/murph-contact-card/route.ts`), which returns `Murph.vcf` built with the existing `buildMurphHostedLinqContactCardVcf` (`apps/web/src/lib/hosted-onboarding/linq-contact-card.ts`): the member's own line as the `mobile` number, a second healthy pool line under the `backup` label, and the chosen avatar embedded as `PHOTO`. Before direct or in-app-browser navigation, the picker uses the existing authenticated handoff-issuance request to prove the selected portrait is available; a failure keeps the picker open with a retry message. The GET and issuance routes read that portrait directly from their traced public assets instead of performing a production self-fetch; if a selected nonblank portrait is unavailable, they return retryable `MURPH_CONTACT_CARD_PHOTO_UNAVAILABLE` rather than silently returning a photo-less card. The optional backup comes from the existing `HostedLinqLine` projection maintained by scheduled provider reconciliation, so a download never performs live provider inventory or reconciliation work; a missing or unreadable projection omits only the backup. `avatar=none` omits `PHOTO`; unknown ids fall back to the default headshot. The route requires an active hosted member session and resolves the member's line server-side from hosted member routing.
- Saving the vCard sets the contact photo in the member's own address book. That is the entire per-user effect.
- When a live `message.delivered` receipt advances a correlated `invite_signup` or `invite_signup_fallback` delivery, the post-commit webhook path considers Linq's native `share_contact_card` endpoint for the receipt's chat (`shareMurphHostedLinqNativeContactCardToChat` in `apps/web/src/lib/hosted-onboarding/linq-contact-card-share.ts`). Before reserving or sharing, it requires exactly one active self handle in the chat roster, resolves that handle's normalized phone number, and fetches the provider contact card for that line. The native share fires only when the card exists, is active, has the same normalized phone number, and has an explicitly present `image_url` that parses to null. Zero or multiple active self handles, an unresolved line, a missing, inactive, or mismatched card, an omitted `image_url`, or a provider read failure skip fail-soft as unverifiable; an explicitly present non-null image skips as imaged. Provider acceptance and `message.sent` alone do not trigger the card. If the delivered receipt arrived before the accepted milestone established correlation, accepted-milestone replay schedules the same best-effort check after its transaction commits. Existing-chat direct and group signup replies are eligible, and the degraded-line fallback checks the newly created direct chat; non-iMessage services are skipped. The check and share never fail or delay reply delivery, and a positively known image-free share uses the per-chat 90-second retry-window reservation shared with the explicit group `share_contact_card` tool so duplicate attempts within one turn/wake cannot double-send. Native `share_contact_card` remains a bodyless POST, so Linq selects the card at send time and the preflight read cannot be atomically bound to that effect; this hardened preflight is therefore a strong best-effort check, not a mathematical guarantee. Skipping an imaged or unverifiable line for a delivery is an explicit, logged product decision, not a silent drop: the member still received the signup reply and can add Murph from the web picker, and the automatic share self-heals for future contacts once the image is removed.
