...
- When a live `message.delivered` receipt advances a correlated `invite_signup` or `invite_signup_fallback` delivery, the post-commit webhook path invokes Linq's native `share_contact_card` endpoint for the receipt's chat (`shareMurphHostedLinqNativeContactCardToChat` in `apps/web/src/lib/hosted-onboarding/linq-contact-card-share.ts`). That endpoint shares the sending line's configured provider contact card: `Murph`, name-only under the current reconciliation policy, with no per-user avatar. Provider acceptance and `message.sent` alone do not trigger the card. If the delivered receipt arrived before the accepted milestone established correlation, accepted-milestone replay schedules the same share after its transaction commits. Existing-chat direct and group signup replies are eligible, and the degraded-line fallback shares into the newly created direct chat; non-iMessage services are skipped. The share is best-effort (it never fails or delays the reply delivery) and shares the per-chat 90-second retry-window reservation with the explicit group `share_contact_card` tool so duplicate attempts within one turn/wake cannot double-send.
- First contact deliberately does not carry a member-chosen avatar because the sending line is pooled and its provider card is name-only. The first-party `.vcf`, with the canonical or member-chosen photo, remains the mechanism for Murph's discretionary in-chat share through the group `share_contact_card` tool and for the web "Add to Contacts" picker download. Native sharing pushes the existing line card into the chat; it does not write a member's avatar choice to that pooled card, so it does not overwrite or leak member-selected photo state.
...
- Do not write member avatar choices to Linq per-line contact cards. Lines are pooled across members, and the contact-card cron (`reconcileHostedLinqContactCards`) only creates missing name-only cards or corrects the shared Murph name. The automatic native share pushes that shared line card into the chat without mutating it; member-selected photos stay confined to first-party `.vcf` generation.
