...
- An eligible run with saved `assistantSupport.notificationStyle: send_scheduled_summary` may receive one concise text progress moment on intervention day four. It celebrates completed work first and treats sparse or unchanged metric data as a caveat rather than a reason to suppress the recap.
...
- With saved scheduled-summary consent, the same closeout may continue into a finite final review: congratulate the member, summarize the result with confidence and confounders in concise text, and ask whether to repeat it, adapt it, or leave it alone. A short voice memo may replace the text on a supported channel. Revoked consent suppresses the message without suppressing outcome persistence. Runs that end early receive neither lifecycle message nor an on-schedule closeout.
- Public progress-card URLs and the CLI URL generator are retired. The legacy GET route and CLI command fail closed; automation instructions must not place reversible health data in URLs. A future automated visual must render directly into private `vault_image` media through a shared renderer.
