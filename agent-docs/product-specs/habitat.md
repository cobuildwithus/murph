...
4. **Voice walkthrough.** The authenticated Environment page can record one continuous memo while showing short topic cards. At zero coverage the script covers all five categories. A partial profile gets only categories and non-declined facts that remain unknown. At 95% coverage or above the script becomes one free-form update prompt, so the member can describe what changed without repeating the walkthrough. The cards are prompts, not fields. The browser uploads the recording directly to Murph instead of opening a chat share sheet. Murph stages it application-encrypted, transcribes it through the existing hosted transcription boundary, and runs one silent Habitat-only maintenance turn that immediately saves explicit, high-confidence catalog facts. For `home-location.location`, the maintenance turn stores only an explicitly stated city or approximate region; exact address details volunteered in a recording are never persisted or sent to the weather provider. The transcript is not added to conversation history, ambiguous details stay unknown, and corrections happen in the normal Murph conversation. Audio is deleted only after the updated vault checkpoint succeeds; a 24-hour R2 lifecycle is the recovery backstop.
...
- Zero-data members see a benefit-led empty state with voice and chat entry points. Sparse records keep the report visible and offer a voice script derived only from unknown, non-declined facts. At 95% coverage or above the fill-gaps action disappears in favor of a free-form voice or chat update. There are no edit forms.
...
- 2026-07-30 — Voice guidance has three modes: five-category first walkthrough, coverage-derived gap filling that excludes declined facts, and one free-form update prompt at 95% coverage or above.
- 2026-07-30 — Voice extraction may retain only a member-stated city or approximate region for live conditions; exact address details are discarded.
