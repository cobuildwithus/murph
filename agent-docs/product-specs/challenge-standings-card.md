...
The group response-card tool accepts the canonical challenge-page slug, the same normalized observation input as `score-challenge`, and the exact canonical projection-scope keys used by each scorecard component. Scorer participants start from the challenge page's participants whose participation state is `in`; current room membership or a sharing grant never creates challenge buy-in. The tool runs the deterministic scorer and derives every point total, target, order, coverage state, coverage count, rank, and tie at the attachment boundary; the model cannot submit those derived values directly. Individual labels come only from the trusted shared read, in scorer/page order, the title comes only from the existing challenge page, and V1 subtitle and footer remain null. The existing response-card pipeline then validates the closed card schema and emits semantic text for non-native routes.

The App Server turn owns one ephemeral shared-read proof. Every attempted scoring read contributes its trusted canonical scope keys and must return the same nonempty ordered current-room-member and authorized-label roster. An unavailable, failed, empty, capacity-omitted, duplicate-participant, roster-changed, or label-changed result invalidates the proof for the rest of the turn. Attachment requires at least one successful batch, unique scorer participant ids that are all present in that trusted roster, and one component-to-scope mapping backed by those successful reads. Additional current room members remain nonparticipants; scorer/page order owns card order.

After those checks, the attachment effect compare-and-set upserts one machine-readable snapshot containing the normalized input, deterministic result, component scope mapping, and trusted read batches onto the existing challenge page. Persistence completes before the card is attached. A missing page, malformed prior snapshot section, concurrent page change, or write failure leaves ordinary text available and attaches no card. This is not a second leaderboard: the existing challenge page remains the only durable owner, and the proof itself resets with the turn.
...
This card does not create a challenge, decide participants, interpret raw evidence, create a second leaderboard, or refresh itself after delivery. It verifies the trusted read/persistence boundary, runs the existing scorer, and records the resulting snapshot on the existing challenge page. A later update is a new immutable message snapshot produced from that canonical challenge state.
