...
- The canonical experiment outcome writer saves one write-once, version-bound outcome with biomarker deltas, confidence, caveats, confounders, protocol references, analysis windows, and the bounded daily points used to calculate each metric summary. Repeated closeout or delivery work validates and returns that referenced artifact instead of refreshing it from later evidence.
- The encrypted browser-vault replica dereferences only the validated `outcomeRef` on a canonical experiment and requires the saved artifact to match the stable experiment identity. The saved artifact owns its historical title, lifecycle status, protocol, windows, metric summaries, and any daily point snapshots it contains; supported later experiment edits do not revoke or rewrite it. Legacy point-free outcomes remain unchanged and aggregate-only. The results read model may pair their saved averages with current browser-safe daily measurements selected by the artifact's saved biomarker identities and windows; these display points are not written into, or represented as historical evidence from, the legacy artifact.
