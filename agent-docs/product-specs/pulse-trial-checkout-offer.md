...
This plan specifies the 14-day Pulse Trial built on the current hosted Murph billing and hosted AI usage allowance system.
...
trial policy is 14 days as of `pulse-trial-2026-07-15-v3`. The previous
...
- `apps/web/src/lib/hosted-onboarding/billing-service.ts` persists one open Checkout attempt before provider I/O, then creates or retrieves its Stripe Checkout Session in `subscription` mode with the plan recurring line item, session metadata, subscription metadata, and card payment methods. Identical concurrent requests reuse the attempt and Session; a different intent must wait for the open Session to finish or expire.
...
| `launch_monthly` | `pulse_trial_7d` | 14-day Pulse Trial checkout |
...
The member billing ref owns at most one open Checkout attempt. Before Stripe is
called, web commits a random attempt id, a server-derived intent hash, and its
creation time under the member billing lock. The intent hash covers the member,
invite, plan, offer policy, price, redirect URLs, and customer/email binding.

An identical retry reuses the same attempt. A different intent returns a typed
...
Creation uses no automatic SDK retry and a bounded request timeout. An unbound
attempt is replayable only inside a 23-hour window, shorter than Stripe's
documented minimum idempotency-key retention. Older ambiguous attempts require
support rather than risking a second subscription.
...
The 4.50 USD trial allowance is enforced without promising an exact token-level cutoff. The operation that crosses the remaining allowance may finish, but subsequent usage-bearing work blocks and accepted conversation input remains pending. Trial members cannot buy usage credit. Expired, pre-start, or malformed trial entitlement still denies independently of spend.
...
7. Persist one member-owned Checkout attempt and bind its intent to the Stripe idempotency key.
...
- `hosted-onboarding-billing-service.test.ts`
  ...
  - identical concurrent requests reuse one persisted attempt and Stripe Session
  - a different price, offer, policy, customer, or email binding cannot replace an open attempt
  - a retry after ambiguous provider failure reuses the attempt id and intent hash
...
- `hosted-execution-usage-allowance.test.ts`
  ...
  - the crossing operation may finish, produces the trial-specific limit notice, and blocks subsequent usage-bearing work
...
- runtime access tests
  - stale-trial member access retains its reason, notice, and future retry time
  - active paid and in-window trial admission composes entitlement with remaining allowance
...
4. Verify runtime admission allows an active paid member with remaining capacity and denies a synthetic stale-trial member with a future `retryAfter`.
...
10. Confirm trial accounting records the 4.50 USD limit and sends the trial-specific notice after a crossing.
11. Confirm a synthetic over-limit period blocks the next usage-bearing invocation while preserving accepted input, and never grants paid Pulse allowance.
...
tables. Checkout concurrency adds only one nullable attempt tuple and its
encrypted Stripe Session reference to the existing member billing ref.
...
- Active in-window trial admission enforces remaining allowance through the composed web-owned access-and-usage decision; the crossing operation may finish and subsequent work blocks.
