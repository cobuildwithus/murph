...
- Settings linked-account removal starts with Privy's unlink operation for the exact app-session principal. Web mutates Murph state only after a bounded management read proves that account type absent and another supported verified email, phone, or Telegram sign-in remains. The transaction locks the member, requires the submitted provider identity to match the current blind index when one exists, and clears only the identity-owned sign-in, sender, alias, and messaging-route projections; billing email and unrelated channels remain intact. A stale tab cannot clear a replacement identity. Telegram replacement completes this revocation before opening the ordinary link flow, so Privy never receives a second same-type link attempt.
