...
- After that exact event has a persisted model-source allow, Web may start one
  separate tool-free first-reply request in parallel with instant-start
  enrollment. Its provider-visible input is only the same bounded first-contact
  text plus fixed instructions and a strict reply-or-defer schema. It receives
  ...
  instructions require deferral for personal or current facts, files, actions,
  diagnosis, treatment, or any request that needs more context. Web also rejects
  ...
  deletion. Classifier failure, non-model allow sources, unsupported routes, or
  model deferral must never enter this reply path.
