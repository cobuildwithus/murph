...
- `web.fetch` and `web.pdf.read` may retrieve public webpages or PDFs over outbound HTTP(S), but they are an explicitly enabled egress surface and must stay guarded read-only tools: strip URL fragments, redact query-bearing URL details from returned metadata and errors, block loopback/private-network targets before fetch, keep redirects bounded and revalidated, enforce timeout plus response-size limits, and bound PDF parsing work. Treat fetched page bodies and extracted PDF text as untrusted external context rather than canonical Murph data, and do not use signed or session-bound URLs as tool inputs. These tools do not imply browser automation, cookie/session replay, or private-network reachability.
