...
- A hosted custom-inference connection is a singular personal-member secret
  ...
  logs. The runtime must also re-read Web's selected custom revision before
  provider input; the active fence and envelope are not current selection
  authority. Runtime egress must validate the active fence before opening the
  envelope and must never fall back to a managed provider.
