...
- The canonical write guard is now legacy/best-effort detection for deprecated direct-write paths rather than the normal Codex assistant-turn boundary. Where that legacy path still runs, it prefers validated committed core write metadata over temporary per-process guard receipts, so a well-formed committed protected write that matches its durable payload receipts may be preserved even if the temporary receipt copy is missing.
