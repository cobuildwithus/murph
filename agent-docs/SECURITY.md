...
- The internal product-feedback digest may disclose only the fixed
  server-owned kind and section labels, truthful grouped per-kind counts, each
  ordinary private row's server-controlled internal member id, and the
  ...
  through the existing Resend transport. The member id groups feedback already
  linked by Web; the digest must not read the member relation, contact data, or
  infer a human from a synthetic group runtime. Unlinked groupchat and truly
  anonymous rows share one final section so the email does not misattribute
  either to a person. The disclosure boundary for summary text is the capture
  side: the recording path stores only a bounded de-identified product-only
