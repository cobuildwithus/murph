...
- The same narrow authority-free presentation exception includes V4/V6/V7 workout
  ...
  member. Schema 7 may also carry a separately domain-separated stable workout
  digest for authenticated read-only refresh. That digest intentionally permits
  same-workout correlation across cards held by one credential owner, but is
  never identity or write authority: the Messages bearer scopes lookup to the
  current member, and runtime requires exactly one matching workout in that
  member's bounded canonical collection. A forwarded card therefore cannot
  read the sender's vault. A destructive batch is invalid when its typed final visible set
