...
- A direct phone call may additionally persist only the bounded trusted
  ...
  for asynchronous result routing.
  ...
  authority into the generation-scoped queued notification, and revalidates it
  again at message provider entry. The runtime may report only the exact call
  ...
  oldest nonterminal call for that member. Group calls omit the enum and keep
