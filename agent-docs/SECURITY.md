...
- For hosted Junction webhook recovery, an active provider result means every
  ...
  literal status `connected`. Lifecycle or resource aliases such as `active`,
  `available`, or `ok`, unknown values, and mixed matching statuses are not
  connection authority.
- Hosted browser device OAuth is a same-host, same-member, initiating-browser boundary. Start must issue one short-lived host-only proof bound to provider, OAuth state, member, and app-session generation. Callback GET must validate that proof against the active session and pass the session member as exact `expectedOwnerId` before state consumption or provider exchange; it then redirects back into the app with no interstitial confirmation. A missing or invalid proof consumes only the OAuth state and redirects to Connect so a transferable provider callback cannot be relayed into the initiating member later. This proof is ephemeral and adds no durable state owner. `DEVICE_SYNC_PUBLIC_BASE_URL` may change the callback path but must use the hostname that served the authenticated browser start; Web start/runtime plus build validation must reject both explicit and derived split-host values before OAuth mutation. Cloudflare preflight verifies explicit callback overrides only. Keep both `__Host-` cookies host-only: do not add a Domain cookie or cross-host handoff. Suppress Vercel Analytics and Speed Insights on both hosted device callback path families so provider authorization query parameters never enter those vendors. Junction `pending_link` and `link_returned` accounts are inert until either proof-verified callback completion or provider-verified hosted webhook recovery. Recovery requires an authenticated, source-attributed Junction event for the exact prepared row, then a live Junction provider-list read outside the transaction. When an active shared connection is missing that canonical source row, hosted Web may create only a disconnected candidate at the prepared receipt instant under the same health-data admission lock; the candidate is not source admission and remains disconnected until the ordinary provider proof succeeds. The runtime rechecks health-data consent, shared provider application, active connection and credential epochs, exact source epoch, and completed disconnect fences under the health-data admission lock. Only an active provider result may atomically commit `source_confirmed`, source admission, callback-equivalent source-scoped initial work, its mandatory mailbox handoff, dirty state, and webhook trace completion. The initial handoff remains mandatory when dirty work already exists. The webhook alone, candidate creation for a missing source row, an inactive provider result, passive runtime work, or sync success cannot confirm setup. The authenticated prepared receipt is not retry-lifetime authority: shared ingress and both hosted lock rechecks compare the current persisted `setupExpiresAt` with the trace claim's one processing-attempt instant. A live setup without exact provider proof releases the trace for retry; an expired setup completes only the trace before provider I/O without mutating setup phase, source authority, freshness, dirty state, signal, mailbox, wake, jobs, or canonical health. Once a shared account is `source_confirmed`, adding or retrying another Junction-backed source must preserve that phase and all established siblings. Keep the target source `disconnected` and exclude only its webhook and pull work until the runtime admission owner atomically commits source admission with durable work; shared ingress must not write that fact independently. Established sources bypass the recovery provider read, and a receipt older than the current `connectedAt` completes only its trace before provider I/O. An explicit disconnect fence, a newer still-pending source start, or a newer connection epoch wins over a late callback or webhook recovery, which fails without admitting the target. Every independently valid completion after source admission advances the existing source lifecycle epoch and reopens only that source's schedule-time history coverage; Junction exposes no durable provider-registration generation that could safely restore an older completion. Retry cleanup may deregister only that target. Whole-account revoke remains exclusive to explicit connection-wide disconnect. Local and tunneled `device-syncd` callbacks remain a separate explicit daemon contract.
...
- Hosted browser and native sessions use the closed encrypted Better Auth adapter. A database selector is never authority: authenticate the complete session record and current canonical member, reject suspended/deleted owners, and compare the original proof under member-before-session locks before protected commits. Only Web owns `HOSTED_BETTER_AUTH_SECRET` and `HOSTED_AUTH_STORAGE_KEY`; never send them to workers, browsers, logs or generic child processes. `HOSTED_APP_SESSION_HMAC_KEY` remains required by independent billing, referral, callback and recovery contracts; do not remove it with the retired session table. Historical generated deployment URLs must not expose retired readers after cutover; follow the exact deployment-protection and drain gates in `docs/hosted-auth-migration.md`.
- Better Auth is private behind explicit browser/native routes; do not expose its catch-all handler or default account-linking/deletion endpoints. The closed encrypted adapter authenticates all fields before reads, count, mutation or one-use consumption, including transaction callbacks and bulk operations. Canonical member/contact writes and OTP consumption/session issuance share one short transaction; known wrong-code failures commit attempt budgets and failures after proof roll back all writes. Delivery and KMS calls occur outside locks. Browser and native transports reject mixed ambient/bearer authority. Invalid replacement credentials never fall back; old browser cookies are anonymous and old native JWTs receive an upgrade response. Issuance can pause without disabling current session read, renewal or logout. The hourly owner drains expired auth rows in bounded indexed batches, and member deletion cascades member-bound auth records. Retirement deployment requires all continuity and protected-account gates in `docs/hosted-auth-migration.md`.
- Credential changes require a current first-party session, proof of the new contact, and approval of the exact member/session/method/old identity/new identity/operation. Consume contact proof, approval, canonical contact/routing updates, encrypted login projection changes and any session revocation in one database-only transaction. Adding a method preserves sessions; replacement/removal revokes other first-party sessions. Never remove the last usable sign-in or transfer another member's verified or pending contact. Telegram uses verified numeric identity and a session-bound, one-use nonce purpose distinct from login; no provider token is retained. Canonical channel changes enqueue the existing durable mailbox wake atomically, with signaling outside the transaction.
- Approval passkeys are scoped to sensitive actions, not primary sign-in. Store at most eight credential IDs, COSE public keys and signature counters in the encrypted member-bound approval aggregate; never persist private keys or browser assertion payloads. Require WebAuthn user verification, the canonical RP/origin and the complete action challenge. Enrollment requires the existing approved factor at both options and registration, with a five-minute member/session/action-bound challenge; an authenticated primary session alone cannot replace established protection. A first-party account with no established approval protection may enroll its first factor within five minutes of primary authentication; silent exchange and renewal do not satisfy this exception. Initial enrollment rechecks absent credential state and current session at commit. Prepare provider verification and encryption outside transactions, then recheck the original session authenticator and exact credential ciphertext under member-before-session locks. Commit credential changes, counter advancement and one-use action acceptance together. Member suspension or deletion fails closed and canonical deletion cascades credentials. Failed or corrupt passkey verification never falls back to another factor. Previously protected accounts must have usable first-party approval or recovery before retirement removes the legacy verifier. The enrollment switch remains disabled until all reachable readers enforce that ownership; see `docs/hosted-auth-migration.md` for activation and rollback floors.
- Approval recovery requires a previously saved random 32-byte key and first-party primary proof within five minutes. A current Murph passkey must authorize key generation; store only its SHA-256 digest in a separately member/field-bound encrypted column on the existing approval aggregate. Return plaintext once and never place it in URLs, browser persistence, logs or notifications. Redeem through a five-minute member/session/key-generation-bound challenge and new user-verified WebAuthn registration. Atomically replace all previous credentials, consume the key/challenge, revoke other first-party sessions. Keep the authorizing browser. Rotation and recovery must compare current credential state under locks, and provider/KMS calls remain outside transactions. An existing primary session, OTP alone, or lost approval factor cannot provision independent recovery. Unresolved protected accounts block vendor retirement; there is no support override based only on a contact channel.

- Initial approval enrollment is an explicit security tradeoff: an already-bound canonical verified first-party sign-in, proved within five minutes, may establish the first Murph passkey without a previously established approval factor. Require a current first-party session with non-future primary proof, an already-adopted encrypted login projection matching canonical verified methods, and strictly absent approval aggregate. Any approval aggregate (including corrupt/empty state) excludes this policy; no failed assertion or client report may enable it. Bound reauthentication selects only the current member's existing verified method, rechecks the original session and canonical projection under locks, and cannot import, link, create, merge, or mint login authority from registration. The registration challenge binds member, session, primary proof and canonical generation; require canonical RP/origin and WebAuthn user verification. Prepare the bounded twenty-session set and crypto outside locks, then atomically recheck eligibility/freshness/exact state, consume the challenge with absent-state CAS, revoke other sessions and stamp the existing credential-change fence. Preserve the authorizing browser. Both existing issuance/enrollment switches gate options and commit. Registration never approves an action: obtain a new exact-action/session challenge and native assertion through the existing decision owner. This consciously accepts compromise risk of the bound email, phone or Telegram account rather than preserving old-factor equivalence. No saved key, routing contact, link possession, old cookie, silent exchange or session renewal supplies enrollment authority. See `docs/hosted-auth-migration.md` for the required pre-retirement canonical-login inventory and rollback floor.
- A Vercel `Ready` status and the aliases reported on a deployment are candidate
  ...
  production target, `READY` state, and exact Git SHA, enumerate every project
  domain without a branch or custom-environment binding, require the configured
  production base host in that set, and prove every alias resolves to the exact
  deployment id. Provider credentials and domain names must not enter logs or
  artifacts. Public Murph may perform this read-only proof but must not promote,
  assign, or roll back production aliases. Because the proof executes from the
  deployed checkout, the first production deployment containing the exact
...
- Primary login and account-linking are separate authority paths. Credential changes must use the exact action-bound settings endpoint. A successful login may replace the browser session: invalidate decrypted browser-vault state at successful response headers, before JSON parsing, and reload the document after an unreadable successful response. A failed request that did not replace the session must preserve the current cache.
...
- First-party login resolves canonical contact ownership before member creation and compares that exact snapshot under contact/member locks before committing. A contact claimed by another member or a disagreement among phone, verified email, Linq handle or Telegram owners fails closed; there is no automatic member transfer or scaffold deletion. Retryable product completion cannot rebind credentials. App Review operations may activate an existing, verified first-party account through canonical product owners; they may not create credentials, return fixed codes or bypass contact proof.
...
- The browser legal-consent route may accept an invite code only as an explicit
  ...
  invite code or authentication proof is never independent enrollment
  authority.
...
- `POST /api/device-sync/companion/admission` is the admission-only native
  account boundary. Its first-party bearer resolves the existing canonical hosted
  member, then uses the existing consent, untouched-member Starter, and access
  owners, but the closed request accepts only an optional validated IANA time
...
- Native iOS and Android public controllers are protected-main production
  ...
  own no database, authentication, Junction, custom-environment deployment, or
