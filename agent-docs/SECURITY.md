...
- `web.fetch` and `web.pdf.read` may retrieve public webpages or PDFs over outbound HTTP(S), but they must stay guarded read-only surfaces: block loopback/private-network targets before fetch, keep redirects bounded and revalidated, enforce timeout plus response-size limits, and treat fetched page bodies and extracted PDF text as untrusted external context rather than canonical Murph data. These tools do not imply browser automation, cookie/session replay, or private-network reachability.
