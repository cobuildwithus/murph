...
checkpoint with zero retained jobs fences canonical cadence publication. The
wake must also carry the exact non-null epoch of the current active connection;
epoch-less legacy, replaced, missing, or terminal records drain without writing
cadence. An unchanged stale tuple may
...
eligible durable completion record then publishes cadence, removes the mailbox
item, and checkpoints that removal inside the same runtime admission. The deterministic
