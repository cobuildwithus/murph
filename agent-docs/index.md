...
| `agent-docs/operations/pr-reviewgpt-loop.md` | Exact-head managed-browser completion workflow: one preliminary pass and the eligible final gate can run concurrently; substantive results pause before mutation for parent-owned, evidence-backed accept/reject dispositions, except a sole accepted behavior-preserving Complexity Collapse may continue without separate user permission; rejected findings need no model withdrawal, while accepted findings retain verification and rerun rules. The loop also owns exact-head packaging, immutable baselines, bounded coverage patches, completion watchers, correction versus full snapshots, moving-base handling, anomaly retrospectives, and the seven-round hard cap. | PR ReviewGPT loops | Medium | 2026-08-23 |
