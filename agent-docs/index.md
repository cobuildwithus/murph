...
architecture/reuse summary, a base-to-synthetic-merge cyclomatic-complexity
result plus agent hotspot judgment, foreground reply-path impact, complete
initial provider-input impact, and categorized added/deleted LOC. User-facing hosted
...
| `agent-docs/operations/completion-workflow.md` | Outcome-based completion with proportional Product UX plans and walkthroughs, claim-based evidence, a concise PR contract including base-to-synthetic-merge cyclomatic-complexity review, ordered exact-head ReviewGPT gates, cross-owner composition mapping with stable composed/E2E proof, parent-owned accept/reject dispositions with specialist remediation continuation and a final-finding pause, focused local proof, broad PR CI, scoped commit and plan closure, and merge-readiness handoff. | Completion workflow | High | 2026-08-31 |
...
| `agent-docs/operations/pr-reviewgpt-loop.md` | Exact-head managed-browser completion workflow: one review-only preliminary pass and the eligible final gate can run concurrently; every invocation serializes frozen root ReviewGPT toolchain reconciliation under an OS lock before the exact pushed-head proof, and readiness paths are post-install assertions rather than cache authority; preliminary results use a parent-owned, evidence-backed accept/reject update and continue accepted remediation, while final `FINDINGS` pause all candidate mutation unless a narrow exception applies. Rejected findings need no model withdrawal, while accepted findings retain verification and rerun rules. The loop also owns exact-head packaging, immutable baselines, completion watchers, correction versus full snapshots, moving-base handling, anomaly retrospectives, and the seven-round hard cap. | PR ReviewGPT loops | Medium | 2026-08-30 |
