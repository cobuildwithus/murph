...
lifecycle owner to tear down a terminal shell when no near-term wake, child
work, or conversation warmth remains. The successful invocation result alone
owns the admission-time interaction generation; the notification carries only
completion identity. These boundaries are specified by
...
sole exception is already-provisioned local Temporal diagnostics: its credential
stays opaque, investigation begins read-only, and every mutation requires
explicit current-task authorization. The canonical trust boundary and Temporal
procedure are specified by `agent-docs/SECURITY.md` and
...
proof, unchanged GitHub main-push verification, one Vercel candidate per
`main` commit, and focused proof are specified by
`agent-docs/operations/verification-and-runtime.md` and
`agent-docs/references/testing-ci-map.md`.

Independent `vault-cli` and runner-entrypoint bundle budget ownership and
focused proof are specified by
`agent-docs/operations/verification-and-runtime.md` and
`agent-docs/references/testing-ci-map.md`.
...
The hosted group-join reply PostgreSQL proof owns synthetic local crypto
authority and private-field fixtures, so its documented full-file command does
not depend on hosted signing keys or a pre-seeded control-domain root. That
verification contract is specified by `agent-docs/references/testing-ci-map.md`.
...
parent's remaining completion checks without a user-resume pause. Product UX,
prompt, frontend, and coverage proof are parent-owned and do not require a
preliminary specialist audit or local subagent. Final `FINDINGS` retain the
turn-ending pause and block candidate mutation. These boundaries are specified by
`agent-docs/operations/completion-workflow.md` and
...
Participant names come only from the authenticated requester's shared Contacts
projection; otherwise the model sees bounded area-code/last-four or generic
email hints. The model clarifies naturally, then hands off only the exact listed
membership id; Web locks and revalidates the requester, membership generation,
expected runtime, and route. Provider titles
...
`agent-docs/product-specs/private-group-consultation.md`.

Required Temporal compatibility resolves private `main` with the scoped App
token before dispatch, accepts only the returned first-attempt run at that exact
SHA and fixed workflow identity, and re-reads private `main` before success so
...
preemption, abort/shutdown admission before every scope, its single forwarded deadline,
authoritative Web-response ownership, typed destination-failure isolation,
25-replacement destination pages, and source-version-marked retry progress
without a destination admission cap, is specified by
...
Hosted cold-start ownership starts the established-member direct ensure after
mailbox/access validation, waits only for actual request dispatch or its
one-second dispatch-only fallback, then sends the durable Temporal signal so
the direct foreground request can win startup admission. The separate
first-contact shell hint is serialized by the
...
proof preserves real parser order through both existing detail caps, and the
focused live journey owns progress-before-read and natural-copy evidence.
...
| `agent-docs/FRONTEND.md` | Frontend implementation guidance for `apps/web`, including design-system sources (`PRODUCT.md`, `DESIGN.md`), model-neutral implementation routing, consistent dashboard states, concise non-repeating copy, synthetic local data personas, live component/consent/section proof destinations, representation updates when needed, parent-owned rendered review, and completion-workflow routing. | Current frontend implementation guidance | Medium | 2026-08-31 |
...
| `agent-docs/product-specs/journal.md` | Private timeline derived from canonical health records, including quiet conversational capture, read-time grouping, web behavior, privacy, and ownership. | Journal product spec | High | 2026-08-24 |
| `agent-docs/product-specs/personal-patterns.md` | Deterministic private context-to-outcome findings, including Journal inputs, bounded comparisons, A-E evidence grades, proactive messages, weekly audit, and ownership. | Personal Patterns product spec | High | 2026-08-24 |
...
| `agent-docs/product-specs/ios-address-book-advisory-names.md` | Optional iOS Contacts projection with bounded one-to-four-label alternatives, member-scoped KMS MAC tokens, encrypted advisory labels, requester-scoped joined-group roster presentation, route-authorized iMessage/SMS roster lookup and sender attribution with one hosted-group access workflow, typed provider capability gaps, participant-access reconciliation, CAS deletion/retention, residual threat model, and rollout order. | Companion/group privacy contract | High | 2026-08-28 |
...
| `agent-docs/product-specs/group-challenge-data-diagnostics.md` | Truthful complete or partial group-challenge standings, evidence-ordered missing-data guidance with explicit first-projection `pending` versus completed-empty `missing` states, group-authorized fresh exact-scope reads, consent-fenced member-reported Manual corrections, source-tagged multi-source health shares without cross-source selection, rollback-readable legacy sleep compatibility, destination-paged projection delivery with source-version retry progress, and connection/source-epoch-coherent privacy-bounded `device-sync-status.v0` observations. | Group challenge diagnostics | High | 2026-08-26 |
...
| `agent-docs/product-specs/personal-group-awareness.md` | Personal Murph read access to hosted-group memberships, including independent opaque pagination for detailed memberships and disclosure grants, inventory-v2 roster compatibility, inventory-v3 exact-route action availability, live chat counts and safe requester-scoped participant labels, owner discoverability for organic routed groups without implicit vault sharing, ordinary-owner composition with existing membership-gated group actions, requested permissions, active self grants, and owner-authorized permission links. | Hosted group self-awareness | High | 2026-08-29 |
| `agent-docs/product-specs/private-group-consultation.md` | Implemented Assistant Ask request/reply primitive plus the separate target-authored private-to-group context handoff, with paged membership inventory, natural safe roster clarification, exact opaque membership-generation handoff, requester-only Contacts names and masked fallbacks, entry-local provider availability, host-owned consented participant attribution, provider-free cached-root admission transactions, first-write-wins stored-target and pinned mailbox replay, canonical durable follow-up context, delimiter-safe untrusted context, plain-text model output with runtime-owned delivery, deterministic causal unavailable handling, bounded foreground-causal draining, bounded opaque failure correlation, and dispatch-first direct latency hints. | Hosted group consultation | High | 2026-09-04 |
| `agent-docs/product-specs/consented-group-disclosure.md` | Exact-grant group-to-member Assistant Ask plus exact-message current-sender requests, with independently paged active grant projections, no lifetime permission or grant admission ceiling, independent valid sources in one accepted turn, model-inferred natural-language audience and clarification, Web-owned exact sender/source/route admission, same-channel private-route proof, group notice before a personal read, fail-closed unmarked group rollout skew, one outgoing allow/deny review, mutually exclusive private/group terminal retry, non-disclosing fallback, replay/concurrency ownership, and bounded private/persisted legacy drain. | Hosted group disclosure | High | 2026-08-26 |
...
| `agent-docs/references/testing-ci-map.md` | Verification map for packages, apps, smoke flows, exact-head PR CI, protected-main runtime proof, canonical executors, and current coverage owners, including independent vault CLI and runner-entrypoint total-output comparison, reviewed bundled-Incur payload and scanner proof, installed scoped-committer staging, parent-owned review proof, staggered six-hour native iOS/Android latest-outcome checkpoints, exact-current-main manual recovery, committed immutable source policy, release-note-only production-alias lag, exact production/current-main canary binding, scoped iOS credential projection from the existing protected environment, Temporal private-`main` pre-resolution and post-proof movement closure, exact returned private dispatch/run identity, exact-public-main Vercel promotion admission against current private `main` and live readers, complete reconciliation fixture frontiers and progress triplets, Android uncertain-dispatch fencing, and canary-only public orchestration. | Testing and CI truth | High | 2026-09-02 |
...
| `agent-docs/operations/agent-workflow-routing.md` | Workflow router and compact agent work contract for task classes, proportional Product UX planning and walkthrough routing, action authority, evidence/tool use, focused local PR proof with exact-head CI ownership, parent-owned proof, final-finding pauses, review gates, commit paths, worktree isolation, and safe retirement. | Agent workflow routing | High | 2026-08-31 |
| `agent-docs/operations/product-ux.md` | Single owner for proportional Product UX planning, post-code walkthroughs, and parent review across affected people, minimum current requirements, current writer/consumer gates, complete journeys, risk-based evidence, and finding dispositions. | Product UX workflow | High | 2026-08-31 |
| `agent-docs/operations/native-android-hosted-e2e.md` | Native Android six-hour production-canary ownership, latest-completed-outcome checkpoint admission, exact-current-main manual recovery aligned with the Security and Reliability owners, committed immutable source policy, exact deployed Web/Android binding, exact production/current-main equality, refreshable repository-scoped GitHub App credentials, receipted and unreceipted dispatch fencing, privacy boundary, and source rotation. | Native Android verification operations | High | 2026-09-01 |
| `agent-docs/operations/verification-and-runtime.md` | Verification ownership by delivery path, including focused local proof, immutable exact-base/exact-synthetic-merge docs-drift CI comparisons, deliberate near-merge Ready admission, broad exact-head PR CI, protected-main live Junction proof, fail-closed tag-release matrices, staggered six-hour native iOS and Android production canaries with latest-completed-outcome checkpoint admission, exact-current-main manual recovery, committed source identity, one managed Vercel candidate per `main` commit, scoped iOS credential projection from the existing protected environment, explicit-rerun recovery, parent-owned claim-based rendered evidence, canonical command and executor semantics, patched-Incur generation idempotence and exact runner patch staging, runtime proof boundaries, and package/app testing surfaces. | Verification policy | High | 2026-09-01 |
...
| `agent-docs/operations/typescript-verification-performance.md` | TypeScript 7 worker budgets, optional local canonical verification with explicit Crabbox static-SSH offload and an exclusive heavyweight lane, focused PR proof, direct-default acceptance, scoped diff and repo-tools caps, incremental CI state, editor/watch guidance, and benchmark method. | Verification performance policy | Medium | 2026-07-29 |
| `agent-docs/operations/completion-workflow.md` | Outcome-based completion with proportional Product UX plans and walkthroughs, parent-owned prompt/frontend/coverage evidence, privacy-gated GitHub CLI media attachments, a concise PR contract including exact synthetic-merge-first-parent cyclomatic-complexity review and automated-producer evidence, one proportional exact-head ReviewGPT gate, cross-owner composition mapping with stable composed/E2E proof, parent-owned accept/reject dispositions with a final-finding pause, focused local proof, broad PR CI, exact-file intermediate commits, directory-resolving final plan closure, and merge-readiness handoff. | Completion workflow | High | 2026-09-02 |
...
| `agent-docs/operations/pr-reviewgpt-loop.md` | Exact-head managed-browser workflow for the eligible final cross-cutting gate. Normal reviews are package-guarded onto regular Chat and fail closed on ChatGPT Work. Every invocation serializes frozen root ReviewGPT toolchain reconciliation under an OS lock before exact pushed-head proof, and readiness paths are post-install assertions rather than cache authority. Final `FINDINGS` pause all candidate mutation unless a narrow exception applies; rejected findings need no model withdrawal, while accepted findings retain verification and rerun rules. The loop also owns exact-head packaging, immutable baselines, completion watchers, correction versus full snapshots, moving-base handling, anomaly retrospectives, and the seven-round hard cap. | Final PR ReviewGPT loop | Medium | 2026-09-02 |
