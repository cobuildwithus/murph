...
| `agent-docs/operations/prod-watch.md` | Local five-minute monitor-only production-watch architecture with bounded PostgreSQL aggregates, status-only Vercel/Stripe availability probes whose output is ignored, advisory non-scorable shell-disabled Cloudflare Codex collection, deterministic incident state, opt-in live database proof, transactional launchd replacement, and an owner-only exact-head runtime whose absolute-Git launch gate ignores ambient configuration and excludes diagnosis, remediation, ReviewGPT, GitHub, merge, deploy, and provider/production mutation paths. | Production monitoring workflow | High | 2026-08-15 |
