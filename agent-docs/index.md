...
| `agent-docs/operations/product-ux.md` | Proportional Product UX planning and post-code walkthroughs across affected people, minimum useful value, calibration examples, ordinary entry-to-outcome proof paths, weak states, and risk-based evidence. | Product UX workflow | High | 2026-08-15 |
| `agent-docs/operations/verification-and-runtime.md` | Verification ownership by delivery path, including focused local proof, broad exact-head CI for PRs, risk-based design-catalog evidence, canonical command and executor semantics, runtime proof boundaries, and package/app testing surfaces. | Verification policy | High | 2026-08-15 |
...
| `agent-docs/operations/completion-workflow.md` | Outcome-based completion with proportional Product UX plans and walkthroughs, risk-based rendered evidence, exact-head specialist review, focused local proof, broad PR CI, scoped commit and plan closure, and merge-readiness handoff. | Completion workflow | High | 2026-08-15 |
...
| `agent-docs/operations/pr-reviewgpt-loop.md` | Exact-head managed-browser completion workflow: one preliminary ReviewGPT pass combines product-experience/prompt/frontend/coverage lenses with risk-based rendered evidence and may return a bounded coverage patch; frontend-only work skips the separate final cross-cutting gate unless another risk trigger applies; eligible final round 1 may start in parallel on the same head with an independent immutable baseline; behavior-preserving merge-boundary updates have a one-update budget, required-check-only waiting, and an explicit moving-base-race stop; one packager-owned head-bound decision makes later rounds re-send a full ZIP for sensitive, undeclared, or large PRs and otherwise send a same-thread correction delta only for explicitly routine small PRs; an explicitly requested new full-audit conversation requires the prior-finding ledger; the loop has a seven-round hard cap before explicit continuation is required. | PR ReviewGPT loops | Medium | 2026-08-15 |
...
| `agent-docs/prompts/` | Reusable completion-review lenses, including Product UX persona walkthrough, prompt, frontend, coverage, and fallback deep-review guidance. | Workflow prompt library | Low | 2026-08-15 |
