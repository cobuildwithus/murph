...
| `agent-docs/operations/pr-reviewgpt-loop.md` | Exact-head managed-browser completion workflow: one preliminary ReviewGPT pass combines Product UX, prompt, frontend, and coverage lenses with claim-based evidence and may return a bounded coverage patch; frontend-only work skips the separate final cross-cutting gate unless another risk trigger applies; eligible final round 1 may start in parallel on the same head with an independent immutable 40-character-SHA baseline after the shared PR-body preflight; behavior-preserving merge-boundary updates have a one-update budget, required-check-only waiting, and an explicit moving-base-race stop; one packager-owned head-bound decision makes later rounds re-send a full ZIP for sensitive, undeclared, or large PRs and otherwise send a same-thread correction delta only for explicitly routine small PRs; an explicitly requested new full-audit conversation requires the prior-finding ledger; the loop has a seven-round hard cap before explicit continuation is required. | PR ReviewGPT loops | Medium | 2026-08-19 |
