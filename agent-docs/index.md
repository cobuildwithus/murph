...
| `agent-docs/operations/completion-workflow.md` | Outcome-based completion with proportional Product UX plans and walkthroughs, claim-based evidence, a concise PR contract, ordered exact-head ReviewGPT gates, cross-owner composition mapping with stable composed/E2E proof, a parent-owned accept/reject disposition boundary before remediation with a narrow behavior-preserving Complexity Collapse continuation, focused local proof, broad PR CI, scoped commit and plan closure, and merge-readiness handoff. | Completion workflow | High | 2026-08-24 |
