...
| `agent-docs/operations/prod-watch.md` | Local five-minute monitor-only production-watch architecture, canonical hosted database aggregation, adaptive bounded Vercel detail and request-sample collection with immediate raw-message minimization, deterministic Stripe aggregates, advisory non-scorable shell-disabled Cloudflare Codex collection, one source-observation owner for recurrence and cumulative-gap baselines, exact-dimension incident signals and drill-down, deterministic incident-source terminal authority, opt-in live database proof, and an owner-only exact-head-pinned direct-Node macOS scheduler that excludes automatic diagnosis, remediation, ReviewGPT, GitHub, merge, deploy, and provider/production mutation paths. | Production monitoring workflow | High | 2026-08-14 |
