...
zero-marker recovery, terminal setup-slot release, invocation-scoped credential projection, exact token-return
...
| `docs/device-sync-hosted-control-plane.md` | Current hosted control-plane direction for device sync, including companion account admission with the canonical capacity-gated signup welcome, no-line activation, Web-only welcome email, exact pending-activation runtime-wake recovery, encrypted dirty-payload retention, generic terminal acknowledgement, and the overnight-PRV lane's bounded replay receipts plus canonical-import acknowledgement. | Device-sync architecture direction | Medium | 2026-08-13 |
