...
| `agent-docs/research/2026-08-23-vault-cli-error-recovery-audit.md` | Point-in-time exhaustive Vault CLI audit of generic, lossy, untyped, misleading-success, and privacy-unsafe error paths, with prioritized model-recovery work packages. | CLI error-recovery audit | Medium | 2026-08-23 |
...
| `agent-docs/operations/completion-workflow.md` | Outcome-based completion with proportional Product UX plans and walkthroughs, claim-based evidence, a concise PR contract, ordered exact-head ReviewGPT gates, a parent-owned accept/reject disposition boundary before remediation with a narrow behavior-preserving Complexity Collapse continuation, focused local proof, broad PR CI, scoped commit and plan closure, and merge-readiness handoff. | Completion workflow | High | 2026-08-23 |
...
| `agent-docs/operations/pr-reviewgpt-loop.md` | Exact-head managed-browser completion workflow: one preliminary pass and the eligible final gate can run concurrently; substantive results use a parent-owned, evidence-backed accept/reject disposition boundary before mutation, except a sole accepted behavior-preserving Complexity Collapse may continue without a turn-ending pause or separate user permission; rejected findings need no model withdrawal, while accepted findings retain verification and rerun rules. The loop also owns exact-head packaging, immutable baselines, bounded coverage patches, completion watchers, correction versus full snapshots, moving-base handling, anomaly retrospectives, and the seven-round hard cap. | PR ReviewGPT loops | Medium | 2026-08-23 |
