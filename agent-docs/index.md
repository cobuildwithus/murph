...
Member-owned provider setup keeps lifecycle, handoff, mailbox continuation,
browser provisioning cleanup, and terminal run release in the existing setup
and computer owners. The model uses ordinary browsing to complete and submit
application metadata. Finite provider registration owns only the credentials
URL and ID/secret/reveal selectors; deterministic capture seals and scrubs
credential values without exposing them, and deletion is gated by an exact
registered on-page versus sealed client-ID comparison. This contract is jointly
...
in-flight browser-create and accepted-mailbox-owned no-handle timeout recovery,
status-owned consent-withdrawal cleanup that preserves ambiguous submit and
cancellation fences,
terminal setup-slot release,
...
`agent-docs/RELIABILITY.md`, with the real-PostgreSQL ordering proof indexed in
`agent-docs/references/testing-ci-map.md`.
...
| `agent-docs/PRODUCT_SENSE.md` | Current product posture for a broad personal health assistant, including immediate value, compounding context, consequential discovery, proactive support, composable primitives, first-visit personalization, conversation-first control, model-driven provider-neutral member-owned setup, channel-native Telegram and iMessage presentation with bounded model-authored Telegram rich content and exclusive semantic-card ownership, uncertainty-aware profile/contact speaker labels, public product-link sharing, pre-save capacity disclosure, and product guardrails. | Current product behavior | High | 2026-08-13 |
| `agent-docs/PRODUCT_CONSTITUTION.md` | Internal product constitution and tradeoff rules, including selective proactivity, progressively deeper member-controlled context, and model-driven provider-neutral integration work. | Product principles | High | 2026-08-13 |
...
| `agent-docs/compliance/2026-07-23-connected-source-launch-gate.md` | Connected-source permission assumption, launch status, and ongoing provider controls, including model-driven member-owned provider setup, blind deterministic capture and exact client-ID deletion, terminal setup-slot release, and the pre-suspension external cleanup gate. | Connected-source release gate | High | 2026-08-13 |
...
| `agent-docs/operations/agent-workflow-routing.md` | Workflow router and compact agent work contract for task classes, action authority, evidence/tool use, progress updates, plans, required edit-task Frog inspection and qualifying friction logging with tracked plan-file edits included and same-task commit ownership, focused local PR proof with exact-head CI ownership, state-transition motion-proof PR evidence, current-base merge-tree readiness with one base update maximum and a terminal moving-base-race stop, frontend-only final-gate exemption, behavior-preserving base-conflict rerun exemption, parallel same-head specialist/final ReviewGPT routing, one acceptance run plus a bounded post-acceptance rebase for direct-default pushes, commit paths, clean-status-proven worktree isolation, serialized hook installation, and safe retirement. | Agent workflow routing | High | 2026-08-15 |
| `agent-docs/operations/verification-and-runtime.md` | Verification ownership by delivery path: focused local proof plus broad exact-head CI for PRs, required-check-only merge waiting, current-base merge-tree readiness with one base update maximum and a terminal moving-base-race stop, evidence-driven local CI diagnosis, one acceptance run plus a bounded unchanged-patch post-acceptance rebase for direct shared-default pushes, canonical command and executor semantics, centralized CLI runtime preparation for reverse-dependent diff fanout, protected-main live Junction WHOOP canary proof with independently owned Codex pins and a credential-free stable-Chrome preflight visible to review, runtime proof boundaries, private Temporal worker/deploy/rollback ownership, required live proof for user-facing Telegram presentation changes when an ignored local preview target is configured, required repo-local Playwright fallback for unattached browser proof, and package/app testing surfaces including the internal product-feedback digest. | Verification policy | High | 2026-08-15 |
