...
| `agent-docs/product-specs/physical-notes.md` | Expressive GPT Image artwork mailed through one Lob effect, with end-to-end HTTP-ambiguity preservation, bounded same-key send replay, accepted-message-only standalone provider reconciliation that cannot send or recall mail, row-scoped recovery, atomic blocker settlement, member-wide unresolved-effect guards, member/group complimentary claims, and ordinary Murph usage. | Hosted physical-note product spec | High | 2026-08-20 |
