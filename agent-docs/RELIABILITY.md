...
- Exact private support escalation remains the bounded in-turn exception to ordinary post-reply feedback persistence. Under the same member-scoped advisory lock, Web writes the fixed member marker plus the deterministic anonymous canonical product-area/problem issue, reads and parses both rows, and ranks the member marker for the three-per-UTC-day alert cap. An eligible provider attempt formats readable labels from the first parsed stored issue rather than callback memory and uses the feedback-derived Resend idempotency key, so replay has one stable body even when a later callback selects different codes; missing, member-linked, free-form, legacy, or malformed stored detail fails before email. Later records remain durable without another alert; missing email configuration or provider failure remains visible to the current turn without adding a retry queue or second delivery owner.
