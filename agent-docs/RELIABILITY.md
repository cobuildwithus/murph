...
- The personal maintenance seed runs nightly and completes memory consolidation before reminder work. It skips connected reads when no eligible automation exists. Reminder-maintenance failures leave completed memory writes intact; failed, partial, stale, or ambiguous provider reads leave the prior unexpired suffix unchanged. Instruction replacement is idempotent and compare-and-swap fenced, so an unchanged snapshot does not advance the automation timestamp and a concurrent user edit wins instead of being overwritten.
