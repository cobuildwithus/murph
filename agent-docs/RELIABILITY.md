...
- Cross-session auto-reply route state removes receipt-inventory reads from foreground unanchored selection. The existing runtime-residue owner performs one idempotent legacy import from its already-trusted outbox and receipt inventories, leaves migration incomplete when either inventory is untrusted or has concurrent non-abandoned running consumers for one route, writes route records monotonically, and writes `auto-reply/route-state-migration.json` last. A foreground read with no valid marker fails optional unanchored context closed without scanning history. After migration, it touches one exact route file and at most the one exact pending receipt. Unanchored provider egress requires the marker plus a running consuming receipt; an exact provider-message anchor remains authoritative before migration and persists its bounded pending claim before live steering, so a crash cannot let migration publish without that witness. Any claim failure prevents provider start and releases an earlier provider-input reservation. A same-turn claim upgrade retains one bounded, receipt-proven prior order so a completed turn can settle the earlier accepted context when a newer steer was abandoned. Runtime residue reconciles pending claims before ordinary receipt retention, advances a completed or deferred claim only through matching context-intent evidence, clears terminal claims that lack any such proof, protects unresolved witnesses, deletes claim-free route files after their last outbox authority disappears, and may prune terminal legacy metadata after compaction. Failed or blocked turns never settle either the current or prior order. At the established old-running cutoff, trusted residue inventories may classify a turn abandoned only when no active outbox authority, pending receipt input, or pending accepted-journal input remains; maintenance then clears its route claim without settling it and prunes the receipt and journal under the same runtime lock, restoring retry eligibility. The bounded receipt retention is the rollback horizon for legacy metadata retained by route-capable turns; once a migration marker or route claim is written, route-capable code is the workspace rollback floor. Deploy by draining older assistant writers before enabling the new writer on a workspace, then keep the marker and route subtree in every hosted snapshot and restore.
