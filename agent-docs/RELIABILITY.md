...
- Managed automation ownership is route-authority-based: member seeds run only on direct/member routes and the group room-model seed runs only on authenticated non-direct Linq/iMessage or Telegram routes. Group email is excluded because its sender is spoofable. Existing managed records bound to the wrong owner are archived in place; caller-supplied unscoped seeds retain their prior route behavior. Only the immutable built-in personal-memory and group-room-model automation ids select silent maintenance policy, so an editable tag, slug, title, or instruction cannot acquire no-delivery or replay-barrier behavior. Both policies reuse the existing isolated exact-skip turn, foreground preemption, and provider-admission replay barrier. The group policy scans at most 192 recent sessions, selects at most 24 route-eligible sessions after filtering, and uses their seven-day overlapping admitted-group transcripts with one full-rewrite knowledge page instead of a cursor or work queue; a missed or preempted run is recovered by overlap, while a run admitted to the provider is consumed because its write may already have committed. The canonical knowledge boundary rejects a `group-room-model` body over 8 KiB of UTF-8 before replacing the prior page, so scheduled and explicit rewrites share one atomic size invariant. Malformed, unreadable, or wrong-type fixed-slug state is a conflict rather than an empty page and cannot be replaced by either path.
