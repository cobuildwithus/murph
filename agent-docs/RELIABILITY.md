...
- Daily nutrition response cards remain one outbox-owned immutable effect. The runtime retains the V1 parser and text rendering for existing outbox and checkpoint state, while V2 adds canonical fiber plus nullable goal snapshots. Ordinary private-direct interactive turns, exact private-direct scheduled turns, and the managed meal closeout use the same attachment tool. Scheduled use requires saved instructions that explicitly request a card; occurrence authority alone is not intent. Because a card replaces the whole final response, it is eligible only when the card alone completely satisfies the current request. New accepted input in the same live turn invalidates an earlier card-only decision, and attachment is rejected after the delivery context advances. Every card copies each total from the immediately preceding single-date canonical meal-totals read. Before deriving, saving, or surfacing numeric targets, before activating a paused proposal, and before attaching a card, the runtime prompt applies the dedicated safety gate even when a complete active bundle exists and during a scheduled closeout. The context snapshot is navigation only: the gate reads the complete canonical Identity, Preferences, Instructions, and Context memory document, while active conditions and active regimens require independent 200-record list reads plus detail reads for every returned record. The same gate requires the bounded lossless canonical measurement read before proposal work and again before activation. A failed or unreadable memory read, or saturated, failed, or unreadable clinical or measurement discovery, suppresses numeric setup, Goal mutation or activation, and cards; an existing paused proposal remains unchanged. A clearly current saved under-18 age or number-sensitive preference also suppresses numeric output, while missing or ambiguous age alone does not universally block it. Prior acceptance is not current safety proof, and scheduled authority does not permit new screening questions or target setup. The managed daily-nutrition path first proves target authority with a 200-record active-Goal list and detail reads for every returned record with targets; saturation fails closed, and title, context visibility, or the default list prefix never selects the authority set. It then requires one complete five-metric active canonical goal bundle before attachment. After an explicit interactive card or target-setting request, absent goals cause it to write or update one paused canonical proposal and send only the ordinary-text values and reasoning. Any write that adds or changes a derived managed target atomically supplies the complete intended array with paused status, even when that Goal was active, so revised values cannot appear before renewed acceptance; overlap removal alone may retain active status when the explicit owner supplies the metric. If an explicit card request caused the proposal, the member's next unambiguous acceptance may reapply the complete current-context safety gate and, only after it passes, activate and read back that record, re-read same-date totals, and attach the first goal-aware card in the same response. Target-setting-only requests, corrections, declines, ambiguous replies, and compound requests remain text-only. A scheduled meal closeout may use an already accepted complete bundle, but it never asks for target inputs or creates, changes, or explains a proposal; without the bundle it retains the ordinary closeout and no card. Existing explicit targets win metric by metric. Saturated or conflicting reads, scalar-incompatible ranges, unsafe numeric contexts, and missing responsible calorie inputs suppress the card instead of producing a goal-less attachment. Missing or partial totals can carry only an `unavailable` status, and an assessed status must not point opposite the frozen total and target. The semantic target status is frozen presentation context for the one message, not durable goal progress and not a threshold recomputed by iOS. The durable V2 schema remains nullable for legacy replay and rendering compatibility, while the model-facing authoring boundary rejects nutrition V1 and requires all five V2 goal snapshots. Deterministic V2 semantic text includes every metric's exact target and frozen status, including an explicit unavailable target/status where applicable, so Telegram, email, and Linq's ordinary-text fallback preserve the same five-goal meaning; V1 text is unchanged. Linq explicitly requests the shipping Messages extension's interactive balloon and retains the provider static layout for recipients without it. The layout contains the date, meal count, available totals, and an explicit partial marker when supporting meal counts are incomplete; for V2, the first non-null goal in canonical calories, protein, carbohydrates, fat, and fiber order contributes its exact target and frozen status. The HTTPS message URL carries the complete immutable V1 or V2 presentation snapshot in the existing bounded Base64URL fragment family. Encoding is not encryption, and the fragment contains no member identity, canonical record reference, credential, or other authority. The value-free fallback names the truthful text-recovery action without triggering Apple data detectors. Interactive rollout requires production-device presentation proof because provider acceptance and delivery receipts do not prove that the extension drew a visible balloon. After the first V2 card-bearing state or effect, the V2-capable Worker and runner are the rollback floor.
