...
- Automatic Google Health Fitbit cutover runs only after the hosted pass has
  ...
  Junction webhook admission derives one source authority from the complete
  data-bearing payload for both summary and timeseries events. Direct records,
  nested records, and grouped records share one hierarchy: the deepest inline
  data source wins, while each containing record, group key, and finally the
  webhook envelope are fallbacks only when their child data has no source.
  Mixed sources at the same authoritative layer and source-unknown data events
  during a non-terminal Fitbit migration are retryable before dirty or payload
  acceptance. A retained terminal Fitbit row does not extend that
  migration-only rejection; redelivery then follows ordinary admission.
  ...
  presentation marker; provider or transport failure advances the existing
  local reconcile wake by 30 seconds. The flow must not create a migration
  queue or depend on a browser poll remaining mounted.
