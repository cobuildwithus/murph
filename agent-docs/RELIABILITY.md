...
- Linq speaker-label resolution is prompt-time, presentation-only work owned by the assistant-runtime reader. Its operation-local memo retains successful labels, valid unnamed results, and fail-soft misses through later live admissions, so an already-seen handle never recontacts Web during the compound operation. One module-private 2,048-entry insertion-ordered `Map` opportunistically reuses validated positives for a fixed hour and valid successful omissions for five minutes across ordinary turns in the same resident process. Keys include the callback-bound runtime member, exact accepted-input route conversation key, channel, and normalized handle; hits do not slide expiry or eviction order. New or expired handles still form one bounded Web batch. Failures remain operation-local and are never process-cached, so cold starts, deploys, multiple processes, timeout, late completion, parser skew, authorization loss, suspension, consent loss, ambiguity, KMS/storage failure, or any other optional source failure falls back unnamed without blocking, retrying, or acknowledging accepted conversation work. The cache has no timer, single-flight, mutation invalidation, persistence, or distributed coordination, and the existing `read_participant_display_names` one-second soft deadline remains unchanged.
