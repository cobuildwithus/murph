...
- Automatic meal-photo uploads are replay-safe only through the capture id derived by the enrolled installation. Each staging attempt must own a distinct object. Under the per-capture mailbox lock, the first accepted item chooses the canonical object for exact duplicates; later attempts delete only their own losing object. Failed or ambiguous appends must reconcile the mailbox claim before cleanup so they never delete an accepted object's bytes. Web must reject conflicting reuse, re-signal exact mailbox duplicates, require an established private home route before staging, lock the hosted member and active sponsorship source rows before rechecking final upload authority and the continued presence of a private route, and acknowledge an upload only after private object staging and canonical mailbox append both succeed. The mailbox wake never copies recipient state. Runtime import must check the canonical external reference before writing, verify staged length and SHA-256 before import, and reconcile the stable automatic-meal closeout on both new-import and existing-meal replay paths using the current Web-owned direct route and hosted route-validation profile. A missing closeout record or current direct route keeps the mailbox item retryable without rereading or duplicating the canonical meal. Staging deletes only through a post-checkpoint effect; cleanup derives the user-namespaced object path without requiring encryption-context rediscovery. After failed cleanup, the R2 lifecycle rule makes staging eligible for asynchronous deletion at 31 days, one day beyond mailbox recovery retention, rather than guaranteeing deletion at that exact age. The scheduled closeout uses one `dailyLocal` occurrence and a bounded lookback so after-9pm/delayed captures are eventually processed. Its saved route is only a scheduler locator: execution resolves current Web member routing before provider work and subsequent effect boundaries, while closeout-only direct Telegram delivery rechecks at provider entry so a stale outbox target makes no provider call. That obsolete intent becomes terminal without becoming a replyable failure input; the existing pending closeout occurrence and ordinary cron backoff policy remain the sole recovery owner. When terminal outbox evidence remains attached after reconciliation fails, cron status derives a stable first-backoff repair deadline from that existing owner pair, publishes it on later passes while the evidence remains retained, and counts it due at the deadline until the ordinary cron repair clears the pending intent. The closeout-only post-checkpoint reread arms the first projected wake; a bounded one-pass safety candidate covers an unavailable read or aggregate status that does not yet expose the exact repair deadline. The next pass creates one fresh intent against the current route. Unrelated automations retain their existing delivery and terminal-failure-note policy. Canonical photo removal is idempotent and receipt-guarded in one write batch: the latest meal revision drops the attachment while the raw image and manifest become matching tombstone proof. A missing control client, private route, staged object, write fence, mailbox append, runtime read, automation record, or photo-retention precondition is a visible retryable/fail-closed outcome rather than false success.
