...
- Automatic meal-photo uploads are replay-safe only through the capture id derived by the enrolled installation. Web must reject conflicting reuse, re-signal exact mailbox duplicates, and acknowledge an upload only after private object staging and canonical mailbox append both succeed. Runtime import must check the canonical external reference before writing, verify staged length and SHA-256 before import, and delete staging only through a post-checkpoint effect; after failed cleanup, the R2 lifecycle rule makes staging eligible for asynchronous deletion at 24 hours rather than guaranteeing deletion at that exact age. A missing control client, staged object, write fence, mailbox append, or runtime read is a visible retryable failure rather than a successful setup/upload.
...
- Call Circle setup and confirmation delivery uses the existing mailbox row as its cancellation/claim owner. Cancellation relabels only unconsumed current asks as superseded, and the signed delivery-time claim locks the member, revalidates current access/enrollment plus pending-match authority, then atomically stamps `consumedAt` immediately before provider entry. A requested-and-consumed retry remains idempotently eligible only while that authority is still current; cancellation-first or stale-authority attempts fail closed. Terminal cancellation, expiry, handoff, and outcome notices are intentionally outside this revocable set.
