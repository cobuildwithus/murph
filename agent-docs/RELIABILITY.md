...
- Linq instant start uses the existing planner twice around the existing no-card Pulse-trial owner. The first transaction may create the canonical member, verified inbound phone identity, pending same-line route, and invite, but it neither counts the inbound nor appends the conversation. The invite records the persisted model-source admission event as the bounded continuation proof. Stripe customer/subscription provisioning, the billing write, and activation then share the existing member lock; their Stripe calls use the existing five-second, no-network-retry authority budget. An overlapping same-chat, same-participant, same-line inbound may reuse that exact pending proof, wait on the same owner, and then take the ordinary active-member path instead of emitting a signup link. A second ordinary planner pass observes active access, promotes the same route, counts each inbound once, and appends each message once. Signup dispatch rechecks that no pending instant start or active access superseded it. Only a genuinely new billing identity can enter this path; an existing Stripe customer falls back before subscription creation so a saved card cannot silently auto-convert. Any classifier, configuration, route, definitive Stripe, or activation failure falls back to the existing signup-link path, while member-lock contention remains retryable, without creating a second entitlement, queue, or runtime.
