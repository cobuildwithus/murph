...
- Tool-enabled assistant provider turns should disable automatic model retries and same-turn provider-route failover once local side-effecting tools are in play, so bounded assistant/vault operations are never replayed implicitly by transport-layer retry or cross-route recovery. Bound tool execution failures should be returned to the model as structured tool results so the model can recover inside the same turn instead of aborting the provider turn.
