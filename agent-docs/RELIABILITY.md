...
- Exact private support escalation remains the bounded in-turn exception to ordinary post-reply feedback persistence. Under the same member-scoped advisory lock, Web writes the fixed member marker plus deterministic anonymous issue detail, reads and validates both rows, and ranks the member marker for the three-per-UTC-day alert cap. An eligible provider attempt formats from the first stored detail rather than callback memory and uses the feedback-derived Resend idempotency key, so replay has one stable body even when the model rewords the same accepted-input callback; a missing, member-linked, or malformed stored detail fails before email. Later records remain durable without another alert; missing email configuration or provider failure remains visible to the current turn without adding a retry queue or second delivery owner. Because consent is runner-bundle behavior and detail disclosure is Web behavior, release Cloudflare/runner first with immediate rollout and exact fingerprint convergence, then Web; roll back Web first. The existing stale-runner admission fence and managed-container smoke own convergence instead of a new consent-version or retry owner.
