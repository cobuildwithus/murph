...
- Transcript rollout is two-phase because ordinary snapshot cleanup deletes settled accepted-turn journals before content retention runs. Phase one deploys the stamping-capable runner with immediate rollout, proves fleet convergence, and leaves legacy unstamped transcript entries plus existing snapshot wake projections untouched. Record that convergence instant. Phase two may begin only after 14 complete days: a separate migration re-arms persisted snapshots, and the runtime may then retire every remaining unstamped user entry without reconstructing receipt state. Do not collapse the interval, infer a receipt from projection time, or add a second receipt index.
