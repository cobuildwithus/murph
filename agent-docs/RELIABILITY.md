...
- Exercise routine cards remain that same one outbox-owned immutable effect. A dedicated tool keeps each model-facing schema below the Codex compaction limit, while both tools produce the same response-card effect. Routine timing fails validation unless the total equals every exercise estimate plus transition time. Images must be bounded HTTPS catalog results with exact alt text and provenance; routine cards never combine with response media. Telegram sends one `sendRichMessage` request with a table, collapsed instructions, and an optional slideshow. Hosted rich sends enter through the same callback-owned authority, secret, fetch, and liveness checks as hosted text sends. It sends deterministic text only after a non-retryable 4xx proves the rich request was rejected before acceptance. That fallback gets one provider attempt and does not start a second retry owner. A timeout, transport failure, 408, 409, 425, 429, or 5xx never triggers text fallback, because a second provider request could duplicate an accepted message. Existing outbox replay and terminal evidence remain the only retry and completion owners. During mixed-version rollout, an older Worker rejects `sendRichMessage` before provider entry, so the new runner can use that one text fallback; an older runner keeps its existing text-only behavior. Linq does not attempt a native Messages-extension routine card and uses deterministic text instead.
