...
- Assistant product-feedback capture accepts at most one in-memory candidate during a successful provider turn. The assistant execution context can only hand that candidate to its hosted invocation synchronously; the existing web-control write remains post-checkpoint and starts after a current-turn member-channel send succeeds or after an exact scheduled occurrence commits an intentional no-reply result. Queue-only scheduled replies stage their candidate until delivery succeeds, while provider or terminal delivery failure discards it. Feedback never counts as a provider side effect for transport retry safety, and persistence remains best-effort with a two-second maximum deadline, no retry queue, and no user-visible delivery state. The authority-derived idempotency key remains the ambiguity fence when a timed-out write may already have reached Web.
...
with retry identity scoped by occurrence, operation, and requested fields; field
projection additionally compares the trusted occurrence time so a delayed callback
cannot overwrite newer human or scheduled state. Retention keeps only the retired
structural mailbox rows still referenced by current per-field causal watermarks, so
that ordering evidence survives without preserving payload content or adding a state
owner; superseded rows return to ordinary pruning. Scheduled Clinical Records calls
return a stable authenticated launcher and create no intent; the ordinary 15-minute,
single-use claim begins only when the member opens that launcher. Scheduled feedback
is flushed only after a committed no-reply result or confirmed delivery.
