...
- Assistant product-feedback capture remains a best-effort foreground tool call: its existing web-control write has a two-second maximum deadline, returns failure to the model without blocking the ordinary fallback reply beyond that bound, and adds no retry queue or user-visible delivery state. The accepted-input-derived idempotency key remains the ambiguity fence when a timed-out write may already have reached Web.
