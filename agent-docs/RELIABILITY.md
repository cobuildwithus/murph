...
- Junction exact-workout webhook work captures that same source lifecycle epoch
  ...
  pre-fetch connected providers and selects one exact remote id. An explicit
  stream connection reference selects that provider id directly and must agree
  with webhook attribution. Without an exact reference, stream
  source/device/app attribution must resolve uniquely; only an identity-less
  stream may fall back to the sole same-slug connection. That exact id must
  remain connected in the post-fetch listing, so another connection with the
  same slug cannot substitute for it. Missing, conflicting, or ambiguous proof,
  an old epoch, a stale or replaced provider instance, or a reconnect racing the
  fetch exits without import.
...
- Junction exact-linked `workout_duration` history treats the bounded sanitized
  row ids as the selected set before value normalization. The combined duration
  and companion-session chunk imports only when every selected id normalizes
  and matches; malformed or partially matched chunks use the existing bounded
  retry ladder and terminally close without publishing coverage.
