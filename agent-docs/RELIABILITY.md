...
- A usage-credit purchase freezes one reconstructible Checkout request before
  Stripe I/O and moves through one durable create claim. Every create retry uses
  the same purchase-derived Stripe idempotency key; an ambiguous response must
  not reopen the claim, mint a replacement purchase, or create a second payable
  Session. The member may begin another purchase only after the existing one is
  terminal.
...
- Matching usage-credit refund or dispute events must never fall through to the
  ...
  must append replay-safe, capped reversals for succeeded refunds and withdrawn
  disputes, and a restoration when dispute funds are reinstated. A failure
  keeps the Stripe receipt retryable; it does not silently complete the event.
