...
- Personal memory consolidation and reminder-availability refresh are separate nightly one-shot seeds, so reminder/provider failure cannot roll back, contaminate, or delay completed memory work. The later reminder seed skips connected reads when no eligible automation exists and exposes one composite refresh per eligible lookup. Host code derives the current seven-day request, rejects incomplete or unsupported provider results, normalizes timestamps, and applies the suffix only when the automation version and exact calendar account binding observed before the read still match. A clean empty read removes the prior suffix; a failed, partial, disconnected, or concurrent refresh leaves canonical instructions unchanged. Replacement is idempotent, and scheduled delivery uses evidence only while the current policy/source/account binding remains exact and the snapshot is canonical, unexpired, and covers an occurrence scheduled within 24 hours of generation. A successful snapshot is a short derived-data lease: disconnect or provider revocation stops future refreshes but does not synchronously cancel it. Policy removal or account replacement invalidates it immediately; a missed first refresh remains pending and delivers normally, while missed later refreshes age out without a retry queue or second state owner.
