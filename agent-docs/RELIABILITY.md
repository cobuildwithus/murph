...
- Personal and Family subscription Checkout binds each provider Session before
  returning its URL. Personal Sessions remain in the member-owned Checkout
  ledger until account deletion makes every retained Session terminal. A
  personal bind rejected by suspension closes the undisclosed Session and any
  newly created customer. Family binding rechecks the exact group owner under
  the owner lock; definitive stale ownership closes the unbound Session. A
  Family redirect with no provider URL clears only a provider-proven expired
  exact Session under that same lock, while completed or ambiguous Sessions
  remain bound for webhook or deletion reconciliation. Account deletion gives
  all bound subscription-Session retrieval and expiry calls one shared
  five-second foreground deadline with provider retries disabled; timeout or
  ambiguous terminal state preserves local ownership for a later
  member-initiated retry.
