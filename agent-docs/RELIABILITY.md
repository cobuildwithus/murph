...
- Automatic meal-photo uploads are replay-safe only through the capture id derived by the enrolled installation. Each staging attempt must own a distinct object. Under the per-capture mailbox lock, the first accepted item chooses the canonical object for exact duplicates; later attempts delete only their own losing object. Failed or ambiguous appends must reconcile the mailbox claim before cleanup so they never delete an accepted object's bytes. Web must reject conflicting reuse, re-signal exact mailbox duplicates, lock the hosted member and active sponsorship source rows before rechecking final upload authority, and acknowledge an upload only after private object staging and canonical mailbox append both succeed. Runtime import must check the canonical external reference before writing, verify staged length and SHA-256 before import, and reconcile the stable automatic-meal closeout on both new-import and existing-meal replay paths using the restored hosted operator home and hosted route-validation profile. A missing closeout route/record keeps the mailbox item retryable without rereading or duplicating the canonical meal. Staging deletes only through a post-checkpoint effect; cleanup derives the user-namespaced object path without requiring encryption-context rediscovery. After failed cleanup, the R2 lifecycle rule makes staging eligible for asynchronous deletion at 31 days, one day beyond mailbox recovery retention, rather than guaranteeing deletion at that exact age. The scheduled closeout uses one `dailyLocal` occurrence and a bounded lookback so after-9pm/delayed captures are eventually processed. Canonical photo removal is idempotent and receipt-guarded in one write batch: the latest meal revision drops the attachment while the raw image and manifest become matching tombstone proof. A missing control client, staged object, write fence, mailbox append, runtime read, automation record, or photo-retention precondition is a visible retryable/fail-closed outcome rather than false success.
