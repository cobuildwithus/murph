...
- Linq speaker-label resolution is prompt-time, presentation-only work owned by that same compound operation. The initial batch sends one bounded set of unique unresolved handles through the existing Web reader; one in-memory memo retains both successful labels and fail-soft misses through later live admissions, so an already-seen handle never recontacts Web and a newly seen handle causes only the necessary bounded read. The memo is discarded with the turn and is never checkpointed or used as profile truth. The Cloudflare `read_participant_display_names` action has a one-second soft deadline (or the lower configured control timeout), so the runtime hot path stops waiting before the existing address-book helper's own two-second deadline. Timeout, late completion, parser skew, authorization loss, suspension, consent loss, ambiguity, KMS/storage failure, or any other optional-source failure leaves the message unnamed and cannot block, retry, or acknowledge accepted conversation work.
