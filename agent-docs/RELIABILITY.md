...
- A pending Environment recording receives high Temporal Activity priority on
  the shared hosted-runtime Task Queue. Ordinary work keeps the default, and
  the global device-sync recovery sweep uses low priority. This ordering does
  not preempt running work or replace durable mailbox retry ownership. Web also
  clears inactive system-frontier projections without reading mailbox rows, so
  ineligible users do not keep retrying stale Temporal pointers.
- On a shared queue, work started by a member that updates the current UI uses
  high priority. Ordinary system work uses the default. Global scans, cleanup,
  and recovery sweeps use low priority. Priority changes dispatch order only;
  it must not add another queue, retry owner, or persisted state owner.
