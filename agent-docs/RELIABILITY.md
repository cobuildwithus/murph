...
- Hosted Linq reaction target reads must distinguish permanent unavailable targets from transient provider failures: permanent misses are acknowledged without context, transient failures remain retryable, and event-id mailbox dedupe makes successful replay once-only. Deferred reaction context is never runnable work and must not contribute foreground counts, retry wakes, or orchestration lag. A supported affirmative reaction to Murph's own fetched target is instead an ordinary conversation-message reply and uses the existing Temporal/direct-ensure handoff; duplicate delivery re-hands off the existing row without reappending it or trusting stale checkpoint facts. The mailbox projection exposes only a bounded newest deferred-reaction suffix through the earliest wakeable message and fast-forwards the omitted prefix through existing durable progress; assistant grouping then pairs by provider occurrence time. Retain at most the newest 32 deferred observations per group and 256 total in the hosted pending set so reaction floods cannot create unbounded foreground reads or prompts.
