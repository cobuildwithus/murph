...
- Assistant product-feedback capture accepts at most one in-memory candidate during a successful provider turn, then attempts the existing web-control write only after the ordinary reply handoff is durable. Failed provider attempts discard their candidate, feedback never counts as a provider side effect for transport retry safety, and persistence remains best-effort with a two-second maximum deadline, no retry queue, and no user-visible delivery state. The accepted-input-derived idempotency key remains the ambiguity fence when a timed-out post-reply write may already have reached Web.
