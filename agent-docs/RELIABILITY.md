...
- Tool-enabled assistant provider turns disable automatic model retries once local side-effecting tools are in play, so bounded assistant/vault operations are never replayed implicitly by transport-layer retry. Code Mode receives each dynamic Murph tool result as one direct string; once its await resolves, later JavaScript rendering failure cannot authorize replay and any confirmation uses canonical readback. Bound tool execution failures remain tool results so the model can recover inside the same turn instead of aborting the provider turn. Codex owns yielded cells: automatic timeout yields block final-answer delivery until the model waits for terminal output, while explicit background yields remain non-blocking. Murph adds no cell-status parser or shadow scheduler.
