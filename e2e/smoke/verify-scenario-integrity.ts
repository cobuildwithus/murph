...
export async function verifyScenarioIntegrity(options?: {
  coverageMode?: boolean;
}): Promise<void> {
  const errors: string[] = [];
  const coverageMode = options?.coverageMode === true;
  ...
  if (scenarioFiles.length === 0) {
    errors.push("No scenario manifests found in e2e/smoke/scenarios");
  }

  const goldenOutputPaths = new Set<string>();
  const sampleImportPaths = new Set<string>();
  const vaultFixturePaths = new Set<string>();
  const scenarioRequiredVaultFixturePaths = new Set<string>();
  ...
  for (const fixture of corpus.vaultFixtures ?? []) {
    ...
    if (fixture.requireSmokeScenarioReference !== false) {
      scenarioRequiredVaultFixturePaths.add(fixture.path);
    }
  ...
  for (const fileName of scenarioFiles) {
    ...
    const scenario = await readJson<ScenarioManifest>(relativeScenarioPath);
  ...
  if (coverageMode) {
    ...
    for (const command of documentedCommands) {
      if (!normalizedScenarioCommands.has(normalizeDocumentedCommand(command))) {
        errors.push(`Missing scenario manifest for documented command: ${command}`);
      }
    }

    for (const command of seenScenarioCommands) {
      if (!documentedCommandSet.has(normalizeDocumentedCommand(command))) {
        errors.push(`Scenario manifest command is not in the documented baseline surface: ${command}`);
      }
    }
    ...
    for (const fixturePath of scenarioRequiredVaultFixturePaths) {
  ...
  console.log(
    `Scenario-manifest ${modeLabel} verification passed for ${scenarioFiles.length} scenarios, ` +
      `${sampleImportPaths.size} sample inputs, and ${goldenOutputPaths.size} golden-output directories.`,
  );
}

async function main(): Promise<void> {
  await verifyScenarioIntegrity({ coverageMode: process.argv.includes("--coverage") });
}

function isEntrypoint(): boolean {
  const entryArg = process.argv[1];
  return entryArg !== undefined && pathToFileURL(entryArg).href === import.meta.url;
}
