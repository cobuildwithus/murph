...
Junction timeseries are the concrete model for this boundary. Their resource policy is static and code-owned: the established default list is unchanged, while `steps`, `distance`, `calories_active`, `heartrate`, and `weight` are explicit opt-ins only. Opted-in `steps` and `distance` reduce to provider/source-partitioned daily observations plus `junction.timeseries_daily_aggregate.v1` evidence. Opted-in `calories_active` and `heartrate` reduce to provider/source-partitioned UTC-hour features, or session features when the upstream row supplies a stable session id or valid session bounds, plus `junction.timeseries_feature_aggregate.v1` evidence. They never emit canonical sample rows or retain full provider snapshots. Opted-in `weight` lands once per sparse reading as a canonical `measurement` with compact `junction.weight_reading.v1` evidence and uses the long summary-history backfill window; the four dense opt-ins retain the bounded dense-timeseries window. Summary and timeseries availability are independent, so a daily activity summary does not suppress an explicitly configured steps or distance timeseries. Provider workout/session streams remain outside this opt-in set and belong under `activity_session.workout.metrics` unless an explicit projector promotes derived facts; the closed companion Strain observation remains the documented exception because its redacted identity cannot be joined safely to Junction's workout session.
