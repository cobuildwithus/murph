...
| Metabolic timeseries | Pull-capable | Fetch glucose in bounded windows and sparse insulin/carbohydrate intervals when exposed by the connected source. | Glucose becomes daily mean/min/max, population standard deviation, and coefficient-of-variation observations plus a maximum of 24 local-hour buckets. Each documented `insulin_injection` interval becomes a `medication_intake`; each timed `carbohydrates` interval becomes a sample-grain `carbohydrate-intake` observation, never a synthetic `meal`. | Retain one versioned compact artifact per glucose source/day and one allowlisted compact artifact per valid insulin/carbohydrate record. The glucose temporal shape is limited to 4,096 serialized bytes and suppresses hourly buckets if that budget is crossed while preserving mean/min/max and canonical SD/CV. Invalid sparse payloads retain only a no-valid-record marker. Never retain full provider arrays or canonical timeseries samples. | Junction-backed glucose, insulin injection, and carbohydrate sources |
...
- metabolic: `glucose`, `lowest-glucose`, `highest-glucose`, `glucose-standard-deviation`, `glucose-coefficient-of-variation`, `carbohydrate-intake`; insulin injections use `medication_intake`
