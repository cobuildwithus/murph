...
Junction timeseries are the concrete model for this boundary. A compile-time policy in `@murphai/contracts` owns membership, default/opt-in admission, history mode, bounded fetch width, and normalization mode. Existing defaults retain their behavior, and all additions remain explicit opt-ins. Opted-in `steps` and `distance` reduce to provider/source-partitioned closed-UTC-day observations plus `junction.timeseries_daily_aggregate.v1` evidence. Opted-in `calories_active` and `heartrate` reduce to provider/source-partitioned UTC-hour or stable-session derived features plus `junction.timeseries_feature_aggregate.v1` evidence. Provider-local days and incomplete transport windows are not promoted. Opted-in `weight` lands once per sparse reading as a canonical `measurement` with compact `junction.weight_reading.v1` evidence. The twelve sparse physiological, intake, alert, and intervention resources in the row above use the summary-history horizon in at most 30-day fetch windows and remain per-reading observations, typed alerts, or insulin interventions rather than daily averages. Each accepted sparse record may produce only one compact allowlisted evidence object plus one or a few canonical events; no path retains grouped provider envelopes, historical arrays, full provider snapshots, or canonical sample rows. Summary and timeseries availability remain independent. Provider workout/session streams remain outside this opt-in set and belong under `activity_session.workout.metrics` unless an explicit projector promotes derived facts; the closed companion Strain observation remains the documented exception because its redacted identity cannot be joined safely to Junction's workout session.
...
- cardiovascular, respiratory, and alerts: `resting-heart-rate`, `average-heart-rate`, `max-heart-rate`, `respiratory-rate`, `spo2`, `forced-expiratory-volume-1`, `forced-vital-capacity`, `peak-expiratory-flow-rate`, `heart-rate-alert`, `sleep-apnea-alert`
