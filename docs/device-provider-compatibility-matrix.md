...
| --- | --- | --- | --- | --- | --- |
| Account/profile identity | Pull-capable | Resolve a stable `externalAccountId` during connect. Fetch profile data only when it materially helps later routing, display, or provenance. | Usually provenance plus a raw `profile` artifact, not a standalone canonical event by itself. | Retain profile payloads only as raw evidence when they are operator-useful. Keep stored runtime metadata shallow. | Garmin/Fitbit through Junction, WHOOP, Oura, Strava |
