...
| Recovery / readiness | Pull-capable | Reconcile recent daily windows; webhook hints optional. | `observation` metrics such as `recovery-score`, `readiness-score`, `sleep-score-delta`, `readiness-score-delta`, `stress-level`, and `body-battery`. | Retain a bounded recovery or readiness evidence part plus day-level provenance. | WHOOP, Oura, Garmin through Junction |
