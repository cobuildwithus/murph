...
| Continuous vitals / timeseries | Pull-capable | Fetch bounded windows only when a current product fact needs them. Keep reconcile windows small enough to avoid duplicate churn. | Compact daily/session `observation` metrics or display-grade metric facts. Glucose retains mean, minimum, maximum, population standard deviation, and coefficient of variation. Junction blood oxygen additionally exposes bounded below-90% sample burden, with run summaries only across qualifying nearby pairs; stress exposes non-clinical daily-mean run and successive-difference summaries across qualifying pairs, plus local-daypart summaries only when both windows have qualifying evidence. Apple HealthKit HRV maps to `hrv-sdnn`; generic wearable HRV retains RMSSD semantics. Do not normalize provider firehose points into canonical sample rows. | Reduce samples in memory and retain only tiny aggregate evidence parts; full-fidelity timeseries retention requires an explicit product/debug policy and tests. | Garmin/Fitbit through Junction, WHOOP, Oura, Strava |
| Sparse physiological readings, intake, alerts, and interventions | Pull-capable; webhook notifications trigger the same pull owner | Junction code-owned configuration admits BMI, carbohydrates, body fat, FEV1, FVC, heart-rate alerts, inhaler usage, insulin injections, lean body mass, peak flow, sleep-apnea alerts, and waist circumference in the production assembly; `fall` remains a product-default sparse alert. BMI, body fat, lean body mass, and waist circumference are contract opt-ins, while the clinical resources, carbohydrates, and insulin are defaults; member or environment overlays cannot change the production set. BMI, body composition, carbohydrates, and insulin retain the generic 14-day horizon; their shared sparse collection contract permits chunks no larger than 30 days without widening that initial window. The seven clinical/safety resources and `fall` likewise retain the generic 14-day horizon with one-day jobs. Each clinical resource admits at most 128 provider rows and retains at most 100 deterministic facts, while metabolic history admits at most 3,840 provider rows and 3,000 canonical facts per bounded chunk. | One per-reading `observation` for body, respiratory, carbohydrates, inhaler, fall, and typed alert counts; one `intervention_session` per insulin dose retaining amount/unit, interval, purpose, delivery form/mode, and insulin type. Irregular-rhythm alerts remain provider alert facts, never ECG diagnoses. | Preserve aggregator and source-provider provenance, source type/instance, timestamps/intervals, and same-time distinct identities. Libre fake-UTC wall times require one exact vault-zone instant; real nonzero offsets remain absolute, while gaps, overlaps, and mixed temporal domains fail closed. The canonical event spine retains the first accepted fallback-zone interpretation across profile-timezone changes; explicit row zones and changed raw wall times still correct the fact. Stable provider row IDs may inform hashed identity but never enter compact evidence. Fallback-zone evidence retains the provider wall-time interval rather than a replay-dependent derived instant. Retain one compact allowlisted evidence object per accepted record, plus a count-only overflow marker when needed and a raw-receipt digest; never retain grouped envelopes or historical arrays. Recheck source lifecycle after provider reads so reconnect races retry the same resource/window. | Explicitly configured Junction-backed sources |
...
| ECG recordings | Pull-capable | Fetch Junction electrocardiogram summaries by default. The code-owned production set separately enables `electrocardiogram_voltage`: list stable summaries for each one-day owner window, then serially fetch the exact source/session interval for each recording. Bind Junction's id-less voltage group only to the summary ID; source, interval, ambiguity, and declared sample-count mismatches retry without invented identity. Admit at most 100,000 samples and 64 recordings, and reduce before snapshot retention. | The summary remains one complete `measurement` per recording. The dense opt-in writes an independent compact `measurement` containing sample count; min/max/mean/RMS, interval, unit, and lead count stay compact evidence. Its canonical owner is the stable provider recording ID plus exact source origin, so corrected timestamps and aggregates revise that fact rather than creating another one. It never diagnoses or classifies rhythm. | Retain one compact feature/evidence record per stable provider recording ID and exact source origin. Never retain or re-emit waveform samples, grouped envelopes, or sample-sized evidence. | Apple HealthKit, Withings, and Kardia through Junction |
| Workout / activity sessions | Pull-capable; foreground companion enrichment for WHOOP-keyed HealthKit metadata | Fetch list and detail endpoints. The code-owned production set separately enables Junction `workout_stream`, which admits at most 32 workouts per one-day index window, then fetches dedicated streams serially with at most 100,000 points and 8 MiB per workout. The exact assembly has 48 production timeseries resources: 7 wide and 41 one-day resources, including 40 ordinary one-day resources plus `workout_stream`. Full-job timeseries continuations run one resource for one closed UTC day at a time. Ordinary collections allow at most three sequential pages, one attempt and eight seconds per page (24 seconds maximum provider wait); page-heavy hourly/session features retry as one complete hour, while daily aggregates remain day-atomic. `workout_stream` uses the same bounded three-page index and persists at most 32 canonical completed workout identities so yield or cancellation can resume between serial stream reads. Each reduced unit is imported before the scalar resource and window coordinate advance. A deployed v1 resource envelope is accepted only as read-only upgrade input and is immediately projected to its validated active scalar resource. Pagination remains in memory, and no vendor page cursor, provider row, waveform sample, or workout point enters job state. | Normal provider workouts remain complete `activity_session` events. The stream opt-in writes one independent compact overall `measurement` for duration, distance, heart-rate shape, cadence, power, and speed plus at most 64 fixed-distance split measurements. Running/walking cadence uses steps per minute, cycling cadence uses rpm, and swimming splits are 100 meters instead of 1 kilometer. Stable provider workout identity plus exact source origin owns every facet; a newer correction withdraws omitted splits. Provider calendar/offset evidence owns the feature day when present; otherwise canonical vault-timezone resolution applies. The rebuildable query projection groups live facets once by the internal hashed identity and stores bounded provider-scoped activity summaries. `wearables activity list` reads those filtered rows and exposes provider, activity type, start time, power in watts, speed in meters per second, other bounded metrics, and splits without provider workout or source-instance IDs. Stream facts cannot replace session calories, route, zones, or source details. | Retain one bounded feature/evidence record per admitted workout and exact source origin. Continuation retains only the scalar resource/day coordinate and bounded workout identity strings, never completed-resource sets, timestamps beyond that coordinate, samples, waveform arrays, coordinates, provider arrays, or stream arrays. Never retain stream points or sample-sized feature lists. | Garmin/Fitbit through Junction, WHOOP, Oura, Strava, WHOOP-keyed metadata through the iOS companion |
...
| Meal / nutrition summaries | Pull-capable | Fetch Junction meal summaries by default for supported connect sources; keep broader nutrition expansions off default polling unless a product need proves them. | Canonical `meal` events with stable Junction-summary-derived `mealId` values, provider IDs as fallback identity only, item names as ingredients, nutrition totals for calories, protein, carbs, fat, fiber, and water, plus 28 bounded documented micronutrients (`nutrition.micros`). Explicit finite zero stays distinct from a null or absent provider field. The assistant-facing nutrient read returns water and those 28 fields with per-field contributing-meal counts. It does not import the source app's daily targets, dashboard percentages, or completeness state. | Retain sanitized `junction-summary-meal` evidence parts for replay. Units follow the pinned Junction meal-summary SDK contract; similarly named Sense database columns can use converted units and must not silently reinterpret persisted summary values. | Junction meal summaries, including Cronometer-backed sources |
...
Junction timeseries are the concrete model for this boundary. A compile-time policy in `@murphai/contracts` owns membership, product-default labeling, history mode, bounded fetch width, and normalization mode. Production may explicitly supply that policy's exhaustive canonical resource list, while omitted runtime configuration resolves to the curated default set; an explicit programmatic list remains exact. This keeps resource admission code-owned, prevents policy additions from silently widening omitted configurations, and leaves no member- or environment-owned expansion surface. `steps` and `distance` reduce to provider/source-partitioned closed-UTC-day observations plus `junction.timeseries_daily_aggregate.v1` evidence. `calories_active` and `heartrate` reduce to provider/source-partitioned UTC-hour features plus `junction.timeseries_feature_aggregate.v1` evidence; provider-local days and incomplete transport windows are not promoted. For both dense daily aggregates and hourly/session features, a documented point timestamp wins; otherwise interval start owns the unsplit provider scalar and its UTC day/hour bucket, with interval end used only as fallback. `weight` lands once per sparse reading as a canonical `measurement` with compact `junction.weight_reading.v1` evidence. Sparse physiological, intake, alert, and intervention resources reuse one generic owner and remain per-reading observations, typed alerts, or insulin interventions rather than daily averages. Among these body-composition and metabolic resources, `weight` alone participates in the package-owned extended set; BMI, body fat, lean body mass, waist circumference, carbohydrates, and insulin retain the generic bounded horizon. The next activity slice uses the same descriptor pipeline: `calories_basal`, `daylight_exposure`, `floors_climbed`, `stand_duration`, `stand_hour`, and `wheelchair_push` reduce to source-partitioned daily aggregates; `handwashing`, `uv_exposure`, `workout_distance`, `workout_duration`, and `workout_swimming_stroke` reduce to bounded UTC-hour features; and `fall` remains one sparse alert fact per provider record. Full-job continuations fetch one resource and one closed day at a time, and page-heavy hourly features adapt to one closed hour; provider calls remain sequential and every collection has a strict page, attempt, and request-time bound. When the code-owned configured set narrows, a structurally valid durable cursor for a known removed resource restarts its same owner window at the first configured resource; unknown and malformed cursors still fail closed. Dense `electrocardiogram_voltage` and dedicated `workout_stream` reduce before snapshot retention into O(recordings/workouts) compact evidence independent of sample count. Their independent compact resource types use the stable provider recording/workout ID plus exact source origin as canonical identity, leaving timestamps and aggregates revisable. Workout streams use the workout index plus serial dedicated stream reads rather than the grouped-timeseries endpoint, and resumable progress uses the existing job payload. The stable workout/source identity and source update version form one authoritative facet set, so a newer correction retracts omitted splits. The rebuildable wearable-summary projection groups live stream facets once by that hashed canonical identity; the activity-list runtime reads date/provider-filtered summary rows and does not expose the provider workout or source-instance identifier. No path retains grouped provider envelopes, historical arrays, vendor page cursors, waveform/stream points, full provider snapshots, canonical sample rows, or invented coverage metadata. Summary and timeseries availability remain independent, and stream-derived facts use independent measurement resource types rather than the richer ECG summary or workout-session references.

The package-owned `m2` 180-day extended-history matrix is exactly `afib_burden`, `basal_body_temperature`, `blood_pressure`, `body_temperature`, `body_temperature_delta`, `caffeine`, `heart_rate_recovery_one_minute`, `mindfulness_minutes`, `note`, `sleep_breathing_disturbance`, `vo2_max`, `water`, and `weight`. It is independent of the generic timeseries and summary settings; every other admitted timeseries resource retains the generic bounded horizon. Twelve schedule-time coordinates per each of 33 semantic sources keep the active-key lookup capped at 396, while blood pressure remains the independent source-first thirteenth resource.

Blood oxygen and stress have an additional closed-day temporal owner. It derives exact half-open UTC bounds from the vault-local IANA day and grants replacement authority only after that day closes plus a conservative 24-hour arrival lag. Scheduled reconciliation queues the entire bounded horizon once per eligible local day, or when the timezone, resources, horizon, or stable source roster/capabilities change. Its hashed scheduling marker commits atomically with the local resource/day queue. Hosted publication waits for the existing retained-wake/completion checkpoint, since SQLite is excluded from workspace snapshots; the marker never grants import authority. Oxygen/stress data events queue only intersecting days in the rolling horizon, including future jobs held until the closure lag passes. Queued or running rows deduplicate active work across restart; completed day jobs can be recreated by later events or the daily safety sweep. Ordinary reconciliation retains its existing cadence. Failed, yielded, unavailable, and dead work grants no authority. Temporal children do not advance generic account completion, and partial webhook or direct resource work cannot gate the ordinary pull floor. A successful temporal resource/day owns its connection/resource/day `temporal-*` facets through the existing authoritative event-set boundary; ordinary daily facts use separate identities. Absolute timestamps are filtered as instants, while floating/date-only values qualify by raw local day only under complete authority. The reducers cap input, collapse exact duplicates without dropping distinct equal-instant values, require resource-specific absolute adjacency caps for continuity claims, and persist only bounded scalar evidence.
