...
| Workout / activity sessions | Pull-capable; foreground companion enrichment for WHOOP-keyed HealthKit metadata | Fetch list and detail endpoints. The code-owned production set separately enables Junction `workout_stream`, which admits at most 32 workouts per one-day index window, then fetches dedicated streams serially with at most 100,000 points per workout. The exact assembly has 48 production timeseries resources: 13 wide and 35 dense, including 34 ordinary dense resources plus `workout_stream`. At the 100-page collection ceiling and existing three-attempt GET policy, one closed dense day is bounded to 3,532 / 10,596 logical GETs / network attempts. Full timeseries ceilings are 57,248 / 171,744 per attempt and 286,240 / 858,720 across five attempts for the 14-day backfill plus six wide windows, and 26,024 / 78,072 per attempt and 130,120 / 390,360 across five attempts for the seven-day reconcile plus one wide window. Its job-owned continuation is one current resource and one closed UTC day plus a versioned bounded exact set of completed configured resource names and, while `workout_stream` is unfinished, an optional at-most-32 canonical set of completed workout identities. Each reduced resource is imported before its name advances. Within the active coordinate, insertion and reordering cannot skip newly configured resources; malformed or removed active identities fail closed. Pagination remains in memory, so a handled retry or cooperative continuation can replay at most the single unfinished resource, not any earlier completed resource. The coordinate clears only after every configured resource completes; resource progress contains names and version metadata only, never provider rows, waveform samples, workout points, or sample-sized evidence. | Normal provider workouts remain complete `activity_session` events. The stream opt-in writes one independent compact `measurement` with duration, distance, heart-rate average/maximum, and stable workout identity. Its canonical owner is the stable provider workout ID plus exact source origin, so corrected timestamps and aggregates revise that fact. It cannot replace session calories, power, route, zones, or source details. | Retain one compact feature/evidence record per admitted workout and exact source origin. Continuation retains only the daily coordinate and bounded identity strings, never timestamps beyond that coordinate, samples, waveform arrays, or stream arrays. Never retain stream points or sample-sized feature lists. | Garmin/Fitbit through Junction, WHOOP, Oura, Strava, WHOOP-keyed metadata through the iOS companion |
