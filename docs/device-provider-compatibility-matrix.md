...
Junction timeseries are the concrete model for this boundary. Their resource lists are static and code-owned: the established default list is unchanged, while `steps`, `distance`, `calories_active`, `heartrate`, and `weight` are explicit opt-ins only. Opted-in `steps` and `distance` reduce to provider/source-partitioned UTC-day observations plus `junction.timeseries_daily_aggregate.v1` evidence. Opted-in `calories_active` and `heartrate` reduce to provider/source-partitioned UTC-hour features plus `junction.timeseries_feature_aggregate.v1` evidence. Those identities match the scheduler's complete closed-UTC-day import boundary; provider-local days and upstream sessions are not promoted from independently incomplete transport windows. They never emit canonical sample rows or retain full provider snapshots. Opted-in `weight` lands once per sparse reading as a canonical `measurement` with compact `junction.weight_reading.v1` evidence and uses the long summary-history backfill window; the four dense opt-ins retain the bounded dense-timeseries window. Summary and timeseries availability are independent, so a daily activity summary does not suppress an explicitly configured steps or distance timeseries. Provider workout/session streams remain outside this opt-in set and belong under `activity_session.workout.metrics` unless an explicit projector promotes derived facts; the closed companion Strain observation remains the documented exception because its redacted identity cannot be joined safely to Junction's workout session.
