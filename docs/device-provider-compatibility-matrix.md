...
Junction timeseries are the concrete model for this boundary. Normal sync may fetch only policy-admitted, product-needed timeseries and must aggregate them before persistence. Blood oxygen and stress level additionally use one `junction.timeseries_temporal_features.v2` envelope in the existing daily artifact plus fixed summary observations. Only the closed date-by-date fetch path supplies transient complete-source-day authority for those observations; precise resource windows and webhook-triggered imports keep ordinary daily summaries but cannot publish temporal features from partial windows. Each authoritative result reconciles the fixed feature facets through canonical event upserts and versioned retractions, so insufficient or capped replacements remove stale features without retaining another merge state. Equal timestamps collapse deterministically by mean before reduction. SpO2 sample burden below the repository-owned 90% oxygen-screening threshold remains a literal, sample-counted low-confidence statistic; run shape requires adjacent pairs no more than five minutes apart and inside the deterministic 2.5× median-cadence boundary. Mechanical stress run and successive variation require pairs inside the same cadence boundary and a 15-minute absolute cap; evening-minus-morning shape additionally requires a qualifying pair in both provider/vault-local windows. Queryable observations carry bounded scalar sample/pair counts, method, and non-high confidence, never timestamps or sample arrays. SpO2 output deliberately reports sample percentages and consecutive-sample counts rather than inferring continuous duration. These are descriptive summaries, not alerts or diagnoses. Fewer than two distinct valid instants emit no temporal feature, isolated stress readings emit no derived claim, and input above 5,000 samples per resource/source/day or 25,000 per resource/import suppresses the whole temporal enhancement while preserving ordinary daily facts. Output is capped at ten feature observations per local day and a 1 KiB feature envelope; suppression never creates another artifact. The vault may keep tiny aggregate evidence such as `junction-timeseries-daily-*`, but it must not persist full `junction-timeseries-*` sample arrays, raw timestamp arrays, hourly rows, or generic provider snapshots for dropped dense resources. Dense/debug streams such as steps, distance, and heart rate, plus sparse resources such as weight, stay out of default sync unless a current product observation needs them. Provider workout/session metrics belong under `activity_session.workout.metrics` unless an explicit projector promotes derived daily facts; the closed companion Strain observation is the documented exception because its redacted identity cannot be joined safely to Junction's workout session. Wearable summaries require compact display-grade facts such as daily activity, sleep, or body observations.
