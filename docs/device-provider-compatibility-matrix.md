...
Junction timeseries are the concrete model for this boundary. Their resource lists and exhaustive history policies are static and code-owned, and `steps`, `distance`, `calories_active`, `heartrate`, and `weight` are included by default. `steps` and `distance` reduce to provider/source-partitioned UTC-day observations plus `junction.timeseries_daily_aggregate.v1` evidence. `calories_active` and `heartrate` reduce to provider/source-partitioned UTC-hour features plus `junction.timeseries_feature_aggregate.v1` evidence. They never emit canonical sample rows or retain full provider snapshots. `weight` lands once per sparse reading as a canonical `measurement` with compact `junction.weight_reading.v1` evidence and uses the extended history window; the four dense resources retain bounded history.

Other admitted sparse daily resources, including AFib burden, VO2 max, heart-rate recovery, body and basal temperatures, sleep-breathing disturbance, caffeine, water, and mindfulness, use extended history through the same source-scoped job owner. Historical-pull status is re-read before coverage, rejected aggregate dates use the bounded retry ladder, and migration continues until no middle gap remains. Summary and timeseries availability remain independent. Provider workout/session streams remain outside this default set and belong under `activity_session.workout.metrics` unless an explicit projector promotes derived facts; the closed companion Strain observation remains the documented exception because its redacted identity cannot be joined safely to Junction's workout session.
