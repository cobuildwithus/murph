...
| Body measurements / composition | Pull-capable | Poll or fetch stable body endpoints only. Junction summary state may use import-day fallback when it lacks a measurement timestamp; sparse Junction `weight`, `fat`, `body_mass_index`, `lean_body_mass`, and `waist_circumference` readings require their own timestamp and documented unit. | Summary `observation` metrics include `weight`, `bmi`, `body-fat-percentage`, `lean-body-mass`, `waist-circumference`, `bone-mass-percentage`, `muscle-mass-percentage`, `visceral-fat-index`, and `body-water-percentage`. Sparse readings retain those existing weight/fat/BMI/lean/waist metric identities with `sample` grain. Visceral fat is an upstream index, never a percentage. | Retain one compact evidence part per sparse reading. Do not retain the provider response array, emit generic sample rows, or derive mass values from percentages. | WHOOP body measurement, Oura daily SpO2, Junction-backed sources when configured |
...
Junction timeseries are the concrete model for this boundary. Dense product-needed resources reduce in memory to compact daily facts and evidence. Sparse body resources instead retain one timestamped observation and one tiny `junction.body_measurement_reading.v1` evidence part per reading. Neither path may persist full `junction-timeseries-*` sample arrays or generic provider response arrays. Dense/debug streams such as steps, distance, and heart rate stay out of default sync unless a current product observation needs them. Provider workout/session metrics belong under `activity_session.workout.metrics` unless an explicit projector promotes derived daily facts; the closed companion Strain observation is the documented exception because its redacted identity cannot be joined safely to Junction's workout session. Wearable summaries require compact display-grade facts such as daily activity, sleep, or body observations.
...
- body and composition: `weight`, `bmi`, `body-fat-percentage`, `lean-body-mass`, `waist-circumference`, `bone-mass-percentage`, `muscle-mass-percentage`, `visceral-fat-index`, `body-water-percentage`, `systolic-blood-pressure`, `diastolic-blood-pressure`
