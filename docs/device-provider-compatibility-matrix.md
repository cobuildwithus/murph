...
| Sparse physiological readings, intake, alerts, and interventions | Pull-capable; webhook notifications trigger the same pull owner | Junction code-owned configuration explicitly opts in BMI, carbohydrates, body fat, FEV1, FVC, heart-rate alerts, inhaler usage, insulin injections, lean body mass, peak flow, sleep-apnea alerts, and waist circumference in the production assembly. Contract defaults remain off, and member or environment overlays cannot change the production set. Initial history uses the 180-day summary/extended horizon in bounded 30-day windows; dense/default resources keep one-day windows. | One per-reading `observation` for body, respiratory, carbohydrates, inhaler, and typed alert counts; one `intervention_session` per insulin dose retaining amount/unit, interval, purpose, delivery form/mode, and insulin type. Irregular-rhythm alerts remain provider alert facts, never ECG diagnoses. | Preserve aggregator and source-provider provenance, source type/instance, stable provider row ID when present, timestamps/intervals, and same-time distinct identities. Retain one compact allowlisted evidence object per accepted record plus a raw-receipt digest; never retain grouped envelopes or historical arrays. | Explicitly configured Junction-backed sources |
