...
Junction timeseries are the concrete model for this boundary. A compile-time policy in `@murphai/contracts` owns membership, default/opt-in admission, history mode, bounded fetch width, and normalization mode. Existing defaults retain their behavior. Opted-in `steps` and `distance` reduce to source-partitioned daily observations; `calories_active` and `heartrate` reduce to source-partitioned UTC-hour or stable-session features; and `weight` lands once per sparse reading. The twelve sparse physiological, intake, alert, and intervention resources in the row above remain separate code-owned opt-ins whose initial history uses the summary horizon in at most 30-day windows. Those resources remain per-reading observations, typed alerts, or insulin interventions rather than daily averages. Each accepted sparse record may produce only one compact allowlisted evidence object plus one or a few canonical events; no path retains grouped provider envelopes, historical arrays, or full provider snapshots. Summary and timeseries availability remain independent, and provider workout/session streams still belong under `activity_session.workout.metrics` unless an explicit projector promotes derived facts.
