...
Junction timeseries are the concrete model for this boundary. Normal sync may fetch only policy-admitted, product-needed resources and must reduce dense streams before persistence. The vault may keep tiny daily aggregate evidence such as `junction-timeseries-daily-*`; it must not persist full provider timeseries arrays, canonical samples, or generic provider snapshots for dropped dense resources. Sparse heart-rate and sleep-apnea alerts, falls, FEV1, forced vital capacity, peak expiratory flow, and inhaler usage are the per-record exception: each lands as one `measurement` event plus compact `junction-timeseries-reading-*` evidence with timing, canonical unit, and provenance. Inhaler usage is a measured count rather than an intervention because Junction supplies no medication/dose identity. Dense/debug streams such as steps, distance, and intraday heart rate, plus unsupported sparse resources such as weight, stay out of default sync unless a current product fact needs them. Provider workout/session metrics belong under `activity_session.workout.metrics` unless an explicit projector promotes derived daily facts; the closed companion Strain observation is the documented exception because its redacted identity cannot be joined safely to Junction's workout session. Wearable summaries require compact display-grade facts such as daily activity, sleep, or body observations.
