...
| ECG recordings | Pull-capable | Fetch Junction electrocardiogram summaries by default. `electrocardiogram_voltage` is a separate exact opt-in: fetch one-day grouped windows, admit at most 100,000 samples and 64 recordings, and reduce before snapshot retention. | Existing summaries remain one `measurement` event per recording. The dense opt-in updates that same owner with sample count; its compact evidence also carries start/end/duration, upstream unit, lead count, and finite min/max/mean/RMS. It never diagnoses or classifies rhythm or synthesizes an AFib/ECG summary. | Retain one compact feature/evidence record per recording with stable provider identity or deterministic semantic fallback. Never retain or re-emit waveform samples, grouped envelopes, or sample-sized evidence. | Apple HealthKit, Withings, and Kardia through Junction |
| Workout / activity sessions | Pull-capable; foreground companion enrichment for WHOOP-keyed HealthKit metadata | Fetch list and detail endpoints. Junction `workout_stream` is a separate exact opt-in that admits at most 32 workouts per one-day index window, then fetches the provider's dedicated per-workout stream endpoint serially with at most 100,000 points per workout. The common single-page maximum is 33 calls; existing pagination keeps the absolute maximum at 132. Use webhooks only when the provider offers reliable session updates or deletes. | Normal provider workouts become `activity_session` events with session-scoped workout detail under `workout`. The stream opt-in revises that same owner with bounded duration, distance, heart-rate average/maximum, sample count, and stable workout identity. Daily/queryable activity rollups still require explicit summary observations or a later projector. Companion-only Strain remains a separate `workout-strain` observation. | Retain one compact activity/evidence record per admitted workout. Stable Junction workout IDs own correction identity, and conflicting same-ID bodies fail closed. Never retain stream points or sample-sized feature lists. | Garmin/Fitbit through Junction, WHOOP, Oura, Strava, WHOOP-keyed metadata through the iOS companion |
...
- body, composition, and sparse intake/action facts: `weight`, `bmi`, `body-fat-percentage`, `lean-body-mass`, `waist-circumference`, `systolic-blood-pressure`, `diastolic-blood-pressure`, `carbohydrates`, `inhaler-usage`; insulin doses remain `intervention_session` events rather than observation metrics
