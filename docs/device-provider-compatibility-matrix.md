...
| Workout / activity sessions | Pull-capable; foreground companion enrichment for WHOOP-keyed HealthKit metadata | Fetch list and detail endpoints. The code-owned production set separately enables Junction `workout_stream`, which admits at most 32 workouts per one-day index window, then fetches dedicated streams serially with at most 100,000 points per workout. The exact assembly has 43 production timeseries resources: 13 wide and 30 dense, including 29 ordinary dense resources plus `workout_stream`. At the 100-page collection ceiling and existing three-attempt GET policy, one closed dense day is bounded to 3,032 / 9,096 logical GETs / network attempts. Full timeseries ceilings are 50,248 / 150,744 per attempt and 251,240 / 753,720 across five attempts for the 14-day backfill plus six wide windows, and 22,524 / 67,572 per attempt and 112,620 / 337,860 across five attempts for the seven-day reconcile plus one wide window. Its job-owned continuation is one coordinate: phase and current window/day plus a versioned bounded exact set of completed configured resource names and, while `workout_stream` is unfinished, an optional at-most-32 canonical set of completed workout identities. Each reduced resource is imported before its name advances. Within the active coordinate, insertion and reordering cannot skip newly configured resources; malformed or removed identities fail closed. Pagination remains in memory, so a handled retry or cooperative continuation can replay at most the single unfinished resource, not any earlier completed resource. The coordinate clears only after every configured resource for the window/day completes; resource progress contains names and version metadata only, never provider rows, waveform samples, workout points, or sample-sized evidence. | Normal provider workouts remain complete `activity_session` events. The stream opt-in writes one independent compact `measurement` with duration, distance, heart-rate average/maximum, and stable workout identity. Its canonical owner is the stable provider workout ID plus exact source origin, so corrected timestamps and aggregates revise that fact. It cannot replace session calories, power, route, zones, or source details. | Retain one compact feature/evidence record per admitted workout and exact source origin. Continuation retains only the daily coordinate and bounded identity strings, never timestamps beyond that coordinate, samples, waveform arrays, or stream arrays. Never retain stream points or sample-sized feature lists. | Garmin/Fitbit through Junction, WHOOP, Oura, Strava, WHOOP-keyed metadata through the iOS companion |
...
Junction timeseries are the concrete model for this boundary. A compile-time policy in `@murphai/contracts` owns membership, default/opt-in admission, history mode, bounded fetch width, and normalization mode. Existing defaults retain their behavior, and additions require explicit code-owned opt-in; the production provider assembly supplies the exact 26-resource addition requested here, with no member- or environment-owned expansion surface. Opted-in `steps` and `distance` reduce to provider/source-partitioned closed-UTC-day observations plus `junction.timeseries_daily_aggregate.v1` evidence. Opted-in `calories_active` and `heartrate` reduce to provider/source-partitioned UTC-hour features plus `junction.timeseries_feature_aggregate.v1` evidence; provider-local days and incomplete transport windows are not promoted. For both dense daily aggregates and hourly/session features, a documented point timestamp wins; otherwise interval start owns the unsplit provider scalar and its UTC day/hour bucket, with interval end used only as fallback. Opted-in `weight` lands once per sparse reading as a canonical `measurement` with compact `junction.weight_reading.v1` evidence. The first twelve sparse physiological, intake, alert, and intervention resources use the summary-history horizon in at most 30-day fetch windows and remain per-reading observations, typed alerts, or insulin interventions rather than daily averages. The next activity slice uses the same descriptor pipeline: `calories_basal`, `daylight_exposure`, `floors_climbed`, `stand_duration`, `stand_hour`, and `wheelchair_push` reduce to source-partitioned daily aggregates; `handwashing`, `uv_exposure`, `workout_distance`, `workout_duration`, and `workout_swimming_stroke` reduce to bounded UTC-hour features; and `fall` remains one sparse alert fact per provider record. Dense resources keep one-day fetch chunks while `fall` uses the bounded 30-day sparse-history chunk. Dense `electrocardiogram_voltage` and dedicated `workout_stream` are separate one-day opt-ins that reduce before snapshot retention into O(recordings/workouts) compact evidence independent of sample count. Their independent compact resource types use the stable provider recording/workout ID plus exact source origin as canonical identity, leaving timestamps and aggregates revisable. Workout streams use the workout index plus serial dedicated stream reads rather than the grouped-timeseries endpoint, and resumable progress is the existing job payload's canonical, contracts-bounded set of exact terminal candidate identities rather than a provider-order cursor. No path retains grouped provider envelopes, historical arrays, continuation timestamps, waveform/stream points, full provider snapshots, canonical sample rows, or invented coverage metadata. Summary and timeseries availability remain independent, and stream-derived facts use independent measurement resource types rather than the richer ECG summary or workout-session references.
