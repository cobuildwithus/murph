...
Junction timeseries are the concrete model for this boundary. Normal sync may fetch only policy-admitted, product-needed timeseries and must aggregate them before persistence. Blood oxygen and stress level additionally use a bounded `junction.timeseries_temporal_features.v2` evidence envelope plus fixed summary observations. Temporal ownership uses the vault's configured IANA timezone: the provider requests one exact half-open UTC window for each vault-local day and grants replacement authority only after that local day has ended plus a conservative 24-hour arrival lag. The temporal reconcile horizon is clamped to `1..14` authoritative local days. Every scheduled reconcile fetches the newest eligible local day immediately for each configured temporal resource, then places older resource/day coordinates on the existing durable job queue in newest-first order. Succeeded job history proves populated or successful-empty completion across scheduler reruns and restarts; failed, yielded, and dead work grants no authority, and dead work can be recreated on the next reconcile. With two temporal resources this is bounded to two immediate collections plus 26 older one-resource/one-day jobs. If both newest resources need durable continuation, at most 28 temporal resource/day rows plus one ordinary reconcile follow-up remain serialized by the existing per-account execution fence. Temporal children never advance generic account completion. Generic completion is account activity state, not proof that the complete ordinary pull floor ran; every scheduled reconcile refetches configured ordinary resources. Each child performs one canonical import transaction and the existing per-account execution fence serializes it with other device work; each provider collection remains bounded to 100 pages and 25,000 records, with at most three GET attempts per page. A higher-priority resource job may run before the catch-up backlog, but partial webhook or direct resource work never gates the ordinary pull floor. Precise resource windows and webhook-triggered imports keep ordinary daily summaries but cannot publish temporal features from partial windows. Each successfully fetched temporal resource/day owns all of that connection/resource/day's `temporal-*` facets through the core device batch's `authoritativeEventSets`; a successful empty or insufficient/capped replacement therefore removes stale source facets atomically, while a failed or yielded resource grants no authority. Ordinary daily facts use different external-ref identities and remain untouched. Absolute provider timestamps are filtered against the exact UTC window; floating and date-only values are compared by raw calendar day only when complete local-day authority exists, and remain ordinary un-authorized input for precise windows rather than being coerced through the process timezone. Fetch dedupe preserves distinct values at the same instant, then the reducer deterministically averages equal-instant observations before calculating features. SpO2 sample burden below the repository-owned 90% oxygen-screening threshold remains a literal, sample-counted low-confidence statistic; run shape requires adjacent pairs no more than five minutes apart and inside the deterministic 2.5× median-cadence boundary. Mechanical stress run and successive variation require pairs inside the same cadence boundary and a 15-minute absolute cap; evening-minus-morning shape additionally requires a qualifying pair in both vault-local windows. Queryable observations carry bounded scalar sample/pair counts, method, and non-high confidence, never timestamps or sample arrays. SpO2 output deliberately reports sample percentages and consecutive-sample counts rather than inferring continuous duration. These are descriptive summaries, not alerts or diagnoses. Fewer than two distinct valid instants emit no temporal feature, isolated stress readings emit no derived claim, and input above 5,000 samples per resource/source/day or 25,000 per resource/import suppresses the whole temporal enhancement while preserving ordinary daily facts. Each resource/day normalization is capped at ten feature observations and a 1 KiB feature envelope, for a composed maximum of twenty observations across the two temporal resources on one local day; four-source stress input can emit nine feature observations for that resource while the hard resource/day cap remains ten. The vault may keep tiny aggregate evidence such as `junction-timeseries-daily-*` and `junction-timeseries-temporal-*`, but it must not persist full sample arrays, raw timestamp arrays, hourly rows, or generic provider snapshots for dropped dense resources. Dense/debug streams such as steps, distance, and heart rate, plus sparse resources such as weight, stay out of default sync unless a current product observation needs them. Provider workout/session metrics belong under `activity_session.workout.metrics` unless an explicit projector promotes derived daily facts; the closed companion Strain observation is the documented exception because its redacted identity cannot be joined safely to Junction's workout session. Wearable summaries require compact display-grade facts such as daily activity, sleep, or body observations.
...
- activity and movement daily/display facts: `daily-steps`, `distance`, `active-calories`, `total-calories`, `floors-climbed`, `activity-minutes`, `low-activity-minutes`, `medium-activity-minutes`, `high-activity-minutes`, and display-grade daily heart-rate facts such as `walking-average-heart-rate`. Session-only workout values such as strain, recording percentage, elevation, speed, and workout heart rate belong under `activity_session.workout.metrics` unless an explicit projector emits display-grade observations.
- sleep and recovery: `sleep-total-minutes`, `time-in-bed-minutes`, `sleep-latency-minutes`, `sleep-efficiency`, `sleep-score`, `recovery-score`, `readiness-score`
- cardiovascular and vitals: `resting-heart-rate`, `average-heart-rate`, `max-heart-rate`, `respiratory-rate`, `spo2`
- body and composition: `weight`, `bmi`, `body-fat-percentage`, `bone-mass-percentage`, `muscle-mass-percentage`, `body-water-percentage`, `visceral-fat-index`, `lean-body-mass`, `waist-circumference`, `systolic-blood-pressure`, `diastolic-blood-pressure`
