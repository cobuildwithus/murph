...
| Workout / activity sessions | Pull-capable; foreground companion enrichment for WHOOP-keyed HealthKit metadata | Fetch list and detail endpoints. The code-owned production set separately enables Junction `workout_stream`, which admits at most 32 workouts per one-day index window, then fetches dedicated streams serially with at most 100,000 points per workout. One closed day is bounded to 132 logical GETs or 396 network attempts with the existing three-attempt GET policy. Composed ceilings are 1,848 / 5,544 per attempt and 9,240 / 27,720 across five attempts for the 14-day backfill, and 924 / 2,772 per attempt and 4,620 / 13,860 across five attempts for the seven-day reconcile. Its job-owned continuation is one coordinate: the current closed-day start plus an optional bounded canonical set of exact completed workout identities for that day. That coordinate remains owned by the whole closed day and clears only after workout streams and every configured ordinary dense resource for the day complete; order, insertion, retry, yield, and cancellation cannot turn a lexical cursor into authority. | Normal provider workouts remain complete `activity_session` events. The stream opt-in writes one independent compact `measurement` with duration, distance, heart-rate average/maximum, and stable workout identity. Its canonical owner is the stable provider workout ID plus exact source origin, so corrected timestamps and aggregates revise that fact. It cannot replace session calories, power, route, zones, or source details. | Retain one compact feature/evidence record per admitted workout and exact source origin. Continuation retains only the daily coordinate and bounded identity strings, never timestamps beyond that coordinate, samples, waveform arrays, or stream arrays. Never retain stream points or sample-sized feature lists. | Garmin/Fitbit through Junction, WHOOP, Oura, Strava, WHOOP-keyed metadata through the iOS companion |
