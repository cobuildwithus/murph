...
Browser-vault session creation requires current launch-required consent before reading hosted vault state. Device-sync connection and reconnection setup do not use launch-document freshness as an admission gate: the member must still have an active authenticated session, take an explicit connect action, and complete the source provider's authorization flow. Current companion device status, token exchange, and sync ingestion likewise remain available to an active bearer-authenticated member when launch-document acceptance becomes stale. These device paths do not require a second connected-source consent grant because the explicit source action and provider authorization supply the feature intent. Future feature gates should follow the same pattern at the boundary where hosted processing would otherwise begin and should introduce separate optional scopes only when they cover distinct data use beyond the explicit source authorization.
...
The authenticated dashboard layout reads consent status before starting the browser-vault provider. A member with stale launch consent sees the current legal card alongside the requested dashboard route; the reminder does not replace or block the device-connect page. Accepting both launch scopes reloads that exact route and restores protected vault-backed features. If the reminder status cannot be read, the layout omits the reminder instead of taking device connection down.

The ordinary Linq inbound webhook, mailbox ingestion, hosted container wake, current-conversation reply path, configured non-Strava device connection/reconnection flow, and current companion device sync do not use launch consent as an admission gate. A stale document version therefore does not stop an existing member from texting Murph, receiving a reply in that active conversation, connecting an available device, or continuing current device sync. Independently guarded browser-vault, clinical-record, export, billing, group-join, iMessage mini-app, meal-photo, and unrelated companion actions still fail closed with `HOSTED_CONSENT_REQUIRED` until the member accepts the current documents. Strava remains disabled for new connections and reconnect offers as a separate provider product gate.
