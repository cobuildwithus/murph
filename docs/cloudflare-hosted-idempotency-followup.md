...
- hosted RevNet issuance already uses invoice-level idempotency plus persisted tx-hash state so retried `invoice.paid` webhooks do not resubmit a second onchain payment
...
- the repo still uses multiple durable-idempotency shapes (`execution_outbox`, receipt-local webhook side effects, invoice-owned issuance state, and the Cloudflare committed side-effect journal) rather than one shared implementation
