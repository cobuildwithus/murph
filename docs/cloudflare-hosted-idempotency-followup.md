...
- the repo still uses multiple durable-idempotency shapes (`HostedWake` / `HostedExecutionCursor`, receipt-local webhook side effects, the hosted Stripe fact queue, invoice-owned issuance state, and the Cloudflare committed side-effect journal) rather than one shared implementation
