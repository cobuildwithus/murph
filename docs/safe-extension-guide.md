...
2. Preserve useful provider evidence as `evidenceParts` on the shared device-batch payload; `@murphai/core` writes the exact bytes into `raw/integrations/**` and compact references into `ledger/integration-ingests/**`. Do not write provider evidence files directly from importer code.
...
- New paths under the vault root preserve Markdown truth, append-only JSONL, immutable `raw/` evidence, and compact provider evidence indexes in `ledger/integration-ingests/**`.
