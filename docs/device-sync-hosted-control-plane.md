...
When a connection transitions from clean to dirty, webhook ingress commits the dirty state, appends one opaque `device-sync.wake` mailbox pointer for that dirty revision, and completes the trace in the same transaction. Additional level hints while already dirty are coalesced without another ingress wake. Durable webhook work appends independent encrypted payload rows under exact trace claim and is acknowledged by explicit payload row id, so concurrent durable deliveries do not need a connection-scoped acceptance lock. If a post-commit Temporal signal is missed, the dirty row, mailbox pointer, or remaining payload rows stay pending and the device-sync dirty sweeper can signal Temporal again later using the same dirty-revision wake identity. Webhook and app paths do not send a user-level runner nudge directly to Cloudflare.
