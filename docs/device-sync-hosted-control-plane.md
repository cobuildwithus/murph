...
and serial. Each lane claims trace idempotency in chunks of eight through one
bounded Prisma transaction (`createMany`, one classification read, and only
rare conditional stale-lease takeovers) instead of opening one claim
transaction per event. Stores without the batch capability keep the serial
claim fallback. Web does not re-run the provider signature verifier or parser.
...
Only trace admission is transaction-batched. Health-data consent,
provider-application revision, setup, source, reconnect, disconnect, dirty
state, exact encrypted payload, mailbox wake, trace completion, and post-commit
Temporal behavior retain their existing per-event authority checks and
independently retryable transactions. Cloudflare consumer concurrency remains
one, so the four-lane Web bound is also the composed database-concurrency bound
for this Queue. Value-free batch logs report input, lane, chunk, disposition,
failure-code count, and duration totals; they never include account, trace,
transport, provider payload, or exception values. No Queue database, Durable
Object state, Vercel Workflow, or Temporal webhook workflow is introduced.
...
lower bound near 58 minutes before the batch-claim savings. At 500 ms/event,
