...
The Fitbit card is a product-facing source abstraction, not a second Google Health app card. Fresh Fitbit and Pixel Watch starts select Junction provider `google_health`; persisted provider `fitbit` remains the legacy provenance, status, and targeted-disconnect identity. While both source identities exist, Google Health canonical import stays fenced until Junction reports supported resources and a fresh successor update, and the member explicitly confirms representative recent history before disconnecting legacy Fitbit. A disconnect in progress keeps the fence closed, a failed disconnect restores the working legacy source, and a successful disconnect admits only successor records strictly after the durable legacy cutover boundary. The control plane does not infer historical completeness or proprietary Fitbit scores.
