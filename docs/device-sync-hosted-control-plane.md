...
When a connection transitions from clean to dirty, webhook ingress commits the dirty state and completes the trace in the same transaction. It then sends a best-effort user-level Temporal recovery nudge without appending foreground mailbox work. Additional level hints while already dirty are coalesced without another ingress wake. Durable webhook work appends independent encrypted payload rows under exact trace claim and is acknowledged by explicit payload row id, so concurrent durable deliveries do not need a connection-scoped acceptance lock. If a post-commit Temporal signal is missed, the dirty row or remaining payload rows stay pending and the device-sync dirty sweeper can signal Temporal again later by dirty user. Webhook and app paths do not send runner nudges directly to Cloudflare.
