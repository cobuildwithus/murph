...
Before provider I/O, the hosted connection-start path stages a short-lived `device_oauth_session` lifecycle marker under the active `hosted_member` row lock, then closes the transaction; Murph never holds a database transaction or member lock across provider calls. After the provider returns, `PrismaDeviceSyncControlPlaneStore.commitConnectionStart` locks the active member again and atomically replaces that marker with the callback state plus the optional `device_connection` seed. Seedless OAuth starts use the same lifecycle fence. If suspension wins before staging, provider I/O never starts. If suspension wins after staging, deletion fails retryably while the unexpired marker means provider creation may still be in flight; once the request resolves it removes the marker after exact cleanup, or deletion waits for expiry before resolving and removing the deterministic Junction owner upstream. A rejected start does not return a Link URL. If finalization wins, the seed and callback state commit before deletion can acquire the member lock, so deletion observes and removes both. Ordinary commit failures retain the marker instead of guessing that a provider user is newly created; the next successful same-provider start collapses older markers, and account deletion remains the destructive cleanup authority.
