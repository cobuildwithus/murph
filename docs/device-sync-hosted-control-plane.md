...
- `apps/web` is the canonical hosted control plane. It owns durable hosted device-sync facts in Postgres, including connection ownership, OAuth/session state, short-lived hosted connect intents, token-audit history, sparse sync signals, per-connection dirty state for webhook freshness, local-agent sessions, and the web-owned internal runtime snapshot/apply/connect-link/reconcile/Fitbit-cutover/dirty-state/pending/ack routes.
...
- request one verified Google Health Fitbit cutover for an exact connection
...
- `POST /api/settings/device-sync/connections/:connectionId/fitbit-migration/cutover`

These are the only browser-facing wearable connection start, completion, and migration-cutover routes. The settings start route resolves direct provider manifests and the connect-target catalog assembled by `@murphai/device-syncd/config`, so `/connect` can expose direct WHOOP/Oura/Strava targets plus Junction-backed Garmin and Google Health-backed Fitbit/Pixel Watch sources when those providers are configured. The first-party `/device/connect/:claim` route is the hosted assistant confirmation path: GET renders login/confirmation state without mutating provider OAuth state, and POST starts provider OAuth only for the authenticated member that owns the claim. Every hosted browser start compares the resolved callback base with the authenticated request hostname before creating shared ingress; `DEVICE_SYNC_PUBLIC_BASE_URL` may change the path, but a split hostname is an operator error because both callback credentials are host-only. Provider callback GET requires the callback proof and active session, passes the member as `expectedOwnerId`, and redirects only after shared ingress verifies the OAuth-state owner; a missing or invalid proof burns the OAuth state and redirects to Connect without mutating. The Fitbit migration route requires the active member session and mutation-origin proof, then invokes the same readiness-checked cutover owner used by the hosted runtime.

The Fitbit card is a product-facing source abstraction, not a second Google Health app card. Fresh Fitbit and Pixel Watch starts select Junction provider `google_health`; persisted provider `fitbit` remains the legacy provenance, status, and targeted-disconnect identity. While both source identities exist, Google Health canonical import stays fenced until its source-scoped historical job completes with representative summary data, Junction reports supported resources, and a fresh successor update arrives. Junction source authority comes from the complete inline payload, including nested and grouped records, rather than only the envelope hint. Mixed inline sources and source-unknown data events are retried before dirty or encrypted-payload acceptance during migration. A Google Health data event received while Fitbit still owns canonical admission stamps successor freshness and appends the existing deterministic reconcile wake under the connection lock, but provider trace completion and data acceptance wait for provider redelivery after Fitbit becomes terminal; lifecycle and data-less historical-complete events remain admissible. After the hosted runtime publishes that evidence, it calls one signed, exact-connection Web cutover operation; Web rechecks the durable source rows and reuses the existing targeted source-disconnect owner, so the browser is presentation and manual recovery rather than the migration lifecycle owner. The member's only required migration action is Google authorization even if the Connect page closes before verification finishes. A disconnect in progress keeps the fence closed, a failed disconnect restores the working legacy source, records a durable retry marker, and schedules the normal device-sync pass for a bounded retry. A successful disconnect admits only successor records strictly after the durable legacy cutover boundary. The control plane never treats provider capability metadata alone as historical-completeness evidence and does not infer proprietary Fitbit scores.
...
- `POST /api/internal/device-sync/fitbit-migration/cutover` on `apps/web`
...
4. The hosted job sends narrow runtime updates back through the signed internal web apply route. When those updates make a Google Health Fitbit successor locally eligible, the same pass calls a separate exact-connection cutover operation; Web rechecks durable readiness before any provider effect. At most four eligible cutovers are attempted per member pass, and a failure advances the existing local reconcile wake instead of adding a queue.
...
For accepted webhooks, provider trace completion means durable audit and dirty acceptance committed. Internal wake delivery is not allowed to force provider retry after that transaction commits. The pre-acceptance Google Health migration fence is deliberately different: under the connection lock it records successor freshness and a deterministic reconcile wake, leaves the provider trace and data work unaccepted, signals the existing runtime best-effort after commit, and returns retryable so provider redelivery can enter the ordinary path after Fitbit becomes terminal. Existing connection-established and disconnect wakes remain immediate lifecycle commands because they are explicit lifecycle commands, not high-cardinality freshness hints.
