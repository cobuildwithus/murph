...
Before provider I/O, every hosted Link, OAuth, and companion SDK connection-start path stages its own short-lived `device_oauth_session` lifecycle marker under the active `hosted_member` row lock, then closes the transaction; Murph never holds a database transaction or member lock across provider calls. After the provider returns, `PrismaDeviceSyncControlPlaneStore.commitConnectionStart` locks the active member again and atomically replaces the Link/OAuth marker with callback state plus the optional `device_connection` seed. The SDK path uses the same lock to atomically upsert its connection and consume only its own marker. Seedless OAuth starts use the same lifecycle fence. A direct-OAuth callback consumes its state under the active-member fence and retains that exact marker through authorization-code exchange, durable connection commitment, and established hooks. Provider denial or a missing authorization code is rejected before provider exchange and completes the marker; once exchange may have begun, a seedless failure without a returned connection or positive cleanup proof retains the consumed marker even past its original expiry. If suspension wins before staging, provider I/O never starts. If suspension wins after staging, deletion fails retryably while the unexpired marker means provider creation may still be in flight. A rejected request returns neither a Link URL nor an SDK token and retains its marker; generic suspension never guesses that the deterministic provider user was created by this request. Deletion waits for marker expiry before resolving and removing the Junction owner upstream. If finalization wins, the local connection and callback state or SDK marker consumption commit before deletion can acquire the member lock, so deletion observes the completed state. Concurrent starts never remove one another's markers, and account deletion remains the only owner of destructive provider cleanup.
