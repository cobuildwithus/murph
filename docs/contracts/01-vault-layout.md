...
- `.runtime/operations/assistant/generated-deliveries/<filename>` is the sole flat assistant-runtime ref exception for restart-safe one-time delivery residue. The same turn that establishes a send obligation may create and adopt one direct single-link regular file there before calling `send_vault_file` with a semantic provider call id; generated-file calls are serialized, and missing identity fails before adoption. Adoption tightens runtime parents to `0700`, the file to `0600`, and transfers the friendly source to its deterministic owned ref without clobbering an existing target. Exact active descriptors preserve the file in encrypted hosted checkpoints, while quiescent cleanup removes only terminal, changed, or orphaned direct files after the complete live inventory and outbox state are trusted; an orphan hardlink removes only the runtime-owned link, while an active hardlink fails closed. After an exact owned ZIP attachment reaches terminal `sent`, cleanup may derive a bounded retirement proof for `exports/packs/<packId>/` directories whose valid manifest-declared file set and bytes match both the sent archive descriptor and the current live derived pack. One interruptible unit inspects at most one archive and retires at most one pack; every active vault-file ref at or beneath that pack defers deletion, and the final outbox check, unchanged-pack revalidation, and removal serialize with outbox creation. The terminal ZIP remains the continuation until every eligible live pack is gone; hosted checkpoint cleanup and local assistant-daemon startup cleanup re-derive the next unit without persisted cursor state. Failed, denied, expired, malformed, missing, incomplete, changed, active-owned, or generic ZIPs grant no pack cleanup authority. Every other hidden vault-file ref remains invalid, and portable support bundles omit this path with the rest of `.runtime/**`. `exports/assistant-deliveries/**` is an ordinary, unreserved vault path.
