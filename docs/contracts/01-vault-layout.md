...
- Event shards use `occurredAt` and retain the logical path `ledger/events/YYYY/YYYY-MM.jsonl`. The current UTC month is plain JSONL; a closed month may instead have one lossless level-6 gzip representation at `ledger/events/YYYY/YYYY-MM.jsonl.gz`. Readers resolve both formats through the logical `.jsonl` path and fail closed if both representations exist. Event append operations and receipts stay representation-neutral; the canonical write owner resolves plain versus gzip from the logical path while holding its existing lock. Backdated writes preserve append receipts and hosted replay semantics by atomically replacing and verifying the gzip archive. The archive API is an explicit consumer-first activation seam and is not called automatically: deploy and drain gzip-capable core/query/runtime readers before creating the first event archive. That first archive makes the compatible reader release the rollback floor; recovery is a forward fix or a compatible lossless conversion back to plain JSONL before downgrade.
