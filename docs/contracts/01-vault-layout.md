...
- `.runtime/operations/assistant/generated-deliveries/<filename>` is the sole flat assistant-runtime ref exception for restart-safe one-time delivery residue. The same turn that establishes a send obligation may create and adopt one direct regular file there before calling `send_vault_file`; adoption tightens runtime parents to `0700` and the file to `0600`. Exact active descriptors preserve the file in encrypted hosted checkpoints, while quiescent cleanup removes only terminal, changed, or orphaned direct files after the complete inventory and outbox state are trusted. Every other hidden vault-file ref remains invalid, and portable support bundles omit this path with the rest of `.runtime/**`. `exports/assistant-deliveries/**` is an ordinary, unreserved vault path.
