...
- Integration-ingest shards use `importedAt`: `ledger/integration-ingests/YYYY/YYYY-MM.jsonl`. Device/provider evidence is retained inline in these append-only records with exact UTF-8 bytes, byte counts, SHA-256 hashes, receipts, logical roles, and canonical output ids. Ordinary and current incrementally filtered writes remain strict `murph.integration-ingest.v1` records; the deterministic incremental-evidence id identifies the latter without changing the v1 shape. Only a historical row rewritten by the Junction duplicate-evidence repair becomes `murph.integration-ingest.v2` with required `evidenceRetention: "filtered"`. That row intentionally retains only a proved subset of the received evidence; it preserves the ingest id, receipt, provenance, output ids, and counts, removes output-role links for removed parts, and must not be treated as complete-delivery evidence during replay. The repair may receipt-guard rewrite live JSONL rows into that state while preserving row order and identity and emitting one metadata-only audit; ordinary writers remain append-only. A reader that understands only the strict v1 shape cannot restore a vault after its first v2 row, making the v2-capable runner bundle a hard rollback floor unless a separate reverse migration is provided. Closed months may replace that logical JSONL shard with `ledger/integration-ingests/YYYY/YYYY-MM.jsonl.gz` or `ledger/integration-ingests/YYYY/YYYY-MM.jsonl.zip`; readers treat the uncompressed `.jsonl` path as the logical shard path. Portable data-bundle staging losslessly replaces closed raw JSONL representations with deterministic gzip archives when both compressed and uncompressed sizes fit the archive reader's bounds, preserves oversized and active-UTC-month shards as appendable JSONL, and explicitly includes existing canonical gzip/ZIP representations while continuing to exclude generic archives; the source vault is not mutated. Generic write paths must not mutate archived integration-ingest months, but the core integration-ingest append planner may explicitly amend an archived month by replacing the single physical archive representation after shard, id, size/hash, replay, and rollback checks pass. Each logical month must have exactly one physical representation, so archiving and amendments are replacement operations rather than copy-and-leave. ZIP archives are bounded before inflate and must contain exactly one entry whose name is the JSONL shard filename, with no path components.
