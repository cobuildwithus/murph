...
- `.runtime/operations/assistant/generated-deliveries/<filename>` is the sole flat assistant-runtime ref exception for restart-safe one-time delivery residue. The same turn that establishes a send obligation may create and adopt one direct single-link regular file there before calling `send_vault_file` with a semantic provider call id; generated-file calls are serialized, and missing identity fails before adoption. Adoption tightens runtime parents to `0700`, the file to `0600`, and transfers the friendly source to its deterministic owned ref without clobbering an existing target. Exact active descriptors preserve the file in encrypted hosted checkpoints, while quiescent cleanup removes only terminal, changed, or orphaned direct files after the complete live inventory and outbox state are trusted; an orphan hardlink removes only the runtime-owned link, while an active hardlink fails closed. After an exact owned ZIP attachment reaches terminal `sent`, the serial send path performs no optional archive work. One finite, interruptible quiescent sweep instead reads each present sent archive once and may retire only unchanged `exports/packs/<packId>/` directories whose complete direct file inventories and bytes match that archive's valid manifest. The sweep visits every archive and candidate once, skips already-missing completed archive refs, stops on present descriptor/path mismatches or unsafe refs, and scans past stale, oversized, or active-owned candidates and archives to reach later eligible work. After manifest and exact-inventory validation, a candidate whose central-directory aggregate exceeds the vault-file byte ceiling is terminally ineligible and is not inflated or hashed. Every active vault-file ref at or beneath a candidate defers only that candidate, and canonical materialization, explicit prune, final outbox checking, unchanged-pack revalidation, and removal serialize through the same assistant runtime write lock. The staged terminal ZIP and its exact sent outbox intent remain continuation evidence only while transient work remains, so ordinary terminal-outbox age and count pruning cannot strand cleanup and terminally ineligible candidates cannot retain the archive. Hosted idle cleanup invokes that sweep directly. Assistantd owns one ephemeral coalesced maintenance task that starts after its listener, aborts for authenticated foreground requests, and restarts after authenticated completion, including outbox drains; it adds no persisted cursor and does not put archive cleanup on the request result path. Failed, denied, expired, malformed, missing, incomplete, changed, active-owned, or generic ZIPs grant no pack deletion authority. Every other hidden vault-file ref remains invalid, and portable support bundles omit this path with the rest of `.runtime/**`. `exports/assistant-deliveries/**` is an ordinary, unreserved vault path.
