...
- Non-device raw imports are copied under stable type-specific folders in `raw/` and remain immutable in place, except for explicit receipt-guarded privacy/retention tombstones owned by canonical core mutations.
- Each non-device raw import directory also stores an immutable manifest sidecar with artifact checksums and import provenance. An explicit privacy/retention tombstone mutation may atomically replace both an artifact's content and the matching manifest integrity/provenance while retaining the historical path.
- Automatically captured meal JPEGs use that exception after the managed closeout inspects the image and reads back any supported structured meal update: the latest meal revision drops the photo attachment, while the historical raw path stores only a small `murph.automatic-meal-photo-tombstone.v1` JSON proof with a matching manifest artifact. The operation rejects non-automatic meals, missing/mismatched manifests, and changed image bytes.
