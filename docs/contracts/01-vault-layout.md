...
- Device/provider API snapshot imports use `ledger/integration-ingests/YYYY/YYYY-MM.jsonl` for live months, may read closed months from `.jsonl.br`, `.jsonl.gz` or `.jsonl.zip` archives, and do not create `raw/integrations` files or manifest sidecars.
