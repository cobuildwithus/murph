...
- `.runtime/operations/assistant/generated-deliveries/<filename>` is the sole flat assistant-runtime ref exception for restart-safe one-time delivery residue. It is non-canonical, private, and portable only through encrypted hosted workspace checkpoints; the compatibility phase can read already-persisted refs but does not yet let initial file-send preparation create them. Every other hidden vault-file ref remains invalid. `exports/assistant-deliveries/**` is an ordinary, unreserved vault path.
