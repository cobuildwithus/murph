...
- `.runtime/operations/assistant/generated-deliveries/<filename>` is the sole flat assistant-runtime ref exception for restart-safe one-time delivery residue. The same turn that establishes a send obligation may create and adopt one direct single-link regular file there before calling `send_vault_file` with a semantic provider call id; generated-file calls are serialized, and missing identity fails before adoption. Adoption tightens runtime parents to `0700`, the file to `0600`, and transfers the friendly source to its deterministic owned ref without clobbering an existing target. Exact active descriptors preserve the file in encrypted hosted checkpoints, while quiescent cleanup removes only terminal, changed, or orphaned direct files after the complete live inventory and outbox state are trusted; an orphan hardlink removes only the runtime-owned link, while an active hardlink fails closed. After an exact owned ZIP attachment reaches terminal `sent`, the serial send path performs no optional archive work. The existing quiescent generated-delivery owner instead drains bounded, interruptible units while progress remains and may retire only an unchanged `exports/packs/<packId>/` directory whose complete direct file inventory and bytes match the archive's valid manifest. Each unit inspects at most one present archive and removes at most one pack; it skips already-missing completed archive refs, stops on present descriptor/path mismatches or unsafe refs, and may scan past stale or active-owned pack candidates to reach a later eligible pack. Every active vault-file ref at or beneath a candidate defers that candidate, and the final outbox check, unchanged-pack revalidation, and removal serialize with outbox creation. The staged terminal ZIP and its exact sent outbox intent remain continuation evidence until every candidate is complete, so ordinary terminal-outbox age and count pruning cannot strand cleanup. Hosted idle cleanup and recurring local runtime maintenance re-derive work without persisted cursor state; assistantd admits requests before its startup pass and cancels that pass for foreground work. Failed, denied, expired, malformed, missing, incomplete, changed, active-owned, or generic ZIPs grant no pack deletion authority. Every other hidden vault-file ref remains invalid, and portable support bundles omit this path with the rest of `.runtime/**`. `exports/assistant-deliveries/**` is an ordinary, unreserved vault path.
