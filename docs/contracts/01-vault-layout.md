...
- `exports/assistant-deliveries/**` is reserved, non-canonical staging for files the assistant creates solely for one-time outbound delivery. Existing user files, canonical records, and durable work stay at their owning paths and must not be moved or copied here merely to send them. The reserved prefix is included in hosted checkpoints while a file's ref, filename/content type, size, and SHA-256 match an active vault-file send, approval, retry, or delivery-confirmation obligation; quiescent pre-checkpoint cleanup may delete only unmatched regular files and empty directories inside this exact prefix.
