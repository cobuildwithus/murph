...
- `raw/inbox/**` is the exception: `ledger/inbox-captures/YYYY/YYYY-MM.jsonl` is the sole committed metadata owner, while `raw/inbox/**` retains attachment bytes that need raw storage and a hash/size-verified `text.txt` only when capture text exceeds the 20,000-character inline projection. Capture text is capped at 64 MiB total. The explicit legacy-envelope owner migration also retains `text.txt` at exactly 20,000 characters because the legacy projection cannot prove whether more text existed; it verifies every non-retained attachment receipt and rejects an existing current owner before deleting the envelope. Current `murph.inbox-capture.v2` records carry the complete sanitized envelope metadata plus content references, so new captures do not write a redundant `envelope.json` or a generic raw-import manifest. Image attachment bytes are normalized to bounded static WebP before they are written here or left unstored, and canonical raw metadata drops size-like provider fields instead of retaining original byte sizes. Raw inbox image/audio/video bytes may expire after 14 days when `ledger/inbox-attachment-retention/YYYY/YYYY-MM.jsonl` records the deleted path, sha256, purge time, reason, and retained parser derivative. A promoted document's duplicate inbox bytes may expire on the same schedule only when its stable capture/attachment correlation points to one live canonical document owner and the source bytes, import audit, event, manifest, and canonical raw artifact still agree; every missing correlation, mismatch, missing materialization, damaged proof, active protection, or durable raw reference retains the inbox copy. The correlation contains IDs only and is written to the existing audit stream when default promotion succeeds, so later text retention cannot strand an otherwise eligible duplicate. Promoted documents use the same bounded admission limits as media, and their admitted receipts share one transient audit/event ledger snapshot for proof. The existing write batch receives each verified source receipt and rechecks it at the final delete mutation, so bytes changed after admission are not removed. Long-form text content is canonical message content and does not use attachment retention.
...
- Event shards use `occurredAt` and retain the logical path `ledger/events/YYYY/YYYY-MM.jsonl`. Current and future UTC months remain untouched by archiving. Closed months use one lossless quality-5 Brotli representation at `.jsonl.br`; existing `.jsonl.gz` remains readable. Event append receipts are representation-neutral: the canonical write owner resolves the physical source under its existing lock, and backdated writes atomically replace and verify that archive.
- Integration-ingest shards use `importedAt`: `ledger/integration-ingests/YYYY/YYYY-MM.jsonl`. Provider evidence remains inline with exact UTF-8 bytes, byte counts, SHA-256 hashes, receipts, logical roles and canonical output IDs. Closed months and active months compacted by idle maintenance use `.jsonl.br`; gzip and legacy single-entry ZIP reads remain supported. Generic writes cannot mutate archived months; the integration append planner may amend the single archive only after shard, ID, size/hash, replay and rollback checks. Gzip/Brotli reads and amendments stream bounded decompressed chunks; legacy ZIP remains bounded and may buffer its single entry. ZIP entries must be the exact JSONL shard filename without path components.
- Hosted idle-shutdown maintenance converts valid closed plain and gzip shards to Brotli within its existing shared 30-second archive budget. It streams source and verification reads, validates every row and the exact complete decompressed size/SHA-256 receipt, checks that the source is unchanged, atomically publishes the archive, then removes the source. Event current/future months stay untouched. Integration current-month plain shards become eligible at 4 MiB; smaller current shards and future months stay untouched. Later imports reuse the existing verified archive-amendment writer. If a gzip/Brotli amendment exceeds the compressed or uncompressed archive cap, it first publishes the byte/hash-verified identical plain base, removes the archive, and uses the ordinary receipt-backed append. Exact-duplicate recovery covers interruption before removal; ordinary append replay and rollback cover interruption afterward. Archive caps never become a new limit on a growing plain journal. Foreground wakes abort the pass. Remaining old representations are the next pass's worklist. Exact interrupted raw/gzip/Brotli duplicates converge only after all copies independently validate and match; mismatches remain fail-closed conflicts. Canonical integration readers and amendment planning repair only an encountered duplicate current or closed shard under the existing canonical write lock. They re-enumerate its representations after acquiring the lock and reuse the same complete validation before removing an identical copy. Conflicts and future-month duplicates remain unchanged and fail closed; ordinary hosted startup performs no archive inventory or repair pass. Portable data-bundle staging preserves existing archives and still gzip-compresses closed raw integration shards without mutating the source vault.
...
- `.runtime/operations/assistant/generated-deliveries/<filename>` is the sole flat assistant-runtime ref exception for restart-safe one-time delivery residue. The same turn that establishes a send obligation may create and adopt one direct single-link regular file there before calling `send_vault_file` with a semantic provider call id; generated-file calls are serialized, and missing identity fails before adoption. Adoption tightens runtime parents to `0700`, the file to `0600`, and transfers the friendly source to its deterministic owned ref without clobbering an existing target. Exact active descriptors preserve the file in encrypted hosted checkpoints. Snapshot construction removes runtime-owned operator-home symlinks before materializing deferred skipped-inline files, then runs generated-delivery cleanup independently before unrelated pending-input compaction and broad residue maintenance. That pass removes only terminal, changed, or orphaned direct files after the complete physical inventory and outbox state are trusted; archive planning cannot reintroduce removed residue. An orphan hardlink removes only the runtime-owned link, while an active hardlink fails closed. Every other hidden vault-file ref remains invalid, and portable support bundles omit this path with the rest of `.runtime/**`. `exports/assistant-deliveries/**` is an ordinary, unreserved vault path.
...
- Device/provider API snapshot imports retain logical `ledger/integration-ingests/YYYY/YYYY-MM.jsonl` paths and may read current or closed months from `.jsonl.br`, `.jsonl.gz` or `.jsonl.zip` archives, and do not create `raw/integrations` files or manifest sidecars.
...
- `raw/inbox/**` may contain canonical attachment bytes without manifest sidecars; its owning current inbox-capture ledger record supplies the metadata boundary. Image/audio/video bytes and exact promoted-document duplicates intentionally expired by retention leave the ledger record intact and are represented by `ledger/inbox-attachment-retention/**` so readers can surface `retention_expired` instead of corruption. Media tombstones remain `murph.inbox-attachment-retention.v1`; document tombstones use `murph.inbox-attachment-retention.v2` with the verified byte size and document-specific reason.
...
Deploy and drain the compatible-reader release across every workspace consumer
before enabling Brotli publication or migrating existing gzip archives. Once Brotli
is written, that reader release is the rollback floor; downgrade requires an
explicit lossless conversion first. Compatibility readers remain until retained
snapshots and supported imports no longer contain those representations.
