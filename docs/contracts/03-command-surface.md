...
An explicit recognized `--source strong|hevy` selects that parser dialect; header inference is used only when the option is absent. Common workout headers are intentionally not treated as Strong-only. If the explicit source conflicts with unambiguous provider-specific headers, inspection and import fail before raw or canonical persistence.
