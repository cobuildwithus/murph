...
Assistant personality settings remain canonical product state in `bank/preferences.json`, owned by core preference usecases. They are intentionally absent from the raw CLI so no registered general command advertises an audience-independent path to them. Private direct conversation turns receive the exact-turn `murph.assistant_style` operation. Authenticated non-direct Linq turns receive only the `murph.group` `read_own_assistant_style` and `update_own_assistant_style` actions: the runtime injects the accepted current sender after model parsing, the request accepts no member selector, and Web may read or update only that resolved active member's tone, voice, Humor, Push, and Detail. Email, indeterminate, ambiguous, and unrecognized senders fail closed. The group-origin update appends through the existing member preference/mailbox owner and affects future private member turns, never the room prompt. Hosted private-direct mutations obtain the current provider batch's runtime-owned causal frontier, equal to the newest exact-successor sequence admitted before provider start; the model cannot supply or rewrite that number. Per-field watermarks live in the bounded companion `bank/assistant-preference-mutations.json` and are committed by the same owner. This is a registered-tool and prompt-surface policy, not a filesystem sandbox around the privileged Codex runtime. The first personality-aware reader/writer release remains the rollback floor because older strict readers can reject `assistant.personality`.
