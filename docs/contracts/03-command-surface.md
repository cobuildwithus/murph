...
- `vault repair-junction-evidence-duplicates` dry-runs by default and mutates only with explicit command-line `--apply`. It inspects bounded live integration-ingest and event history and filters only same-month Junction device evidence for an identified account when an earlier valid row already proves the exact provider/account/role/content/media/metadata fingerprint and every event association. Missing, malformed, repeated, or revised event spines protect the associated evidence; changed evidence, new associations, receipts, provenance, row ids, output ids, and counts are preserved. Apply receipt-guards each live JSONL rewrite, changes affected rows to `murph.integration-ingest.v2` with required `evidenceRetention: "filtered"`, removes only the matching output-role links, and writes a metadata-only audit. Replay reuses the append planner's one authoritative full-shard id-and-evidence proof scan when bounded novelty inspection misses; neither the marker nor the existing row id alone authorizes a no-op. Archived shards are never rewritten by this command and are explicitly reported in `skippedArchivedShardCount`.
...
vault-cli vault repair-junction-evidence-duplicates --vault <path> [--dry-run] [--apply] [--max-ingest-shards <count>] [--max-ingest-bytes <bytes>] [--max-event-rows <count>] [--request-id <id>]
