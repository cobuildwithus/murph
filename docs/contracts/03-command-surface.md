...
Workout CSVs do not carry a source revision. Murph assigns a dated importer-mapping revision to the canonical result: an unchanged replay is a no-op, while changed content for the same source session fails closed instead of guessing which version is newer. The planner exposes a provider-neutral, privacy-safe session key derived from the raw source timestamp; the public identity remains provider-scoped. For a refreshed snapshot, the usecase boundedly verifies up to 100 prior workout batches and 50 MiB of raw artifacts, replans them with recorded provenance, and resolves their attached live events through one core lookup. Overlapping session keys reuse those authoritative external references and current canonical payloads; only genuinely new sessions receive new identities and attach to the new raw snapshot. Conflicting, partial, over-limit, changed, or ambiguous overlap fails before raw or canonical writes.
