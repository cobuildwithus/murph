...
Assistant personality settings remain canonical product state in `bank/preferences.json`, owned by core preference usecases. They are intentionally absent from the raw CLI because an assistant shell could bypass conversation audience authority through any registered command. Private direct conversation turns receive the exact-turn `murph.assistant_style` operation instead. The first personality-aware reader/writer release remains the rollback floor because older strict readers can reject `assistant.personality`.
