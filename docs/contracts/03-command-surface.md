...
`workout import csv` validates the complete structured batch before writing. A malformed mapped workout rejects the whole structured import without storing its raw batch. A valid import stores one immutable raw CSV and manifest, then commits all canonical `activity_session` decisions through one batch operation. An invalid, over-range, or absent source duration is omitted with an aggregate warning while otherwise valid exercises and sets remain importable. Replaying the same source sessions returns skipped-existing counts and does not store another raw copy. Response identifier and ledger-path lists are capped at ten entries with explicit truncation flags, so large history imports do not expand assistant context in proportion to the workout count.
