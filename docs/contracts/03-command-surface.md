...
- In hosted runtime, `device account show|reconcile|disconnect` use the signed hosted bridge and the same web-owned connection authority as browser settings. `disconnect --confirm --expectedConnectedAt <shown-connectedAt>` is accepted only after conversational approval of the shown connection scope and only while that exact connection epoch remains current; for a Junction account, approval covers the entire shared Junction connection and every upstream wearable source on it. Lease activation is a two-web-release cutover: the first source release deploys every lease-aware writer guard while production disconnect remains lease-less, then the exact production alias must remain current through the complete prior-function drain before a separate source release activates claims. Once activated, the canonical owner leases that approved connection epoch before upstream revoke, makes the refresh and disconnect leases mutually exclusive, rejects token export/reconnect/reconcile/runtime connection writes and local heartbeats while any disconnect lease remains unresolved, and consumes the lease in the same transaction that commits the terminal disconnect result. Provider revoke has a web-owner subdeadline below the disconnect lease lifetime and hosted control-plane deadline, and cancellation propagates through the runner proxy to every provider request; an upstream timeout stops further source requests, then the canonical owner commits the disconnected state and returns the existing revoke warning instead of letting transport timeout obscure the result. An expired non-null disconnect lease is ambiguous provider-effect evidence: a retry adopts it without replaying provider revoke, commits the disconnected state plus manual-removal warning and durable receipts, and only then permits reconnect. The durable mailbox item owns post-commit recovery, so a stalled Temporal wake cannot obscure the committed receipt. The result preserves upstream revoke warnings and `historicalResetIncomplete` manual-removal guidance.
