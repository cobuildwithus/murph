...
- `vault` exposes `show | stats | repair | repair-junction-hr-zones | update`.
...
- `audit show|list|tail` and `vault show|stats|repair|repair-junction-hr-zones|update` are first-class vault noun commands layered on top of the read model and core metadata write/repair paths.
