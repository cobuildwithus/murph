...
vault-cli medication save <title> --vault <path> [--id <regimenId>] [--slug <slug>] [--status <status>] [--started-on <date>] [--stopped-on <date>] [--schedule <text>] [--substance <text>] [--dose <number>] [--unit <unit>] [--group <text>] [--note <text>] [--related-goal-id <id> ...] [--related-condition-id <id> ...] [--related-regimen-id <id> ...] [--request-id <id>]
...
`medication save` and `medication history add` are medication-only facades over the private regimen registry. They create `kind: medication` regimen records; `medication history add` defaults to `status: completed`, `group: medication/history`, and a date-qualified slug derived from the title plus `startedOn`/`stoppedOn` so repeated historical courses do not collide or become active regimens or point-in-time intake events.
