...
vault-cli assistant style show --vault <path>
vault-cli assistant style set <humor|push|detail> <0-10> --vault <path>
vault-cli assistant style reset <humor|push|detail|all> --vault <path>
...
`vault-cli assistant ask|chat|deliver|status|doctor|run|stop|session` persist or inspect assistant runtime state only. `vault-cli assistant style` is the narrow canonical exception: it reads or mutates the member's typed Humor, Push, and Detail preferences in `bank/preferences.json` through the core preference owner. Hosted style mutations obtain the active turn's exact mailbox sequence from the authenticated local runtime bridge; the model cannot supply or rewrite that number. Per-field watermarks live in the bounded companion `bank/assistant-preference-mutations.json` and are committed by the same owner. Durable user-facing memory is managed through the top-level canonical `memory` noun backed by `bank/memory.md`, and durable scheduled assistant prompts are managed through the top-level canonical `automation` noun backed by `bank/automations/*.md`. Accepted inbound assistant-automation captures may still auto-preserve stored document attachments into the canonical document import surface before reply behavior runs, while leaving the original inbox capture evidence in place under `raw/inbox/**`. Coarse system-written turn receipts, replay-safe outbox intents, diagnostics snapshots, persisted status snapshots, and other assistant runtime artifacts live under `vault/.runtime/operations/assistant/**` for read-only `status` / `doctor` inspection. Session JSON keeps only public provider headers; secret-bearing provider headers live in private sidecars under `vault/.runtime/operations/assistant/secrets/**`, and `assistant doctor --repair` can tighten permissive runtime modes in place. Assistant ask/chat and the root `chat` shorthand are Codex App Server surfaces only: they accept Codex launch/config overrides such as `--codexCommand`, `--codexHome`, `--modelProvider`, `--reasoningEffort`, `--sandbox`, `--approvalPolicy never`, and `--profile`. Assistant-originated writes are rebound to the real host-side user turn instead of trusting client-supplied provenance text, and the canonical vault remains authoritative.

`assistant style show` returns all three effective scores with `source: default|custom` and never writes. `assistant style set` accepts one fixed setting id plus one integer from 0 through 10. It preserves an explicit choice even when it equals the product default. `assistant style reset` removes one override or all personality overrides. Every style result includes `vault`, `preferencesPath`, `updated`, `recordedAt`, and the complete effective `settings` snapshot. A no-op reports `updated: false`; callers must not claim a change from that result. Invalid scores and setting ids fail instead of clamping or partially writing.

Deploy compatible strict preferences readers before enabling `assistant style set` or `reset`. Once a personality override has been stored, binaries that predate `assistant.personality` can reject the canonical document. The first compatible reader/writer release is the rollback floor unless a current compatible binary removes the field through this canonical command surface.
...
The existing assistant runtime commands remain runtime inspection/control only. `assistant style` is the explicit canonical product-state exception. Any other future user-facing, queryable, or durable assistant setting must pass the persisted-state placement gate: canonical product state is not an `assistant` runtime CRUD surface, so it must use a core-owned canonical record or explicit derived materialization.
...
- In hosted runtime, `device account show|reconcile` use the typed hosted bridge and web-owned connection authority. Hosted `device account disconnect` is unavailable during the disconnect-lease guard rollout and fails before any control-plane or provider request. The authenticated browser settings flow remains the existing disconnect entrypoint and intentionally stays on its compatible lease-less target-compare path in this first release so a safety change does not disable a user-critical flow.
- A later hosted-disconnect activation must not treat a model-supplied flag or repeated text as approval. It requires trusted, later accepted member input bound to the member, action, connection id, shown `connectedAt` epoch, and—for Junction—the entire shared connection plus every upstream wearable source; the canonical owner must consume that approval atomically with the matching mutation authority. The same activation must preserve pending or ambiguous provider effects through a typed, sanitized error envelope across web, Cloudflare, hosted runtime, and CLI, then permit one bounded post-expiry recovery that commits terminal disconnected state and durable receipts without replaying provider revoke. Only after those prerequisites exist may a separate web release activate disconnect-lease claims and a later runner release expose hosted disconnect.
- Lease activation remains a two-web-release cutover: this first source release deploys every lease-aware writer guard while production browser disconnect remains lease-less, then the exact production alias must remain current through the complete prior-function drain before a separate source release activates claims. Once activated, the canonical owner leases the approved connection epoch before upstream revoke, makes refresh and disconnect leases mutually exclusive, rejects token export/reconnect/reconcile/runtime connection writes and local heartbeats while any disconnect lease remains unresolved, and consumes the lease in the same transaction that commits the terminal result. The writer-guard release is the rollback floor while any non-null lease evidence can exist.
