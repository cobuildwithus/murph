...
- `vault compact-inbox-parser-attempts` dry-runs by default and mutates only with explicit command-line `--apply`. It reconstructs one versioned parser result bundle from an exact legacy attempt, verifies identity, paths, schema, and semantic equality, then deletes only the proved sidecars. Apply is bounded to 100 eligible attempts; conflicts, incomplete attempts, symlinks, and unexpected entries remain untouched and are reported as counts.
