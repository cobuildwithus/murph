...
The confirmed `--weight-unit` applies to every positive unitless load field, including bodyweight, assistance, and added weight. Unit-bearing values and headers remain authoritative; conflicts block the batch. Auxiliary loads are converted to canonical kilograms before persistence. If a member previously confirmed the wrong unit, rerun the exact original CSV with the corrected unit and `--correct-units`; this explicit route requires exact prior raw evidence, reparses with the original evidence timezone, proves that non-unit session and set fields remain unchanged, assigns a newer correction revision, supersedes the same event identities atomically, and stores no duplicate raw batch. It cannot be combined with `--store-raw-only`. If the original timezone cannot be proved from the attached event or manifest, correction fails before mutation.

Workout CSVs do not carry a source revision. Murph assigns a dated importer-mapping revision to the canonical result: an unchanged replay is a no-op, while changed content under the same mapping revision fails closed instead of guessing which version is newer. A deliberate parser correction bumps that mapping revision so the corrected canonical event can supersede in place. New imports use a privacy-safe timestamp-derived identity. Before selecting identities, the usecase finds prior workout manifests whose source artifact matches the submitted bytes by length and SHA-256, regardless of the manifest's prior provider label, verifies the raw bytes again, and asks core for the live workout events already attached to that evidence. Those events retain their authoritative external references and are updated in place; a current-mapping replay remains a no-op even if the vault timezone preference later changes. This also lets an exact ambiguous file previously mislabeled Strong adopt explicit Hevy semantics without creating a second history. An exact raw batch with a partial, conflicting, or ambiguously ordered event set blocks instead of guessing, duplicating history, or writing another raw copy.
