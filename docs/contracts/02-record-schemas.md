...
`encounter` events may carry visit-scoped clinical context: `clinician`, `facility`, `reasonForVisit`, `assessmentText`, `planText`, `instructionsText`, `followUpText`, and `diagnoses[]`. `diagnoses[]` is encounter-scoped only; it does not automatically promote durable `condition` records. Visit facts such as vitals, ordered procedures, and tests stay as linked canonical events using `links[]` with `type: "related_to"` back to the encounter and shared `rawRefs` for provenance.
