...
- Assistant-engine owns one reusable Codex App Server slot per Node runtime/container for the current stable process identity. A turn is an RPC into that process; clean successful turns leave it idle, while overlapping direct turns fail busy instead of creating a process map. Identity/config mismatch, abort cleanup, malformed active-turn output, parent-thread output/tool/terminal state that cannot be scoped to the active turn, process failure, or idle explicit shutdown stops or poisons it before a later turn can reuse it. Typed idle lifecycle/global/foreign-thread notifications may be tolerated off-turn only when they cannot mutate later-turn output or execute parent-turn tools.
