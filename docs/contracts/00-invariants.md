...
- PR 465 usage-limit Telegram notice rollout is a three-web-release handoff. First deploy PR 495 so no active web binary clears the period marker, prove Vercel's production alias points at that exact commit, and drain prior functions. Then deploy PR 501 so the legacy direct Telegram sender retains ambiguous post-dispatch claims, prove and drain that exact commit, and keep it as the rollback floor. Only then merge PR 465 and deploy its Worker from protected `main` with the normal full-gate `pnpm cf:deploy` workflow while Vercel builds the new web release; the Vercel predeploy wrapper applies the additive `retryAfterAt` migration before promotion. The repository-wide deploy currently defaults `container_rollout=immediate`, but this PR does not change the runner bundle or add a separate runner-convergence requirement. Worker-first skew is inert. If new web reaches an old Worker without the route, web records a retryable pre-provider failure and can reclaim the exact delivery later. Mixed prerequisite/PR 465 web is safe after both prerequisite drains because no active release clears an ambiguous marker. One transaction reads current and legacy keys, selects the sole executable delivery, claims the period marker as the generation/rollback fence, and creates or reclaims the delivery row before dispatch; acceptance later advances only that exact delivery row. A legacy row is executable only for the Linq source that can reuse its provider idempotency key; cross-source, Telegram-source, multiple-legacy, current-plus-legacy, marker-only, provider-correlated, and terminally ambiguous states suppress a new send. Only failures proven pre-provider and explicit Telegram rate limits are durably retried; generic post-dispatch control errors and provider 5xx responses are terminally ambiguous. After the rollback window closes and production has no recent legacy-key attempts, complete the deletion tracked in [issue 499](https://github.com/cobuildwithus/murph/issues/499). Keep the Worker route and PR 501 rollback floor until PR 465 web instances have drained.
