...
- PR 465 usage-limit Telegram notice rollout: apply the additive web schema migration first, configure `HOSTED_TELEGRAM_USAGE_LIMIT_NOTICE_AUTHORITY_SECRET` in both hosted web and the Cloudflare Worker, deploy the Worker with the hosted-control `telegram/usage-limit-notice` route before deploying hosted web that calls it, then deploy hosted web after old web instances have drained from the hosted usage-denial path. During web-before-Worker or missing-authority-secret skew, route/control failures record retryable unavailable delivery rows and do not mark the period sent. New web selects exactly one executable delivery row for each notice send: the current period key when claimable, otherwise an already-owned claimable legacy notice-code key. Legacy notice-code rows are compatibility inputs only; new web does not create guard rows or reserve every legacy key. Do not run mixed old/new hosted-web quota-notice senders concurrently, because pre-PR old web only honors `limitNoticeSentAt`. No runner-container bundle change is required and `container_rollout=immediate` is not required. Roll back web before rolling back the Worker route or shared authority secret, and keep the Worker route until new web instances have drained.
