...
- Codex config file content is not part of the warm launch key. Only per-thread-safe config values may vary while a warm app-server process is reused; process-scoped config values must remain stable or become part of process launch identity.
