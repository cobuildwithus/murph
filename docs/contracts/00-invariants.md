...
- Native Codex thread resume must receive the current non-instruction execution context through RPC, including cwd, model/provider, sandbox, and approval policy. Config-backed model/provider defaults are resolved before each turn so resume validation compares against the current effective context. Resume state is continuity, not permission to keep stale execution policy from the original thread or process launch.
- Whole Codex config file content is not part of the warm launch key. Per-thread-safe config values such as model selection may vary while a warm app-server process is reused. Provider table authority such as endpoint, env key, or wire API is launch-affecting identity; changing it under the same provider id must restart the app-server process.
