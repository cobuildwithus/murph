...
- PR 465 usage-limit Telegram notice rollout is a two-web-release handoff. First deploy the marker-preservation prerequisite from PR 495, prove that Vercel's production alias points at it, wait for prior web functions to drain, and keep that release as the rollback floor. Then merge PR 465 and deploy its Worker from protected `main`, preferably before Vercel promotes the new web deployment; the Vercel predeploy wrapper applies the additive `retryAfterAt` migration before that promotion. Worker-first skew is inert. If new web reaches an old Worker without the route, web records a retryable pre-provider failure and can reclaim the exact delivery later. Mixed prerequisite/PR 465 web is safe after the prerequisite drain because neither release clears the period marker. One transaction reads current and legacy keys, selects the sole executable delivery, claims the period marker as the generation/rollback fence, and creates or reclaims the delivery row before dispatch; acceptance later advances only that exact delivery row. A legacy row is executable only for the Linq source that can reuse its provider idempotency key; cross-source, Telegram-source, multiple-legacy, current-plus-legacy, marker-only, provider-correlated, and terminally ambiguous states suppress a new send. Only failures proven pre-provider and explicit Telegram rate limits are durably retried; generic post-dispatch control errors and provider 5xx responses are terminally ambiguous. After the rollback window closes and production has no recent legacy-key attempts, complete the deletion tracked in [issue 499](https://github.com/cobuildwithus/murph/issues/499). No runner-container bundle change is required and `container_rollout=immediate` is not required. Keep the Worker route and marker-preserving rollback floor until PR 465 web instances have drained.
