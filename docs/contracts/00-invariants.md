...
- The company-wide fulfilled usage-top-up lifetime total increments from the
  first successful purchase-status transition to fulfilled, inside that
  transaction. It is one anonymous count with no member, purchase,
  payment-provider, event, or timing reference; fulfillment replay and rollback
  cannot increment it, and account deletion cannot decrement it.
