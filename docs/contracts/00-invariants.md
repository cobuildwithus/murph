...
- Hosted process cleanup may exempt only the verified Codex App Server root process. If packaging adds a launcher wrapper, collapse it at image build or launch time so the verified root is the long-lived app-server process; do not wildcard direct children as Codex-owned.
