...
- When closed-day authority spans a bounded multi-day horizon, use the existing durable job queue and history as the completion ledger: process the newest useful day inline, enqueue stable resource/day jobs newest-first, reuse succeeded populated or empty work across restarts, and let ordinary retry/dead-letter behavior own failures. Do not add a second cursor, lifecycle service, retained sample stream, or provider-specific completion table.
