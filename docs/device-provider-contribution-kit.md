...
- A closed-day derived fact must use one explicit vault timezone from scheduling through fetch and normalization. Request an exact half-open datetime window, wait for any documented safety lag, and grant `authoritativeEventSets` replacement only for resources whose fetch succeeded. Account-wide job success is not proof that a resource-specific authoritative pull ran. Absolute timestamps may be filtered as instants; floating or date-only timestamps may be filtered by their raw calendar day only under complete local-day authority and must not be interpreted through the worker process timezone. Successful empty responses may replace an owned facet set; failed, aborted, or yielded responses must not.
- When closed-day authority spans a bounded multi-day horizon, use the existing durable job queue: process the newest useful day inline, enqueue stable resource/day jobs newest-first, deduplicate queued or running work across restarts, and let ordinary retry/dead-letter behavior own failures. Keep succeeded rows as execution history rather than permanent completion proof when source topology or provider availability can widen. Let the next bounded scheduled pull converge through authoritative replacement instead of adding a second cursor, lifecycle service, retained sample stream, provider-specific completion table, or topology generation.
