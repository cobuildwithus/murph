...
4. Settings vault export uses the same encrypted browser-vault session machinery as dashboard reads through `POST /api/settings/vault-export/session`. It additionally requires the one-time authorization and refuses to release a session unless the underlying browser-vault replica is fresh and no device-sync refresh is still pending, so a stale or in-flight replica cannot be downloaded as a "complete" export and the user's one-time challenge is not consumed on a stale response.
