...
2. Suspend the hosted member for account deletion, resolve any staged wearable connection-start markers, then best-effort terminate the per-user hosted Temporal runtime workflow with reason `account-deleted` before provider revocation, billing cancellation, or local row deletion starts. A hosted start stages its marker under this same member-row lifecycle lock before provider I/O and replaces it with the optional connection seed plus callback state under the lock afterward. Deletion therefore either prevents provider I/O, observes the completed local rows, or retains retry authority for in-flight upstream cleanup.
