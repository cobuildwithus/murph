...
4. Remove wearable/device provider access before local device rows are deleted. Account deletion uses a provider's destructive `deleteAccount` hook when available and otherwise falls back to `revokeAccess`. For a staged start without a committed connection, Junction derives the same blinded owner key used for creation, resolves the provider user, and deletes it; an unexpired marker with no resolved user fails retryably because creation may still be in flight, and a provider cleanup failure retains the marker for the next deletion attempt. Ordinary device disconnect remains per-source deregistration. Providers without either hook remain local-reference deletion only.
