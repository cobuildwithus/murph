...
- Assistant-captured product feedback rows with published changelog item ids and safe row metadata, while omitting internal feedback ids.
...
| `prisma.hosted_product_feedback` | Live delete | Confirmed data export | Deletes assistant-captured product feedback rows. Confirmed export includes published changelog item ids and safe row metadata while omitting internal feedback ids. |
