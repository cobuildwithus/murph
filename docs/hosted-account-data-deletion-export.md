...
Settings supports private vault export and canonical account deletion. Vault export requires a fresh one-use, session-bound WebAuthn assertion with user verification, then downloads the decrypted browser-vault replica JSON that powers dashboard pages. Deletion requires the current Murph session, same-origin admission and an exact typed confirmation phrase; it stays available without an approval passkey. Deletion cancels Stripe billing and owns external cleanup through a durable encrypted receipt. Historical Privy obligations must converge before the retirement release, as defined in `hosted-auth-migration.md`.

## User-facing entry points

- `/settings` includes a **Data & privacy** section.
- **Export vault** opens a confirmation dialog that requires a sensitive-data acknowledgement, creates a short-lived authorization challenge, signs it with an enrolled approval passkey, then loads the current browser-vault replica through `/api/settings/vault-export/session` and downloads it as JSON in the browser.
- **Delete account** explains that deletion is permanent and requires the exact phrase `DELETE MY ACCOUNT`. It uses the current authenticated Murph app session without creating or signing a passkey challenge, and remains available if secure approval is unavailable while vault export stays disabled. After success the Settings page shows a short confirmation and redirects home; the hosted session is revoked server-side.

## Security model

Account deletion is intentionally deliberate but does not introduce a separate passkey identity requirement. The current authenticated Murph app session identifies the member, browser mutation-origin protection blocks cross-origin admission, and the exact typed phrase records explicit destructive intent before any vendor cancel, Prisma delete, or vendor-account cleanup runs. Vault export remains an explicit user-intent confirmation gesture for the bulk JSON download, not a stricter session-trust upgrade: an active hosted session can already read the same encrypted browser-vault replica through `POST /api/browser-vault/session` to render the dashboard, so the user-verified passkey assertion on export protects against confused-deputy and forced-action paths (browser extensions, embedded iframes, accidental UI activation) rather than against an attacker who already has the live session cookie.
...
2. Read the exact encrypted Stripe target rows and decrypt them in one
...
| `prisma.hosted_auth_record` | Member cascade | Not exported secret | Deletes member-bound login users, accounts and sessions. Independent pre-member verification/rate-limit records expire through bounded retention. No login record or bearer is exported. |
| `prisma.hosted_member_approval_credentials` | Member cascade | Not exported secret | Deletes the encrypted approval credential aggregate and saved recovery-key digest. |
| `prisma.hosted_sensitive_action_challenge` | Live delete | Not exported secret | Deletes hashed authorization challenges and durable Assistant approval decisions stored in the same member-scoped table. Raw tokens, signatures, action hashes, and approval material are never exported. |
| `prisma.hosted_member_identity` | Live delete | Confirmed data export | Deletes encrypted contact hints. Confirmed export includes user-facing phone/contact fields and omits lookup keys and active phone-code attempt IDs. |
...
| `providers.stripe` | Best-effort delete | Documented only | Cancels the Stripe subscription before local deletion (fail-closed), then deletes the Stripe customer through receipt-owned retry after the local wipe. Outcomes are reported in the deletion result. |
...
Stripe vendor accounts are actively deleted by the deletion workflow itself: the subscription is canceled fail-closed before the local wipe, and the encrypted deletion receipt retains retry ownership for the customer afterward. Local usage-credit purchase rows and their encrypted Stripe references are deleted with the account. Stripe retains records it is legally required to keep (for example invoices and payment records) under its own documented processes after the customer object is deleted.

The owner and owned thread-container members are locked and suspended before
the external-target snapshot is prepared. Runtime and direct/Family Stripe
relationship writers serialize on that lifecycle fence and reject a
suspended owner; the destructive transaction re-reads the complete target set
under the canonical group/Family/member lock order and aborts if it changed.
The retirement gate resolves historical provider receipts before removing their
lookup columns; new receipts retain independent target progress and retry ownership.
