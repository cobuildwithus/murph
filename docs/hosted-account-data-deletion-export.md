...
2. Suspend the hosted member for account deletion, resolve any staged wearable connection-start markers, then best-effort terminate the per-user hosted Temporal runtime workflow with reason `account-deleted` before provider revocation, billing cancellation, or local row deletion starts. Every hosted Link, OAuth, and companion SDK start stages its own marker under this same member-row lifecycle lock before provider I/O. Link/OAuth replaces only that marker with the optional connection seed plus callback state; a direct-OAuth callback retains its consumed marker through provider exchange and local commitment, including past the original expiry when an exchange failure has no positive cleanup proof; SDK atomically upserts its connection and consumes only that marker. Deletion therefore either prevents provider I/O, observes the completed local rows, or retains retry authority for every in-flight upstream effect.
...
4. Remove wearable/device provider access before local device rows are deleted. Account deletion uses a provider's destructive `deleteAccount` hook when available and otherwise falls back to `revokeAccess`. Any unexpired staged start fails deletion retryably because provider creation may still be in flight, even if a provider user is already visible. After marker expiry, Junction derives the same blinded owner key used for creation, resolves the provider user, and deletes it. A missing owner-cleanup capability, unsupported result, or provider failure retains the marker and fails deletion retryably. Ordinary device disconnect remains per-source deregistration. Providers without either committed-connection cleanup hook remain local-reference deletion only; an unresolved staged effect always fails closed.
...
9. Immediately attempt the receipt-owned Cloudflare Durable Object/R2, Stripe-customer, and Privy-user cleanup. Persist completion independently per target. Each target has a five-second response-path budget plus a small receipt-settlement margin. Unconfigured, partial, timed-out, and failed targets remain pending.
...
| `prisma.device_oauth_session` | Live delete | Metadata/counts | Deletes callback state and pre-provider lifecycle markers. A hosted start stages the marker before provider I/O, then replaces it atomically with any connection seed under the member lifecycle lock, including seedless OAuth starts. A consumed direct-OAuth marker remains live through exchange and commitment and is retained indefinitely when provider cleanup is ambiguous. Account deletion resolves or retains markers before the local wipe, so deletion cannot be followed by an untracked late provider user or state recreation. |
