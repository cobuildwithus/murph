...
| `prisma.hosted_group_participant_observation` | Live delete | Not exported derived data | Deletes blinded roster observations that match the member's phone or verified-email lookup key before identity rows. Unmatched observations expire after 14 days. User exports omit this derived evidence and its lookup keys. |
