...
| `postgres.hosted_runtime_log` | Receipt-owned retry delete with permanent anti-recreation fence | Documented only | Deletes isolated redacted runtime diagnostics under the same advisory lock used by append. The cleanup receipt retries failures after canonical member deletion and permanently retains one opaque digest-only fence with no raw member identifier solely to prevent late diagnostic recreation. |
