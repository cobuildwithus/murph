...
Calendar approval presentation prioritizes the recognizable connected account, event name, start, end or duration, timezone, and optional location and details. Each field is bounded independently so a long description cannot hide the event or time. The immutable fingerprint still binds the complete provider request, including the exact provider version, account id, and server-forced arguments that are not useful as primary consent copy.
