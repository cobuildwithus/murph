...
9. An approved decision returns the browser to the originating Murph conversation with `I approved the secure request.` prefilled. That message only resumes the foreground conversation; the durable approval row and consume-time recheck remain the authorization evidence. Denial returns through the existing bare conversation link.
