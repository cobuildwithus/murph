...
Calendar approval presentation prioritizes the recognizable connected account, event name, start, end or duration, timezone, and optional location and details. Each field is bounded independently so a long description cannot hide the event or time. Connected-app action ids select the explicit fact-row renderer; legacy automatic approvals remain one prose block even when their untrusted content contains the connected-app row delimiter. Connected-app preview values strip control and bidirectional-format characters before row assembly so untrusted aliases or provider text cannot visually reorder trusted consent labels. The immutable fingerprint still binds the complete provider request, including the exact provider version, account id, and server-forced arguments that are not useful as primary consent copy.
