...
2. Rows with `kind = assistant.action.approve` are durable action approvals. They contain a stable approval locator, exact-action hash, bounded presentation, and `pending | approved | denied` decision. Legacy approval presentation remains in the original plaintext columns. Connected-app presentation uses only the encrypted columns and the `hosted-member-private-field` lane, with AAD bound to the member, approval locator, table, and individual field.
...
Connected-app calendar creation, account rename, and disconnect also use this owner. Web first resolves the callback member's owned connected-account id, applies every forced server argument, and builds the immutable request from the exact member, account, operation or calendar slug, provider version, and canonical complete arguments. A pending result returns the ordinary approval URL without provider mutation. An approved generation is consumed with a fresh server-generated per-egress consumer immediately before Composio mutation, so concurrent calls have one winner and an exact replay cannot repeat a non-idempotent provider action. Reads, search, list, connection, and accountless services do not use this mutation gate.

Connected-app mutations are intentionally not persisted as pending effects. Their approval wake can make the decision visible and return the member to Murph, but it cannot reconstruct or execute a non-durable provider request. The pending assistant response says that approval saves permission without performing the action, and the null-contact approval page tells the member to return to the originating conversation and ask Murph to continue. After the member returns and still wants the action, one fresh invocation must rebuild the same exact request and consume it. A changed account or argument produces a different approval; no connected-app queue or lifecycle state is added.

Calendar approval presentation prioritizes the recognizable connected account, event name, start, end or duration, timezone, and optional location and details. Web first validates and canonicalizes the provider-bound account, alias, and complete calendar arguments, then uses that same prepared representation for presentation, identity hashing, and provider execution. Values that are overlong, contain controls or bidirectional controls, contain the fact-row separator, or contain malformed surrogate pairs fail before approval and provider egress instead of being cleaned or truncated. Connected-app action ids select the explicit fact-row renderer; legacy automatic approvals remain one prose block. Account presentation always includes toolkit, alias, and word id; when word id is missing or duplicated, a stable 16-character SHA-256 account-id fingerprint disambiguates the hidden complete id. The immutable fingerprint still binds the complete provider request, including the exact provider version, account id, and server-forced arguments that are not useful as primary consent copy.
...
9. An approved or denied decision returns through the existing bare originating-conversation link. For a durable parked effect, the system wake resumes the approved owner automatically while the approval row and consume-time recheck remain the authorization evidence. A non-durable connected-app mutation has no parked owner to resume, so the wake never executes it; the returning member's next exact invocation performs the one-time consume and provider call.
...
Approval rows contain no action payload, file bytes, raw recipient identifier, signature, or wallet authorization material. They store only an exact-action hash and bounded presentation; connected-app presentation is encrypted at rest and decrypts only for the authenticated owner, while legacy presentation retains its established plaintext form. Missing, corrupt, owner-swapped, row-swapped, or field-swapped connected-app ciphertext fails closed. The pending-effects mailbox payload contains only its control kind, member owner, timestamp, and a hashed generation-scoped event identity; it contains no file or destination data. Rows are member-scoped, cascade with the hosted member, and are explicitly covered by account deletion. User exports omit the rows and their security metadata.
