...
description: Run Murph's bounded monitor-only production-watch collector, collect aggregate and provider-availability evidence, and investigate claimed redacted incidents without treating monitoring state as product truth.
---
...
- Provider reads must either aggregate before returning data or return status only. The Cloudflare child ignores user config, uses the reviewed model/effort and disabled-feature list, and may use only the lockfile-pinned Cloudflare Observability transport; its parent verifies that this is the complete effective MCP set before launch. Vercel and Stripe use one fixed availability/authentication command each, discard both output streams at the process boundary, and remain partial/non-scorable. The child returns only `prod-watch.provider-evidence.v1`; it rejects rather than summarizes raw text into a free-form field.
...
- Scheduled provider collection must use the installer-pinned Codex standalone path and digest plus the reviewed repository-local MCP transport. Installation and every launch must verify the pinned runtime with the absolute system Git executable in an isolated environment that ignores ambient Git configuration and disables fsmonitor. Installation must run its final preflight from the pinned runtime before cutover and restore the exact prior managed plist/job state if candidate activation fails. Production runs must reject test-only environment controls; never route production through the test dependency-injection entrypoint.
...
Do not pass `--scheduled` for an interactive run and do not manually create or supply a provider envelope. The supervisor always owns deterministic database collection, Vercel/Stripe status-only probes, and the temporary Cloudflare child. It removes the temporary envelope after merge or failure. A `partial` snapshot is not healthy. Availability-only or missing provider evidence must stay explicit. Only fresh, complete, authenticated, successful source evidence may contribute production counters, latency, fingerprints, or provider release context; availability-only, degraded, partial, stale, failed, or unauthenticated evidence contributes monitor-health incidents only.
