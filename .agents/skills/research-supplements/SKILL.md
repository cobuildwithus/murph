...
4. **Dry-run against the DB**
   ...
   - Treat repair preview `structured_ready` rows as the only candidates for automated backfill. Keep `partial_parse` and `needs_better_parser` rows in manual review/refetch/OCR queues, even when some facts were recovered.
   - Before deleting raw evidence fields such as `bodyText`, `rawPageText`, or `allProductFactsText`, confirm the row has production-quality `label.ingredientRows` and `label.servingSizes`; preserve raw evidence for partial, uncertain, image-only, OCR-fragmented, or whole-page evidence rows.
