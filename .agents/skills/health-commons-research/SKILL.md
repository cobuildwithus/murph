...
- Do not treat one active wake as a hard reason to stop send fanout. Wakes are the heavier harvest/polling work; sends are usually acceptable while a lane has a wake running as long as the browser profile remains responsive and stays under a reasonable open-tab budget. As a working default, avoid pushing a managed browser profile beyond about 20 open ChatGPT tabs unless the user explicitly wants a deeper queue.
