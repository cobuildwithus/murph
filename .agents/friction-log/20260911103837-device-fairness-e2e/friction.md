...
The terminal backlog check also reused the four-minute observation budget even though the scheduler permits 30-second, two-minute, and ten-minute no-progress retries. A Linux integration run delivered the reminder but timed out with 46 resources remaining. A local replay confirmed that cooperative cancellation can process 63 jobs, then four jobs, then a zero-job pass before resuming the remaining 46 queued jobs. The terminal drain now has a separate 15-minute budget covering those retry levels and one bounded two-minute device pass. The enclosing test adds that allowance to its existing setup and intermediate-stage budget so it cannot cut the terminal wait short. Reminder timing, positive receipt-bounded progress, the first admission window, and exact delivery assertions retain their original budgets.

A later full integration run observed a completed pass with zero processed jobs before the positive receipt boundary. The runtime's lifecycle classifier permits this ordinary no-work completion, but the observer accepted only zero-job yields. It now releases either completed or yielded empty passes while retaining the original deadline and rejecting failed, unavailable, superseded, or malformed outcomes. The positive yielded pass, exact retained backlog, single-admission window, one reminder send, and eventual full drain remain required; an empty completion cannot satisfy those assertions.
