...
After preparing production smoke output, the instrumentation external-identity test also assumes Turbopack chunk syntax, although the production build uses Webpack. It must recognize the direct Webpack external while retaining the same physical-client identity assertion.
