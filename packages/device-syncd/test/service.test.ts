...
test("sqlite store migrates legacy observation tables and backfills conservative local revision counters", async () => {
  const vaultRoot = await makeTempDirectory("murph-device-syncd-migrate-observation-revisions");
  const databasePath = path.join(vaultRoot, ".runtime", "device-syncd.sqlite");
  ...
  try {
    ...
    `);
    const insertLegacyAccount = database.prepare(`
    ...
    `);
    const insertLegacyCredentials = database.prepare(`
    ...
    `);
    const insertLegacyObservation = database.prepare(`
    ...
    insertLegacyAccount.run(
      ...
      "2026-03-20T10:00:00.000Z",
      "2026-03-20T10:00:00.000Z",
      "2026-03-20T10:07:00.000Z",
    );
    insertLegacyCredentials.run(
      "dsa_legacy_advanced",
      "enc:legacy-access-advanced",
      "enc:legacy-refresh-advanced",
      "2026-03-28T00:00:00.000Z",
      "2026-03-20T10:00:00.000Z",
  ...
  const migratedSynced = store.getAccountById("dsa_legacy_synced");
  const migratedAdvanced = store.getAccountById("dsa_legacy_advanced");

  assert.ok(migratedSynced);
  assert.equal(migratedSynced?.hostedObservedUpdatedAt, "2026-03-20T10:05:00.000Z");
  assert.equal(migratedSynced?.hostedObservedTokenVersion, 4);
  assert.equal(migratedSynced?.hostedObservedConnectionRevision, 0);
  assert.equal(migratedSynced?.hostedObservedTokenRevision, 0);
  assert.equal(migratedSynced?.localConnectionRevision, 0);
  assert.equal(migratedSynced?.localTokenRevision, 0);

  assert.ok(migratedAdvanced);
  assert.equal(migratedAdvanced?.hostedObservedUpdatedAt, "2026-03-20T10:05:00.000Z");
  assert.equal(migratedAdvanced?.hostedObservedTokenVersion, 4);
  assert.equal(migratedAdvanced?.hostedObservedConnectionRevision, 0);
  assert.equal(migratedAdvanced?.hostedObservedTokenRevision, 0);
  assert.equal(migratedAdvanced?.localConnectionRevision, 1);
  assert.equal(migratedAdvanced?.localTokenRevision, 1);
  assert.deepEqual(readTableColumns(store, "device_observation_state"), [
    "account_id",
    "hosted_observed_updated_at",
    "hosted_observed_token_version",
...
test("sqlite store still rejects newer schema versions even when stale legacy tables remain", async () => {
  ...
  assert.throws(
    ...
    /device sync runtime database schema version 3 is newer than supported version 2/,
  );
});
