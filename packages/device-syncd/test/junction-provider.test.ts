...
test("Junction shallow workout stream webhook imports only a bounded feature envelope", async () => {
  ...
  const webhook = createJunctionSvixWebhook({
    body: {
      ...
      data: {
        ...
        source_id: "provider-garmin-synthetic",
        source: { device_id: "device-synthetic-1", provider: "garmin", type: "watch" },
        sport: { slug: "running" },
      },
    },
  ...
  jobPayload.sourceLifecycleEpoch = 1;
  const account = createAccount({
    sources: [{
      displayName: "Garmin",
  ...
  assert.match(String(jobPayload.sourceInstanceId), /^source-[a-f0-9]{24}$/u);
...
test("Junction source-less workout stream uses fetched identity for final local authority", async () => {
  ...
  const result = await executeJunctionJob(
    ...
    createJob("resource", {
      ...
      resource: "workout_stream",
      resourceCategory: "timeseries",
      sourceLifecycleEpochs: "fitbit:2,garmin:1",
      windowStart: "2026-04-02T00:00:00.000Z",
      windowEnd: "2026-04-03T00:00:00.000Z",
    }),
  );
...
test.each([
  {
    label: "rejects a same-slug remote replacement",
    preFetchProviders: [
      { id: "provider-garmin-before", slug: "garmin", status: "connected" },
    ],
    postFetchProviders: [
  ...
  },
  {
    ...
    preFetchProviders: [
      { id: "provider-garmin-stable", slug: "garmin", status: "connected" },
      { id: "provider-fitbit-before", slug: "fitbit", status: "connected" },
    ],
    postFetchProviders: [
      { id: "provider-garmin-stable", slug: "garmin", status: "connected" },
  ...
  const provider = createJunctionProvider(async (input) => {
    ...
    if (url.endsWith("/v2/timeseries/workouts/workout-source-less-remote/stream")) {
  ...
  await executeJunctionJob(
    ...
    createJunctionJobContext({
      ...
      upsertConnectionSource: (input) => {
        sourceCatalogMutationCount += 1;
        return createConnectionSource(input);
      },
    }),
    createJob("resource", {
      objectId: "workout-source-less-remote",
      resource: "workout_stream",
      resourceCategory: "timeseries",
      sourceLifecycleEpochs: "fitbit:1,garmin:1",
      windowStart: "2026-04-02T00:00:00.000Z",
      windowEnd: "2026-04-03T00:00:00.000Z",
    }),
  );

  assert.equal(providerListCount, 2);
  assert.equal(sourceCatalogMutationCount, 0);
  assert.equal(streamRequestCount, 1);
  assert.equal(importedSnapshots.length, expectedImportCount);
});

test("Junction workout stream fences a superseded source epoch before provider work", async () => {
  const requests: string[] = [];
  const importedSnapshots: unknown[] = [];
  const provider = createJunctionProvider(async (input) => {
    const url = readUrl(input);
    requests.push(url);
    throw new Error(`Unexpected request for a superseded workout lifecycle: ${url}`);
  });
  const account = createAccount({ sources: [{
    displayName: "Garmin",
    firstSeenAt: "2026-04-01T00:00:00.000Z",
...
test("Junction workout stream fences a reconnect that races its stream fetch", async () => {
  const requests: string[] = [];
  const importedSnapshots: unknown[] = [];
  let localAuthorityChecks = 0;
  const provider = createJunctionProvider(async (input) => {
    const url = readUrl(input);
    requests.push(new URL(url).pathname);
    if (url.endsWith("/v2/user/providers/junction-user-1")) {
      return createJsonResponse({ providers: [{
        id: "provider-garmin-racing-reconnect",
        slug: "garmin",
        status: "connected",
        resource_availability: { workouts: true },
      }] });
    }
    if (url.endsWith("/v2/timeseries/workouts/workout-racing-reconnect/stream")) {
      return createJsonResponse({
        distance: [0, 1_000],
        time: [1_776_859_200, 1_776_859_260],
      });
  ...
  const result = await executeJunctionJob(
    ...
    createJunctionJobContext({
      account,
      listConnectionSources: async () => {
        localAuthorityChecks += 1;
        return [createConnectionSource({
          lifecycleEpoch: localAuthorityChecks === 1 ? 1 : 2,
          resourceAvailabilitySummary: { workouts: true },
        })];
      },
      importSnapshot: async (snapshot) => {
    ...
    createJob("resource", {
      objectId: "workout-racing-reconnect",
      resource: "workout_stream",
      resourceCategory: "timeseries",
      sourceLifecycleEpoch: 1,
      sourceProviderSlug: "garmin",
  ...
  assert.deepEqual(result, {});
  assert.equal(localAuthorityChecks, 2);
  assert.deepEqual(requests, [
    "/v2/user/providers/junction-user-1",
    "/v2/timeseries/workouts/workout-racing-reconnect/stream",
    "/v2/user/providers/junction-user-1",
  ]);
  assert.equal(importedSnapshots.length, 0);
});
...
test("Junction workout stream fences a remote provider instance replaced during fetch", async () => {
  ...
  const result = await executeJunctionJob(
    ...
    createJob("resource", {
      objectId: "workout-remote-reconnect",
      resource: "workout_stream",
      resourceCategory: "timeseries",
      sourceLifecycleEpoch: 1,
      sourceProviderSlug: "garmin",
  ...
  assert.deepEqual(requests, [
    "/v2/user/providers/junction-user-1",
    "/v2/timeseries/workouts/workout-remote-reconnect/stream",
    "/v2/user/providers/junction-user-1",
  ]);
  assert.equal(importedSnapshots.length, 0);
});

test("Junction workout stream rejects a stale webhook source instance before its stream fetch", async () => {
  const requests: string[] = [];
  const provider = createJunctionProvider(async (input) => {
    const url = readUrl(input);
    requests.push(new URL(url).pathname);
    if (url.endsWith("/v2/user/providers/junction-user-1")) {
      return createJsonResponse({ providers: [{
        id: "provider-garmin-current",
        slug: "garmin",
        status: "connected",
        resource_availability: { workouts: true },
      }] });
    }
    throw new Error(`Unexpected request for a stale workout instance: ${url}`);
  }, {
    webhookSecret: "whsec_d2ViaG9vay10ZXN0LXNlY3JldA==",
  });
  const webhook = createJunctionSvixWebhook({
    body: {
      ...
      user_id: "junction-user-1",
      data: {
        workout_id: "workout-stale-instance",
        source_id: "provider-garmin-superseded",
        source: { provider: "garmin", type: "watch" },
      },
    },
    messageId: "msg_workout_stale_instance",
    timestamp: "1775174400",
  });
  const parsed = await requireJunctionWebhookHandler(provider).verifyAndParseWebhook({
    headers: webhook.headers,
  ...
  const parsedJob = requireValue(parsed.jobs[0], "Stale workout webhook should build one job.");
  ...
  assert.deepEqual(result, {});
  assert.deepEqual(requests, ["/v2/user/providers/junction-user-1"]);
});
...
test("Junction sparse-history scheduling caps global fanout and rotates across every pair", () => {
  ...
  const availability = Object.fromEntries(
    [
      ...JUNCTION_EXTENDED_TIMESERIES_BACKFILL_RESOURCES.map((resource) => [resource, true] as const),
  ...
  const sources = JUNCTION_CONNECT_SOURCE_TARGETS.map((target) => ({
    ...
    displayName: null,
    status: "connected" as const,
    resourceCount: JUNCTION_EXTENDED_TIMESERIES_BACKFILL_RESOURCES.length + 1,
  ...
  assert.equal(JUNCTION_CONNECT_SOURCE_TARGETS.length, 33);
  assert.equal(candidateCount, 429);
...
test("Junction does not schedule workout-duration history without advertised workout summaries", () => {
  const provider = createJunctionDeviceSyncProvider({
    apiKey: "sk_us_test_123",
    clientUserIdSecret: "junction-client-user-id-secret",
    environment: "sandbox",
  ...
  const source = {
    ...
    status: "connected" as const,
    resourceCount: 1,
    resourceAvailabilitySummary: { workout_duration: true },
    lastErrorCode: null,
    lastErrorMessage: null,
    firstSeenAt: "2026-04-03T00:00:00.000Z",
    lastSeenAt: "2026-08-11T00:00:00.000Z",
  ...
  assert.equal(scheduled?.jobs.some((job) =>
    job.payload?.resource === "workout_duration" && job.payload?.historicalBackfill === true
  ), false);
});

test.each([404, 422])(
  "Junction workout-duration history stops before duration fetch when workout summaries return %d",
  async (status) => {
    ...
    const provider = createJunctionDeviceSyncProvider({
      ...
      fetchImpl: async (input) => {
        ...
        if (url.pathname === "/v2/user/providers/junction-user-1") {
          return createJsonResponse({ providers: [{
            id: "provider-garmin-workout-unsupported",
            slug: "garmin",
            status: "connected",
            resource_availability: { workout_duration: true, workouts: true },
          }] });
        ...
        if (url.pathname === "/v2/summary/workouts/junction-user-1") {
          ...
          return createJsonResponse({ error: "unsupported_resource" }, status);
        }
        if (url.pathname === "/v2/timeseries/junction-user-1/workout_duration/grouped") {
    ...
    const initialJob = requireValue(
      requireValue(provider.jobExecutor).createScheduledJobs?.(
        createStoredAccount({ sources: [source] }),
        "2026-08-11T12:00:00.000Z",
      ).jobs.find((job) => job.payload?.resource === "workout_duration"),
    );
    ...
    assert.equal(
      ...
      `summary.workouts.${status}.unsupported`,
    );
    assert.equal(hasJunctionExtendedTimeseriesHistoryBackfillCoverage(
...
async function runUnmatchedWorkoutDurationHistoryScenario(input: {
  durationData?: (
    workoutId: string,
    index: number,
  ...
  const provider = createJunctionDeviceSyncProvider({
    ...
    fetchImpl: async (request) => {
      ...
      if (url.pathname === "/v2/user/providers/junction-user-1") {
        return createJsonResponse({ providers: [{
          id: "provider-garmin-workout-unmatched",
          slug: "garmin",
          status: "connected",
          resource_availability: { workout_duration: true, workouts: true },
        }] });
      ...
      if (url.pathname === "/v2/summary/workouts/junction-user-1") {
        summaryRequestCount += 1;
        return createJsonResponse({ workouts: input.summaries });
      }
      if (url.pathname === "/v2/timeseries/junction-user-1/workout_duration/grouped") {
        durationRequestCount += 1;
        return createJsonResponse({ groups: { garmin: input.workoutIds.map((workoutId, index) => ({
          data: input.durationData?.(workoutId, index) ?? [{
            start: `2026-02-13T${String(10 + index).padStart(2, "0")}:00:00.000Z`,
            end: `2026-02-13T${String(10 + index).padStart(2, "0")}:30:00.000Z`,
            unit: "minutes",
            value: 30,
          }],
          source: { provider: "garmin", type: "watch", workout_id: workoutId },
        })) } });
      }
      throw new Error(`Unexpected request: ${url.toString()}`);
    },
  ...
  let lastResult: Awaited<ReturnType<typeof executeJunctionJob>> = {};

  for (let execution = 0; execution < input.executions; execution += 1) {
    lastResult = await executeJunctionJob(
      provider,
      createJunctionJobContext({
        account: createAccount({ sources: [source] }),
        importSnapshot: async () => {
        ...
        now: new Date(Date.parse("2026-08-11T12:00:00.000Z") + execution * 24 * 60 * 60_000).toISOString(),
      }),
      job,
    );
    const continuation = lastResult.scheduledJobs?.find((candidate) =>
      candidate.payload?.resource === "workout_duration"
    );
    if (execution < input.executions - 1) {
      const requiredContinuation = requireValue(
        continuation,
        `missing workout-duration continuation after execution ${execution + 1}: ${JSON.stringify(lastResult)}`,
      );
      job = {
...
test("Junction empty workout companions exhaust bounded retry without import or coverage", async () => {
  const result = await runUnmatchedWorkoutDurationHistoryScenario({
    executions: 5,
    summaries: [],
    workoutIds: ["workout-empty-companion"],
  });
...
test.each([
  {
    label: "partial multiple-ID companions",
    summaries: [{
      id: "workout-matched",
      source: { provider: "garmin", type: "watch" },
      time_start: "2026-02-13T10:00:00.000Z",
    ...
    workoutIds: ["workout-matched", "workout-missing"],
  },
  {
    label: "matching non-normalizable companion",
    summaries: [{
      id: "workout-not-normalizable",
      source: { provider: "garmin", type: "watch" },
    }],
    workoutIds: ["workout-not-normalizable"],
  },
...
test.each([
  {
    ...
    label: "malformed exact rows",
    summaries: [{
      id: "workout-malformed",
      source: { provider: "garmin", type: "watch" },
      time_start: "2026-02-13T10:00:00.000Z",
      time_end: "2026-02-13T10:30:00.000Z",
    }],
    workoutIds: ["workout-malformed"],
  },
  {
    durationData: (_workoutId: string, index: number) => [{
      start: `2026-02-13T${String(10 + index).padStart(2, "0")}:00:00.000Z`,
      ...(index === 0
        ? {
            end: "2026-02-13T10:30:00.000Z",
    ...
    label: "mixed valid and malformed exact rows",
    summaries: [
      {
        id: "workout-valid",
        source: { provider: "garmin", type: "watch" },
      ...
      },
      {
        id: "workout-malformed",
        source: { provider: "garmin", type: "watch" },
        time_start: "2026-02-13T11:00:00.000Z",
        time_end: "2026-02-13T11:30:00.000Z",
      },
    ],
    workoutIds: ["workout-valid", "workout-malformed"],
  },
])("Junction workout-duration history never certifies $label", async ({
  durationData,
  summaries,
  workoutIds,
}) => {
...
test.each([
  ...
  const expectedExecutionCount = 6;
  const expectedRequestCount = 180;
...
test("Junction exhausted malformed dates advance one lineage to later valid history", async () => {
  const dayMs = 24 * 60 * 60_000;
  const windowStart = "2026-08-01T00:00:00.000Z";
  const malformedDays = new Set([
  ...
  const provider = createJunctionDeviceSyncProvider({
    ...
    fetchImpl: async (input) => {
      ...
      if (url.pathname === "/v2/user/providers/junction-user-1") {
        return createJsonResponse({ providers: [{
          id: "provider-garmin-malformed-lineage",
          slug: "garmin",
          name: "Garmin",
          status: "connected",
          resource_availability: { caffeine: true },
      ...
      if (url.pathname === "/v2/introspect/historical_pull") {
        return createJsonResponse({ data: [{
          provider: { garmin: { not_pulled: [], pulled: {
            caffeine: { days_with_data: 5, status: "success" },
          } } },
          user_id: "junction-user-1",
        }] });
      }
      if (url.pathname === "/v2/timeseries/junction-user-1/caffeine/grouped") {
        const day = `${url.searchParams.get("start_date")}T00:00:00.000Z`;
        requestsByDay.set(day, (requestsByDay.get(day) ?? 0) + 1);
        return createJsonResponse({ groups: { garmin: [{
          data: [{
            start: day,
            value: malformedDays.has(day) ? "not-a-number" : 0.095,
          }],
          source: { provider: "garmin", type: "watch" },
        }] } });
      }
  ...
  const accountMetadata = {
    ...
    junctionHistoricalBackfillLastEmptyAt: null,
    junctionHistoricalBackfillStatus: "coverage_v3_complete",
    junctionHistoricalBackfillWindowEnd: "2026-08-01T00:00:00.000Z",
    junctionHistoricalBackfillWindowStart: "2026-07-31T00:00:00.000Z",
  };
  const initial = requireValue(executor.createScheduledJobs?.(
  ...
  const retryDelaysByDay = new Map<string, number[]>();
  let executionCount = 0;

  while (!hasJunctionExtendedTimeseriesHistoryBackfillCoverage(
    metadata,
    "garmin",
    "caffeine",
    1,
    ...
    const result = await executeJunctionJob(
      ...
      createJunctionJobContext({
        account: createAccount({ metadata, sources: [source] }),
        importSnapshot: async (snapshot) => {
          const importedAt = typeof snapshot === "object" && snapshot
            && "importedAt" in snapshot && typeof snapshot.importedAt === "string"
            ? snapshot.importedAt
            : null;
          if (importedAt && !malformedDays.has(
            new Date(Date.parse(importedAt) - dayMs).toISOString(),
          )) {
            importedDays.push(new Date(Date.parse(importedAt) - dayMs).toISOString());
          }
          return { canonicalEventCount: 1, durableDeliveryAccepted: true };
        },
        now: executionNow,
    ...
    const coveragePatchValues = Object.values(result.metadataPatch ?? {}).filter((value) =>
    ...
    if (!continuation) {
      assert.equal(coveragePatchValues.length, 1);
      break;
    }
    ...
    if (retryAttempts > 0 && continuation.availableAt) {
      const retryDay = String(continuation.payload?.windowStart ?? attemptedDay);
      retryDelaysByDay.set(retryDay, [
        ...(retryDelaysByDay.get(retryDay) ?? []),
        Date.parse(continuation.availableAt) - Date.parse(executionNow),
      ]);
    }
    now = continuation.availableAt ?? now;
    job = {
  ...
  assert.equal(coverageValues.length, 1);
  assert.match(String(coverageValues[0]), /^m2\|/u);
  assert.equal(hasJunctionExtendedTimeseriesHistoryBackfillCoverage(
    metadata,
    "garmin",
    "caffeine",
...
test("Junction exhausted malformed date waits for historical pull readiness before coverage", async () => {
  ...
  const provider = createJunctionDeviceSyncProvider({
    ...
    fetchImpl: async (input) => {
      ...
      if (url.pathname === "/v2/user/providers/junction-user-1") {
        return createJsonResponse({ providers: [{
          id: "provider-garmin-readiness-gate",
          slug: "garmin",
          name: "Garmin",
          status: "connected",
          resource_availability: { caffeine: true },
      ...
      if (url.pathname === "/v2/introspect/historical_pull") {
        return createJsonResponse({ data: [{
          provider: { garmin: { not_pulled: [], pulled: {
            caffeine: { days_with_data: 1, status: historicalPullStatus },
          } } },
          user_id: "junction-user-1",
        }] });
      }
  ...
  const pending = await executeJunctionJob(provider, context, saturatedJob);
  assert.equal(hasJunctionExtendedTimeseriesHistoryBackfillCoverage(
    pending.metadataPatch ?? {},
    "garmin",
    "caffeine",
    1,
  ), false);
