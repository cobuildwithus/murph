...
test("Junction webhook jobs use complete inline provenance and reject mixed sources", async () => {
  ...
  for (const authorityCase of [
    {
      envelopeSourceProviderSlug: "fitbit",
      inlineSourceProviderSlug: "google_health",
    },
    {
      envelopeSourceProviderSlug: "google_health",
      inlineSourceProviderSlug: "fitbit",
    },
  ]) {
    const parsed = await parseWebhook({
      data: {
        id: `activity-${authorityCase.inlineSourceProviderSlug}`,
        provider: authorityCase.envelopeSourceProviderSlug,
        results: [{
          date: "2026-04-02",
          sourceProviderSlug: authorityCase.inlineSourceProviderSlug,
          steps: 4321,
        }],
      },
      messageId: `msg_activity_${authorityCase.envelopeSourceProviderSlug}_${authorityCase.inlineSourceProviderSlug}`,
    });
    assert.equal(
      parsed.dataSourceProviderSlug,
      authorityCase.inlineSourceProviderSlug,
    );
    assert.equal(
      parsed.jobs[0]?.payload?.sourceProviderSlug,
      authorityCase.inlineSourceProviderSlug,
    );
  }

  const nestedTimeseries = await parseWebhook({
    data: {
      provider: "fitbit",
      records: [{
        sourceProviderSlug: "google_health",
    ...
    messageId: "msg_steps_nested_google_health",
  });
  assert.equal(nestedTimeseries.dataSourceProviderSlug, "google_health");
  assert.equal(
    nestedTimeseries.jobs[0]?.payload?.sourceProviderSlug,
    "google_health",
  );
  assert.equal(nestedTimeseries.jobs[0]?.payload?.webhookDataJson, undefined);

  const groupedTimeseries = await parseWebhook({
    data: {
      groups: {
        fitbit: [{
          data: [{
    ...
    eventType: "daily.data.steps.created",
    messageId: "msg_steps_grouped_google_health",
  });
  assert.equal(groupedTimeseries.dataSourceProviderSlug, "google_health");
  assert.equal(
    groupedTimeseries.jobs[0]?.payload?.sourceProviderSlug,
    "google_health",
  );
...
test("Junction migration cleanup requires a live successor before revoking Fitbit", async () => {
  const requests: Array<{ method: string; url: string }> = [];
  const provider = createJunctionProvider(async (input, init) => {
    const request = {
      method: String(init?.method ?? "GET"),
...
test("Junction migration cleanup is already complete when Fitbit is absent", async () => {
  const requests: Array<{ method: string; url: string }> = [];
  const provider = createJunctionProvider(async (input, init) => {
    const request = {
      method: String(init?.method ?? "GET"),
...
test("Junction direct resource jobs use nested source authority and fetch when it is missing", async () => {
  const cases: Array<{
    label: string;
    directRecord: Record<string, unknown>;
    expectedInlineSteps: number | null;
    restSteps: number;
  }> = [
    {
  ...
  for (const testCase of cases) {
    ...
    assert.equal(
      ...
      testCase.expectedInlineSteps === null,
      testCase.label,
    );
    assert.equal(importedSnapshots.length, 1, testCase.label);
    const snapshot = importedSnapshots[0] as {
    ...
    if (testCase.expectedInlineSteps === null) {
      assert.equal(
        snapshot.summaries?.activity?.[0]?.steps,
        testCase.restSteps,
        testCase.label,
      );
    } else {
      assert.match(
        JSON.stringify(snapshot.summaries),
        new RegExp(`\\b${testCase.expectedInlineSteps}\\b`, "u"),
        testCase.label,
      );
    }
    assert.deepEqual(snapshot.timeseries, {}, testCase.label);
