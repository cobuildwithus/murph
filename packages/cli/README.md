...
- The CLI now also owns an `assistant` command group for provider-backed local chat turns, assistant-session metadata outside the vault, and an always-on inbox triage loop via `src/assistant-runtime.ts`.
