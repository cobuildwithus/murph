...
- The dedicated `@murph/assistant-core` package owns the headless assistant/inbox/vault/operator-config modules directly; the CLI now imports that package in place and keeps only the daemon-aware wrappers that genuinely belong at the CLI transport boundary.
