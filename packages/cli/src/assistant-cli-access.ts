...
export function buildAssistantCliGuidanceText(
  ...
  return [
    ...
    `If the user simply shares a meal photo, default to logging it through \`${access.rawCommand} meal add\` instead of treating it as generic chat. If the user only says what they ate or drank and no meal photo/file path is available, still default to logging it through the narrowest note or journal path instead of just chatting, preserving whether it was a meal, snack, or drink. Use the same meal surface for meals, snacks, and drinks whenever media-backed capture is available, and preserve "snack" in the note when that is the right label.`,
  ].join('\n\n')
}
