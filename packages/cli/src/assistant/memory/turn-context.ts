...
import { HEALTHYBOB_VAULT_ENV, VAULT_ENV } from '../../operator-config.js'
import { normalizeNullableString } from '../shared.js'

const ASSISTANT_MEMORY_TURN_VAULT_ENV =
  'ASSISTANT_MEMORY_BOUND_VAULT'
const LEGACY_ASSISTANT_MEMORY_TURN_VAULT_ENV =
...
const LEGACY_ASSISTANT_MEMORY_TURN_PRIVATE_CONTEXT_ENV =
...
const LEGACY_ASSISTANT_MEMORY_TURN_SOURCE_PROMPT_ENV =
...
export const assistantMemoryTurnEnvKeys = [
  ...
  LEGACY_ASSISTANT_MEMORY_TURN_VAULT_ENV,
  ASSISTANT_MEMORY_TURN_PRIVATE_CONTEXT_ENV,
  LEGACY_ASSISTANT_MEMORY_TURN_PRIVATE_CONTEXT_ENV,
  ASSISTANT_MEMORY_TURN_SOURCE_PROMPT_ENV,
  LEGACY_ASSISTANT_MEMORY_TURN_SOURCE_PROMPT_ENV,
  ASSISTANT_MEMORY_TURN_SESSION_ID_ENV,
  LEGACY_ASSISTANT_MEMORY_TURN_SESSION_ID_ENV,
  ASSISTANT_MEMORY_TURN_ID_ENV,
  LEGACY_ASSISTANT_MEMORY_TURN_ID_ENV,
] as const
...
export function createAssistantMemoryTurnContextEnv(
  ...
  return {
    ...
    [LEGACY_ASSISTANT_MEMORY_TURN_ID_ENV]: input.turnId,
    [ASSISTANT_MEMORY_TURN_PRIVATE_CONTEXT_ENV]: input.allowSensitiveHealthContext
      ? '1'
      : '0',
    [LEGACY_ASSISTANT_MEMORY_TURN_PRIVATE_CONTEXT_ENV]: input.allowSensitiveHealthContext
      ? '1'
      : '0',
    [ASSISTANT_MEMORY_TURN_SESSION_ID_ENV]: input.sessionId,
    [LEGACY_ASSISTANT_MEMORY_TURN_SESSION_ID_ENV]: input.sessionId,
    [ASSISTANT_MEMORY_TURN_SOURCE_PROMPT_ENV]: input.sourcePrompt,
    [LEGACY_ASSISTANT_MEMORY_TURN_SOURCE_PROMPT_ENV]: input.sourcePrompt,
    [ASSISTANT_MEMORY_TURN_VAULT_ENV]: resolvedVault,
    [LEGACY_ASSISTANT_MEMORY_TURN_VAULT_ENV]: resolvedVault,
  }
}
...
export function resolveAssistantMemoryTurnContext(
  ...
  const vault = readFirstEnvValue(env, [
    ASSISTANT_MEMORY_TURN_VAULT_ENV,
    LEGACY_ASSISTANT_MEMORY_TURN_VAULT_ENV,
  ])
  const sourcePrompt = readFirstEnvValue(env, [
    ASSISTANT_MEMORY_TURN_SOURCE_PROMPT_ENV,
    LEGACY_ASSISTANT_MEMORY_TURN_SOURCE_PROMPT_ENV,
  ])
  const sessionId = readFirstEnvValue(env, [
    ASSISTANT_MEMORY_TURN_SESSION_ID_ENV,
    LEGACY_ASSISTANT_MEMORY_TURN_SESSION_ID_ENV,
  ])
  const turnId = readFirstEnvValue(env, [
    ASSISTANT_MEMORY_TURN_ID_ENV,
    LEGACY_ASSISTANT_MEMORY_TURN_ID_ENV,
  ])
