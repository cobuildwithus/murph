...
describe('ReviewGPT PR context guard', () => {
  ...
  it.each(['pr-review', '--preset=pr-deep-review'])(
    ...
    (preset) => {
      ...
      expect(readFileSync(harness.capturePath, 'utf8')).toBe(
        `pr=42\nphase=final\nargs=exec cobuild-review-gpt --config scripts/review-gpt.config.sh --minimum-marked-response-time 180s ${preset} --dry-run\n`,
      )
  ...
  it('leaves generic presets outside the PR workflow', () => {
    ...
    expect(result.status, result.stderr).toBe(0)
    expect(readFileSync(harness.capturePath, 'utf8')).toBe(
      'pr=\nphase=\nargs=exec cobuild-review-gpt --config scripts/review-gpt.config.sh --minimum-marked-response-time 180s simplify --dry-run\n',
    )
  })
