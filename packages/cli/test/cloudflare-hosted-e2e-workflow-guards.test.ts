...
describe('cloudflare hosted e2e workflow guards', () => {
  it('provisions a real local postgres service for hosted local e2e jobs', () => {
    ...
    expect(workflow.match(/image: postgres/g)).toHaveLength(2)
    expect(workflow.match(/POSTGRES_DB: murph_test/g)).toHaveLength(2)
    expect(workflow.match(/POSTGRES_PASSWORD: postgres/g)).toHaveLength(2)
    expect(workflow.match(/POSTGRES_USER: postgres/g)).toHaveLength(2)
    expect(workflow.match(/--health-cmd pg_isready/g)).toHaveLength(2)
    expect(workflow.match(/- 5432:5432/g)).toHaveLength(2)
