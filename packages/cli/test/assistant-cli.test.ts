...
type AssistantStyleSettingState = {
  source: 'custom' | 'default'
  value: number
}
type AssistantStyleResult = {
  preferencesPath: string
  recordedAt: string | null
  settings: {
    detail: AssistantStyleSettingState
    humor: AssistantStyleSettingState
    push: AssistantStyleSettingState
  }
  updated: boolean
}

const DEFAULT_ASSISTANT_STYLE_SETTINGS = {
...
test.sequential(
  'assistant style commands expose defaults and round-trip a canonical humor override',
  async () => {
    const parent = await mkdtemp(path.join(tmpdir(), 'murph-assistant-style-'))
    const vaultRoot = path.join(parent, 'vault')
    cleanupPaths.push(parent)
    await initializeVault({ vaultRoot })

    const initial = requireData(
      await runSourceCli<AssistantStyleResult>([
        'assistant',
        'style',
        'show',
        '--vault',
    ...
    assert.deepEqual(initial.settings, DEFAULT_ASSISTANT_STYLE_SETTINGS)
    ...
    assert.equal(updated.settings.humor.value, 9)
    assert.equal(updated.settings.humor.source, 'custom')
    assert.notEqual(updated.recordedAt, null)
    ...
    assert.equal(unchanged.recordedAt, updated.recordedAt)

    const rejected = await runSourceCli<AssistantStyleResult>([
      'assistant',
      'style',
      'set',
      'humor',
    ...
    assert.deepEqual(reset.settings.humor, {
      source: 'default',
      value: 3,
    })
  },
...
test.sequential(
  'assistant style commands update the remaining dials and reset every override',
  async () => {
    const parent = await mkdtemp(path.join(tmpdir(), 'murph-assistant-style-all-'))
    const vaultRoot = path.join(parent, 'vault')
    cleanupPaths.push(parent)
    await initializeVault({ vaultRoot })
    ...
    for (const [setting, value] of [
      ...
      assert.equal(setResult.settings[setting].source, 'custom')
      assert.equal(setResult.settings[setting].value, Number(value))
    }

    const resetAll = requireData(
      await runSourceCli<AssistantStyleResult>([
        'assistant',
        'style',
        'reset',
    ...
    assert.deepEqual(resetAll.settings, DEFAULT_ASSISTANT_STYLE_SETTINGS)
  },
  ASSISTANT_CLI_TIMEOUT_MS,
)
