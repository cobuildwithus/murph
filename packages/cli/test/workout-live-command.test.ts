...
test('ordinary later-local-day lifecycle preserves the stale workout and logs only the stated new set', async () => {
  const { parentRoot, vaultRoot } = await createTempVaultContext(
    'murph-live-workout-reminder-context-',
  )
  cleanupPaths.push(parentRoot)
  const cli = createWorkoutCli()

  const initialized = await run<{ created: boolean }>(cli, [
    'init', '--vault', vaultRoot, '--timezone', 'America/Los_Angeles',
  ])
  assert.equal(requireData(initialized.envelope).created, true)

  await run(cli, [
    'workout', 'format', 'save', 'Earlier routine',
    '--vault', vaultRoot,
    '--type', 'strength-training',
    '--duration', '90',
    '--exercise',
    'order=1;name=Bench press;sourceExerciseId=EX_PRIOR;mode=weight_reps;unitOverride=lb',
    '--set-template',
    'exercise=1;order=1;targetReps=8;targetWeight=135;targetWeightUnit=lb',
    '--set-template',
    'exercise=1;order=2;targetReps=8;targetWeight=135;targetWeightUnit=lb',
  ])
  await run(cli, [
    'workout', 'format', 'save', 'Later routine',
    '--vault', vaultRoot,
    '--type', 'strength-training',
    '--duration', '30',
    '--exercise',
    'order=1;name=Goblet squat;sourceExerciseId=EX_LATER;mode=weight_reps;unitOverride=lb',
    '--set-template',
    'exercise=1;order=1;targetReps=10;targetWeight=50;targetWeightUnit=lb',
    '--set-template',
    'exercise=1;order=2;targetReps=10;targetWeight=50;targetWeightUnit=lb',
  ])

  const earlierStartedAt = '2026-08-10T06:30:00.000Z'
  const laterMessageAt = '2026-08-10T18:00:00.000Z'
  ...
  assert.equal(earlierStartedAt.slice(0, 10), laterMessageAt.slice(0, 10))
  assert.equal(localDate(earlierStartedAt), '2026-08-09')
  assert.equal(localDate(laterMessageAt), '2026-08-10')

  const earlier = requireData((await run<WorkoutResult>(cli, [
    'workout', 'start',
    '--routine', 'earlier-routine',
    '--started-at', earlierStartedAt,
    '--vault', vaultRoot,
  ])).envelope)
  const earlierId = earlier.eventId
  await run<ShowResult>(cli, [
    'workout', 'set', 'log',
    '--workout-id', earlierId,
    '--exercise-id', 'EX_PRIOR',
    '--set-order', '1',
    '--require-existing-set',
    '--reps', '7',
    '--weight', '130',
  ...
  const staleBlock = await run(cli, [
    'workout', 'start',
    '--routine', 'later-routine',
    '--started-at', laterMessageAt,
    '--vault', vaultRoot,
  ])
  assert.equal(staleBlock.envelope.ok, false)

  const earlierFinished = requireData((await run<ShowResult>(cli, [
    'workout', 'finish',
    '--workout-id', earlierId,
    '--ended-at', '2026-08-10T07:15:00.000Z',
    '--vault', vaultRoot,
  ])).envelope)
  assert.equal(earlierFinished.entity.data.durationMinutes, 45)
  assert.equal(
    earlierFinished.entity.data.workout.endedAt,
    '2026-08-10T07:15:00.000Z',
  )
  assert.deepEqual(earlierFinished.entity.data.workout.exercises[0]?.sets, [
    { order: 1, reps: 7, weight: 130, weightUnit: 'lb' },
    { order: 2 },
  ])

  const later = requireData((await run<WorkoutResult>(cli, [
    'workout', 'start',
    '--routine', 'later-routine',
    '--started-at', laterMessageAt,
    '--vault', vaultRoot,
  ])).envelope)
  const laterId = later.eventId
  assert.deepEqual(later.workout?.exercises[0]?.sets, [
    { order: 1 },
    { order: 2 },
  ])

  const loggedLater = requireData((await run<ShowResult>(cli, [
    'workout', 'set', 'log',
    '--workout-id', laterId,
    '--exercise-id', 'EX_LATER',
    '--set-order', '2',
    '--require-existing-set',
    '--reps', '6',
    '--weight', '45',
  ...
  assert.deepEqual(loggedLater.entity.data.workout.exercises[0]?.sets, [
    { order: 1 },
    { order: 2, reps: 6, weight: 45, weightUnit: 'lb' },
  ])

  const sameDayBlock = await run(cli, [
    'workout', 'start', 'Ambiguous same-day workout',
    '--started-at', '2026-08-10T19:00:00.000Z',
    '--vault', vaultRoot,
  ])
  assert.equal(sameDayBlock.envelope.ok, false)

  const correctedEarlier = requireData((await run<ShowResult>(cli, [
    'workout', 'edit', earlierId,
    '--workout-exercise',
    'order=1;name=Bench press;sourceExerciseId=EX_PRIOR;mode=weight_reps;unitOverride=lb',
    '--workout-set',
    'exercise=1;order=1;reps=8;weight=130;weightUnit=lb',
    '--workout-set',
    'exercise=1;order=2',
    '--vault', vaultRoot,
  ])).envelope)
  assert.deepEqual(correctedEarlier.entity.data.workout.exercises[0]?.sets, [
    { order: 1, reps: 8, weight: 130, weightUnit: 'lb' },
    { order: 2 },
  ])

  const stillActive = requireData((await run<ShowResult>(cli, [
    'workout', 'active', '--workout-id', laterId, '--vault', vaultRoot,
  ])).envelope)
  assert.equal(stillActive.entity.id, laterId)
})
