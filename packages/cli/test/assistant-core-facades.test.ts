...
test('cli and split owner packages publish the expected owner dependencies', async () => {
  ...
  assert.equal(vaultUsecasesManifest.exports?.['./usecases/runtime'] !== undefined, true)
  assert.equal(vaultUsecasesManifest.exports?.['./usecases/shared'] !== undefined, true)
  assert.equal(vaultUsecasesManifest.exports?.['./health-cli-descriptors'] !== undefined, true)
