...
describe('monorepo release flow coverage audit', () => {
  ...
  it('exposes only the package-backed review-gpt runner', () => {
    ...
    expect(reviewGptReadme).toContain('The threshold defaults to 5 minutes')
    ...
    const tooFastGuardCall = reviewGptDriver.indexOf(
      'assertMarkedResponseDurationTrusted(responseResult, responseFile);',
      tooFastGuardEnd,
    )
    const completedResponseBranch = reviewGptDriver.indexOf(
      "if (responseResult?.status === 'completed') {",
      tooFastGuardCall,
    )
    expect(tooFastGuardCall).toBeGreaterThan(tooFastGuardEnd)
    expect(completedResponseBranch).toBeGreaterThan(tooFastGuardCall)
