...
describe('monorepo release flow coverage audit', () => {
  ...
  it('exposes only the package-backed review-gpt runner', () => {
    ...
    expect(rootPackageJson.devDependencies?.['@cobuild/review-gpt']).toBe('^0.5.94')
    expect(pnpmWorkspace).toContain('@cobuild/review-gpt@0.5.94')
    ...
    expect(prDeepReviewPrompt).toContain(
      'if you cannot read the PR diff or the touched files through the connected repository',
    )
    expect(prDeepReviewPrompt).toContain(
      'do not review from memory or from the PR description alone',
    )
