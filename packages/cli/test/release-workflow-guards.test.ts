...
describe('release workflow guards', () => {
  ...
  it('keeps prerelease routing, primary-package release notes, and ordered publish helper usage', () => {
    ...
    expect(workflow).toContain('falling back to trusted publishing')
    ...
    expect(publishHelper).toContain('Configure npm trusted publishing for this repository/package or provide NPM_TOKEN to the publish job.')
  })
})
