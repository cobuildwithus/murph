...
- Treat `packages/web` as an operator-facing observability surface by default, not a marketing site.
- Default to utility copy, visible status, and obvious next actions.
- Use Tailwind utility classes only; theme tokens come from `packages/web/app/globals.css`.
