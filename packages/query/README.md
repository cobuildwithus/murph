...
Owns read helpers, filters, derived retrieval state, and export-pack generation over canonical vault state. Query code must not mutate canonical vault data. It may rebuild local projection artifacts such as the optional SQLite search index at `.runtime/projections/search.sqlite`.
