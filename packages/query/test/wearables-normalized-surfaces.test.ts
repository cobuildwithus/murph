...
test("heart-rate-zone projection uses the multi-source workout-day rollup without false attribution", () => {
  const vault = makeVault([
    makeActivitySession({
...
test("daily workout rollup adds distinct multi-origin sessions with metric-specific reducers", () => {
  const vault = makeVault([
    makeObservation({
      dayKey: "2026-02-14",
      entityId: "evt_garmin_daily_distance",
      metric: "distance",
      occurredAt: "2026-02-14T23:00:00.000Z",
      provider: "garmin",
      recordedAt: "2026-02-14T23:05:00.000Z",
      unit: "km",
      value: 14.5,
    }),
    makeObservation({
      dayKey: "2026-02-14",
      entityId: "evt_garmin_daily_max_hr",
    ...
    makeActivitySession({
      ...
      dataOrigin: {
        aggregatorProvider: "junction",
        originConfidence: "high",
        sourceInstanceId: "garmin-watch-run",
        sourceProviderSlug: "garmin",
        sourceType: "watch",
        version: 1,
      },
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
    ...
    makeActivitySession({
      ...
      dataOrigin: {
        aggregatorProvider: "junction",
        originConfidence: "high",
        sourceInstanceId: "garmin-watch-strength",
        sourceProviderSlug: "garmin",
        sourceType: "watch",
        version: 1,
      },
      dayKey: "2026-02-14",
      durationMinutes: 15,
      endAt: "2026-02-14T22:15:00.000Z",
      entityId: "evt_garmin_strength",
      occurredAt: "2026-02-14T22:00:00.000Z",
      provider: "junction",
      recordedAt: "2026-02-14T22:17:00.000Z",
      startAt: "2026-02-14T22:00:00.000Z",
      workoutMetrics: {
        activeCalories: 90,
        maxHeartRate: 172,
        workoutStrain: 6,
  ...
  assert.equal(latest?.activity?.sessionMinutes.selection.value, 75);
  assert.equal(latest?.activity?.sessionCount.selection.value, 2);
  assert.deepEqual(latest?.activity?.activityTypes, ["Functional strength training", "Running"]);
  assert.equal(latest?.activity?.activeCalories.selection.value, 690);
  assert.equal(latest?.activity?.distanceKm.selection.value, 14.5);
  assert.equal(latest?.activity?.totalElevationGainMeters.selection.value, 200);
  assert.equal(latest?.activity?.maxHeartRate.selection.value, 180);
  assert.equal(latest?.activity?.workoutStrain.selection.value, 12.5);
  assert.equal(pointValue("workout-minutes"), 75);
  assert.equal(pointValue("workout-count"), 2);
  assert.equal(pointValue("activity-minutes"), null);
});

test("daily workout rollup suppresses a mirrored overlapping session", () => {
  const vault = makeVault([
    makeActivitySession({
      activityType: "Running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_garmin_run_direct",
      occurredAt: "2026-02-14T12:00:00.000Z",
      provider: "garmin",
    ...
    makeActivitySession({
      ...
      dataOrigin: {
        aggregatorProvider: "junction",
        originConfidence: "high",
        sourceInstanceId: "garmin-watch-mirror",
        sourceProviderSlug: "garmin",
        sourceType: "watch",
        version: 1,
      },
      ...
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_garmin_run_junction",
      occurredAt: "2026-02-14T12:00:00.000Z",
      provider: "junction",
      recordedAt: "2026-02-14T13:02:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
  ...
  assert.equal(latest?.activity?.sessionMinutes.selection.value, 60);
  assert.equal(latest?.activity?.sessionCount.selection.value, 1);
  assert.equal(latest?.activity?.activeCalories.selection.value, 600);
  assert.equal(
    projection.metricPoints.find((point) => point.metricKey === "workout-minutes")?.value,
    60,
  );
});

test("activity-session mirror overlap requires at least 80% coverage of both windows", () => {
  const summarizeOverlap = (shiftedStartAt: string, shiftedEndAt: string) =>
    summarizeWearableLatest(makeVault([
      makeActivitySession({
        activityType: "running",
        dayKey: "2026-02-14",
        durationMinutes: 100,
        endAt: "2026-02-14T13:40:00.000Z",
        entityId: "evt_overlap_base",
        occurredAt: "2026-02-14T12:00:00.000Z",
      ...
      makeActivitySession({
        ...
        durationMinutes: 100,
        endAt: shiftedEndAt,
        entityId: "evt_overlap_shifted",
        occurredAt: shiftedStartAt,
        recordedAt: "2026-02-14T14:03:00.000Z",
        startAt: shiftedStartAt,
      }),
    ]))?.activity;

  const eightyPercentOverlap = summarizeOverlap(
    "2026-02-14T12:20:00.000Z",
    "2026-02-14T14:00:00.000Z",
  );
  assert.equal(eightyPercentOverlap?.sessionMinutes.selection.value, 100);
  assert.equal(eightyPercentOverlap?.sessionCount.selection.value, 1);

  const seventyNinePercentOverlap = summarizeOverlap(
    "2026-02-14T12:21:00.000Z",
    "2026-02-14T14:01:00.000Z",
  );
  assert.equal(seventyNinePercentOverlap?.sessionMinutes.selection.value, 200);
  assert.equal(seventyNinePercentOverlap?.sessionCount.selection.value, 2);
});

test("activity-session mirror preference favors declared duration consistent with its window", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 10,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_inconsistent_duration",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:01:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
    }),
    makeActivitySession({
      ...
      dayKey: "2026-02-14",
      durationMinutes: 50,
      endAt: "2026-02-14T12:50:00.000Z",
      entityId: "evt_consistent_duration",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:02:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
    }),
  ...
  assert.equal(activity?.sessionMinutes.selection.value, 50);
  assert.equal(activity?.sessionCount.selection.value, 1);
});

test("activity-session mirror preference tolerates plausible paused elapsed time", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
    ...
    makeActivitySession({
      ...
      dayKey: "2026-02-14",
      durationMinutes: 49,
      endAt: "2026-02-14T12:49:00.000Z",
      entityId: "evt_unpaused_duration",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:02:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
    }),
...
test("activity-session mirror merge keeps a preferred duration-derived window coherent", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      entityId: "evt_derived_window",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:01:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
    }),
    makeActivitySession({
      ...
      durationMinutes: 50,
      endAt: "2026-02-14T12:50:00.000Z",
      entityId: "evt_shorter_explicit_window",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:02:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
    }),
    makeActivitySession({
      ...
      dayKey: "2026-02-14",
      durationMinutes: 40,
      endAt: "2026-02-14T12:40:00.000Z",
      entityId: "evt_nested_after_merge",
      occurredAt: "2026-02-14T12:00:00.000Z",
  ...
  assert.equal(activity?.sessionMinutes.selection.value, 100);
  assert.equal(activity?.sessionCount.selection.value, 2);
});

test("activity-session mirror overlap falls back from a reversed explicit end", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T11:59:00.000Z",
      entityId: "evt_reversed_end",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:02:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
    }),
...
test("activity-session mirror overlap does not absorb a nested short workout", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
    ...
    makeActivitySession({
      ...
      dayKey: "2026-02-14",
      durationMinutes: 10,
      endAt: "2026-02-14T12:30:00.000Z",
      entityId: "evt_nested_short",
      occurredAt: "2026-02-14T12:20:00.000Z",
      recordedAt: "2026-02-14T12:31:00.000Z",
      startAt: "2026-02-14T12:20:00.000Z",
    }),
  ]))?.activity;

  assert.equal(activity?.sessionMinutes.selection.value, 70);
  assert.equal(activity?.sessionCount.selection.value, 2);
});

test("activity-session mirror type matching understands aliases and generic fallbacks", () => {
  const cases = [
    {
      expectedCount: 1,
      label: "run alias",
    ...
    {
      expectedCount: 1,
      label: "compound run alias",
      leftType: "Outdoor Run",
      rightType: "running",
    },
    {
      expectedCount: 1,
      label: "compound cycling alias",
      leftType: "Indoor Cycling",
      rightType: "ride",
    },
    {
      expectedCount: 1,
      label: "strength variant",
      leftType: "Functional strength training",
      rightType: "strength-training",
    },
    {
      expectedCount: 1,
      label: "generic workout fallback",
      leftType: "workout",
      rightType: "running",
    },
    {
      expectedCount: 2,
      label: "specific incompatible types",
      leftType: "running",
      rightType: "cycling",
    },
  ] as const;
  ...
  for (const scenario of cases) {
    const activity = summarizeWearableLatest(makeVault([
      makeActivitySession({
        activityType: scenario.leftType,
        dayKey: "2026-02-14",
        durationMinutes: 60,
        endAt: "2026-02-14T13:00:00.000Z",
        entityId: `evt_type_left_${scenario.expectedCount}`,
        occurredAt: "2026-02-14T12:00:00.000Z",
        recordedAt: "2026-02-14T13:01:00.000Z",
        startAt: "2026-02-14T12:00:00.000Z",
      }),
      makeActivitySession({
        activityType: scenario.rightType,
        dayKey: "2026-02-14",
        durationMinutes: 60,
        endAt: "2026-02-14T13:00:00.000Z",
        entityId: `evt_type_right_${scenario.expectedCount}`,
        occurredAt: "2026-02-14T12:00:00.000Z",
        recordedAt: "2026-02-14T13:02:00.000Z",
        startAt: "2026-02-14T12:00:00.000Z",
      }),
    ...
    assert.equal(activity?.sessionCount.selection.value, scenario.expectedCount, scenario.label);
    assert.equal(
      activity?.sessionMinutes.selection.value,
      scenario.expectedCount * 60,
      scenario.label,
    );
  }
  ...
  for (const genericType of [
    ...
    const activity = summarizeWearableLatest(makeVault([
      makeActivitySession({
        activityType: genericType,
        dayKey: "2026-02-14",
        durationMinutes: 60,
        endAt: "2026-02-14T13:00:00.000Z",
        entityId: "evt_generic_type",
        occurredAt: "2026-02-14T12:00:00.000Z",
        recordedAt: "2026-02-14T13:01:00.000Z",
        startAt: "2026-02-14T12:00:00.000Z",
      }),
    ...
    assert.equal(activity?.sessionCount.selection.value, 1, genericType);
    assert.equal(activity?.sessionMinutes.selection.value, 60, genericType);
  }

  const missingTypeActivity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_missing_type",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:01:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
    }),
    makeActivitySession({
      ...
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_known_type",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:02:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
    }),
  ]))?.activity;
  assert.equal(missingTypeActivity?.sessionCount.selection.value, 1, "missing type");
  assert.equal(missingTypeActivity?.sessionMinutes.selection.value, 60, "missing type");
});

test("activity-session mirror heuristics require timestamps", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "run",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      entityId: "evt_missing_time_left",
      recordedAt: "2026-02-14T13:01:00.000Z",
    }),
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      entityId: "evt_missing_time_right",
      recordedAt: "2026-02-14T13:02:00.000Z",
    }),
  ]))?.activity;

  assert.equal(activity?.sessionMinutes.selection.value, 120);
  assert.equal(activity?.sessionCount.selection.value, 2);
});

test("stable activity resource identity dedupes mirrors and retains the full window", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_stable_full",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:01:00.000Z",
      resourceId: "stable-workout-resource",
      startAt: "2026-02-14T12:00:00.000Z",
      workoutMetrics: {
        activeCalories: 600,
      },
  ...
  assert.equal(activity?.sessionMinutes.selection.value, 60);
  assert.equal(activity?.sessionCount.selection.value, 1);
  assert.equal(activity?.activeCalories.selection.value, 600);
});

test("activity-session overlap merge retains a secondary stable resource identity", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_identityless_preferred",
      occurredAt: "2026-02-14T12:00:00.000Z",
    ...
    makeActivitySession({
      ...
      durationMinutes: 50,
      endAt: "2026-02-14T12:50:00.000Z",
      entityId: "evt_stable_identity_window",
      occurredAt: "2026-02-14T12:00:00.000Z",
      provider: "garmin",
      recordedAt: "2026-02-14T13:02:00.000Z",
      resourceId: "shared-three-way-resource",
      startAt: "2026-02-14T12:00:00.000Z",
    }),
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 50,
      entityId: "evt_stable_identity_without_window",
      provider: "garmin",
      recordedAt: "2026-02-14T13:03:00.000Z",
      resourceId: "shared-three-way-resource",
    }),
...
test("session-aware exact dedupe preserves complementary workout evidence", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_exact_details_running",
      heartRateZones: [{
        durationMinutes: 12,
        zone: 1,
      }],
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:01:00.000Z",
      resourceId: "exact-details-resource",
      startAt: "2026-02-14T12:00:00.000Z",
      workoutMetrics: {
        activeCalories: 600,
      },
    ...
    makeActivitySession({
      ...
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_exact_details_generic",
      heartRateZones: [{
        durationMinutes: 20,
        zone: 2,
      }],
  ...
  assert.deepEqual(activity?.heartRateZones, [
    { durationMinutes: 12, zone: 1 },
    { durationMinutes: 20, zone: 2 },
  ]);
});

test("daily workout reducers attribute each metric to its actual contributors", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_contributor_garmin",
      occurredAt: "2026-02-14T12:00:00.000Z",
      provider: "garmin",
      recordedAt: "2026-02-14T13:01:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
  ...
  assert.equal(activity?.sessionCount.selection.provider, "multiple");
  ...
  assert.equal(activity?.maxHeartRate.selection.provider, "garmin");
  assert.equal(activity?.workoutStrain.selection.provider, "multiple");
});

test("mirrored metric ties retain both contributors without duplicating the session", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_mirror_contributor_garmin",
      occurredAt: "2026-02-14T12:00:00.000Z",
      provider: "garmin",
      recordedAt: "2026-02-14T13:01:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
    ...
    makeActivitySession({
      ...
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_mirror_contributor_oura",
      occurredAt: "2026-02-14T12:00:00.000Z",
      provider: "oura",
      recordedAt: "2026-02-14T13:01:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
  ...
  assert.equal(activity?.sessionMinutes.selection.value, 60);
  assert.equal(activity?.sessionCount.selection.value, 1);
  assert.equal(activity?.sessionMinutes.selection.provider, "garmin");
  assert.equal(activity?.activeCalories.selection.provider, "multiple");
  assert.equal(activity?.maxHeartRate.selection.provider, "multiple");
});

test("mirrored workout fields follow the provider whose value was selected", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_mirror_selected_garmin",
      occurredAt: "2026-02-14T12:00:00.000Z",
      provider: "garmin",
      recordedAt: "2026-02-14T13:02:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
    ...
    makeActivitySession({
      ...
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_mirror_selected_oura",
      occurredAt: "2026-02-14T12:00:00.000Z",
      provider: "oura",
      recordedAt: "2026-02-14T13:01:00.000Z",
      startAt: "2026-02-14T12:00:00.000Z",
  ...
  assert.equal(activity?.sessionCount.selection.value, 1);
  assert.equal(activity?.sessionMinutes.selection.provider, "garmin");
  assert.equal(activity?.activeCalories.selection.provider, "garmin");
  assert.equal(activity?.distanceKm.selection.provider, "oura");
  assert.equal(activity?.maxHeartRate.selection.provider, "oura");
  assert.equal(activity?.workoutStrain.selection.provider, "oura");
});

test("session-aware exact dedupe keeps incompatible activity types distinct", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_exact_incompatible_run",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:01:00.000Z",
      resourceId: null,
      startAt: "2026-02-14T12:00:00.000Z",
    ...
    makeActivitySession({
      ...
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_exact_incompatible_cycle",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:01:00.000Z",
      resourceId: null,
      startAt: "2026-02-14T12:00:00.000Z",
  ...
  assert.equal(activity?.sessionMinutes.selection.value, 120);
  assert.equal(activity?.sessionCount.selection.value, 2);
  assert.deepEqual(activity?.activityTypes, ["cycling", "running"]);
});

test("stable activity resource identity overrides a conflicting activity label", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_stable_label_run",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:01:00.000Z",
      resourceId: "stable-label-resource",
      startAt: "2026-02-14T12:00:00.000Z",
    }),
    makeActivitySession({
      activityType: "cycling",
      ...
      durationMinutes: 60,
      endAt: "2026-02-14T13:00:00.000Z",
      entityId: "evt_stable_label_cycle",
      occurredAt: "2026-02-14T12:00:00.000Z",
      recordedAt: "2026-02-14T13:02:00.000Z",
      resourceId: "stable-label-resource",
      startAt: "2026-02-14T12:00:00.000Z",
...
test("stable activity identity includes resource type and facet", () => {
  const activity = summarizeWearableLatest(makeVault([
    makeActivitySession({
      activityType: "running",
      dayKey: "2026-02-14",
      durationMinutes: 30,
      entityId: "evt_resource_type_session",
      provider: "oura",
      recordedAt: "2026-02-14T13:01:00.000Z",
      resourceId: "endpoint-local-id",
    ...
    makeActivitySession({
      ...
      dayKey: "2026-02-14",
      durationMinutes: 45,
      entityId: "evt_resource_type_workout",
      provider: "oura",
      recordedAt: "2026-02-14T13:02:00.000Z",
      resourceId: "endpoint-local-id",
    ...
    makeActivitySession({
      ...
      dayKey: "2026-02-14",
      durationMinutes: 20,
      entityId: "evt_resource_facet_primary",
      facet: "primary",
      provider: "oura",
      recordedAt: "2026-02-14T18:01:00.000Z",
      resourceId: "faceted-endpoint-local-id",
      resourceType: "workout",
    }),
    makeActivitySession({
      ...
      dayKey: "2026-02-14",
      durationMinutes: 25,
      entityId: "evt_resource_facet_secondary",
      facet: "secondary",
      provider: "oura",
  ...
  assert.equal(activity?.sessionMinutes.selection.value, 120);
  assert.equal(activity?.sessionCount.selection.value, 4);
});

test("activity-session mirror preference does not depend on private candidate ids", () => {
  const summarizeConflictingMirror = (runId: string, runningId: string) =>
    summarizeWearableLatest(makeVault([
      makeActivitySession({
        activityType: "run",
        dayKey: "2026-02-14",
  ...
  const first = summarizeConflictingMirror("evt_z_run", "evt_a_running");
  const second = summarizeConflictingMirror("evt_a_run", "evt_z_running");

  assert.equal(first?.sessionCount.selection.value, 1);
  assert.equal(second?.sessionCount.selection.value, 1);
  assert.equal(first?.activeCalories.selection.value, second?.activeCalories.selection.value);
  assert.deepEqual(first?.activityTypes, second?.activityTypes);
});

test("activity-session reconciliation order ignores private ids in non-transitive overlap chains", () => {
  const summarizeChain = (ids: {
    full: string;
    medium: string;
    short: string;
  }) =>
  ...
  const mediumFirstById = summarizeChain({
    full: "evt_z_full",
    medium: "evt_a_medium",
    short: "evt_m_short",
  });
  const fullFirstById = summarizeChain({
    full: "evt_a_full",
    medium: "evt_m_medium",
    short: "evt_z_short",
  });

  assert.equal(mediumFirstById?.sessionCount.selection.value, 2);
  assert.equal(mediumFirstById?.sessionMinutes.selection.value, 100);
  assert.equal(fullFirstById?.sessionCount.selection.value, 2);
  assert.equal(fullFirstById?.sessionMinutes.selection.value, 100);
});
