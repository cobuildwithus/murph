...
Consumers that need the local vault-backed gateway adapter should depend on `@murphai/gateway-local` explicitly. That package owns the rebuildable `.runtime/gateway.sqlite` serving store plus the local send/read wrapper surface.
