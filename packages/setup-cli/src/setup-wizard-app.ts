...
import { wrapSetupWizardIndex } from './setup-wizard-core.js'
import {
...
} from './setup-wizard-runtime-status.js'
import {
...
export function SetupWizardApp(
  ...
  const [selectedScheduledUpdates, setSelectedScheduledUpdates] = React.useState<
    string[]
  ...
  const latestChannelsRef = React.useRef<SetupChannel[]>(input.initialChannels)
  const latestScheduledUpdatesRef = React.useRef<string[]>(
    input.initialScheduledUpdates,
  )
  const latestWearablesRef = React.useRef<SetupWearable[]>(input.initialWearables)
  ...
  const selectionSteps: Record<SetupWizardSelectionStep, SetupWizardSelectionConfig> =
    {
      ...
      'scheduled-updates': {
        lines: setupWizardScheduledUpdateOptions.map((option, index) => ({
          ...
          badges: buildSetupWizardScheduledUpdateBadges({
            isStarter: input.defaultScheduledUpdateIds.has(option.id),
          }),
          description: option.description,
      ...
      channels: {
        lines: setupWizardChannelOptions.map((option, index) => {
          const status = resolveSetupWizardChannelStatus(
            input.channelStatuses,
            option.channel,
          )
          return {
            active: index === channelIndex,
            badges: buildSetupWizardRuntimeBadges(status),
            description: option.description,
            detail: describeSetupWizardRuntimeStatus(status),
            key: option.channel,
      ...
      wearables: {
        lines: setupWizardWearableOptions.map((option, index) => {
          const status = resolveSetupWizardWearableStatus(
            input.wearableStatuses,
            option.wearable,
          )
          return {
  ...
  const selectedReadyNow = [
    ...
    ...selectedWearables.flatMap((wearable) =>
      resolveSetupWizardWearableStatus(input.wearableStatuses, wearable).ready
        ? [formatSetupWearable(wearable)]
        : [],
    ),
  ]
