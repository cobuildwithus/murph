...
export const workoutImportInspectResultSchema = z.object({
  ...
  requiresWeightUnit: z.boolean(),
  requiresDistanceUnit: z.boolean(),
...
export const workoutImportCsvResultSchema = z.object({
  ...
  weightUnit: z.enum(['lb', 'kg']).nullable(),
  distanceUnit: z.enum(['m', 'km', 'mi']).nullable(),
  parsedWorkoutCount: z.number().int().nonnegative(),
  ...
  skippedRowCount: z.number().int().nonnegative(),
  rawOnly: z.boolean(),
  lookupIds: z.array(z.string().min(1)).max(10),
  lookupIdsTruncated: z.boolean(),
  ledgerFiles: z.array(pathSchema).max(10),
  ledgerFilesTruncated: z.boolean(),
  warnings: z.array(z.string()).max(12),
})
