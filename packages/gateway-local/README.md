...
The local store now persists one unified `gateway_source_events` input table plus capture attachments, permission state, and event/meta state. Conversation, message, and attachment snapshots are derived from those source events on demand instead of being persisted as a second serving-table layer.
