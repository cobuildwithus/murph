...
async function removeAutomaticMealPhotoLocked(
  ...
  for (const attachment of photoAttachments) {
    ...
    tombstones.push({
      ...
      manifestPath: manifestSnapshot.relativePath,
      originalIntegrity,
  ...
  for (const tombstone of tombstones) {
    const snapshot = manifestSnapshots.get(tombstone.manifestPath);
    if (!snapshot) {
      throw new VaultError(
        "MEAL_PHOTO_RETENTION_MANIFEST_INVALID",
...
async function loadPhotoManifestSnapshot(input: {
  ...
  for (const relativePath of manifestPaths) {
    ...
    if (
      !parsed.success ||
      parsed.data.importId !== input.mealId ||
      parsed.data.importKind !== "meal" ||
      parsed.data.owner.kind !== "meal" ||
      parsed.data.owner.id !== input.mealId ||
      parsed.data.rawDirectory !== rawDirectory ||
