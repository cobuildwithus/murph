...
test("automatic meal photo removal preserves the structured meal and tombstones retained image bytes", async () => {
  const vaultRoot = await makeTempDirectory("murph-vault");
  const sourceRoot = await makeTempDirectory("murph-source");
  await initializeVault({ vaultRoot });
  ...
  assert.deepEqual(JSON.parse(tombstoneText), {
    ...
    originalByteSize: Buffer.byteLength(originalPhoto),
    originalSha256: createHash("sha256").update(originalPhoto).digest("hex"),
  });
...
test("automatic meal photo removal fails closed without partial mutation when retained bytes changed", async () => {
  const vaultRoot = await makeTempDirectory("murph-vault");
  const sourceRoot = await makeTempDirectory("murph-source");
  await initializeVault({ vaultRoot });
  const photoPath = await writeExternalFile(
...
test("automatic meal photo removal rejects a mismatched manifest without deleting image bytes", async () => {
  const vaultRoot = await makeTempDirectory("murph-vault");
  const sourceRoot = await makeTempDirectory("murph-source");
  await initializeVault({ vaultRoot });
  ...
  const photoArtifact = manifest.artifacts.find(
    (artifact) => artifact.relativePath === meal.photo?.relativePath,
  );
  if (!photoArtifact) {
    throw new Error("Expected the automatic meal manifest to describe its photo.");
  }
  photoArtifact.sha256 = "0".repeat(64);
  ...
  assert.equal(await fs.readFile(manifestPath, "utf8"), mismatchedManifest);
  assert.deepEqual(
    await readJsonlRecords({ vaultRoot, relativePath: meal.eventPath }),
    ledgerBefore,
  );
