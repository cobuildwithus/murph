...
- own the shared `HostedIngressEvent` / `HostedRun` / `HostedExecutionCursor` contracts, signed callback routes, and narrow execution status vocabulary
- provide the typed shared ingress/control helpers while keeping deployed auth adapters app-local
...
- device-sync runtime snapshot/apply/token contracts stay on `@murphai/device-syncd/hosted-runtime`; this package only carries the outer hosted ingress/run-control seam plus the shared device-sync wake-hint shape needed by that seam
