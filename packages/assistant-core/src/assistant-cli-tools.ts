...
function createWebPdfReadToolDefinitions() {
  ...
  return [
    defineAssistantTool({
      name: 'web.pdf.read',
      description:
        'Fetch one public PDF over HTTP(S) from Murph\'s guarded web-read surface, block private-network targets, redact query/fragment-bearing URL details in tool output, and extract readable text with bounded page and character limits. Use this for menus, manuals, reports, or docs that are published as PDFs. Use only stable public URLs; do not pass signed or session-bound links.',
      inputSchema: z.object({
        url: z.string().url(),
