...
export function buildAssistantCliGuidanceText(
  ...
  return [
    ...
    `When the user wants a durable local wiki page, dossier, or synthesis that should keep adding up inside the vault, synthesize the page in the current assistant turn and then persist it with \`${access.rawCommand} knowledge compile <prompt> --body "<markdown>" --source-path <path> ...\` instead of freehand editing markdown. Knowledge compile writes a non-canonical page under \`derived/knowledge/pages/**\`, rebuilds \`derived/knowledge/index.md\`, and keeps the page inspectable and rebuildable without launching a second model run. Use vault-relative sources, or absolute paths that still resolve inside the selected vault, and do not use \`derived/**\`, \`.runtime/**\`, or assistant-state runtime files.`,
