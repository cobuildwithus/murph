...
export function buildAssistantCliGuidanceText(
  ...
  return [
    ...
    'If `knowledge search` or `knowledge show` surfaces an existing page that already matches the topic, prefer refreshing that slug instead of creating a near-duplicate page.',
    `When the user wants a durable local wiki page, dossier, or synthesis that should keep adding up inside the vault, synthesize the page in the current assistant turn and then persist it with \`${access.rawCommand} knowledge upsert --title "<title>" --body "<markdown>" --source-path <path> ...\` instead of freehand editing markdown. Knowledge upsert writes a non-canonical page under \`derived/knowledge/pages/**\`, rebuilds \`derived/knowledge/index.md\`, and keeps the page inspectable and rebuildable without launching a second model run. Use vault-relative source files, or absolute source files that still resolve inside the selected vault, and do not use \`derived/**\`, \`.runtime/**\`, or assistant-state runtime files.`,
