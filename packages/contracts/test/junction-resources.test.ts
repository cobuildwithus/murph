...
describe("Junction wearable resource policy", () => {
  ...
  it("derives the existing admitted resource surfaces from policy", () => {
    ...
    expect(JUNCTION_DEFAULT_TIMESERIES_RESOURCES).toHaveLength(24);
    expect(JUNCTION_ALLOWED_TIMESERIES_RESOURCES).toEqual(
      JUNCTION_DEFAULT_TIMESERIES_RESOURCES,
    );
    expect(JUNCTION_KNOWN_TIMESERIES_RESOURCES).toHaveLength(29);
  });

  it("admits sparse clinical readings as bounded per-record facts", () => {
