...
describe("Junction wearable resource policy", () => {
  ...
  it("admits only bounded workout retention paths", () => {
    expect(JUNCTION_RESOURCE_POLICIES.workout_duration).toMatchObject({
      admission: "default",
      frequency: "sparse",
  ...
  it("selects only compact supported sparse resources for long history", () => {
    ...
    expect(JUNCTION_RESOURCE_POLICIES.workout_duration.historyChunkDays).toBe(
      JUNCTION_SPARSE_HISTORY_CHUNK_DAYS,
    );
    expect(JUNCTION_RESOURCE_POLICIES.workout_duration.historyAnchor).toBe("schedule_time");
