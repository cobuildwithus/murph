...
export const MURPH_ATTACH_RESPONSE_MEDIA_TOOL = {
  ...
  description:
    `Attach up to ${ASSISTANT_AUTHORED_RESPONSE_MEDIA_MAX_ITEMS} images to the current final assistant response. Accept intentionally public catalog image URLs or an exact vault_image descriptor returned by a trusted Murph command. Never invent or modify a private descriptor. Replaces the current response media batch for this turn only. It does not send directly. For charts, include the key source numbers in the final reply text.`,
  inputSchema: {
    type: 'object',
    additionalProperties: false,
    properties: {
