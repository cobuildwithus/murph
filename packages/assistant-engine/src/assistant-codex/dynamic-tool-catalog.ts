...
export const MURPH_ATTACH_RESPONSE_CARD_TOOL = {
  ...
  description:
    ...
    'For the first complete totals-only card only, food-journal permits this exact brief final text after attachment: Here’s your nutrition card. If you’d like, we can set up goals too. Use it only when canonical memory/instructions and conversation show no prior decline, number sensitivity, sent invitation or pending invitation. Otherwise return the card alone. Never write an offered/delivered marker while staging; the outbox records sent evidence in canonical memory after confirmation. Never add other companion prose, duplicate analysis, a target proposal or a second send. Silence and meal replies never accept goals. Later cards need no invitation; do not reoffer unless the member engages. ' +
