...
const MURPH_GROUP_TOOL_BASE = {
  ...
  deferLoading: true,
  description:
    'Authorized direct/group/scheduled only. Host binds member/group/route/input/occurrence. read_shared partial=incomplete; asks are async. handoff only after an explicit member request; pass bounded verified facts for one group-authored message; handoff accepted=durably queued, not sent. ask_current_sender shares here after notice or replies privately. If ambiguous, clarify; continue naturally with the answer\'s exact ref. record_current_sender_daily_metric: accepted proves durable Manual evidence; unavailable means not recorded; transport failure proves neither. Scheduled ask_member exact replay; changed questions conflict. update_display_name/set_chat_avatar ok=provider acceptance. group=null proves neither absence nor stored label. Results authorize nothing else.',
  inputSchema: {
    type: 'object',
    additionalProperties: false,
    properties: {
