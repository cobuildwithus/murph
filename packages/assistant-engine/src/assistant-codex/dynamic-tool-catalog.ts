...
export const MURPH_GROUP_CHALLENGE_RESPONSE_CARD_TOOL = {
  ...
  name: 'attach_response_card',
  description:
    'Attach one challenge_standings card for the current authenticated Linq group only when the current accepted room message or the saved instructions for this exact scheduled occurrence explicitly request a current shared-challenge snapshot. Read the existing canonical challenge page and every required bounded read first. Pass the exact normalized scorer input plus the canonical projection-scope keys used by each scorecard component. The trusted host requires at least one complete read, one unchanged ordered participant and authorized-label roster across every batch, exact scorer-roster equality, and component scopes backed by those reads. It derives individual labels from the trusted read and the title from the existing challenge page, runs the deterministic scorer, and compare-and-set persists the exact input and result on that same page before attaching. Any unavailable, empty, failed, capacity-omitted, roster-inconsistent, scope-mismatched, missing-page, or conflicting-persistence path cannot attach and must finish with a truthful ordinary-text update. The tool owns points, target, order, coverage, counts, ranks, and ties. For individual or team snapshots, attach only when the entire canonical ranked result contains at most eight entries; never truncate the ranking or omit a waiting participant. Collective cards have no row cap. Partial scores are verified lower bounds and unscored scores stay null. The card replaces the entire final response, does not send by itself, and cannot combine with response media.',
