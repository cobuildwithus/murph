...
export const MURPH_ATTACH_RESPONSE_CARD_TOOL = {
  ...
  description:
    ...
    'For routine daily_nutrition fulfillment, do not call murph.send_progress_update merely to announce safety, totals, estimation, or target-resolution checks. Complete the bounded workflow before the single card or concise truthful fallback. For every daily_nutrition instruction below, the calorie slot accepts one applicable active exact-point legacy calories target in kcal when no dietary-calories owner exists. New authoring remains dietary-calories. If a canonical owner exists, use it only when every applicable legacy alias is an identical compatible point; a different value, incompatible alias, or multiple legacy-only owners is a conflict. Never rename or mutate a Goal merely to repair the legacy key. Apply the 1,200-kcal boundary and residual calculations to the resolved calorie value. For active condition or regimen detail reads below, retry a failed, explicitly truncated, or unreadable family-specific show exactly once through vault-cli show <same-id> --format json. Continue only with one complete, unambiguous canonical record; otherwise fail closed, and never omit fields, shrink the safety set, or retry indefinitely. ' +
