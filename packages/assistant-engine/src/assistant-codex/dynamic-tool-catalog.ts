...
export const MURPH_ATTACH_RESPONSE_CARD_TOOL = {
  ...
  description:
    'For daily_nutrition cards and numeric nutrition targets, use the member request plus context already known in the conversation or vault. Do not run a universal medical-history or measurement checklist. A routine card with complete accepted goals needs no repeated screening unless new context raises a concern. Before proposing or changing numeric targets, if current known context has not established suitability, ask one compact question confirming that the member is at least 18, is not pregnant or breastfeeding, does not have an eating-disorder or number-sensitive tracking context, and is not following clinician-managed nutrition or using medication that materially changes nutrition targets. If known current context makes self-directed numeric guidance unsuitable—including those cases, underweight, frailty or malnutrition, glucose-lowering medication, kidney disease, advanced liver disease, significant heart disease, relevant endocrine disease, or post-bariatric care—use helpful non-numeric text, make no Goal mutation, and attach no card. Read only the context needed to resolve a concrete concern. A missing unrelated fact or failed unrelated read does not block a routine card. A scheduled occurrence asks no screening questions and never activates provisional targets. Treat a calorie target below 1,200 kcal/day as unsuitable; do not floor it upward. ' +
