...
export const MURPH_GROUP_SHARED_READ_TOOL = {
  ...
  name: 'group',
  description:
    'Read current consent-aware shared group facts after the turn has started. Request one to three unique exact projectionScopes. The result includes every current member and each requested scope, distinguishing not_granted from granted-but-missing data. Its participantId is scoped to the group membership and carries no account, device, provider, or route identity. Detached reads have empty currentTurnHandles. A missing metric means only that this current read lacks it; its cause is unverified. device-sync-status.v0 permits only literal status and timestamp statements, never an explanation for why a metric is absent. activity-days.v0 is usable as movement only with data.metricSemantics="broad-movement"; workout-days.v0 is usable as a canonical combined day only with data.metricSemantics="canonical-workout-day". Otherwise the record is ambiguous and unusable, not zero. Record timestamps do not establish projection completeness. Treat every same-local-day value as provisional. The trusted host resolves current authority; never supply member, share, group, runtime, mailbox, session, or route identifiers.',
  inputSchema: {
    type: 'object',
    additionalProperties: false,
    properties: {
...
export const MURPH_GROUP_TOOL = {
  ...
  description:
    ...
    'Use action="read_shared" only when current group standings or diagnostics need exact consent-aware shared facts. Request one to three exact projectionScopes. The result includes every current member and each requested scope, distinguishing not_granted from granted-but-missing data. Each participantId is scoped to this group membership and carries no account, device, provider, or route identity. On an interactive iMessage turn, currentTurnHandles may identify the exact current prompt Sender on that same row; never infer identity from names, order, data, or a global id. A missing metric means only that this current read lacks it; its cause is unverified. device-sync-status.v0 permits only literal status and timestamp statements, never an explanation for why a metric is absent. activity-days.v0 is usable as movement only with data.metricSemantics="broad-movement"; workout-days.v0 is usable as a canonical combined day only with data.metricSemantics="canonical-workout-day". Otherwise the record is ambiguous and unusable, not zero. Record timestamps do not establish projection completeness. Treat every same-local-day value as provisional. It resolves current authority only after this tool call; never supply sender handles, member, share, runtime, group, or route identifiers. ' +
