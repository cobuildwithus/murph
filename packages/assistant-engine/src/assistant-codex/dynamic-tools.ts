  ...
  inputSchema: {
    ...
    properties: {
      ...
      projectionScopes: {
        ...
        items: GROUP_VAULT_SHARE_PROJECTION_SCOPE_SCHEMA,
        description:
          'Optional bounded health projections that liking the server-owned offer message will grant as a fixed snapshot. The server-filled {{share_scope}} placeholder always states that preferred display name is shared too.',
      },
      messageTemplate: {
        type: 'string',
        minLength: 1,
        ...
        description:
          'Required for action="post_join_offer". Write one short natural group-chat message, not a fixed script. Lead with the exact words "Like this message" and immediately say what liking it will do. Include {{share_scope}} exactly once where the server inserts the exact shared-scope phrase. Include {{join_url}} exactly once as the customize link so members can share more or less. Do not include any other URL.',
      },
    },
    required: ['action'],
  },
