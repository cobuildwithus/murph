  ...
  inputSchema: {
    ...
    properties: {
      ...
      displayName: {
        ...
        description:
          'Group display name. Required for action="update_display_name", which requests the iMessage group chat title update and stores the same hosted group label when the chat has a hosted group record; optional for action="create_join_link" or action="post_join_offer" only when it is the name the group chose.',
      },
      membershipId: {
        type: 'string',
        minLength: 1,
