...
export const MURPH_GROUP_TOOL = {
  ...
  description:
    ...
    'Use action="read_usage" when trusted turn context says this conversation\'s Murph usage is running low, or when the current connected group asks about its Murph usage or adding more usage. The result reports only a coarse healthy, low, or exhausted state, the current period end, and a first-party funding URL when available. When the group has no funding link yet, this action also mints it: the bare group record and join code only, never a membership, sharing grant, or payment. Never infer or disclose internal currency accounting, contributor identity, purchase history, or payment status from this action. ' +
