...
export const MURPH_DEVICE_TOOL = {
  ...
  name: 'device',
  description:
    'Work with the current authenticated member’s wearable and health-device accounts. list_accounts returns matching accountId, provider, status, last sync, and safe error context. connect returns a short-lived connectUrl for a supported provider. reconcile queues a refresh for one returned accountId; queued does not mean completed. configure_no_data_outreach changes Garmin (sourceProvider garmin) or Apple Health (sourceProvider apple_health_kit) check-in timing: use after_days with 5–30 days, off, or default only when the current private member message states that preference. Call configure_no_data_outreach at most once for that message; after any result, do not retry. A saved result confirms the check-in preference: acknowledge it once and finish. Off stops only these check-ins; connection and syncing stay unchanged. Never call it from a group or scheduled turn or for another provider. No data is not proof of disconnection or app closure; reserve reconnect guidance for explicit authentication failure. Never ask for or pass provider credentials, tokens, delivery routes, or generic commands.',
