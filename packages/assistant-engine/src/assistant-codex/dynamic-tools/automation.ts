...
export const MURPH_AUTOMATION_TOOL = {
  ...
  name: 'automation',
  description:
    'Create, update, or reconcile durable Murph automations for the current authenticated conversation. save binds delivery to this conversation and accepts no route fields. Every new active scheduled non-direct Linq automation must include one immutable scheduledTask: { kind: "group_notification" } for ordinary group messages, { kind: "group_health_update" } for selector-free current consented health projections, or { kind: "group_challenge", knowledgeSlug, projectionScopeKey } for one exact active challenge page and selectable projection with a finite activeUntil. The task binding and route are immutable. patch preserves the stored route unless retargetToCurrentConversation=true is explicit. reconcile archives members of one supportSeriesId that are absent from desiredAutomationIds. Use patch status to pause, reactivate, or archive. Never pass credentials, delivery targets, filesystem paths, reserved system tags, or generic commands.',
