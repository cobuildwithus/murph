...
export async function executeAutomationDynamicTool(input: {
  ...
  try {
    ...
    if (isAutomationConflictError(error)) {
      const existingAutomationId = readExistingAutomationConflictId(error)
      return automationTextResult(
        false,
...
function isAutomationConflictError(
  error: unknown,
): error is { code: 'VAULT_AUTOMATION_CONFLICT'; details?: unknown } {
  return isUnknownRecord(error) && error.code === 'VAULT_AUTOMATION_CONFLICT'
}

function readExistingAutomationConflictId(error: {
  details?: unknown
}): string | null {
  if (!isUnknownRecord(error.details)) {
    return null
  }
  const automationId = error.details.existingAutomationId
  return typeof automationId === 'string' ? automationId.trim() || null : null
}
