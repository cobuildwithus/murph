...
import { ASSISTANT_APPOINTMENT_REMINDER_SOURCE_REF_PATTERN } from '../../assistant/appointment-reminder-source-ref.js'
import {
...
const saveAutomationArgumentsSchema = z.object({
  ...
  createOnlySourceRef: z.string()
    .regex(ASSISTANT_APPOINTMENT_REMINDER_SOURCE_REF_PATTERN)
    .optional(),
  ...
  if (
    value.createOnly === true
    && (
      value.createOnlyEffectKey === undefined
      || value.createOnlySourceRef === undefined
    )
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      message:
        'Create-only saves require an appointment source ref and appointment-reminder:<one-based source ordinal>.',
      path: value.createOnlySourceRef === undefined
        ? ['createOnlySourceRef']
        : ['createOnlyEffectKey'],
    })
  }
  if (
    ...
    context.addIssue({
      code: z.ZodIssueCode.custom,
      message:
        'createOnlyEffectKey and createOnlySourceRef are valid only with createOnly=true.',
      path: value.createOnlySourceRef !== undefined
        ? ['createOnlySourceRef']
        : ['createOnlyEffectKey'],
    })
...
export function readAutomationDynamicToolRequest(input: {
  ...
  if (parsed.args.action === MURPH_ONBOARDING_FIRST_PERSONAL_READ_ACTION) {
    return {
      kind: 'automation',
      onboardingFirstReadCompletionRequested: true,
      request: buildOnboardingFirstPersonalReadAutomationSaveRequest(),
    }
  }
  if (parsed.args.action === 'save_newsletter') {
    return {
      kind: 'automation',
      request: buildGroupNewsletterAutomationSaveRequest({
        configuration: {
  ...
  if (parsed.args.action === 'save' && parsed.args.createOnly === true) {
    const { createOnlySourceRef, ...request } = parsed.args
    return {
      createOnlySourceRef,
      kind: 'automation',
      request,
  ...
  return { kind: 'automation', request: parsed.args }
}
