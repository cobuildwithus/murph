...
const GROUP_ROOM_MODEL_TOOL_DESCRIPTION = [
  ...
  'During engine-authorized silent consolidation, keep the complete page roughly under 20,000 UTF-8 bytes as a soft curation target, not a hard write gate. When the shown page is near or above that size, compact it before adding more: merge duplicate lore, summarize older low-value detail, and retire stale or contradicted bits while preserving high-signal current canon, explicit corrections, participant preferences, open callbacks, and valid photo refs. Never invent facts or remove still-current context merely to hit an exact number.',
  ...
  'Show first, then pass the returned expectedDigest to upsert or delete. Upsert must contain the complete compact Markdown page and must not contain raw participant handles. If show fails or a write reports stale or unavailable state, stop. This tool is unavailable in group email and never changes participant identity, permissions, health sharing, or personal memory.',
].join(' ')
