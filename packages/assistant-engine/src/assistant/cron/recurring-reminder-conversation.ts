...
export const ASSISTANT_CRON_RECURRING_REMINDER_CADENCE_INSTRUCTIONS = [
  ...
  '- If a relevant human reply followed that output, use it when composing the current reminder; do not ask another cadence question or apply the silence-based skips below.',
  '- Confirmed outputs may include this reminder’s attached one-time follow-up. If that check remains unanswered, skip this ordinary nonclinical occurrence; do not create another check or add a competing cadence question. Never attach a follow-up to a cadence question.',
  ...
  '- If no relevant human reply followed and no exception or skip condition above applies, send the current concise cue and ask one natural question about whether to keep, change, or pause these interruptions.',
