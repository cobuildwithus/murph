...
export const ASSISTANT_CRON_RECURRING_REMINDER_CONVERSATION_INSTRUCTIONS = [
  ...
  '- The silence policy below does not apply to medication, prescribed treatment, clinician-directed care, clinical monitoring, or safety-critical reminders. Send those cues normally unless the direct-conversation completion rule above applies, the member explicitly changes or pauses them, or another authoritative skip condition applies.',
  ...
  '- If a relevant human reply followed that output, first apply the direct-conversation completion rule above; otherwise use the reply when composing the current reminder.',
