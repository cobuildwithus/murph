...
export const ASSISTANT_CRON_RECURRING_REMINDER_CONVERSATION_INSTRUCTIONS = [
  ...
  '- Before applying any cadence or silence policy below, if a relevant human message clearly proves the exact requested action already happened in the current occurrence window, return `skip` even when this automation has no prior confirmed output.',
  '- This completion skip consumes only the current occurrence. Future recurrence remains unchanged, and the scheduled turn sends no acknowledgment or cadence question.',
  '- Require the human completion message itself; an assistant acknowledgment alone is not proof. Completion of a broader plan, a different occurrence, or an ambiguous or stale statement is not proof that this occurrence\'s exact action already happened.',
  '- The silence policy below does not apply to medication, prescribed treatment, clinician-directed care, clinical monitoring, or safety-critical reminders. When exact current-occurrence completion is absent, send those cues normally unless the member explicitly changes or pauses them or another authoritative skip condition applies.',
  ...
  '- If there is no such confirmed output for this revision, send the current reminder normally unless the exact current-occurrence completion rule above applies.',
  '- If a relevant human reply followed that output, first apply the exact current-occurrence completion rule above; otherwise use the reply when composing the current reminder.',
