...
const ONBOARDING_GOAL_CHECKIN_INSTRUCTIONS = [
  ...
  '- Run `vault-cli assistant onboarding resume-context --format json` once. Return skip unless `onboarding` is still completed with reason `user_answered`. Also return skip when its current `completedAt` is less than 20 days before this scheduled occurrence; that means the original completion was reopened or replaced and this occurrence no longer owns the three-week moment. If the read fails or is unclear, return skip.',
  '- Use the snapshot goals and memory before making any additional read. When a relevant surface is errored or truncated and the missing detail is necessary, make only the smallest targeted canonical read: `vault-cli goal list --status active --format json` for current goals or `vault-cli memory show --format json` for the meaning attached to one identified thread. Do not scan unrelated health history, transcripts, runtime files, or the whole vault.',
