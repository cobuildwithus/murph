...
function buildAssistantComputerUseGuidanceText(): string {
  return [
    ...
    "- If the user asks to see the current paused browser screen without taking over, call `murph.computer_pause_for_user` with `handoffPurpose=\"screen_inspection\"` to create or refresh the handoff URL, then include the returned URL. Do not restart the browser task just to get a screen link.",
...
function buildAssistantConnectedAppsGuidanceText(): string {
  return [
    ...
    "- Before asking the user to repeat task-relevant information that a connected email or calendar account may answer, use `connected_apps_manage` to list accounts when account choice is unclear, `connected_apps_search` to discover the exact current read tool and schema, then `connected_apps_execute` with the exact returned account selector. Narrow search to `gmail`, `googlecalendar`, `outlook`, or `zoho_mail` when useful.",
...
function buildAssistantFamilyPlanGuidanceText(): string {
  return [
    "Murph Family:",
    "- Murph Family is Murph product setup for a family account with up to 4 people total. The owner pays for family access and can invite family members by phone number and/or Telegram username when `murph.family_plan` is available.",
    ...
    "- If `start_checkout` returns an inactive checkout URL without `preparedInvite`, keep the flow simple: explain that they should click the link to activate Family, then come back and say it is done if they want you to create an invite. If it returns `preparedInvite`, do not ask them to come back just to create the invite. If `start_checkout` returns `unavailableReason=\"already_sponsored\"`, explain that they already have sponsored Family access and must leave that Family before starting their own.",
    ...
    "- For general questions about what Murph Family is, answer from these rules and use `read_status` only when account-specific state would help. Do not invent pricing, billing dates, official launch terms, or unsupported admin controls.",
...
- For longitudinal visual tracking requests such as progress photos, body-composition photos, skin/acne/tretinoin tracking, posture/form photos, or wound/lesion follow-up, treat the user's request as intent to preserve relevant images. Save available attachment paths with \`vault-cli capture add --media <path> --collection <stable-series-slug>\`, adding body-site, concise notes, tags, or related ids when useful. The durable copy belongs in the capture record; the duplicate \`raw/inbox/**\` media bytes may still expire.
