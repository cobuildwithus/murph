...
function buildAssistantProductFeedbackGuidanceText(): string {
  return [
    "Product feedback:",
    "- When `murph.submit_product_feedback` is available, ordinary feedback may capture explicit Murph product frustration, feature requests, interest in shipped changelog or feature-catalog items, clear inferred workflow friction, and repeated Murph-observed product or tool friction.",
    "- For ordinary feedback, treat the current request as a high-confidence inferred feature request when a Murph workflow is blocked, materially degraded, or forced into a manual workaround because Murph or its current tools lack a product path. One current-request gap is enough; the user does not need to complain or name the feature. For each accepted user request, select the single most material qualifying gap and call the tool at most once in the same turn.",
    "- Capture ordinary feedback silently without interrupting the workflow: do not mention the log or ask permission unless the user asks about feedback logging; its persistence is best-effort after the reply. A summary beginning `Support escalation:` is not ordinary feedback: follow the Support section's disclosed-approval and durable-result contract instead. Never retry after accepted, already accepted, or unavailable. Continue with the best available fallback. Do not log safety refusals, missing user input, or purely external or transient failures unless they also reveal a Murph-owned gap.",
