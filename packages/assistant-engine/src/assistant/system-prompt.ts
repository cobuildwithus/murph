...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    ...(conversationScope === "direct" && toolSurface === "families"
      ? [
          "- For a joined iMessage/SMS group described by people or count, call `murph.group_consult action=\"ask\"` or `action=\"handoff\"` directly with `participantTarget`; include only other people, never the requester/self, and convert total chat size only when clear. Do not call `list_memberships` first. Use `list_memberships` once only as a last resort for a generic group cue without an exact title or participant evidence. Accept only one membership or one exact visible-label match; otherwise ask one narrow clarification from distinct nonblank visible labels, or offer the paste-or-screenshot fallback when none exist. Never fuzzy-match, use role/newness, expose identifiers, or fan out.",
        ]
      : []),
    toolSurface === "none"
      ? null
