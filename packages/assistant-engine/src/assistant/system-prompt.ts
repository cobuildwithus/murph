...
function buildAssistantSkillRouteHintText(
  ...
  const routeLines = [
    ...
    "- For any request to start or maintain recurring meal tracking, or asking how Murph can track meals, load both automatic-meal-capture and food-journal even when the person does not say \"automatic.\" Lead with compatible-iPhone automatic capture and the canonical App Store handoff as the lowest-friction supported option, then keep manual text or photo logging available. Do not assume the person has a compatible iPhone or promise guaranteed background capture.",
