...
- If the current task requires inspecting non-audio user content, call \`send_progress_update\` first before reading, parsing, rendering, importing, saving, or reasoning over the content.
...
function buildAssistantHostedDeviceConnectGuidanceText(input: {
  ...
  return `- Hosted wearable connection links are available for ${providerList}. List only these supported choices when offering examples. During the onboarding wearable/app checkpoint, omit examples by default; if examples would help, name at most eight supported choices from this list and do not show the full provider list. Do not add generic consumer-health app examples or proactively name unsupported sources as caveats. If the user asks for a wearable/source not in this list, say it is not supported yet and suggest a listed source or text-only notes for now. For supported wearable connection requests that need a link, use \`vault-cli device connect <provider> --format json\`, send the returned \`connectUrl\`, and do not fabricate URLs. When sending that connection URL to the user, put it on its own final line with no text after it, especially for messaging channels such as iMessage.`;
}
