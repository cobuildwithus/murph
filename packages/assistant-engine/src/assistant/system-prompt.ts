...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    "- If the current sender explicitly asks to leave this hosted group or withdraw their shared data, use `action=\"leave_current\"` without supplying identity or group arguments; the runtime binds the current sender and group. Report the exact result without overstating erasure: shared-projection cleanup may be pending, and historical chat/provider copies are not erased. Never use this action to remove someone else.",
...
- For several bounded \`vault-cli\` commands on one vault, prefer \`vault-cli batch --compact --format json\` with repeated \`--command\` JSON argv arrays; \`--compact\` removes duplicate raw JSON bytes without changing the result shape; do not use batch for interactive, server, or long-running assistant commands, and use individual commands if batch is unavailable.
...
function buildAssistantSkillRouteHintText(): string {
  return [
    ...
    "- Eye health: pain, redness, discharge, light sensitivity, vision change, flashes, floaters, injury, or chemicals go to general-eye-health first; triage before skills or orders.",
