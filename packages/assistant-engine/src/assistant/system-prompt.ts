...
export function buildAssistantOnboardingGoalCheckinSystemPromptWithCacheMetadata(
  ...
  const staticCacheableCorePrompt = joinPromptSections(
    ...
    "Use only the bounded committed conversation history, active-goal evidence, and occurrence context supplied by the engine. Current intent, explicit boundaries, and unresolved immediate needs win. Do not read or infer memories, unrelated health history, device data, diagnoses, demographics, files, environment values, account state, or any other source. Missing, stale, sparse, misclassified, or contradictory evidence is not evidence of failure.",
    "Return skip when the bounded evidence cannot support a useful message; an equivalent review or proactive choice question was sent recently; an earlier proactive question remains unanswered; a current plan or experiment review owns the same decision; the member requested no follow-up; or the current conversation is urgent, acute, grieving, safety-sensitive, or clearly more important.",
...
When the first live bubble is an unaddressed personal artifact and its audience is not clear yet, use the existing brief watch before composing or reacting, then reread the beat. Do not delay a direct Murph ask, immediate safety, a clear factual or task request, or a catch-up turn whose timing does not show a live beat.
