...
function buildAssistantSkillRouteHintText(
  ...
  const routeLines = [
    ...
    "- Food-journal owns requested-card incomplete-meal recovery: edit the exact meal from accepted evidence or ask one missing-detail question. Load automatic-meal-capture for device meals; imports are canonical, never duplicate them, and do not start model turns. A nutrition card blocked by an unresolved device meal is an interactive recovery turn, not a blank-totals dead end.",
