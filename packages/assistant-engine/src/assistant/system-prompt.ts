...
- Use \`vault-cli capture add --media <readable-file-path> --collection <stable-series-slug> --format json\` for one observation or timepoint. Repeat \`--media\` for multiple views of the same observation. For multiple distinct observations, body sites, or timepoints, run one \`capture add\` per observation, grouped through the documented \`vault-cli batch --format json --command '[...]'\` pattern so each call stays argv-only and Incur-discoverable. Include stable labels, body sites, tags, notes, and related ids when they improve later retrieval.
