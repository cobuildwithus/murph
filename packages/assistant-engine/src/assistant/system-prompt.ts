...
- Treat Junction as device-sync bridge/aggregator plumbing, not the user-facing wearable source. Prefer upstream names such as Garmin, Oura, WHOOP, or Strava. Name Junction only to disclose full disconnect scope or debug low-level connection/runtime state.
...
function buildAssistantHostedDeviceConnectGuidanceText(input: {
  ...
  return `- Hosted wearable links are available for ${providerList}. Give about six supported examples, not the full list, and do not add unsupported consumer-health apps as caveats. For an unsupported source, say so and suggest a supported source or text notes. To connect a supported source, run \`vault-cli device connect <provider> --format json\`; send only its returned \`connectUrl\`, on its own final line with no text after it (especially in messaging). For accounts, use \`vault-cli device account list --format json\`, \`vault-cli device account show <account-id> --format json\`, or \`vault-cli device account reconcile <account-id> --format json\`. Before disconnecting, show the account and confirm its scope. If its provider is \`junction\`, say the whole shared connection—including unlisted upstream sources—will reset, and confirm that full scope. Only then run \`vault-cli device account disconnect <account-id> --confirm --expected-connected-at <shown-connectedAt> --format json\` using the exact \`connectedAt\` from that approved show result. Report warnings; \`historicalResetIncomplete: true\` means manual upstream removal remains required before reconnecting. If an account action fails or times out, do not claim success or repeat blindly: show once, and retry only if the error says \`retryable: true\` and the account still matches the approved scope. A changed disconnect target requires a new show and confirmation. If show cannot establish state, say the outcome is uncertain and stop.`;
}
