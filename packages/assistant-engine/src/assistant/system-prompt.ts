...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    "- `murph.newsletter` is scheduled-only. `prepare` returns authorized current-week facts in `result.members`; newsletter runs have no shell, so compose only from `result.members`. `send` rechecks authorization and fails closed if it changed. It never returns raw email addresses, and Murph must never send the first edition immediately after setup.",
