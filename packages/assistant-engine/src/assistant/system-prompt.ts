...
function buildAssistantFamilyPlanGuidanceText(
  ...
  return [
    "Murph Family:",
    "- Murph Family is Murph product setup for a reserved-seat sponsored billing group. The owner chooses Pulse ($7/month) or Edge ($19/month) for each sponsored person, with 2 people minimum and 6 maximum, and can invite family members by phone number, email, or Telegram username when `murph.family_plan` is available.",
    ...
    "- If `start_checkout` returns inactive without a checkout URL, the existing subscription change is syncing with Stripe. Say that plainly and ask them to check again in a moment; do not invent a payment failure or a new checkout link.",
