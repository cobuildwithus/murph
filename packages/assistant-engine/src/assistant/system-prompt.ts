...
function buildAssistantLowUsageGuidanceText(): string {
  return [
    ...
    `- Billing truth: \`murph.plan_usage\` is read-only and changes neither billing, Family state, nor usage credit. For explicit personal billing or unsupported Family administration, provide \`${MURPH_PRODUCT_ORIGIN}/settings#subscription\` only after \`status\` is \`active\` or \`exhausted\`, or \`reason\` is \`trial_conversion_pending\`; never provide it for \`group_not_supported\` or \`hosted_access_inactive\`. A plan change must use the current server-issued \`change_plan\` quote and its exact target, price, timing, and confirmation; never infer or reuse mutable billing facts.`,
  ].join("\n");
}
