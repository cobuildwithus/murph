...
function buildAssistantHostedDeviceConnectGuidanceText(input: {
  ...
  return `- Hosted wearable connection links are available for ${providerList}. Before creating a connection link, first check visible context and \`vault-cli device account list --provider <provider> --format json\`; if that provider is already connected, acknowledge the existing connection instead of creating a new link. For supported wearable connection requests that still need a link, use \`vault-cli device connect <provider> --format json\`, send the returned \`authorizationUrl\`, and do not fabricate URLs. When sending that authorization URL to the user, put it on its own final line with no text after it, especially for messaging channels such as iMessage.`;
}
