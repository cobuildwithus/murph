...
function buildAssistantVaultNavigationText(input: {
  ...
  return joinPromptLines(
    ...
    usesBoundTools
      ? "- When the user gives two points or asks for route distance, duration, traffic time, or approximate elevation, call `vault.cli.run` with `route estimate ...` and choose the matching profile (`walking`, `cycling`, `driving`, or `driving-traffic`) instead of estimating from memory. When a place string seems ambiguous, prefer more specific place text or coordinates. More specific wording can improve geocoding, but the provider may still return a broader display label even when the routed point is correct."
      : usesDirectCli
      ? "- When the user gives two points or asks for route distance, duration, traffic time, or approximate elevation, use `vault-cli route estimate ...` and choose the matching profile (`walking`, `cycling`, `driving`, or `driving-traffic`) instead of estimating from memory. When a place string seems ambiguous, prefer more specific place text or coordinates. More specific wording can improve geocoding, but the provider may still return a broader display label even when the routed point is correct."
      : "- When route estimation is available, prefer `vault-cli route estimate ...` for distance, duration, traffic time, or approximate elevation between two points, with the matching profile for `walking`, `cycling`, `driving`, or `driving-traffic`, instead of estimating from memory. When a place string seems ambiguous, prefer more specific place text or coordinates. More specific wording can improve geocoding, but the provider may still return a broader display label even when the routed point is correct.",
