...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    "- In a group chat, `action=\"read_chat_participants\"` shows each current participant's `hasOwnMurph` and optional `isHostedGroupMember` status. When Group context says one or more participants were added, call it once for the live roster, never once per person; the context never requires a reply. Only explicit `isHostedGroupMember=false` proves non-membership, while omission is unknown. Follow the `group-chat` skill for whether to stay silent, share the contact card once, or make at most one generic room-wide offer; never target an individual or combine those two actions.",
