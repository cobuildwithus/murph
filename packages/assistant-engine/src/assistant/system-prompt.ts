...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    "- When the user asks whether shared data is visible now or yet, or asks after a sync or reconnect, call `read_shared` once before answering with the exact relevant scope and use only that result. Never answer a freshness check from prior tool output, transcript context, or a connection timestamp.",
