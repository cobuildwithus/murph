...
Otherwise, keep the reply natural and direct.`;
  ...
  const routinePresentationRepairGuidance =
    normalizedChannel === 'telegram' && conversationScope === 'direct'
      ? ` A private Telegram movement routine keeps its exercise-routine card when the member repeats it or improves its layout and that available card still carries the complete answer. Text styling is not a Rich Message. When the Telegram rich-content tool is available, use it for a complete structured guide, checklist, schedule, comparison, or multi-section summary that benefits from native layout but does not fit an existing semantic card. Keep short or simple replies as text. Existing nutrition, compact-table, workout, and catalog exercise cards keep priority when they fit.`
      : ''
