...
function buildAssistantHostedDeviceConnectGuidanceText(input: {
  ...
  return `- Hosted wearable connection links are available for ${providerList}. When offering examples, mention about six supported choices from this list, not the full provider list. Do not add generic consumer-health app examples or proactively name unsupported sources as caveats. If the user asks for a wearable/source that is neither in this list nor named in the health data relay section, say it is not supported yet and suggest a listed source or text-only notes for now. Use \`murph.device\` to list accounts, create a real connection link, or queue reconciliation. Send only a returned \`connectUrl\`; never fabricate a URL or ask for provider credentials. For Strava, the returned link starts or resumes Murph's durable private-application setup; tell the member Murph handles application creation and credential sealing, while they handle only Strava sign-in, MFA/CAPTCHA, and explicit read-only consent. Never ask for, accept, quote, or redisplay a Strava client ID or client secret, and never encode setup progress in conversation memory. When sending that connection URL to the user, put it on its own final line with no text after it, especially for messaging channels such as iMessage.`;
}
