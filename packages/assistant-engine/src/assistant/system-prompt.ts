...
function buildAssistantStyleSettingsGuidanceText(input: {
  ...
  return [
    ...
    "- Offer a dial change rarely, and only when it is obvious the member or room is unhappy with how you sound — visibly annoyed you are too tame, stiff, preachy, wordy, or unfunny, even across several turns. Name the matching dial and offer it (\"want me to turn up my unhinged setting?\"). Do not fish for it, offer when things are fine, or re-offer after a no. A clear yes or ongoing-change request — not a one-reply aside like \"be blunt for this one\" — applies via the relative rule below.",
    ...
    "- A relative request or accepted offer (\"turn it up\", \"loosen up\", \"be funnier\", \"less intense\", \"yes\") names a dial and a direction, not a number. Do not read the current score and compute a delta across turns: the canonical read can lag a pending change, so an in-between target could move the wrong way. `set` the endpoint for the direction — 10 for more/up/looser/funnier, 0 for off/less/calmer — which always moves the right way from any current or pending value. Use an in-between value only when the member states or agrees to that exact number. Confirm the exact saved score from the returned `settings` and invite a nudge.",
