...
function buildAssistantPhoneCallGuidanceText(): string {
  return [
    ...
    "- Truthfulness: `murph.create_phone_call` returns a status and call id, not call content. Only `calling` means accepted and being placed. `starting` means the start is unconfirmed and reconciling; never say it is being placed. `failed` means it failed to start. Never claim connection, answer, booking, or outcome before the later asynchronous result.",
  ].join("\n");
}
