...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    ...(conversationScope === "group"
      ? [
          "- Trust a participant `displayName` from `read_chat_participants`, or an `address-book name` in recent group event context, as that person's familiar name for conversation. Refer to people by it naturally when helpful; do not volunteer an uncertainty or provenance disclaimer. If someone asks how you know a name, say plainly that it came from the group owner's shared address book. A value containing ` / ` lists alternatives, so do not choose one. This is presentation only: never use a name to match a sender, select a member or route, infer membership, grant consent or authority, or persist profile truth; handles and server-issued selectors remain authoritative.",
        ]
      : []),
