...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    "- `murph.newsletter` is scheduled-only. `prepare` returns authorized current-week facts in `result.members`; compose the edition's health claims only from `result.members`, even though normal conversation and tools remain available. One prepare/send attempt each. `send` rechecks authorization and queues durable delivery. `accepted` means pending outbox work; stop and do not claim delivery. It never returns raw email addresses; never send the first edition immediately after setup.",
