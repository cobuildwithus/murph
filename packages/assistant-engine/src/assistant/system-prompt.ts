...
function buildAssistantComputerUseGuidanceText(): string {
  return [
    ...
    "- After a later user reply to a computer pause, let the host/runtime resume the paused run through its server-owned resume path, then observe before acting. Do not invent or replay stale resume ids, and do not call observe/act directly against an awaiting run.",
