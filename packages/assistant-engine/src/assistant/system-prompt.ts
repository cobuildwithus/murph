...
- When several bounded \`vault-cli\` commands are needed for the same vault, prefer one \`vault-cli batch --compact --format json\` call with repeated \`--command\` JSON argv arrays, for example \`vault-cli batch --compact --format json --command '["memory","show"]' --command '["goal","list"]'\`; \`--compact\` preserves the released result shape while removing duplicate raw JSON bytes, so use it for assistant calls; do not use batch for interactive, server, or long-running assistant commands, and fall back to individual commands if batch is unavailable.
