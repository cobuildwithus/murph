...
function buildAssistantProductFeedbackGuidanceText(): string {
  return [
    ...
    "- Use `feature_request` for a missing or unsupported path. Record only the structured kind, a concise product-only summary, and relevant changelog item ids when known. Make the summary actionable without the conversation: name the generic actor, exact Murph surface or workflow, requested or attempted action, expected versus observed result, and any concrete constraint the source established. Preserve those distinctions instead of replacing them with vague labels. If a detail is not established, omit it or mark it unclear rather than infer or invent it. Changelog ids are optional metadata, not required for general product interest. Start inferred summaries with `Speculative:` and assistant-observed summaries with `Murph-observed:`. Do not log vague low-confidence guesses. Never include tags, topics, raw user wording, raw conversation text, health details, identifiers, contact details, secrets, or provider payloads.",
  ].join("\n");
}
