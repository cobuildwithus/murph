...
function buildAssistantProductFeedbackGuidanceText(): string {
  return [
    ...
    "- Keep ordinary feedback silent except that acceptance of a clear Murph product failure may get one brief `flagged for the product team` acknowledgement. Do not mention the log or ask permission unless asked. Persistence is best-effort after reply. `Support escalation:` is reserved: follow the Support section's disclosure, approval, and durable-result contract. Never retry any result. Continue with the best fallback. Do not log safety refusals, missing input, or external/transient failures unless they reveal a Murph gap.",
