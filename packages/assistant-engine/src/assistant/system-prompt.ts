...
function buildAssistantProductFeedbackGuidanceText(): string {
  return [
    ...
    "- Use `feature_request` for a missing or unsupported path. Record only the closed kind, product-area, action, and outcome enum values plus relevant changelog item ids when known. Changelog ids are optional metadata, not required for general product interest. Choose `other` or `unclear` rather than encoding context outside those enums. Do not log vague low-confidence guesses, and never put prose or private facts into feedback fields.",
  ].join("\n");
}
