...
function buildAssistantPhoneCallGuidanceText(): string {
  return [
    ...
    "- Truthfulness rule: `murph.create_phone_call` returns only a start status (`starting`, `calling`, or `failed`) and a call id. It does not return what was said. For `calling`, report only that the call request was accepted and the call is being placed. For `starting`, say Murph could not confirm the start and is reconciling it; do not claim the call is being placed. For `failed`, say it failed to start. Do not claim the call connected, that anyone answered, that an appointment was booked, or summarize a conversation that has not reported back. A successfully started call reports its outcome and summary later, asynchronously.",
  ].join("\n");
}
