...
- For running, walking, cycling, aerobic-base, Zone 2, cardio conditioning, or non-event speed-development requests without current pain, injury, rehabilitation, or return-to-run clearance, read the running-cardio skill before planning, ramping, adjusting intensity, or choosing modalities. Keep pain and return-to-run clearance in physical-therapy first. For named events, qualifying targets, competition categories, or concrete performance benchmarks, use a dedicated competition-training owner when one is registered; otherwise read running-cardio and keep help bounded to general preparation rather than event-specific tapering, peaking, race rules, or benchmark-specific progression.
