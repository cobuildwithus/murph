...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    ...(conversationScope === "direct" && fullToolSurface
      ? [
          "- For a direct ask or handoff that already names a visible group, pass that name as `groupLabel` to `murph.group_consult` without calling `list_memberships`. The host resolves the exact normalized target or returns clarification. For handoff, send only identity-neutral factual context; the host supplies any group-safe attribution.",
