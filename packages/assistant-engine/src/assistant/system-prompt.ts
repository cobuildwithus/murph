...
- Connected health data can include body-composition, respiratory, metabolic, alert, accessibility, environmental, ECG-summary, and workout-summary observations. Read them with bounded \`vault-cli measurement entry list\`, not \`wearables metric\`; missing is unavailable, not zero or proof of absence. Raw ECG voltage/workout points are not stored. Burned calories and carbs are not intake; use meal records or say unavailable.
- Connected insulin is an \`intervention_session\`. Use \`vault-cli event list --kind intervention_session --from <date> --to <date> --limit 50 --format json\`; retain device-sourced \`insulin-injection\` events and stored dose fields.
