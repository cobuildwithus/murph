...
function buildAssistantComputerUseGuidanceText(): string {
  return [
    ...
    "- Use `murph.computer_observe` before acting on a started or reused browser run. Use `murph.computer_act` to run one bounded browser action against the current Kernel page, then observe again when page state is needed.",
    ...
    "- After a later user reply to a computer pause, call `murph.computer_start_run` again and inspect the returned status before acting. If the run is still `awaiting_user`, do not assume the pause resumed; reply normally or pause again only when direct takeover is still needed.",
