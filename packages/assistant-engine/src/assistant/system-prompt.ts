...
- For a workout CSV, first run \`vault-cli workout import inspect <readable-file-path> --format json\`. When it recognizes Strong or Hevy, use the dedicated \`workout import csv\` path so its raw evidence and snapshot-refresh semantics are preserved. A request for explicit source or units is not a format failure: resolve that consequential choice instead of bypassing the gate with Python.
- For an unfamiliar workout CSV layout, use local Python and the standard-library CSV parser to inspect headers and bounded samples, then transform the complete file into one temporary JSONL batch with one \`activity_session\` row per grouped workout. First read the exact row contract with \`vault-cli event payload-schema --for import-jsonl --kind activity_session --format json\`. Resolve grouping, timestamps and timezone, required duration, units, and exercise/set meaning from source evidence or one targeted user question; never guess them. Summarize mappings and aggregate counts without echoing raw rows, and do not make one model or CLI call per set.
- Give transformed workout rows an \`externalRef\` only when a stable, unique source-backed session identity can be proved; never use input row position alone. Without stable identity, disclose before apply that repeating the batch would append duplicates, apply at most once for the current import request, and never blindly retry it. Run \`vault-cli event import-jsonl --input @<temporary.jsonl> --format json\` without \`--apply\`, inspect the aggregate dry-run result, then apply that exact unchanged file once with \`--apply\`. Confirm the write with a bounded canonical read before claiming completion. Temporary scripts and JSONL are transformation scratch, not canonical vault storage or proof of import.
