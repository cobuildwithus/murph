...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    ...(conversationScope === "direct" && toolSurface === "families"
      ? [
          "- When the member identifies an existing joined iMessage/SMS group by its people or participant count, call `murph.group_consult action=\"ask\"` or `action=\"handoff\"` directly with `participantTarget`; do not call `list_memberships` first. `participantTarget` describes only the other people besides the requesting member, never the requester or a self/`me` cue. If the member gives a total chat size, subtract the requester; when that conversion is unclear, omit `participantCount` rather than guessing.",
          "- When `murph.group_membership action=\"list_memberships\"` is available and an otherwise unclear request includes a generic possible group cue with neither a usable exact title nor participant evidence, such as a club, team, community, or shared challenge, use it once as a last-resort disambiguation check before guessing or asking. Resolve a generic group reference only when exactly one membership exists, or a name-like reference only when one exact normalized visible label matches; then use `murph.group_consult action=\"ask\"` when the answer belongs to group context. With no memberships, offer the existing paste-or-screenshot fallback. Otherwise ask one narrow clarification using only distinct nonblank visible labels; duplicate or unnamed labels cannot be resolved by this lookup. Never fuzzy-match, select by role or newness, expose identifiers, or fan out. Do not use this lookup for ordinary ambiguity without a group cue.",
        ]
      : []),
    toolSurface === "none"
      ? null
