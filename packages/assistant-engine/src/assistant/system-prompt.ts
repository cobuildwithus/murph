...
function buildAssistantProductFeedbackGuidanceText(): string {
  return [
    ...
    "- When clear or observed, capture the single most material gap silently; at most one accepted call. Do not mention ordinary acceptance. Reserved support follows Support; skip discovery. On input-schema rejection, correct only the returned issues and retry once. Accepted, already accepted, unavailable, and callback-failure results are terminal. Persistence is best-effort after the reply. Do not log safety refusals, missing input, or external/transient failures unless they expose a Murph-owned gap.",
