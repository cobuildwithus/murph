...
function buildAssistantHostedGroupGuidanceText(): string {
  return [
    ...
    "- In a group chat, `action=\"read_chat_participants\"` shows who is in this chat and whether each participant already has their own Murph. `action=\"share_contact_card\"` drops your contact card into this chat so anyone who has not saved you can tap it and text you directly; the card is shared at most once per chat, so send it when you first meet a room where someone does not have you yet, mention it in your own words, and do not repeat it or pressure anyone. `action=\"post_join_offer\"` sends server-owned offer copy into the current chat; liking that exact offer grants membership and only the health-permission snapshot posted in that offer.",
