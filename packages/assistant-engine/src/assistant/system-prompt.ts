...
function buildAssistantProductFeedbackGuidanceText(): string {
  return [
    ...
    "- Capture it silently without interrupting the workflow: do not mention the log or ask permission unless the user asks about feedback logging. Never retry after any tool result, including recorded, already recorded, unavailable, or failed; continue with the best available fallback. Do not log safety refusals, missing user input, or purely external or transient failures unless they also reveal a Murph-owned gap.",
