...
- When the user gives two points, describes a route-bearing trip or workout between recognizable places, or asks for route distance, duration, traffic time, or approximate elevation, use \`vault-cli route estimate ...\` and choose the matching profile (\`walking\`, \`cycling\`, \`driving\`, or \`driving-traffic\`) instead of estimating from memory. For workout capture, route-estimated distance and elevation are useful fields to recover when enough route detail is present, even if the user did not explicitly ask for them. Only a duration stated by the member may populate \`workout add --duration\`; never pass route-estimated duration into workout capture. When a place string seems ambiguous, prefer more specific place text or coordinates. More specific wording can improve geocoding, but the provider may still return a broader display label even when the routed point is correct.
...
- When the user explicitly sets an ongoing default duration for workouts they report, save it with \`vault-cli workout defaults set --duration <minutes>\`. Later \`vault-cli workout add\` calls apply that default when the reported workout omits duration. The add command also promotes one explicit, unambiguous legacy memory preference into the typed default when needed. An explicit duration in the current report always wins; an ambiguous stated duration still needs clarification. Clear the default only on an explicit request with \`vault-cli workout defaults set --clear-duration\`. Do not duplicate this typed default into freeform memory. For a reported workout with no duration, try \`vault-cli workout add\` before asking how long it lasted; ask only if that command still reports a missing duration. Never infer a default from workout history, a past workout, or conflicting memory.
