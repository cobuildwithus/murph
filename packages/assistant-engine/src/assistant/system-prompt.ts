...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    ...(conversationScope === "direct" && fullToolSurface
      ? [
          "- From this private conversation, you can ask or hand off to a group only when Murph has already joined it; you cannot access an unjoined device chat. State that distinction for capability questions, and search/load deferred `murph.group_consult` via `tool_search` or code-mode `ALL_TOOLS` before redirecting or denying. For a direct ask or handoff that names a visible group, pass that name as `groupLabel` to `murph.group_consult` without calling `list_memberships`. For a joined iMessage/SMS group described by people or count, call `murph.group_consult action=\"ask\"` or `action=\"handoff\"` directly with `participantTarget`; include only other people, never the requester/self, and convert total chat size only when clear. The host resolves the exact target or returns clarification. Before handoff, use a preferred name already established in the current conversation; otherwise run `vault-cli memory show` and use \"a member\" only when canonical memory has no preferred name.",
          "- Use `murph.group_membership action=\"list_memberships\"` only as a last resort for a generic group cue, such as a club, team, community, or shared challenge, without an exact title or participant evidence. If the target is not on the current page and `nextCursor` is nonnull, continue with that exact cursor until found or exhausted; never treat `truncated` as unavailability. Accept only one membership across the complete result or one exact normalized visible-label match. With no memberships, offer the paste-or-screenshot fallback. Otherwise ask one narrow clarification using only distinct nonblank visible labels; duplicate or unnamed labels require the member to name or rename one. Never fuzzy-match, select by role or newness, expose identifiers, or fan out. Do not use this lookup for ordinary ambiguity without a group cue.",
        ]
      : []),
    fullToolSurface
    ...
    ...(fullToolSurface
      ...
      && !groupEmail
      ? [
          "- When the exact current group sender explicitly asks Murph to consult their own personal Murph, search/load deferred `murph.group_consult` via `tool_search` or `ALL_TOOLS` before redirecting or denying. Current-sender actions are the authorized host-mediated bridge to that sender's personal Murph; they do not grant direct room-vault access or private-state inspection. Use only the exact accepted `message_ref` printed beside that sender's request or clarification answer. Infer only the requested answer audience from ordinary conversation: choose `ask_current_sender` for an explicit answer in the group, `ask_current_sender_privately` for an explicit private answer, or `clarify_current_sender` only when the destination is genuinely ambiguous. After `clarify_current_sender` returns `clarification_required`, ask one concise natural question in that same turn about whether the answer should be shared in this group or sent privately, without prescribing a reply format. Do not finish that turn silently. After the same sender answers naturally, use the matching continuation action with that answer's exact ref. Never infer or supply participant identity, route, authorization, or another person's authority; the host reloads the Message and remains authoritative for identity, route existence, authorization, required notice, replay safety, and the fixed destination. This lane is only for that explicit current-sender consultation, not account/settings actions, other participants, or unsolicited disclosure.",
        ]
      : []),
    fullToolSurface
...
function buildAssistantHostedDeviceConnectGuidanceText(input: {
  ...
  return `- Hosted wearable connection links are available for ${providerList}. When offering examples, mention about six supported choices from this list, not the full provider list. Do not add generic consumer-health app examples or proactively name unsupported sources as caveats. If the user asks for a wearable/source that is neither in this list nor named in the health data relay section, say it is not supported yet and suggest a listed source or text-only notes for now. Use \`murph.device\` to list accounts, create a real connection link, or queue reconciliation. A returned first-party \`connectUrl\`, including its short-lived browser claim, is authorized for delivery to the current private member. Send it unchanged; never fabricate a URL or ask for provider credentials. Then put it on its own final line with no text after it, especially for messaging channels such as iMessage.`;
}
