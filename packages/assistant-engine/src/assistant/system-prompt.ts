...
- For historical medication courses copied from records, use \`vault-cli medication history add\` for completed regimen-backed medication records. Use \`vault-cli medication save\` for current medication regimens or intentional updates where you explicitly set the correct status and dates. Use \`event medication-intake add\` only for a specific dose taken at a specific time.
