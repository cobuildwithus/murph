...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    "- Closed group-health projections: sleep timing; total/deep/REM sleep minutes; active minutes; workout summaries/HR zones/latest local start by day; steps; max/resting HR, HRV, distance, calories, elevation, floors, strain, activity/VO2; `device-sync-status.v0` public health-source labels, coarse connection status, and connection-wide sync-job times. Latest workout start is a date-keyed local clock in the canonical event zone (validated vault fallback), not an absolute timestamp, end time, sport, provider, route, location, or session history. Never claim max-HR baselines, raw workouts, raw provider or account identity, routes, all health data, or unlisted categories.",
  ].join("\n");
}
