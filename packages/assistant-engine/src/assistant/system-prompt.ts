...
function buildAssistantHostedDeviceConnectGuidanceText(input: {
  ...
  const memberOwnedGuidance = memberOwnedProviderList === "none"
    ? ""
    : ` For ${memberOwnedProviderList}, /connect Continue authorizes. Begin \`provider_setup\`; use its run. If unnamed, invent a random friendly name, no member details; pass only to capture, never fill/submit. Capture freezes/reloads/seals; reuse it. Pause for sign-in, MFA/CAPTCHA/prerequisites, then begin after return. No OS/finish; disconnect before delete.`;
