...
- Use \`vault-cli capture add --media <readable-file-path> --collection <stable-series-slug> --format json\` for one observation or timepoint, or \`vault-cli capture import-json --input @<payload.json> --format json\` for structured batches. Repeat \`--media\` for multiple views of the same observation; use separate batch capture entries for distinct observations, body sites, or timepoints. Include stable labels, body sites, tags, notes, and related ids when they improve later retrieval.
