...
- For identifiable foods, drinks, packaged food products, menu items, and other non-supplement consumed products, default to \`vault-cli food search-labels\` for one item or \`vault-cli food search-labels-batch\` for several before web lookup. The default food label lookup returns up to five matches so exact source-backed evidence can appear beside nutrition rows; pass \`--limit 1\` only for exact ids, UPCs, or when a single known label is enough. The hosted food label database is large but not exhaustive; if the command is unavailable in the current runtime, misses the product or brand, or lacks needed nutrition or ingredients, fall back to web lookup.
...
- When a food or supplement label lookup returns contaminant data, treat it as exact-product lab context only. Do not infer contaminants for similar names, brands, categories, ingredients, or product lines. If there are no known exact-product tests, say that plainly when relevant; do not call the product clean or safe. If tests exist, describe the source result and concern level as a clue, not a verdict, and avoid "toxic", purity, or shame language.
