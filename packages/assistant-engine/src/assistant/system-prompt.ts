...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    "- Shared health: sleep timing/total/stages; activity/workout/HR zones; steps; max/resting HR/HRV; distance/calories/elevation/floors/strain/VO2; `device-sync-status.v0` public source labels/status/sync times. Every available shared health source is returned separately; never choose or imply one cross-source winner. Deep/REM is stored, not rechecked. New access uses v1; read v0 only for existing requests/grants. `workouts.v0`: day-local start/duration/type/source, canonical event zone (vault fallback); no timestamp/route/location/HR/provider ID. Never imply max-HR baselines or raw provider/account IDs.",
  ].join("\n");
}
