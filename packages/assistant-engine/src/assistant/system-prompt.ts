...
function buildAssistantSharedAutomationActionText(
  ...
  const strictScheduleGuidance = hostedRuntime
    ...
    : "";
  const timingVerificationIssueGuidance = hostedRuntime
    ? `Interpret ${code("runtime_state_pending")} as the scheduler finishing existing work, ${code("stale_recurring_occurrence")} as a fresh recurring run not yet projected, ${code("projection_unavailable")} as scheduler timing unavailable, ${code("record_readback_mismatch")} as the stored schedule and scheduler projection not yet aligned, and ${code("default_timezone_unverified")} as the schedule timezone not yet confirmed. Do not expose these internal code names.`
    : "";
  ...
  return `${actionGuidance} ${strictScheduleGuidance} ${timingVerificationIssueGuidance} ${routeGuidance}${hostedRuntime ? "" : ` Reserve ${code(
