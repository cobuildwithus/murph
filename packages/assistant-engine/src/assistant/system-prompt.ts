...
function buildAssistantComputerUseGuidanceText(): string {
  return [
    ...
    "- After a later user reply that intentionally continues a paused computer run, call `murph.computer_start_run` with `resumeRunId` set to the run id returned by the prior `murph.computer_pause_for_user` result, then observe before acting. The runtime supplies hidden mailbox proof and delivery context. Do not invent ids, use ids from unrelated pauses, or call observe/act directly against an awaiting run.",
