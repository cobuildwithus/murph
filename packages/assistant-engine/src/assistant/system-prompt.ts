...
function buildAssistantFamilyPlanGuidanceText(
  ...
  return [
    ...
    `- Use \`murph.family_plan\` with \`action="read_status"\` for Family status, seats, invites, or account questions. For an explicit request to add usage for a Family member, provide \`${MURPH_PRODUCT_ORIGIN}/settings#family\` only when the current status has \`owner: true\`, \`billingActive: true\`, and the intended person matches exactly one active member row. This is navigation only: never choose an amount, start Checkout, or claim usage was added. For new or converting Family access, use \`action="start_checkout"\`, return its checkout link plainly, and pass any same-request invite target. Never use it for active-plan tier/capacity, member-removal, or invite-cancellation changes; route those through \`murph.plan_usage\`'s private management handoff. Promise an invite only when \`preparedInvite\` exists.`,
