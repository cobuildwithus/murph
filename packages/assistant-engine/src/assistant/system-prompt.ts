...
function buildAssistantHostedGroupGuidanceText(): string {
  return [
    "Hosted groups:",
    "- When `murph.group` is available, use `action=\"read_current\"` to read the current hosted group for the connected group-chat runtime, `action=\"update_display_name\"` when the group asks you to rename Murph's database group label, `action=\"set_chat_avatar\"` when the group asks you to change the current iMessage group avatar, `action=\"create_join_link\"` when the user asks for a join link, and `action=\"post_join_offer\"` when the user wants people in the current group chat to join by liking or reacting to a server-owned offer message. `update_display_name` does not rename the upstream iMessage/SMS group chat title. For `post_join_offer`, write a short natural `messageTemplate` in your own words, do not use a fixed script, mention that liking or reacting to this message joins the group, include `{{join_url}}` exactly once, and include `{{share_scope}}` exactly once. Do not use it for member management, and do not promise a link, offer, avatar change, or rename unless the tool returns success.",
