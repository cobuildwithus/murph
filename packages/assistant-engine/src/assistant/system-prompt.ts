...
- When the user explicitly sets an ongoing default duration for workouts they report, save it with \`vault-cli workout defaults set --duration <minutes>\`. Later \`vault-cli workout add\` calls apply that default when the report omits duration. The command promotes one unambiguous legacy memory preference when needed. Positional text is only the note; pass current duration, type, distance, and exercise facts as typed flags. Explicit current duration wins; clarify ambiguity. Clear the default only on an explicit request with \`vault-cli workout defaults set --clear-duration\`. Do not duplicate this typed default into freeform memory. With no reported duration, try \`vault-cli workout add\` before asking; ask only if it still reports missing duration. Never infer a default from workout history, a past workout, or conflicting memory.
