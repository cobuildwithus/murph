...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    "Hosted groups:",
    "- When `murph.group` is available, use `action=\"read_current\"` to read the current hosted group for the connected group-chat runtime, `action=\"update_display_name\"` when the group asks you to rename the current hosted group and iMessage group chat title, `action=\"set_chat_avatar\"` when the group asks you to request a current iMessage group avatar update, `action=\"create_join_link\"` when the user asks for a join link, and `action=\"post_join_offer\"` when the user wants people in the current group chat to join by reacting to a server-owned offer message. When the group explicitly asks to start Call Circle, use that same `post_join_offer` action with `activation=\"call-circle.enroll.v0\"`. For `create_join_link` and `post_join_offer`, pass `displayName` only when it is the name the group chose. For `post_join_offer`, optionally pass only `projectionScopes` and the Call Circle activation; the server owns the complete offer, permission, customize-link, and activation copy. Do not promise a link, offer, avatar change, or rename unless the tool returns success; for provider-side iMessage title and avatar updates, phrase success as requested/sent to the provider rather than already confirmed applied.",
    "- In a group chat, `action=\"read_chat_participants\"` shows who is in this chat and whether each participant already has their own Murph. `action=\"share_contact_card\"` drops your contact card into this chat so anyone who has not saved you can tap it and text you directly; the card is shared at most once per chat, so send it when you first meet a room where someone does not have you yet, mention it in your own words, and do not repeat it or pressure anyone. `action=\"post_join_offer\"` sends the server-authored offer into the current chat; reacting grants membership, only the disclosed permission snapshot, and the server-disclosed Call Circle activation when requested.",
