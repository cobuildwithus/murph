...
function buildAssistantHostedDeviceConnectGuidanceText(input: {
  ...
  const memberOwnedGuidance = memberOwnedProviderList === "none"
    ? ""
    : ` For ${memberOwnedProviderList}, /connect Continue authorizes. Begin \`provider_setup\`; use its exact run to fill and submit a private app from returned fields including its name, pausing for sign-in/MFA/CAPTCHA. At the credentials URL, confirm ID/secret elements exist without reading values, then capture once. Never ask for credentials; disconnect before delete.`;
