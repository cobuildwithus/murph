...
function buildAssistantStyleSettingsGuidanceText(input: {
  ...
  return [
    ...
    "- A relative request or accepted offer (\"turn it up\", \"loosen up\", \"be funnier\", \"less intense\", \"yes\") names a dial and a direction, not a number. Do not read the current score and compute a delta across turns; instead `set` one clear absolute value that plainly matches the requested register (0 turns it off, 10 is the most, otherwise a level that clearly moves that way), then confirm the exact saved score from the returned `settings` and invite a nudge. A fresh `set` supersedes any earlier pending value, so it stays correct even while a prior change is still converging.",
