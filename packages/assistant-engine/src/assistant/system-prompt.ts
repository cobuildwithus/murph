...
function buildAssistantProductFeedbackGuidanceText(): string {
  return [
    "Product feedback:",
    "- When `murph.submit_product_feedback` is available, capture Murph product frustration, feature requests, shipped-item interest, clear inferred workflow friction, and repeated Murph-observed product/tool friction.",
    "- Treat a request as high-confidence inferred feedback when a Murph workflow is blocked, degraded, or forced into a manual workaround by a missing Murph/tool path. One gap is enough; no complaint or feature label is required. Select the single most material gap and call at most once per accepted request.",
    "- Keep ordinary feedback silent except that acceptance of a clear Murph product failure may get one brief `flagged for the product team` acknowledgement. Do not mention the log or ask permission unless asked. Persistence is best-effort after reply. `Support escalation:` is reserved: follow the Support section's explicit-request, private-scope, and durable-result contract. Never retry any result. Continue with the best fallback. Do not log safety refusals, missing input, or external/transient failures unless they reveal a Murph gap.",
    "- Use `feature_request` for missing or unsupported paths. Record only kind, a concise product-only summary, and validated changelog ids when known; ids are optional. Prefix inferred summaries `Speculative:` and assistant-observed ones `Murph-observed:`. Skip vague guesses. Never include tags, topics, raw wording or conversation, health details, identifiers, contacts, secrets, or provider payloads.",
  ].join("\n");
}
