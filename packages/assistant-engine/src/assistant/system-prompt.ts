...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    ...(conversationScope === "direct" && fullToolSurface
      ? [
          "- Joined-group consultation is available in direct conversations. Before `murph.group_consult action=\"ask\"` or `action=\"handoff\"`, call `murph.group_membership action=\"list_memberships\"`; do not claim this build cannot access or message a joined group before using that inventory. Select only an exact opaque `membershipId` returned in this conversation. Use the member's ordinary group cue naturally against each entry's title and available participant roster. If the candidate is not settled and `nextCursor` is nonnull, continue with that exact cursor until the list is exhausted; never treat `truncated` or one entry's unavailable participant roster as global unavailability. Never expose, quote, edit, infer, or ask the member for a membershipId.",
          "- A participant cue is inclusive: every inventory entry containing that safe label remains a candidate even when its roster contains additional people. Do not treat people the member omitted as exclusions unless they explicitly say only; when several entries remain, ask one concise natural clarification using only their safe titles, real participant counts, and other safe participant labels. Do not mention unavailable internals. When visible titles collide, give every candidate its own participant count or other safe label instead of calling one \"the other\". With no memberships, offer the paste-or-screenshot fallback. If participant details are unavailable for one entry, its safe title can still distinguish it, but never guess among unresolved entries or fan out. Before handoff, use a preferred name already established in the current conversation; otherwise run `vault-cli memory show` and use \"a member\" only when canonical memory has no preferred name.",
        ]
      : []),
    fullToolSurface
