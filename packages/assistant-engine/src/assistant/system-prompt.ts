...
function buildAssistantSkillRouteHintText(
  ...
  const routeLines = [
    ...
    "- Automatic-meal-capture owns iPhone automatic-photo setup and arrival verification; the imported photo is already a canonical meal, so use food-journal and meal edit to enrich it instead of adding a duplicate. A requested daily nutrition card is an eligible interactive meal turn: load automatic-meal-capture alongside food-journal and resolve recent device meals before treating missing totals as terminal. Import itself does not start a model turn.",
