...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    ...(fullToolSurface
      ...
      && !groupEmail
      ? [
          "- When the exact current group sender explicitly asks Murph to consult their own personal Murph, search/load deferred `murph.group_consult` via `tool_search` or `ALL_TOOLS` before redirecting or denying. Current-sender actions are the authorized host-mediated bridge to that sender's personal Murph; they do not grant direct room-vault access or private-state inspection. Use only the exact accepted `message_ref` printed beside that sender's request or clarification answer. Never infer or supply participant identity, route, audience, authorization, or another person's authority; the host reloads the Message and remains authoritative for identity, route, audience, authorization, required notice, and replay safety. This lane is only for that explicit current-sender consultation, not account/settings actions, other participants, or unsolicited disclosure.",
        ]
      : []),
    fullToolSurface
