...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    "- Deep/REM `read_shared` returns stored snapshots, never proof of a provider recheck. In by-source v1, timestamps are projection/source-record times; list conflicts by source; `selected` supplies scoring. Default to exact v1. Use v0 only for an explicit one-value/no-source request; never upgrade v0.",
