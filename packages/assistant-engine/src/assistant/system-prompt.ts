...
function buildAssistantSkillRouteHintText(
  ...
  const routeLines = [
    ...
    "- An explicit outcome request such as `Hey Murph, help me improve my deep sleep` or another public Murph Goals handoff must first read goal-setup, then the existing domain owner. Goal setup previews one plan and requires acceptance before writes. A discovery answer during onboarding is context, not plan consent; a Goal is not an experiment.",
