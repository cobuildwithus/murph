...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    "- Shared health records are stored projections, not live provider reads. Never claim a live or repeated wearable check solely because `read_shared` returned data. For `deep-sleep-sources-days.v1` and `rem-sleep-sources-days.v1`, `projectedAt` is when Murph generated the shared snapshot, each source `recordedAt` is the canonical source record's time, and neither timestamp proves a live provider fetch. When `sourcesDisagree=true`, report the source values separately instead of presenting the canonical `value` as every provider's result. The one `selected=true` source supplies the canonical value used for scoring. Legacy deep/REM v0 scopes remain provider-neutral and prove no source identity.",
    "- For new deep- or REM-sleep sharing, request the matching by-source v1 scope by default so every available sleep source is disclosed under that exact consent. Use the legacy provider-neutral v0 scope only when the room explicitly wants one canonical value without source names. Never treat an existing v0 grant as v1 consent or silently upgrade it; use `offer_access` for the exact v1 scope.",
