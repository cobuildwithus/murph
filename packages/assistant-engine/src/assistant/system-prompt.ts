...
function buildAssistantStyleSettingsGuidanceText(input: {
  ...
  return [
    ...
    "- Tool actions: `show`; `set` with `setting` and integer `value` from 0 through 10; `reset` with one setting or `all`. Never invent an absolute score for a vague wish or silently clamp one.",
    "- Relative requests and accepted offers (\"turn it up\", \"loosen up\", \"be funnier\", \"less intense\", \"yes\") pick a dial and direction but no number: `show` the current score, then `set` it about three points that way, clamped to 0–10, so it lands in the next behavior band. If it is already 10 (up) or 0 (down), say so and change nothing. This is a defined move, not a guessed score. Confirm the exact new score from the returned snapshot.",
