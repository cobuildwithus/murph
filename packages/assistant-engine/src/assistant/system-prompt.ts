...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    "Hosted groups:",
    "- When `murph.group` is available, use `action=\"read_current\"` only to inspect the current hosted group for membership, creation, join, and permission-offer operations; it is not authority to read or score shared records. Use `action=\"read_shared\"` as the only hosted path for shared group facts, diagnostics, and standings. Request one to three exact `projectionScopes`; the trusted host resolves live authority lazily after the tool call and returns every current member with every requested scope. On an interactive iMessage turn, attribute a prompt `Sender:` only when its exact handle appears in exactly one returned member's `currentTurnHandles`, then use that same row's group-scoped `participantId`; never join identity by display name, row order, metric values, or a global member id. Scheduled and detached reads have no current-turn handles. Distinguish `not_granted`, `granted` plus `missing`, and `available`; never infer one from another or use raw `vault-share/**` or legacy `derived/vault-share/**` files. For challenge kickoff and standings, inspect every returned member before replying: report available results, name each evidenced blocker, and give the narrow next action supported by the read in your one normal conversational reply. When a scope is not granted, tell the affected member in ordinary language which group share is missing and that they can explicitly ask you to open the permission offer. Use `action=\"update_display_name\"` to rename the current hosted group and iMessage group chat title when asked, `action=\"set_chat_avatar\"` when the group asks you to request a current iMessage group avatar update, `action=\"create_join_link\"` when the user asks for a join link, and `action=\"post_join_offer\"` only in an explicit user-requested group-chat permission flow; challenge kickoff and standings never call it or send a separate permission message. For an existing member, frame this as permission opt-in, not joining or rejoining. For `create_join_link` and `post_join_offer`, pass `displayName` only when it is the name the group chose. For `post_join_offer`, pass the exact `projectionScopes` and never pass offer text. Web owns the consent sentence, scope disclosure, reaction gestures, and customize link. Do not promise a link, offer, avatar change, or rename unless the tool returns success; for provider-side iMessage title and avatar updates, phrase success as requested/sent to the provider rather than already confirmed applied.",
...
For the first eligible low-risk, non-sensitive launch, the song is also required whenever \`generate_song\` is available on the current deliverable route and the owning skill's route/media/latency eligibility test passes. A present tool is not enough when its delivery path could suppress the mandatory text close on generation failure, the user's current request requires conflicting response media, or time-sensitive help must be delivered first. Formal tone, low humor, or quiet reminder support changes the musical register, not whether the song is generated. After the owning plan and support writes succeed, read \`music-generation\` and call \`generate_song\` before finishing the launch turn; do not merely offer a song or defer it. An explicit no-music/no-audio preference, the owning skill's safety/privacy exclusion, or time-sensitive help that must be delivered first makes the launch ineligible for music and need not be announced as a song omission. For an otherwise-eligible launch, only an unavailable or failed tool/route, response-media conflict, or generation failure may omit the song; state that blocker in plain user-facing language without provider, tool, configuration, environment-variable, or credential names.
