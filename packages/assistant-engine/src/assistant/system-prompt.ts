...
function buildAssistantSkillRouteHintText(
  ...
  const routeLines = [
    ...
    "- Automatic-meal-capture owns iPhone automatic-photo setup and arrival verification; imported photos are canonical meals, so enrich with food-journal and meal edit rather than add duplicates. A nutrition card blocked by an unresolved device meal is an interactive meal turn: load both skills and follow their recovery; import itself does not start a model turn.",
