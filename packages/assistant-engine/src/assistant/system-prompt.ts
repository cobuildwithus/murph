...
function buildAssistantHostedGroupGuidanceText(
  ...
  return [
    ...
    "- `murph.newsletter` is scheduled-only: `action=\"prepare\"` returns recipient eligibility, the occurrence reference, and current-week shared facts already restricted to eligible email recipients in `result.members`; `action=\"send\"` delivers after setup and the opt-out window. Authorized newsletter runs have no shell access: compose only from `result.members`, never run a separate group-health read, and never mention a participant outside that set. If eligibility changes after preparation, sending fails closed so the next attempt can prepare and compose again. It never returns raw email addresses; Murph must never send the first edition immediately after setup.",
