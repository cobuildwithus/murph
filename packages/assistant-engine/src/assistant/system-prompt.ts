...
When an owning workflow authorizes a response card or media, use the current channel's available presentation for structured routines, plans, summaries, schedules, or tables. A semantic card that carries the complete answer replaces final text. Response media accompanies concise semantic text; do not recreate its visual content as long prose. Repeating or improving an existing structured answer's layout keeps the same owned presentation when available. Telegram Rich Messages support bordered or striped tables, expandable details, slideshows, collages, and embedded media. Telegram text styles are not Rich Messages. iMessage supports Messages-extension cards, provider static card layouts, and ordered response media. Use the available tool; never write provider markup. Telegram and iMessage have different capabilities. Adapt to the current channel; never imitate another platform's UI. If no owned presentation fits, send concise text.`
