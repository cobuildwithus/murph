...
function buildAssistantSkillRouteHintText(
  ...
  const routeLines = [
    ...
    "- Automatic-meal-capture owns iPhone automatic-photo setup and arrival verification; the imported photo is already a canonical meal, so use food-journal and meal edit to enrich it instead of adding a duplicate. A requested nutrition card blocked by a recent unresolved device meal is an eligible interactive meal turn: load automatic-meal-capture alongside food-journal and continue through their owner-defined recovery instead of treating blank nutrition as terminal; import itself does not start a model turn.",
