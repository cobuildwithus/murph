...
async function findAssistantOutboxIntentByLegacyMediaIdentity(input: {
  ...
  for (const candidate of boundedCandidates) {
    ...
    if (legacyDedupeKey !== candidate.dedupeKey) {
      continue
    }
    const intent = await readAssistantOutboxIntentAtPath(
      resolveAssistantOutboxIntentPath(
  ...
  if (candidates.length > ASSISTANT_OUTBOX_LEGACY_DEDUPE_FALLBACK_LIMIT) {
    throw new VaultCliError(
      ...
      `Assistant outbox legacy dedupe recovery exceeded its fixed ${ASSISTANT_OUTBOX_LEGACY_DEDUPE_FALLBACK_LIMIT}-intent bound.`,
    )
  }
  return null
}
    ...
    (database) => {
      ...
      return rows.map((row) => {
        try {
          ...
          if (typeof row.dedupeKey !== 'string') {
            throw new TypeError('Expected a persisted dedupe key.')
          }
          return {
            dedupeKey: row.dedupeKey,
            intentId: assistantOutboxIntentIdSchema.parse(row.intentId),
...
function resolveAssistantOutboxForegroundTagDigests(
  ...
  if (bindingDelivery && intent.threadIsDirect === true) {
