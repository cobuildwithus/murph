...
export const MURPH_ONBOARDING_FIRST_PERSONAL_READ_INSTRUCTIONS = [
  ...
  '',
  'Durable dedupe:',
  '- When a new send-worthy read exists, settle the complete outbound text first. Best-effort run `vault-cli knowledge append-section weekly-health-insights "First read <Occurrence local date>" --title "Weekly Health Insights" --position prepend --body <markdown> --source-path <path> ...`. The body contains only the compact claim, evidence, uncertainty, and exact outbound text. Cite only canonical source paths actually used, never `derived/**` or `.runtime/**` paths.',
