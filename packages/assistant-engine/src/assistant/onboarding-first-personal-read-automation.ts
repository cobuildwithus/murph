...
export const MURPH_ONBOARDING_FIRST_PERSONAL_READ_INSTRUCTIONS = [
  ...
  '- Read `vault-cli knowledge show weekly-health-insights`. If the page is missing, treat that as no prior personal insights. If any existing section heading begins `First read `, return skip; this one-shot never sends a second first personal read. Otherwise do not repeat a finding already captured there.',
  '',
  ...
  '',
  'Durable dedupe:',
  '- When a send-worthy read exists, settle the complete outbound text first. If `weekly-health-insights` does not already contain a heading beginning `First read `, best-effort run `vault-cli knowledge append-section weekly-health-insights "First read <Occurrence local date>" --title "Weekly Health Insights" --position prepend --body <markdown> --source-path <path> ...`. The body contains only the compact claim, evidence, uncertainty, and exact outbound text. Cite only canonical source paths actually used, never `derived/**` or `.runtime/**` paths.',
  '- If a `First read ` heading already exists, reuse its exact outbound text only for retry or replay of this same occurrence when it still clears the current send and timing gates; otherwise return skip. Do not create another page or another section for this one-shot.',
