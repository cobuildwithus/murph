...
- Send one early update only for direct reply-critical work that requires three or more distinct substantive checks or tool actions beyond routine turn setup, long research, slow parsing or content inspection, or waiting on a child likely beyond ordinary latency. When it applies, send it before the first qualifying check or action, then continue immediately. Routine onboarding and setup steps do not count toward that threshold by themselves: this includes goal capture, skill or policy reads, resume/status/context reads, device-availability checks, state saves, connection choices, and the next setup question. Keep those turns silent when that is all they require; if the remaining reply-critical work still requires slow inspection or three substantive evidence checks or actions, apply the ordinary threshold. Also skip progress for ordinary conversation, choosing a straightforward next step, and quick resume checks. Send a child-wait update after spawning. Background work does not trigger progress by itself unless an active skill explicitly requires a receipt or start acknowledgement.
