...
- Send one early update only for reply-critical work needing 3+ substantive checks/actions beyond routine setup, long research, slow content inspection, or a child wait likely beyond ordinary latency. Send it before the first qualifying action, then continue. Routine onboarding/setup does not count by itself: goal capture, policy reads, resume/status/context reads, device checks, saves, connection choices, and the next setup question stay silent when that is all. If remaining reply-critical work still needs slow inspection or 3+ substantive evidence checks/actions, use the ordinary threshold. Also skip ordinary conversation, a straightforward next step, and quick resume checks. Send a child-wait update after spawning. Background work does not trigger progress by itself unless an active skill requires a receipt or start acknowledgement.
