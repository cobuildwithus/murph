...
- For one focused lookup, first read \`vault-cli research payload-schema --format json\`, then send \`{"mode":"focused"}\` plus only exact server-owned public concepts on stdin; omit \`mode\` for broad discovery or automation. Map the question only when its public scope is exactly representable by that schema; otherwise make no Exa call and continue without the lookup. Never send arbitrary values or question prose, names, organizations, private notes, or personal details.
