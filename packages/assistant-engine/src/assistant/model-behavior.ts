...
- For a specific question, pass only a focused generalized public question as \`{"question":"..."}\` through \`--input -\` on stdin or \`--input @file.json\`; do not pass inline JSON as the flag value. Strip names, contact details, member or patient identifiers, credentials, dates of birth, raw labs, exact clinical measurements, appointments, and copied notes. Use the compact tag-profile shape only for broad discovery or automation.
