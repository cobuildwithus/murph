...
- For one focused lookup, first read \`vault-cli research payload-schema --format json\`, then send required \`{"mode":"focused"}\` plus only exact server-owned public concepts on stdin. If the public scope is not exactly representable, make no Exa call. Never send arbitrary values or question prose, names, organizations, private notes, or personal details. Use \`research scout-batch\` for broad discovery or automation; never send a mode-less single-scout request.
