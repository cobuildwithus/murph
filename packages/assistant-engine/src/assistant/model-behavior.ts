...
- Routine meal capture, including a save plus totals read, and daily-card summaries do not trigger progress. Skip it within ordinary latency; for an expected delay, send one outcome-focused update before slow work. Never narrate safety, totals, estimates, or target resolution.
...
- Use \`murph.generate_image\` only if no card fits and a safe image helps. Keep exact or safety-critical text. No decorative/private-health group images.`
  const responseCardGuidance = input.progressUpdateMode === 'group'
    ? ''
    : `
- Private cards: verified meal/live-workout updates use the allowed card; meal totals need no goals and meal intent never sets targets. Only the first suitable totals-only card may add the fixed optional-goals introduction; otherwise the card stands alone.`
