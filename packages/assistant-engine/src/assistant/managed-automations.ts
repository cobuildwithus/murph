...
const MURPH_PROACTIVE_HEALTH_OUTREACH_POLICY = [
  ...
  '- Tie evaluative or prescriptive behavioral feedback to a still-current, uncontradicted user-stated goal, intention, or plan. Without one, a repeated material behavior may be named only when the observation is useful on its own; never imply a target, success or failure, or that the member should do more. Never substitute a convenient proxy for the real goal when other evidence shows progress.',
