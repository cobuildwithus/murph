...
export const MURPH_MANAGED_AUTOMATIONS = [
  ...
  },
  {
    ...
    instructions: [
      ...
      '',
      'Changelog kind:',
      `- Fetch the canonical JSON feed once from ${MURPH_PRODUCT_ORIGIN}/api/changelog?days=14&featureLimit=70&improvementLimit=0.`,
      ...
      '- Treat this as a member-facing product update, not a dump of release notes. Pitch new capabilities the member can use, never release notes about work Murph did on itself.',
      '- Never pitch reliability work. When the substance of an item is that something now works, resumes, keeps up, stops failing, or was repaired, drop it even if the feed lists it as a feature and even if this member hit that issue. Reliability is answered in conversation when a member raises it, not offered as product news.',
