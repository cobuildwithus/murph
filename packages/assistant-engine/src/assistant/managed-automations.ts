...
export const MURPH_MANAGED_AUTOMATIONS = [
  {
    ...
    instructions: [
      ...
      '- Suppress: If the week was ordinary — numbers inside the user\'s usual ranges, no notable context, no experiment movement — or if there are no connected device accounts, no live wearable, no recent manual logs, and no experiment movement worth mentioning, return `{"kind":"skip","privateSummary":"No weekly digest cleared the memorability bar."}` and suppress the scheduled message. Skipping an unremarkable week is the expected outcome, not a failure. Do not send a process note or a "quiet week" message.',
      '',
  ...
  },
  {
    ...
    instructions: [
      ...
      '- Before calling external research, name at least one current experiment, plan, metric, symptom, lab, device trend, live tradeoff, recent change, or clinician question that retrieved research could answer. If none exists, suppress the scheduled message without calling `vault-cli research scout-batch` and do not append to the wiki.',
