...
export const MURPH_MANAGED_AUTOMATIONS = [
  ...
  },
  {
    ...
    instructions: [
      ...
      '- Define 1-4 focused research lanes from that compact profile. Group related tags into one lane; do not create one lane per tag when the tags are variants of the same question.',
      '- If unsure about the batch file body, run `vault-cli research scout-batch-payload-schema --format json` before the scout call.',
      '- Use `vault-cli research scout-batch` once. The `--input` body is `{"lanes":[{"label":"sleep","profile":{...}}]}`. Each lane profile uses bucket fields `topics`, `biomarkers`, `behaviors`, `supplements`, `conditionsOrConcerns`, `goals`, and `activeExperiments`; do not use a generic `tags` field. Example body: `{"lanes":[{"label":"sleep","profile":{"topics":["sleep"],"behaviors":["morning light"]}},{"label":"training recovery","profile":{"topics":["recovery"],"behaviors":["resistance training"]}}]}`.',
      '- Pass publication bounds as `--since` and `--until`; YYYY-MM-DD dates or full ISO timestamps are accepted. Prefer the last two years and cap `--maxCandidatesPerLane` at 5 for this automation.',
      ...
      'If something clears the bar:',
      '- Send at most 1 item.',
      '- Treat the batch lanes and `--maxCandidatesPerLane` as the retrieval budget, not the final output count; review and dedupe the lane-tagged candidate pool, then choose only the single best item if one clears the bar.',
