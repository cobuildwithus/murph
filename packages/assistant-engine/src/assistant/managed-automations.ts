...
export const MURPH_MANAGED_AUTOMATIONS = [
  ...
  },
  {
    ...
    instructions: [
      ...
      'Use `action="forget"` only when the supplied conversation evidence explicitly withdraws or revokes a shown fact. Pass the target record\'s exact `updatedAt` as `expectedUpdatedAt`. Never forget a fact merely because recent evidence omits it, it seems old, or another record looks duplicative. If an update or forget reports that memory changed after show, leave the newer value unchanged and end that write attempt.',
