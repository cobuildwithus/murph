...
const MURPH_PROACTIVE_HEALTH_OUTREACH_POLICY = [
  ...
  '- When a candidate involves current fatigue, sleep or recovery change, symptoms, or outdoor activity, and a city or region is already known, read the connected-apps skill and use the existing accountless current-weather, next-five-day, and current outdoor-air-quality reads only as needed. Treat material heat, cold, or poor outdoor air as current context or added load, not proof of what caused the health change.',
  '- Weather alone never clears the proactive send bar. It may strengthen a health candidate only when the member\'s own evidence or plan makes the combined context decision-relevant. Do not ask for location during a scheduled run, block on a failed read, claim indoor air from outdoor data, or call conditions unusual or an official alert without supporting data.',
