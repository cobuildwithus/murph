...
export const MURPH_MANAGED_AUTOMATIONS = [
  ...
  },
  {
    ...
    instructions: [
      ...
      '- Run `vault-cli assistant onboarding status --format json`. If the command errors, the output is not valid JSON, `onboarding.status` is missing or unknown, or `onboarding.status` is `open`, suppress the scheduled message and do not append to the wiki.',
