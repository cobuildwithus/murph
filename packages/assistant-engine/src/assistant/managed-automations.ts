...
export const MURPH_MANAGED_AUTOMATIONS = [
  ...
  },
  {
    ...
    instructions: [
      ...
      'Read existing saved context by calling `murph.member_memory` with `action="show"` first. Existing memory may support faithful shortening of that same record or expiry of its own explicit temporary Context window; it cannot support new facts, inferred traits, or broader preferences.',
      ...
      'First apply clear user corrections and withdrawals to the exact existing record. Then shorten unnecessarily verbose non-health records in place without changing their meaning. Never combine records, delete apparent duplicates, drop a qualification to fit a budget, or rewrite an already concise record. Preserve source dates in the text; updatedAt records an edit, not fresh confirmation by the user. Make at most one mutation per shown record in this pass.',
      ...
      'For temporary non-health Context only, use forget when the entire record describes only that temporary circumstance and the record itself states an unambiguous finite window that has fully ended before the trusted current local date, and no supplied user evidence extends it. Never infer expiry from updatedAt, age, silence, relative dates without an explicit anchor, or a mere deadline for an unfinished goal. Leave ambiguous dates unchanged. Do not recreate an expired or withdrawn fact from older messages in the overlapping evidence window.',
      'Before returning, validate every addition or factual change against supplied conversation evidence; validate faithful shortening and explicit expiry against the exact shown record as well. Skip anything uncertain, duplicated, sensitive, or merely transient task detail.',
