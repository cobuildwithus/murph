...
export const MURPH_MANAGED_AUTOMATIONS = [
  ...
  },
  {
    ...
    instructions: [
      ...
      '- Search the knowledge wiki for related prior findings, including prior `weekly-health-insight-*` pages and any legacy `weekly-health-insights` page if it exists.',
      ...
      '- Save it to the derived knowledge wiki as one immutable, date-scoped page for this scheduled occurrence, using the current local date in the slug: `weekly-health-insight-YYYY-MM-DD`.',
      '- First check the current-date page with `vault-cli knowledge show weekly-health-insight-YYYY-MM-DD`.',
      '- If that current-date page already exists, treat it as this run\'s finding: read it, do not overwrite it, do not create another page, and still send the concise note from that page. Only older pages should disqualify a candidate as a duplicate.',
      '- Do not write a shared rolling page or append to older weekly insight pages. This avoids losing history from concurrent wiki writes.',
      '- If the current-date page is missing, create it with the no-overwrite knowledge write surface, for example: `vault-cli knowledge upsert --create-only --slug weekly-health-insight-YYYY-MM-DD --title "Weekly health insight - YYYY-MM-DD" --body <markdown> --source-path <canonical-vault-path>`. Cite only canonical vault source paths, never `derived/**` or `.runtime/**` paths.',
      '- If create-only reports that the page already exists, another run created it first: read it with `vault-cli knowledge show weekly-health-insight-YYYY-MM-DD` and send the concise note from that page.',
