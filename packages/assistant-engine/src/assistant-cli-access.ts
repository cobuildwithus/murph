...
export function buildAssistantCliGuidanceText(
  ...
  return [
    ...
    '`stage` names the CLI failure. `retryable: true` permits an unchanged repeat; fixing a named `fieldErrors` field or `hint` prerequisite is a new attempt. Without concrete guidance, stop; never guess or echo omitted details.',
  ].join('\n\n')
}
