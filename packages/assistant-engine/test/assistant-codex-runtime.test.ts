...
} from '../src/assistant-codex/files.ts'
...
describe('assistant codex runtime', () => {
  ...
  it('serializes byte-backed Codex file inputs and rejects local file URLs', () => {
    expect(
      buildCodexInjectedFileMessageItems({
        files: [
          {
    ...
    expect(() =>
      buildCodexInjectedFileMessageItems({
        files: [
          {
            type: 'file',
            data: new URL('file:///tmp/private.pdf'),
            mediaType: 'application/pdf',
          },
        ],
      }),
    ).toThrowError('Codex app-server file input does not support URL scheme "file:".')
    ...
    expect(() =>
      buildCodexInjectedFileMessageItems({
        files: [
          {
            type: 'file',
            data: 'data:application/pdf;base64,not base64',
            mediaType: 'application/pdf',
          },
        ],
      }),
    ).toThrowError('Codex app-server file input data URL contains invalid base64.')
    ...
    expect(
      buildCodexInjectedFileMessageItems({
        files: [
          {
            type: 'file',
            data: 'data:application/pdf;name=private-name.pdf;base64,JVBERi0xLjcK',
            mediaType: 'application/pdf',
          },
        ],
      }),
    ...
    ).toThrowError('Codex app-server native file inputs currently support PDF files only.')
  })
  ...
  it('injects raw Responses file items before starting the Codex turn', async () => {
    const workingDirectory = await createTempDir('assistant-codex-inject-')
    ...
    codexMocks.spawn.mockImplementation(() => {
      ...
      queueMicrotask(() => {
        void (async () => {
          ...
          const injectItems = await waitForRpcMethod(child, 'thread/inject_items')
          expect(injectItems).toMatchObject({
            id: 3,
          ...
          expect(readTurnStartInputItems(turnStart)).toEqual([
            {
              type: 'text',
    ...
    await expect(
        ...
        injectedResponsesItems: [
          {
            ...
            content: [
              ...
              },
              {
                ...
                file_data:
                  'data:application/pdf;name=private-source-name.pdf;base64,JVBERi0xLjcK',
              },
            ],
          },
        ],
      ...
      sessionId: 'thread-injected',
      threadId: 'thread-injected',
      turnId: 'turn-injected',
    })
  })

  it('fails before turn/start when Codex thread item injection fails', async () => {
    const workingDirectory = await createTempDir('assistant-codex-inject-fail-')
        ...
        void (async () => {
          ...
          child.stdout.write(
            jsonLine({
              ...
              result: {
                thread: {
                  id: 'thread-inject-failed',
                },
              },
            }),
          )

          await waitForRpcMethod(child, 'thread/inject_items')
          child.stdout.write(
            jsonLine({
              id: 3,
              error: {
                message: 'thread/inject_items failed: file rejected',
              },
            }),
          )
          queueMicrotask(() => {
            sawTurnStart = child.stdin.writes.some((line) =>
    ...
    expect(sawTurnStart).toBe(false)
  })
