  ...
  it.each([
    {
      ...
      label: 'immediately before receipt expiry',
    },
    {
      boundaryOffsetMs: 0,
      historyExpected: false,
      label: 'at inclusive receipt expiry',
    },
    {
      boundaryOffsetMs: 1,
      historyExpected: false,
      label: 'after receipt expiry',
    },
  ])('rechecks bounded history at one-shot turn/start $label', async ({
    boundaryOffsetMs,
    historyExpected,
  }) => {
    const workingDirectory = await createTempDir(
      'assistant-codex-provider-history-authority-work-',
    )
    const codexHome = await createTempDir(
      'assistant-codex-provider-history-authority-home-',
    )
    const deadlineMs = Date.parse('2026-07-20T17:30:00.000Z')
    let currentTimeMs = deadlineMs - 60_000
    ...
    codexMocks.spawn.mockImplementation(() => {
      const child = new MockChildProcess()
      child.pid = 40_500 + spawnedChildren.length
      spawnedChildren.push(child)
      child.stdin.onWrite = (write) => {
        for (const line of write.split('\n')) {
          const trimmed = line.trim()
          ...
          queueMicrotask(() => {
            switch (message.method) {
              case 'initialize':
                ...
                break
              case 'thread/start':
                currentTimeMs = deadlineMs + boundaryOffsetMs
                child.stdout.write(jsonLine({
                  id: message.id,
                  result: {
                    thread: {
                      id: 'thread-history-authority',
                    },
                  },
                }))
                break
    ...
    await expect(
      executeCodexAssistantTurnAttempt({
        ...
        providerConfig,
        systemPrompt: 'Stable Murph instructions.',
        userPrompt: 'Offer one low-pressure health direction choice.',
        workingDirectory,
      }),
    ...
    const prompt = readTurnStartInputItems(turnStarts[0] ?? {})[0]?.text
    expect(prompt).toContain(
      'Offer one low-pressure health direction choice.',
    )
    if (historyExpected) {
