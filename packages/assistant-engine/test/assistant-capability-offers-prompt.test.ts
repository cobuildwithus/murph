...
describe('assistant capability-offers prompt contract', () => {
  ...
  it('names generic group-email and new-group permission mechanics inside hosted-group guidance', () => {
    ...
    expect(prompt).toContain('Every available shared health source is returned separately')
    expect(prompt).toContain('never choose or imply one cross-source winner')
  ...
  it('keeps bounded device diagnostics inside the closed group permission contract', () => {
    ...
    expect(prompt).toContain('Every available shared health source is returned separately')
    expect(prompt).toContain('never choose or imply one cross-source winner')
    expect(prompt).toContain('`workouts.v0`: day-local start/duration/type/source')
