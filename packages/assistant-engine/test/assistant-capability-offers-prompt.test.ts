...
describe('assistant capability-offers prompt contract', () => {
  ...
  it('names newsletter and new-group permission mechanics inside hosted-group guidance', () => {
    ...
    expect(section).not.toContain('"now"/"yet"/post-sync/reconnect checks')
    expect(section).not.toContain('call exact-scope `read_shared` once first')
    expect(section).not.toContain('answer only from it, never prior context or sync times')
  ...
  it('keeps bounded device diagnostics inside the closed group permission contract', () => {
    ...
    expect(prompt).toContain('"now"/"yet"/post-sync/reconnect checks')
    expect(prompt).toContain('call exact-scope `read_shared` once first')
    expect(prompt).toContain('answer only from it, never prior context or sync times')
    expect(prompt).toContain(
