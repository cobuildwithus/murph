...
describe('assistant capability-offers prompt contract', () => {
  ...
  it('names newsletter and new-group permission mechanics inside hosted-group guidance', () => {
    ...
    expect(section).toContain('Shared health records are stored projections, not live provider reads')
    expect(section).toContain('Never claim a live or repeated wearable check')
    expect(section).toContain('every available sleep source is disclosed under that exact consent')
    expect(section).toContain('Never treat an existing v0 grant as v1 consent or silently upgrade it')
    expect(section).toContain('When `sourcesDisagree=true`, report the source values separately')
