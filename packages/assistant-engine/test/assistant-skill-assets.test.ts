...
describe('assistant skill assets', () => {
  ...
  it('keeps group newsletter setup and opt-out behavior in the group-chat skill', async () => {
    ...
    expect(raw).toContain('action="leave_current"')
    expect(raw).toContain('cleanup of their shared projections')
    expect(raw).toContain('Leaving does not erase messages')
    expect(raw).toContain('never accept or supply a member id, group id')
    expect(raw).toContain('`owner_cannot_leave`')
    expect(raw).toContain('If `cleanupPending` is true')
