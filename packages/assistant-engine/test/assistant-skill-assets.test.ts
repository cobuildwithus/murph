...
describe('assistant skill assets', () => {
  ...
  it('keeps group challenge guidance aligned with selectable scoring projections', async () => {
    ...
    expect(raw).toMatch(/For\s+`presentation="native"`, the host may have posted or reused native consent UI[\s\S]*do not infer, announce, or append a companion\s+message claiming a card is visible/u)
    ...
    expect(raw.replace(/\s+/gu, ' ')).toContain(
      'When native consent is the only user-facing outcome, call `murph.finish_without_reply`',
    )
