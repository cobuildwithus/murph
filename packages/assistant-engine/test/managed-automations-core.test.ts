...
describe('applyMurphManagedAutomations core integration', () => {
  ...
  it('creates managed health automations through the canonical automation registry', async () => {
    ...
    expect(record?.instructions).toContain('Wearable explicitly requires reconnect')
    expect(record?.instructions).toContain('Missing or stale data alone never authorizes outreach')
    expect(record?.instructions).not.toContain('sources show no new data for roughly a week')
