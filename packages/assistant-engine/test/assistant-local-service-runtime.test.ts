...
test('sendAssistantMessageLocal clears hosted auto-reply resume state after over-budget terminal provider failures', async () => {
  const terminalError = new Error('provider failed after an expensive request')
  const failedProviderSession = createAssistantSession({
    resumeState: {
      routeFingerprint: 'route-existing',
      threadId: 'provider-session-expensive',
    },
    sessionId: 'session-provider-expensive-failed',
  })
  const { mocks, sendAssistantMessageLocal } = await loadLocalServiceModule({
    providerOutcome: {
      attemptCount: 1,
      ...
      codexThreadId: 'provider-session-expensive',
      providerTurnId: 'provider-turn-expensive-failed',
      rawEvents: [],
      route: {
        provider: 'codex-cli',
        providerOptions: {
  ...
  await assert.rejects(
    () =>
      sendAssistantMessageLocal({
        ...
        prompt: 'Reply to this hosted message',
        turnTrigger: 'automation-auto-reply',
        vault: '/vaults/test',
      }),
    (error) => {
  ...
  assert.equal(mocks.saveAssistantSession.mock.calls[0]?.[0], '/vaults/test')
  assert.equal(mocks.saveAssistantSession.mock.calls[0]?.[1]?.codexResume, null)
  assert.equal(mocks.saveAssistantSession.mock.calls[0]?.[1]?.resumeState, null)
  assert.equal(
    mocks.saveAssistantSession.mock.calls[0]?.[1]?.sessionId,
    'session-provider-expensive-failed',
  )
})

test('sendAssistantMessageLocal clears hosted auto-reply resume state after terminal provider failures with unknown usage', async () => {
  const terminalError = new Error('provider failed without usage metadata')
  const failedProviderSession = createAssistantSession({
    resumeState: {
      routeFingerprint: 'route-existing',
      threadId: 'provider-session-unknown-usage',
    },
    sessionId: 'session-provider-unknown-usage-failed',
  })
  const { mocks, sendAssistantMessageLocal } = await loadLocalServiceModule({
    providerOutcome: {
      attemptCount: 1,
      ...
      codexThreadId: 'provider-session-unknown-usage',
      providerTurnId: 'provider-turn-unknown-usage-failed',
      rawEvents: [],
      route: {
        provider: 'codex-cli',
        providerOptions: {
  ...
  assert.equal(
    mocks.saveAssistantSession.mock.calls[0]?.[1]?.sessionId,
    'session-provider-unknown-usage-failed',
  )
})
