...
describe('applyMurphManagedAutomations', () => {
  ...
  it('keeps the managed weekly improvement coach seed as the baseline Tuesday early-evening recurrence', () => {
    ...
    expect(seed.instructions).toContain('`action: "knowledge_get"` and `slug: "improvement-opportunities"`')
    expect(seed.instructions).toContain('call `murph.scheduled_knowledge` once')
    expect(seed.instructions).toContain('do not supply an action, date, timezone, heading, position, slug, or title')
  ...
  it('runs alternating product notes every two weeks', () => {
    ...
    expect(seed.instructions).toContain('`murph.product_source` once with `source: "changelog"`')
    expect(seed.instructions).toContain('`murph.product_source` once with `source: "feature_catalog"`')
    expect(seed.instructions).toContain('`action: "knowledge_get"` and `slug: "murph-product-notes"`')
    ...
    expect(seed.instructions).toContain('only a `body` that records this run\'s kind and the chosen item ids')
    ...
    expect(seed.instructions).toContain('another run already recorded this occurrence-local-date note')
  ...
  it('keeps overnight memory consolidation as a hosted-only every-other-night maintenance seed', () => {
    ...
    expect(seed.instructions).toContain('`murph.scheduled_read` and `action: "memory_show"`')
    expect(seed.instructions).toContain('Write durable memory only with `murph.maintenance_memory`')
    expect(seed.instructions).not.toContain('vault-cli memory upsert')
    expect(seed.instructions).not.toContain('vault-cli memory update')
  ...
  it('creates the managed automations in a fresh vault', async () => {
    ...
    expect(insightRecord?.instructions).toContain('`action: "knowledge_get"` and `slug: "weekly-health-insights"`')
    ...
    expect(insightRecord?.instructions).toContain('call `murph.scheduled_knowledge` once')
    expect(insightRecord?.instructions).toContain('do not supply an action, date, timezone, heading, position, slug, or title')
    expect(insightRecord?.instructions).toContain('another run created this occurrence-local-date section first')
    ...
    expect(insightRecord?.instructions).toContain('the finding as `body`')
    expect(insightRecord?.instructions).toContain('canonical vault source paths')
    ...
    expect(insightRecord?.instructions).not.toContain('web search')
    ...
    expect(improvementCoachRecord?.instructions).toContain(
      '`action: "knowledge_get"` and `slug: "improvement-opportunities"`',
    )
    expect(improvementCoachRecord?.instructions).toContain('call `murph.scheduled_knowledge` once')
    expect(improvementCoachRecord?.instructions).toContain('do not supply an action, date, timezone, heading, position, slug, or title')
    expect(improvementCoachRecord?.instructions).toContain(
      '{"kind":"skip","privateSummary":"No improvement opportunity cleared the evidence bar."}',
    )
    expect(improvementCoachRecord?.instructions).toContain(
    ...
    expect(researchScoutRecord?.instructions).toContain('`action: "knowledge_get"` and `slug: "weekly-health-research-scout"`')
    expect(researchScoutRecord?.instructions).not.toContain('EXA_API_KEY')
    ...
    expect(researchScoutRecord?.instructions).toContain('Call `murph.research_scout_batch` once')
    expect(researchScoutRecord?.instructions).toContain('If none exists, suppress the scheduled message without calling `murph.research_scout_batch`')
    expect(researchScoutRecord?.instructions).not.toContain('vault-cli research scout')
    ...
    expect(researchScoutRecord?.instructions).toContain('not a generic `tags` field')
    expect(researchScoutRecord?.instructions).toContain('Example lanes: `[{"label":"evening light and sleep"')
    ...
    expect(researchScoutRecord?.instructions).toContain('Call `murph.scheduled_knowledge` once')
    expect(researchScoutRecord?.instructions).toContain('do not supply an action, date, timezone, heading, position, slug, or title')
    ...
    expect(productUpdatesRecord?.instructions).toContain('`murph.product_source` once with `source: "changelog"`')
    expect(productUpdatesRecord?.instructions).toContain('`murph.product_source` once with `source: "feature_catalog"`')
    expect(productUpdatesRecord?.instructions).toContain('`action: "knowledge_get"` and `slug: "murph-product-notes"`')
    ...
    expect(productUpdatesRecord?.instructions).toContain('call `murph.scheduled_knowledge` once')
    expect(productUpdatesRecord?.instructions).toContain('do not supply an action, date, timezone, heading, position, slug, or title')
    ...
    expect(productUpdatesRecord?.instructions).toContain('only a `body` that records this run\'s kind and the chosen item ids')
    ...
    expect(productUpdatesRecord?.instructions).toContain('another run already recorded this occurrence-local-date note')
  ...
  it('creates the hosted overnight memory consolidation automation in hosted runtime', async () => {
    ...
    expect(memoryRecord?.instructions).toContain('`murph.scheduled_read` and `action: "memory_show"`')
    expect(memoryRecord?.instructions).toContain('Write durable memory only with `murph.maintenance_memory`')
    expect(memoryRecord?.instructions).not.toContain('vault-cli memory upsert')
    expect(memoryRecord?.instructions).not.toContain('vault-cli memory update')
  ...
  it('migrates existing weekly product notes to the two-week cadence', async () => {
    ...
    expect(productUpdatesRecord?.instructions).toContain(
      '`murph.product_source` once with `source: "feature_catalog"`',
    )
    expect(productUpdatesRecord?.instructions).toContain('only a `body` that records this run\'s kind and the chosen item ids')
    expect(productUpdatesRecord?.instructions).toContain('Do not include reasons, user context, health details, raw user wording, provider data, or copied catalog/changelog text')
    expect(productUpdatesRecord?.instructions).toContain('Do not append again and do not switch kinds')
