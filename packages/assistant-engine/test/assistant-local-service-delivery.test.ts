...
test('sendAssistantMessageLocal persists compound initial inputs as separate ordered transcript entries', async () => {
  const context = await createTempVaultContext(
    'assistant-local-service-compound-initial-transcript-',
  )
  tempRoots.push(context.parentRoot)
  const olderText = `Background result: ${'x'.repeat(4_500)}`
  const freshText = 'Fresh user detail: use the green comet label.'
  const createInput = async (input: {
    ...
    event: {
      ...
      conversation: {
        ...
        actorIsSelf: false,
        source: 'telegram',
        threadId: 'thread-compound-transcript',
        threadIsDirect: true,
      },
      ...
      replyTarget: {
        channel: 'telegram',
        messageId: `${input.eventId}-message`,
        threadId: 'thread-compound-transcript',
      },
      sourceMetadata: {
        externalThreadRouteAuthorityPresent: true,
  ...
  const olderInput = await createInput({
    eventId: 'evt_compound_older',
    laneSeq: '1',
    occurredAt: '2026-04-22T10:00:00.000Z',
    text: olderText,
  })
  const freshInput = await createInput({
    eventId: 'evt_compound_fresh',
    laneSeq: '2',
    occurredAt: '2026-04-22T10:00:01.000Z',
    text: freshText,
  })
  const acceptedInputs = [
    { event: olderInput, promptFallbackText: olderText },
    { event: freshInput, promptFallbackText: freshText },
  ].map(({ event, promptFallbackText }) => ({
    ...assistantInputCandidateFromStoredEvent(event).acceptedInput,
    promptFallbackReason: 'system-input' as const,
    promptFallbackText,
  }))
  const aggregatePrompt = `${olderText}\n\n${freshText}`
  const { mocks, sendAssistantMessageLocal } = await loadLocalServiceModule()

  await sendAssistantMessageLocal({
    acceptedTurnInput: {
      initialInputs: acceptedInputs,
    },
    prompt: aggregatePrompt,
    vault: context.vaultRoot,
  })

  const persistedUserEntries =
    mocks.appendAssistantTranscriptEntriesWithRefs.mock.calls.flatMap(
      (call) => call[2].filter((entry) => entry.kind === 'user'),
    )
  expect(persistedUserEntries).toEqual([
    expect.objectContaining({
      contentReceivedAt: '2026-04-22T10:00:00.000Z',
      text: olderText,
    }),
    expect.objectContaining({
      contentReceivedAt: '2026-04-22T10:00:01.000Z',
      text: freshText,
    }),
  ])
  expect(persistedUserEntries).not.toContainEqual(
    expect.objectContaining({ text: aggregatePrompt }),
  )
  expect(
    mocks.runtimeState.turns.acceptedInputs.updateTranscriptRefs,
  ).toHaveBeenCalledWith({
    refs: [
      expect.objectContaining({ inputId: olderInput.inputId }),
      expect.objectContaining({ inputId: freshInput.inputId }),
    ],
    turnId: 'turn-1',
  })
})
