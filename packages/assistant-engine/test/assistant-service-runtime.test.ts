...
vi.mock('../src/assistant/local-service.js', () => ({
  openAssistantConversationLocal: seamMocks.local.openAssistantConversationLocal,
...
vi.mock('../src/assistant/session-resolution.js', async () => {
  ...
  return {
    ...actual,
    buildResolveAssistantSessionInput: seamMocks.buildResolveAssistantSessionInput,
  }
})
...
import {
  persistPendingAssistantUsageEvent,
...
beforeEach(() => {
  ...
  seamMocks.buildAssistantCliGuidanceText.mockReset().mockReturnValue(
    'CLI guidance block.',
  )
  ...
  seamMocks.createAssistantUsageId.mockReset().mockImplementation(
    ({
      attemptCount,
      turnId,
    }: {
  ...
  seamMocks.markAssistantFirstContactSeen.mockReset().mockResolvedValue(undefined)
  seamMocks.normalizeAssistantDeliveryError
    .mockReset()
    .mockReturnValue(createDeliveryError({
...
describe('assistant turn finalizer seam', () => {
  it('persists the user prompt after provider success when failure persistence is disabled', async () => {
    ...
    expect(saved.providerBinding?.providerState?.resumeRouteId).toBe('route-existing')
  })
  ...
  it('skips duplicate user persistence when failure persistence already happened and rewrites the resume route on provider change', async () => {
    ...
    expect(saved.providerBinding?.providerSessionId).toBe('provider-session-new')
    expect(saved.providerBinding?.providerState?.resumeRouteId).toBe('route-new')
  })
