...
- For connected or saved-meal nutrient questions, also read `food-journal` and use its bounded meal-nutrient workflow. Keep recorded intake separate from lab status: missing or partial meal coverage is not zero intake, complete-looking selected stored-meal coverage does not prove a complete day, source-app targets and percentages are not imported, and one day of food records cannot diagnose a deficiency.
