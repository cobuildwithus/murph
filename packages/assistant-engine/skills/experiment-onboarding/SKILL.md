...
- Behavior-followthrough may satisfy planned-session support with quiet or review-only support when the user prefers that or when per-session reminders are likely to become noise. Record the support style and schedule only the review/check-in automation needed; do not create per-session cue messages just to satisfy default-on support.
...
- When a repair message or proposal shows the support loop is stale, do not leave related future session-support automations blindly active. Use stored `session_support_automation_slugs` first; otherwise inspect future active automations by experiment slug, `session-support` tag, or stable slug prefix. Update or archive only future behavior-support automations that would repeat the same stale policy. Do not silently archive clinical or safety-relevant support; ask or switch to safer review-only support.
