...
- First-week habit support reminders: use bounded one-shot `automation save` calls with slugs such as `experiment-week-one-<experiment-slug>-<YYYY-MM-DD>` after the run exists. The stable slug lets rescheduling update the same automation instead of creating duplicates. Use generic tags by default with repeated `--tag`: `assistant`, `scheduled`, `experiment`, and `first-week-support`. Add protocol-specific tags only when they are necessary and non-sensitive.
- Session-support automations: use bounded one-shot `automation save` calls with slugs such as `experiment-session-support-<experiment-slug>-<YYYY-MM-DD>` after the run exists. The stable slug lets rescheduling update the same automation instead of creating duplicates. Use generic tags by default with repeated `--tag`: `assistant`, `scheduled`, `experiment`, and `session-support`.
...
- First-week support automation instructions should state that this is bounded early habit support, not a missed-log follow-up or weekly digest, with the skip conditions and a short daily reminder as the outcome. The scheduled assistant verifies current state with full vault access; do not enumerate the surfaces it must read.
