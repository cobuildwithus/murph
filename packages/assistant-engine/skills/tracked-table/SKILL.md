...
2. After resolving, pass `--workout-id` on every ordinary CLI live-workout mutation, including exercise additions and finish. For every ordinary set write, pass `--workout-id`, one explicit exercise selector, and `--set-order`. Prefer a stable `--exercise-id`; otherwise use exact exercise order or the exact canonical name. The host-bound scheduled rollover tool below instead takes `previousWorkoutId` because it owns the close/start/log transition. These explicit coordinates make a repeated agent attempt correct the same set rather than append a duplicate.
...
A new scheduled workout reminder creates one narrow exception to explicit finish intent. When saving or patching that reminder, set `scheduledReply` to `{ "kind": "workout_rollover", "routineId": "<canonical wfmt_* id>" }`; omit that field from every other reminder or automation. Use rollover only when the current direct message is an accepted native reply to the exact reminder and `murph.log_scheduled_workout_set` is available on the current root turn. The host-bound routine plus current reply must uniquely identify one exercise name and canonical order, one existing set order, and at least one numeric actual result stated by the member. Read the current canonical workout and the host-bound saved routine; use the exact `evt_*` prior workout id. Never select the routine, exercise, or set from conversational recency.

The prior workout must be the sole active Murph live workout, must contain at least one set, and every existing set coordinate must already contain a logged result. Zero pending coordinates do not ordinarily finish a workout; they authorize rollover only at this boundary. Call `murph.log_scheduled_workout_set` with `previousWorkoutId`, `exerciseName`, `exerciseOrder`, `setOrder`, and only the actual fields stated in the current reply. Never pass or infer the routine, authority timestamps, or an operation id; the host injects those from the exact accepted reply. The tool preserves the prior event and all of its exercise, set, result, note, routine, and native-action receipt data; closes it at its persisted final activity duration rather than the later reply time; starts the saved routine at the scheduled occurrence; and logs only the exact authorized set. Planned targets remain plan data and are never copied into actuals.

Do not use rollover for a group message, an unquoted or non-native reply, a turn where the host-bound tool is absent, a pending prior set, a zero-set prior workout, a missing actual result, an inexact or ambiguous coordinate, more than one matching scheduled session, or any unrelated active workout. Do not split it into ordinary finish/start/log commands. Let the composite tool fail closed so its existing live-workout lock and persisted operation receipt can converge an interrupted replay without retargeting.
