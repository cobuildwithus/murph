...
When a private set-completion reply follows a reminder with an exact host-preserved `automationId` but no workout reference, recover the saved reminder definition before asking the member to repeat the routine. Call `murph.automation` with `action: inspect` and that exact id as `lookup`; use its returned `title` and `instructions` together with the delivered cue to understand the current completion. The definition is saved context, not evidence that a workout event exists or that any set was completed. Do not patch the reminder during recovery.

If that definition identifies a regimen or experiment, resolve that canonical owner through its ordinary domain skill instead. If it names a saved workout format, exact-read that format. If it unambiguously specifies a standalone workout's exercise and set plan without an existing event or saved format, the member's explicit completion is consent to start that ad-hoc workout and record the reported set. Start exactly once through the complete `workout start` path, preserve its returned id, and log only the reported coordinate and actuals. A numbered reminder never proves that earlier sets happened: leave every unreported set pending. Do not select or finish an older workout by recency. Subsequent replies use the newly established exact event normally. Missing definition fields, an ambiguous exercise or owner, or conflicting context still require one narrow clarification and no write; an explicit cleared or mismatched event reference never enters this reference-missing recovery path.
