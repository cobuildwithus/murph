...
3. Pass only values the member stated, values already present on that exact canonical set, or one exact active-workout prescription the member explicitly applied to every set of one exercise. That prescription counts as member-stated for those set coordinates only while the same workout is active and its establishing message remains available in the current direct conversation. For a later unqualified completion such as “set 2 done,” pass the prescribed actual field instead of asking again or writing a note-only completion. A value stated with the completion overrides the prescription. Ask for the actual value when the prescription is a range, AMRAP or qualitative instruction, conflicts with another value, could apply to more than one exercise, or is no longer available in the current conversation. Never treat a saved-plan target, prior workout, card target, or assistant-authored suggestion as this prescription. Preserve stated units and wording.
