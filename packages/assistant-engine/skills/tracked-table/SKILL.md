...
5. After every verified private workout mutation that changes the snapshot—including an ordinary set log, correction, clear, exercise addition, start, resume, or finish—build and attach the refreshed structured workout card as the complete response on a supported private card route, except for the bounded later-day transition below. That compound transition uses its complete ordinary-text result because one workout card cannot represent both canonical mutations. Do not send a text-only acknowledgement or companion prose for an ordinary single-workout mutation.
...
6. After every command succeeds, send one concise ordinary-text response as the complete channel result. Name the earlier workout, the member-readable rendering of its exact recorded end time, the newly started routine, and the exact set result saved. Do not attach a single-workout card or companion prose on this compound turn; subsequent ordinary updates resume card-only responses.
...
Run `vault-cli workout finish --workout-id <evt_id>` under exactly two authorities: the member explicitly or unmistakably closes that session, or every check in the bounded later-member-local-day reminder transition above succeeds. The bounded transition must pass the explicit canonical `--ended-at` derived there; elapsed time or a reminder by itself never authorizes a finish. No other implicit finish is allowed. The returned event must contain the verified `endedAt` and final elapsed duration. Do not fabricate missing set values or copy targets into the event.
