...
If the user sends lab PDFs, Lab Results of Record documents, pasted lab results, or other blood-test documents and full parsing/import is not needed for the current visible reply, prefer the delegated-save path above. The lab child should preserve and work from durable evidence, extract panels/analytes/dates/units/reference ranges/flags/provenance, and save the full recoverable structure with `blood-test import-json` or typed blood-test save commands. If synchronous inspection is needed for the current reply, call `send_progress_update` before reading the content or using file/import tools.
