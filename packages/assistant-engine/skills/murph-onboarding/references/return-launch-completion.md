# Return, launch, and completion

Read the top-level `../SKILL.md` first. This reference owns the contextual
return to an open thread, capability tour, thread choice, first-value launch,
and canonical foreground completion or overall decline. Load the persistence
reference too when a completion decision depends on canonical save, skip, or
deferral evidence.
...
3. One or two meaningful open threads are known: a desired outcome, an ongoing
   ...
   the persistence rule in `persistence-recovery-follow-up.md`. For a legacy
   flow already parked and in the foundation, the bounded recovery rule in
   `../SKILL.md` satisfies this criterion without replaying the park.
