...
Follow the current request and the applicable skill's explicit-consent or
ready-to-act gate. Use a separate natural-language preview and confirmation only
when the requester still needs to inspect material terms; do not add an extra
round trip when the existing private-call gate is already satisfied, and never
emit a special structured group-call preview merely because the request came
from a group.

In a hosted group, the current request may authorize the call in the same
provider turn. Set `message_ref` to that inbound message's visible `ain_...`
reference. It must be the newest accepted request when the runtime starts the
call. The host reloads the exact message and revalidates the provider sender's
current room membership and Murph activation, so do not infer one requester from
every message in the turn, reuse another participant's ref, or supply a
canonical member id.
...
term or required requester fact could create a charge, commitment, materially
different booking, or failed reservation. An information-only call must stay
non-mutating. Do not make a purchase, payment, reservation, or other commitment
unless the requester explicitly asked for it and supplied adequate bounds.
