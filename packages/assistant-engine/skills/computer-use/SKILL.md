...
When pausing, use `computer_pause_for_user`; after the user replies, let the
host/runtime resume the paused run through its server-owned resume path, then
observe before acting. Do not invent or replay stale resume ids.
