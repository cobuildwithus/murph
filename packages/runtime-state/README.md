...
- hosted execution working snapshots use the same portable inclusion policy as full/base snapshots, preserve raw artifact refs when unchanged, write changed artifacts through the normal artifact ref format, set `browserVaultReplicaRef` to null unless a replica was generated from the effective base-plus-delta state, and fail closed when required Codex native resume state is missing the exact rollout JSONL and manifest.
