...
Hosted full/base snapshots keep that broad continuity policy, including safe filtered `.codex-hosted/**` as opaque provider continuity state. Hosted hot-state snapshots are deliberately narrower: they capture an explicit latest assistant-runtime correctness subset, clear those hot paths before restore, exclude diagnostics/status/run logs unless they are read for correctness, and do not include operator-home Codex state. Hot snapshots are for user-visible mailbox/input and pre-delivery outbox checkpoint boundaries; full/base snapshots remain the cold restore, compaction, and provider-continuity image.
...
- hosted execution full/base snapshots canonical `vault/**`, durable operational runtime state under `vault/.runtime/operations/**` except explicit unsafe/process-local exclusions, the minimal operator-home hosted config needed for bootstrap, and safe non-secret `.codex-hosted/**` Codex continuity state; they do **not** snapshot device-sync control/token stores, parser executable-selector config, rebuildable projections, caches, tmp state, secrets, quarantine/repair payloads, locks, pid files, sockets, Codex auth/credential/key/cert files, Codex cache/tmp/log/history files, or Codex prompt-history files. Hosted Codex-home diagnostics stay count/class only plus keyed relative-name hashes when the hosted log fingerprint secret is configured.
- hosted execution hot-state snapshots use an explicit assistant-runtime include list plus byte/file budgets and must not externalize artifacts; restore treats the latest hot bundle as authoritative for those vault hot paths while preserving Codex home continuity from the base/full layer unless a full snapshot replaces it.
