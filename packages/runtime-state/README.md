...
Hosted full/base snapshots keep that broad runtime continuity policy, but Codex provider continuity is precise: only active `.codex-hosted/sessions/YYYY/MM/DD/rollout-*.jsonl` files referenced by live assistant session resume state are included, alongside a tiny continuity manifest. Hosted hot-state snapshots are deliberately narrower: they capture an explicit latest assistant-runtime correctness subset, clear those hot paths before restore, exclude diagnostics/status/run logs unless they are read for correctness, and include only the exact Codex rollout files required by the hot assistant session resume state. Hot snapshots are for user-visible mailbox/input and pre-delivery outbox checkpoint boundaries; full/base snapshots remain the cold restore and compaction image.
...
- hosted execution hot-state snapshots use an explicit assistant-runtime include list plus byte/file budgets and must not externalize artifacts; when the hot assistant session state has Codex native resume state, the hot bundle must also carry the exact matching active rollout JSONL and manifest, and otherwise fails closed instead of relying on unrelated base continuity.
