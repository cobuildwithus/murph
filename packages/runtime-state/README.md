...
`vault/.runtime/operations/assistant/**` is the assistant runtime residue root. Hosted execution treats this root as runtime continuity by default so Cloudflare can stay a thin runner over the local assistant runtime. Assistant descriptors still document/audit known files, but new assistant continuity files do not need a descriptor to survive hosted checkpoint/restore. The hosted denylist stays small and safety-oriented: secrets, device-sync runtime state, parser executable-selector config, quarantine/repair payloads, locks, pid/socket files, temp lock files, legacy assistant usage queues, global `.runtime/cache/**`, `.runtime/tmp/**`, and rebuildable `.runtime/projections/**` stay out of hosted workspace snapshots. Assistant diagnostics, journals, status snapshots, cron run logs, outbox state, receipts, sessions, transcripts, hosted inbox runtime state, parser runtime state outside executable selectors, and mailbox/runtime continuity files move with the encrypted hosted workspace. Hosted assistant usage records directly to the web-owned usage ledger through the hosted-execution usage contract instead of using assistant-runtime snapshot state.

Hosted full/base snapshots keep that broad runtime continuity policy and write a portable workspace manifest into bundle metadata. Codex provider continuity is precise: only active `.codex-hosted/sessions/YYYY/MM/DD/rollout-*.jsonl` files referenced by live assistant session resume state are included, alongside a tiny continuity manifest. Foreground assistant turns do not build workspace checkpoint snapshots; after a foreground pass they may schedule a best-effort Codex continuity artifact that reads only assistant session resume requirements plus those referenced rollout files, writes one small hosted bundle without a portable workspace manifest, and skips vault/raw/derived workspace scans. That tiny artifact is not a complete independent restore path until web-owned workspace metadata records a durable `codexContinuityRef`. `canonical_runtime_commit` stores exact hosted canonical write receipts in supervisor-owned artifacts and records the authoritative receipt-log ref on the workspace checkpoint, so restore never replays assistant-local receipt files. `activation_bootstrap` and `idle_shutdown` are the only full/base snapshot producers, with `idle_shutdown` compacting the effective restored state into a new full/base snapshot that can include or supersede the tiny continuity artifact.
...
- hosted execution full/base snapshots canonical `vault/**`, durable operational runtime state under `vault/.runtime/operations/**` except explicit unsafe/process-local exclusions, the minimal operator-home hosted config needed for bootstrap, and only manifest-referenced active Codex rollout JSONL files; they do **not** snapshot device-sync control/token stores, parser executable-selector config, rebuildable projections, caches, tmp state, secrets, quarantine/repair payloads, locks, pid files, sockets, Codex auth/credential/key/cert files, Codex cache/tmp/log/history files, Codex prompt-history files, Codex SQLite metadata, unreferenced Codex sessions, or archived Codex sessions. Hosted Codex continuity diagnostics stay count/byte/counter only plus keyed rollout-relative hashes when the hosted log fingerprint secret is configured.
- hosted foreground continuity must stay separate from full/base workspace snapshots. The tiny Codex continuity artifact is limited to manifest-referenced rollout JSONL files and the continuity manifest; it does not carry portable workspace manifests, canonical vault files, raw artifacts, derived outputs, projections, caches, or temporary state.
...
- large raw artifacts under `vault/raw/**` may be externalized into separate encrypted content-addressed objects. Hosted execution restores assistant attachment-evidence refs under `raw/inbox/**`, `raw/assistant-input/**`, `derived/inbox/**`, and `derived/assistant-input/**` before local runtime code reads them.
