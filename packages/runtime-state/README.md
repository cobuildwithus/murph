...
- narrow `@murphai/runtime-state/node/*` subpaths expose hosted-safe Node helpers such as assistant-usage parsing, loopback bearer auth, runtime path constants, SQLite warning filters, and ULID helpers without forcing callers through the broad `./node` barrel
