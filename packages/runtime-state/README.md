...
- hosted execution snapshots canonical `vault/**`, durable operational runtime state under `vault/.runtime/operations/**` except explicit unsafe/process-local exclusions, the minimal operator-home hosted config needed for bootstrap, and safe `.codex-hosted` Codex resume continuity files (`config.toml`, `session_index.jsonl`, and `sessions/**`); they do **not** snapshot device-sync control/token stores, parser executable-selector config, rebuildable projections, caches, tmp state, secrets, quarantine/repair payloads, locks, pid files, sockets, Codex auth/credential files, or Codex cache/tmp/log/history files
