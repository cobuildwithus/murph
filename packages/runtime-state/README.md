...
Hosted full/base snapshots keep that broad continuity policy. Hosted hot-state snapshots are deliberately narrower for vault files: they capture an explicit latest assistant-runtime correctness subset, clear those hot paths before restore, and exclude diagnostics/status/run logs unless they are read for correctness. When the hosted Codex home exists, hot-state snapshots also include the safe filtered `.codex-hosted/**` tree as opaque provider continuity state. Hot snapshots are for user-visible checkpoint boundaries; full/base snapshots remain the cold restore and compaction image.
...
- hosted execution hot-state snapshots use an explicit assistant-runtime include list plus byte/file budgets, include safe filtered `.codex-hosted/**` whenever the hosted Codex home exists, and must not externalize artifacts; restore treats the latest hot bundle as authoritative for those paths.
