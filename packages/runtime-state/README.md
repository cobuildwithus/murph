...
Hosted full/base snapshots keep that broad continuity policy. Hosted hot-state snapshots are deliberately narrower: they capture an explicit latest assistant-runtime correctness subset, clear those hot paths before restore, and exclude diagnostics/status/run logs unless they are read for correctness. Hot snapshots are for user-visible checkpoint boundaries; full/base snapshots remain the cold restore and compaction image.
