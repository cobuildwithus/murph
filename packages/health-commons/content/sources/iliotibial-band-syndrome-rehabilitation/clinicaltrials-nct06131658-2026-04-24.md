...
summary: 'Merged 1 candidate row(s) from shard(s): snowball-gap-fill. Registry tracking only; do not use as efficacy evidence unless a linked publication/results record is extracted separately. Direct runner motor-control trial registry; preserve but do not extract efficacy until results are available.'
status: draft
quality: usable
aliases:
