...
protocol:
  ...
  tips:
  - First run is a tolerance read, not a proof of willpower or a durable metabolic win.
  - Shortest duration that answers the question. Longer is not automatically better.
  - Keep training, heat, alcohol, diet, sleep, supplements, and other experiments stable.
  - Have a stop plan and a refeed plan before the fast starts.
  - Wearables, CGM, ketones, BP, weight, and labs are context only — never override symptoms or clinician guidance.
  - No large catch-up meal after the fast; the first refeed is a safety checkpoint.
  keepInMind:
