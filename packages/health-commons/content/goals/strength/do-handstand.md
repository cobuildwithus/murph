...
key: goal_template:do-handstand
slug: do-handstand
title: Do a Handstand
summary: Build overhead strength, body-line control, and balance from the wall toward a freestanding handstand.
...
goal:
  category: strength
  outcomeKind: skill
  goalPhrase: do a handstand
  successSignals:
    - id: handstand_practice
      kind: behavior
      label: Short handstand practices happen consistently
    - id: wall_handstand_control
      kind: capacity
      label: Wall holds become stable and comfortable
    - id: freestanding_handstand
      kind: milestone
      label: A freestanding handstand is held under control
  evidenceSourceKeys:
  ...
  workflow:
    ...
    ownerSkillIds:
      - strength-training
      - mobility-posture
  startPrompt: Hey Murph, help me do a handstand.
  indexable: true
safety:
...
Practice on a clear, nonslip surface away from furniture and other people. Avoid unsupported inversions when dizziness, uncontrolled blood pressure, relevant eye conditions, or a recent wrist, elbow, shoulder, head, or neck injury makes them unsafe. Coaching can shorten the learning curve and improve spotting.
