...
key: goal_template:run-faster-10k
slug: run-faster-10k
title: Run a Faster 10K
summary: Improve 10K pace by raising sustainable speed while keeping enough easy volume to finish strongly.
...
goal:
  ...
  goalPhrase: run a faster 10K
  successSignals:
    - id: ten_kilometer_time
      kind: milestone
      label: A faster evenly paced 10K
    - id: sustained_strong_pace
      kind: capacity
      label: More time at a controlled strong pace
    - id: long_run_durability
      kind: capacity
      label: Less fade late in longer easy runs
  evidenceSourceKeys:
  ...
  workflow:
    ...
    ownerSkillIds:
      ...
      - running-cardio
  startPrompt: Hey Murph, help me run a faster 10K.
  indexable: true
safety:
