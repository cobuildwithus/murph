...
key: goal_template:run-marathon
slug: run-marathon
title: Run a Marathon
summary: Prepare for 26.2 miles with months of consistent running, progressive long runs, practiced fueling, and a conservative race plan.
...
goal:
  category: cardio
  outcomeKind: event
  goalPhrase: run a marathon
  successSignals:
    - id: durable_training_weeks
      kind: behavior
      label: Durable, consistent marathon-training weeks
    - id: long_run_and_fueling
      kind: capacity
      label: Long runs completed with a workable fueling plan
    - id: marathon_finish
      kind: milestone
      label: A controlled 26.2-mile finish
  evidenceSourceKeys:
  ...
  workflow:
    ...
    ownerSkillIds:
      ...
      - running-cardio
  startPrompt: Hey Murph, help me run a marathon.
  indexable: true
safety:
