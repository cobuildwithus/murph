...
key: goal_template:run-first-10k
slug: run-first-10k
title: Run My First 10K
summary: Extend a comfortable running base until 10 kilometers is a manageable endurance event rather than a leap in distance.
status: field-testing
quality: usable
aliases:
  - run a first 10K
  - train for my first 10K
categories:
  - goals
  - cardio
  - running
...
goal:
  category: cardio
  outcomeKind: event
  goalPhrase: run my first 10K
  successSignals:
    - id: weekly_running_consistency
      kind: behavior
      label: Three or four consistent running days each week
    - id: long_easy_run
      kind: capacity
      label: A comfortable long run approaching event duration
    - id: ten_kilometer_finish
      kind: milestone
      label: Ten kilometers completed with controlled pacing
  evidenceSourceKeys:
  ...
  workflow:
    ...
    ownerSkillIds:
      ...
      - running-cardio
  startPrompt: Hey Murph, help me run my first 10K.
  indexable: true
safety:
