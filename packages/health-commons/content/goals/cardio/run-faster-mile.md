...
key: goal_template:run-faster-mile
slug: run-faster-mile
title: Run a Faster Mile
summary: Improve mile speed by combining easy running, controlled fast repetitions, strength, and enough recovery to absorb the work.
status: field-testing
quality: usable
aliases:
  - improve my mile time
  - get faster at the mile
categories:
  - goals
  - cardio
  - running
...
goal:
  ...
  goalPhrase: run a faster mile
  successSignals:
    - id: mile_time
      kind: milestone
      label: A faster well-paced mile
    - id: repeat_speed_quality
      kind: capacity
      label: Faster repetitions with controlled recovery
    - id: stable_weekly_running
      kind: behavior
      label: Consistent easy running around one quality session
  evidenceSourceKeys:
  ...
  workflow:
    ...
    ownerSkillIds:
      ...
      - running-cardio
      - strength-training
  startPrompt: Hey Murph, help me run a faster mile.
  indexable: true
safety:
...
Fast running raises impact and cardiovascular demand. Stop for chest pain, fainting, or severe unusual breathlessness, and pause speed work for pain that changes your stride or worsens during the session.
