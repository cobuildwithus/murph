...
- `vault-cli experiment progress-card <id> [--confounder "<YYYY-MM-DD:label>"] --format json` to build a shareable progress image for an active run; attach the returned `url` with the response-media tool when showing how an experiment is going. Log durable confounders with `experiment context log`; pass the ones worth surfacing on the card via `--confounder`.
