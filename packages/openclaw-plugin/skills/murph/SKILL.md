...
10. Resolve first-session prep separately from first-week support. First-session prep teaches the user how to do the protocol the first time, either in the current reply or through one one-shot prep reminder when the first intervention session time and a deliverable route are known.
11. Resolve bounded first-week habit support as a required onboarding decision: schedule it, record that the user declined it, or record the concrete route/cadence blocker. Default the support window to the first 7 calendar days of the experiment, or the first 3-5 planned intervention sessions for non-daily protocols. Use bounded one-shot `automation save ... --schedule-kind at` reminders by default, not indefinite recurring reminders.
12. Save first-week support traceability as setup answers when possible: `first_week_support_status` (`scheduled`, `declined`, or `blocked`), `first_week_support_cadence`, `first_week_support_window`, `first_week_support_automation_slugs`, and `first_week_support_blocked_reason` when blocked. If `experiment start` cannot write them, apply them immediately after run creation with repeated key/value flags, for example `vault-cli experiment edit <id> --setup-answer first_week_support_status=scheduled --setup-answer first_week_support_cadence=daily --setup-answer first_week_support_window=<YYYY-MM-DD>..<YYYY-MM-DD> --setup-answer first_week_support_automation_slugs=<comma-separated-slugs>`.
13. Scheduled first-session prep and first-week support reminders should read `vault-cli experiment show <id> --format json`, `vault-cli commons protocol show <key-or-route> --format json`, and `vault-cli experiment progress <id> --as-of <date> --format json` before sending. Skip if the experiment is inactive, reminders were declined or cancelled, the scheduled session or log is already complete, the saved plan changed, or the bounded support window has ended.
...
16. Use neutral language for reminders and missed-log checks. A missed-log check should ask whether the session happened, not imply failure. First-week support should stay short and non-pressuring: what to do or log today, the safety stop rule when relevant, and what to record.
