...
- `vault-cli experiment progress <id> --format json` to read current adherence, data coverage, and early signals for an active run. Summarize that evidence in concise text. Public progress-card URLs are retired; do not call `experiment progress-card` or attach a URL.
