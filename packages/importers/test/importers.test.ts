...
test("importMeal remains a thin compatibility alias for addMeal", async () => {
  const photoPath = await createTempFile("lunch.jpg", "image-placeholder");
  const { calls, corePort } = createCorePortSpy();
  ...
  assert.equal(calls.meals.length, 1);
  assert.equal(mealPayload.photoPath, photoPath);
  assert.equal(mealPayload.note, "rice bowl");
  assert.equal(mealPayload.occurredAt, "2026-03-11T12:00:00.000Z");
});

test("createImporters exposes canonical addMeal and compatibility importMeal", async () => {
  const { calls, corePort } = createCorePortSpy();
  const importers = createImporters({ corePort });

  assert.equal(typeof importers.addMeal, "function");
  assert.equal(typeof importers.importMeal, "function");

  await importers.importMeal({
    note: "  soup and toast  ",
    source: "derived",
  });
  ...
  assert.ok(mealPayload);
  assert.equal(calls.meals.length, 1);
  assert.equal(mealPayload.note, "soup and toast");
