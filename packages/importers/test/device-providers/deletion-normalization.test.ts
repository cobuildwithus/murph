...
function expectSyntheticDeletionArtifact(
  ...
  expect(normalized.events?.[0]?.rawArtifactRoles).toEqual([evidencePart?.role]);
...
describe("pushDeletionObservation", () => {
  it("persists a metadata-only deletion artifact", () => {
    ...
    expect(events[0]?.rawArtifactRoles).toEqual([evidenceParts[0]?.role]);
  });
  ...
  it("collapses exact duplicate delete observations that share identical metadata", () => {
    ...
    expect(events.map((event) => event.rawArtifactRoles)).toEqual([[evidenceParts[0]?.role]]);
  });
});
...
describe("normalizeOuraSnapshot", () => {
  ...
  it("does not keep full deleted sleep payload artifacts", () => {
    ...
    expect(normalized.events?.[0]?.rawArtifactRoles).toEqual([normalized.evidenceParts?.[0]?.role]);
  });
