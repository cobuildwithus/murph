...
Junction keeps its existing daily timeseries summaries as the compatibility surface. For `glucose`, `blood_oxygen`, and `stress_level`, the importer also derives one deterministic 24-hour feature envelope and one measurement of scalar facts per source/day; coverage and episode durations are explicitly estimated from capped gaps between discrete readings. Dense reconcile and direct-resource jobs both use the closed-calendar-day importer, so there is no precise-window path that can publish a partial day under a complete-day identity. Every scheduled reconcile enters that bounded importer, whose UTC-12 closure filter decides which dates are publishable without consulting an account-global completion clock. For sparse `caffeine`, `water`, and `mindfulness_minutes`, every admitted interval becomes one exact-start measurement with start/end qualifiers and compact per-record evidence. Precise direct-resource snapshots emit only those per-record intervals; the calendar-day path is the sole writer of the retained daily sums, so an early partial set cannot freeze or block later records. Date-only transport preserves the provider-local calendar response instead of trimming it to UTC, and calendar-day imports wait until the date has closed even at UTC-12 before publishing, so a non-UTC source day stays complete. Daily sums and dense feature envelopes are unversioned complete-set facts: child-row revisions do not order the collection, serialized calendar imports reconcile set growth or removal through the canonical event spine, and exact replays collapse. Stable sparse intervals retain their explicit provider revisions; one versioned interval may migrate an earlier unversioned baseline, after which stale, changed unversioned, or equal-revision conflicting interval bodies fail closed. ID-less records keep semantic tuple identity. Both paths are bounded before persistence and never retain raw dense sample arrays.
