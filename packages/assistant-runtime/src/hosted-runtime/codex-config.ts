...
export function buildHostedCodexConfigToml(input: {
  ...
  return [
    ...
    "# Multi-agent V2 spawn tools back Murph's standing prompt/skill-directed",
    "# delegation for slow, non-reply-critical sidecar work such as lab PDF",
    "# imports and supplement-label lookup. Codex >=0.143 accepts applicable",
    "# skill/AGENTS instructions as explicit delegation authorization, so the",
    "# prompt and skills own the policy; do not add a custom mode hint here.",
    "[features]",
    "plugins = false",
    "multi_agent_v2 = true",
