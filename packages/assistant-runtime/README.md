...
- seed one finite hosted signup onboarding follow-up after successful signup welcome delivery; the exact current seed, its immediate predecessor, or the bounded original legacy seed is reconciled without granting execution authority to editable metadata; the ordinary one-shot notification is spread deterministically from 1:30 PM through 2:29 PM on the next local day, reserves at least 30 minutes for execution before delivery authority closes at 3:00 PM on that local day, checks onboarding resume context without mutating onboarding, and is consumed after either one reply-oriented continuation or a skip
