...
- expose invocation-scoped automation and device authority only through narrow typed tools on an attended active root turn, never through Codex App Server or descendant shell env; the runtime supplies existing domain ports directly, canonical automation records remain owned by the already-bound vault, route writes use the trusted current destination rather than model-supplied locators, and scheduled turns and descendants receive neither authority
- leave the attended Codex App Server warm while running each scheduled notification or maintenance occurrence in a separate exact-owned one-shot process with a sterile temporary cwd, no native execution environment or selected capability roots, and a launch-time catalog that disables model-selected code mode and multi-agent handlers; bounded canonical and bundled-skill reads plus required writes and exact external fetches remain task-bound parent-owned typed operations with effect-time source revalidation and no shell or operator-CLI fallback
