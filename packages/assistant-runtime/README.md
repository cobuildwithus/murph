...
- preserve actor-scoped and native-reply-scoped hosted Linq group turns while serializing room output through existing auto-reply provenance, outbox, and turn-receipt truth: stop and freshly continue the shared-history scan after the first group reply intent, defer pending room input behind an active proven auto-reply intent, terminally suppress only input received before a sent auto-reply turn began, exclude generic notification carriers, and keep mid-turn input or failed/abandoned intents replyable
