...
- re-read Web's effective core-provider choice immediately before every
  ...
  consuming the input; custom inference instead uses the exact revision already
  pinned to the invocation write fence, while its effective Codex model/provider
  stays separate from the dormant OpenAI/Venice operator preference
