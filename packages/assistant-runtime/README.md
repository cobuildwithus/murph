...
- keep automation timing verification explicit and operation-local: time-based responses include typed, content-free verification issues; an unverified write gets at most one model-requested read-only inspection before the reply; failure and recovery diagnostics reuse the existing assistant automation-detail log path without retries, new persistence, or a second scheduler owner
