...
- apply every `member.preferences.updated` system-mailbox delta in mailbox order, with an older retry blocking newer preference deltas instead of latest-wins supersession; the canonical preference owner reserves bounded per-setting revisions so an older retry stale-no-ops only fields superseded by a newer conversational write while current siblings still apply, and retains a bounded handled receipt so replay remains idempotent until the mailbox item is removed
