...
- record each `phone-call.resulted` system-mailbox event once as internal context on the exact existing direct session that initiated the call, without session creation, rerouting, a provider turn, or delivery; when result and conversation rows import together, independently admit only phone results at or before the accepted conversation causal frontier without replacing preference intent time, terminally settle a missing or non-direct origin as a fail-closed no-op while retaining transient retries, and clear native resume metadata so the next attended turn rebuilds from the committed transcript that includes the call outcome
