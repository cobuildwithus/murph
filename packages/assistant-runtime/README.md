...
- seed one finite hosted signup onboarding follow-up after successful signup welcome delivery; the managed ownership fingerprint, not its public slug alone, selects the scoped onboarding decision policy; the one-shot is spread deterministically from 1:30 PM through 2:29 PM on the next local day, reserves at least 30 minutes for execution before delivery authority closes at 3:00 PM on that local day, checks onboarding resume context, applies any completion only after final cron commit authority, and is consumed after either one reply-oriented continuation or a skip
