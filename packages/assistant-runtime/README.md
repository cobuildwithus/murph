...
- admit at most one mailbox-backed input per hosted provider turn and expose its exact causal sequence to hosted personality commands through the authenticated invocation-local CLI bridge, leaving later inputs pending rather than steering across causal anchors
