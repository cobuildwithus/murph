...
- record each `phone-call.resulted` system-mailbox event once as internal context on its exact bound direct session, without a provider turn or delivery, and clear native resume metadata so the next attended turn rebuilds from the committed transcript that includes the call outcome
