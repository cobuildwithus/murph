...
describe("runHostedWorkspaceAssistantPhase runtime logs", () => {
  ...
  it("reports a degraded onboarding goal check-in stage without failing the pass", async () => {
    const logRequests: HostedRuntimeLogRequest[] = [];
    mocks.applyMurphManagedAutomations.mockResolvedValueOnce({
      created: 2,
      onboardingGoalCheckinFailure: Object.assign(
        new Error("Onboarding state is malformed."),
        { reason: "invalid-json" },
      ),
      skipped: 0,
      updated: 0,
    ...
    expect(logRequests.flatMap((request) => request.entries)).toContainEqual(
      expect.objectContaining({
        ...
        redactedJson: expect.objectContaining({
          ...
          murphManagedAutomationStage: "onboarding_goal_checkin",
        }),
      }),
    );
  });

  it("keeps the bounded retry ladder for a transient onboarding goal check-in failure", async () => {
    const transient = Object.assign(new Error("onboarding state busy"), {
      code: "EBUSY",
    });
    mocks.applyMurphManagedAutomations.mockResolvedValueOnce({
      created: 2,
      onboardingGoalCheckinFailure: transient,
      skipped: 0,
      updated: 0,
    });
