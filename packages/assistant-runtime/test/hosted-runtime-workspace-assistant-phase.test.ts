...
describe("runHostedWorkspaceAssistantPhase runtime logs", () => {
  ...
  it("recovers a transient post-contact Telegram seed failure without replaying member input", async () => {
    const defaultRoute = {
      channel: "telegram",
      deliverySource: null,
      deliveryTarget: "telegram_seed_recovery_target",
      identityId: null,
      participantId: null,
      threadIsDirect: true,
      threadId: "telegram_seed_recovery_thread",
    };
    mocks.readAssistantInputEvent.mockResolvedValueOnce({
      conversation: {
        accountId: "telegram_seed_recovery_identity",
        actorId: null,
        actorIsSelf: false,
        source: defaultRoute.channel,
        threadId: defaultRoute.threadId,
      ...
      replyTarget: {
        channel: defaultRoute.channel,
        messageId: "telegram_seed_recovery_message",
        threadId: defaultRoute.deliveryTarget,
      },
    });
    const transientWriteFailure = new VaultCliError(
      "MURPH_MANAGED_AUTOMATION_SETUP_TRANSIENT",
      "canonical automation write temporarily unavailable",
      { retryable: true },
    );
    mocks.applyMurphManagedAutomations
      .mockRejectedValueOnce(transientWriteFailure)
      .mockResolvedValueOnce({
        created: 1,
    ...
    const inboundResult = await runHostedWorkspaceAssistantPhase(createPhaseInput({
      importedCount: 1,
      now: () => "2026-04-27T00:00:00.000Z",
    }));
    expect(inboundResult.afterCheckpoint).toEqual(expect.any(Function));
    const failedSeedCheckpoint = await inboundResult.afterCheckpoint?.();
    expect(failedSeedCheckpoint).toEqual(expect.objectContaining({
      nextWakeAt: "2026-04-27T00:00:30.000Z",
      redactedStatus: expect.objectContaining({
        murphManagedAutomationSetupRetryAttempt: 1,
        murphManagedAutomationSetupRetryable: true,
    ...
    const retryLogs: HostedRuntimeLogRequest[] = [];
    const retryResult = await runHostedWorkspaceAssistantPhase(createPhaseInput({
      assistantInputIds: [],
      importedCount: 0,
      logRequests: retryLogs,
    ...
    expect(mocks.applyMurphManagedAutomations).toHaveBeenCalledTimes(2);
    expect(mocks.applyMurphManagedAutomations).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ defaultRoute }),
    );
    expect(mocks.applyMurphManagedAutomations).toHaveBeenNthCalledWith(
      2,
      expect.not.objectContaining({ defaultRoute: expect.anything() }),
    );
    ...
    expect(retryLogs.flatMap((request) => request.entries)).toContainEqual(
      expect.objectContaining({
        component: "runtime",
        eventCode: "assistant.onboarding_followup_reconciled",
        level: "info",
