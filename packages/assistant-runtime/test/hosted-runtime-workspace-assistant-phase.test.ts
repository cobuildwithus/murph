  ...
  it("executes modern Dexcom recovery only while the live status exposes that target", async () => {
    ...
    const deviceSyncPort = {
      ...
      async createConnectLink(request) {
        connectLinkRequests.push(request);
        return {
          authorizationUrl: "https://connect.example.test/dexcom",
          connectUrl: "https://connect.example.test/dexcom",
          expiresAt: "2026-04-29T00:05:00.000Z",
          provider: request.connectTarget,
      ...
      async fetchSnapshot(request) {
        if (request?.sourceProviderSlug !== "dexcom_v3") {
          return {
            connections: [],
            generatedAt: "2026-04-29T00:00:00.000Z",
            userId: "member_synthetic_phase",
        ...
        return {
          connections: [{
            connection: {
              ...
              createdAt: "2026-04-28T00:00:00.000Z",
              displayName: null,
              externalAccountId: "synthetic-dexcom-account",
              id: "conn_synthetic_dexcom",
              metadata: {},
              provider: "junction",
              scopes: [],
    ...
    expect(prompt).toContain("vault-cli device connect dexcom_v3 --format json");
    await expect(deviceTool.request({
      action: "connect",
      provider: "dexcom_v3",
    })).resolves.toEqual({
