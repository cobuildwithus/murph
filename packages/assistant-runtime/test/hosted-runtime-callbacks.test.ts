...
describe("hosted runtime callbacks", () => {
  ...
  it("marks post-success Linq text liveness aborts as possibly committed", async () => {
    const effect = createEffect({
      bindingDeliveryTarget: "linq_chat_123",
      channel: "linq",
      explicitTarget: "linq_chat_123",
    ...
    const assertLiveness = vi.fn()
      .mockResolvedValueOnce(undefined)
      .mockResolvedValueOnce(undefined)
      .mockRejectedValueOnce(new Error("aborted after Linq text response"));
    const providerFetch = vi.fn<typeof fetch>(async () =>
      new Response(null, { status: 204 }));
    mocks.sendLinqMessage.mockImplementationOnce(async (_request, dependencies) => {
      await dependencies.fetchImplementation("https://api.linq.example/messages", {
        headers: {},
        method: "POST",
      });
      return {
    ...
    mocks.dispatchAssistantOutboxIntent.mockImplementationOnce(async ({ dependencies }) => {
        ...
        return createDispatchResult(
          {
            ...
            lastError: {
              code: "ASSISTANT_DELIVERY_AMBIGUOUS",
              message: "Ambiguous Linq text delivery.",
            },
            status: "abandoned",
          },
          {
      ...
      throw new Error("expected post-success Linq text liveness failure");
    });
    ...
    expect(capturedError).toMatchObject({
      deliveryMayHaveSucceeded: true,
      message: "aborted after Linq text response",
    });
    expect(outcomes).toEqual([
      expect.objectContaining({
        deliveryStatus: "failed_ambiguous",
    ...
    expect(providerFetch.mock.calls[0]).toMatchObject([
      "https://api.linq.example/messages",
      expect.objectContaining({ method: "POST" }),
    ]);
    const [typingUrl, typingInit] = providerFetch.mock.calls[1] ?? [];
    expect(String(typingUrl)).toContain("/chats/linq_chat_123/typing");
    expect(typingInit).toMatchObject({ method: "DELETE" });
  });

  it("marks post-success Linq voice memo liveness aborts as possibly committed", async () => {
    const effect = createEffect({
      bindingDeliveryKind: "thread",
      bindingDeliveryTarget: "linq_chat_123",
      channel: "linq",
    ...
    const assertLiveness = vi.fn()
      .mockResolvedValueOnce(undefined)
      .mockResolvedValueOnce(undefined)
      .mockRejectedValueOnce(new Error("aborted after Linq voice memo response"));
    const providerFetch = vi.fn<typeof fetch>(async () =>
      new Response(null, { status: 204 }));
    mocks.sendLinqVoiceMemoMessage.mockImplementationOnce(async (_request, dependencies) => {
      await dependencies.fetchImplementation("https://api.linq.example/voice", {
        headers: {},
        method: "POST",
      });
      return {
    ...
    mocks.dispatchAssistantOutboxIntent.mockImplementationOnce(async ({ dependencies }) => {
        ...
        return createDispatchResult(
          {
            ...
            lastError: {
              code: "ASSISTANT_DELIVERY_AMBIGUOUS",
              message: "Ambiguous Linq voice memo delivery.",
            },
            status: "abandoned",
          },
          {
      ...
      throw new Error("expected post-success Linq voice memo liveness failure");
    });
    ...
    expect(capturedError).toMatchObject({
      deliveryMayHaveSucceeded: true,
      message: "aborted after Linq voice memo response",
    });
    expect(outcomes).toEqual([
      expect.objectContaining({
        deliveryStatus: "failed_ambiguous",
