...
describe("runHostedWorkspaceUntilIdleOrBudget", () => {
  ...
  test("runs non-gating assistant enrichment warmup before the assistant phase", async () => {
    const vaultRoot = await mkdtemp(path.join(tmpdir(), "murph-workspace-runner-"));
    const events: string[] = [];
    const items = [
      createMailboxItem({
        id: "mailbox_item_runner_warmup",
        laneSeq: "1",
      }),
    ];
    const { mailboxPort } = createMailboxPort({ items });
    ...
    try {
      await runHostedWorkspaceUntilIdleOrBudget({
        checkpointRequestBuilder: createHostedWorkspaceCheckpointRequestBuilder({
          attemptId: "attempt_synthetic_runner_warmup",
          expectedWorkspaceVersion: "0",
          leaseGeneration: "1",
          nextWakeAt: null,
          nextWakeReason: null,
        ...
        prepareAssistantEnrichment(input) {
          events.push("warmup");
          assert.equal(input.initialMailboxImport.checkpoint?.checkpointed, true);
        ...
        requestId: "request_synthetic_runner_warmup",
        runtimeLogContext: {
          attemptId: "attempt_synthetic_runner_warmup",
          leaseGeneration: "1",
          workspaceVersion: "0",
      ...
      const warmupLog = logRequests.flatMap((request) => request.entries)
        .find((entry) => entry.eventCode === "runtime.inbox_enrichment_warmup_finished");
      assert.equal(warmupLog?.level, "info");
      assert.deepEqual(warmupLog?.redactedJson, {
        elapsedMs: 12,
        ok: true,
        rebuiltCaptures: 2,
        timedOut: false,
  ...
  test("continues to assistant phase when enrichment warmup fails", async () => {
    const vaultRoot = await mkdtemp(path.join(tmpdir(), "murph-workspace-runner-"));
    const events: string[] = [];
    const { mailboxPort } = createMailboxPort({
      items: [
        createMailboxItem({
          id: "mailbox_item_runner_warmup_failed",
          laneSeq: "1",
        }),
      ],
    });
    ...
    try {
      await runHostedWorkspaceUntilIdleOrBudget({
        checkpointRequestBuilder: createHostedWorkspaceCheckpointRequestBuilder({
          attemptId: "attempt_synthetic_runner_warmup_failed",
          expectedWorkspaceVersion: "0",
          leaseGeneration: "1",
          nextWakeAt: null,
          nextWakeReason: null,
        ...
        async prepareAssistantEnrichment() {
          events.push("warmup");
          throw Object.assign(new Error("inbox warmup failed"), {
            code: "INBOX_CONFIG_INVALID",
          });
        },
        requestId: "request_synthetic_runner_warmup_failed",
        runtimeLogContext: {
          attemptId: "attempt_synthetic_runner_warmup_failed",
          leaseGeneration: "1",
          workspaceVersion: "0",
      ...
      assert.deepEqual(events, ["import", "warmup", "assistant"]);
      const warmupLog = logRequests.flatMap((request) => request.entries)
        .find((entry) => entry.eventCode === "runtime.inbox_enrichment_warmup_finished");
      assert.equal(warmupLog?.level, "warn");
      assert.equal(warmupLog?.errorCode, "INBOX_CONFIG_INVALID");
      assert.deepEqual(warmupLog?.redactedJson, {
        elapsedMs: 0,
        ok: false,
