...
describe("hosted workspace runtime entrypoint", () => {
  ...
  test("collapse invariant 1: fresh conversation wake runs before idle shutdown checkpointing", async () => {
  ...
  test("collapse invariant 3a: due assistant wake committed by checkpoint is serviced in the same attempt", async () => {
    const vaultRoot = await mkdtemp(path.join(tmpdir(), "murph-collapse-invariant-"));
    const events: string[] = [];
    const checkpointRequests: HostedWorkspaceCheckpointRequest[] = [];
  ...
  test("collapse invariant 7: redacted status counters survive a checkpointed follow-up pass", async () => {
    const vaultRoot = await mkdtemp(path.join(tmpdir(), "murph-collapse-invariant-"));
    const events: string[] = [];
    const checkpointRequests: HostedWorkspaceCheckpointRequest[] = [];
    const idleCheckpointDelayMs = 1;
  ...
  test("retained post-checkpoint consumed conversation replay checkpoints dirty import before returning", async () => {
    const vaultRoot = await mkdtemp(path.join(tmpdir(), "murph-runtime-idle-checkpoint-"));
    const events: string[] = [];
    const checkpointRequests: HostedWorkspaceCheckpointRequest[] = [];
    const mailboxItems: HostedMailboxItem[] = [];
  ...
  test("shutdown does not abort an accepted foreground runtime wake import", async () => {
    const vaultRoot = await mkdtemp(path.join(tmpdir(), "murph-workspace-entrypoint-"));
    const events: string[] = [];
    const checkpointRequests: HostedWorkspaceCheckpointRequest[] = [];
    const shutdownController = new AbortController();
...
describe("hosted runtime shutdown signal", () => {
  ...
  test("a pre-shutdown no-work runtime wake is not serviced after shutdown checkpointing", async () => {
    const vaultRoot = await mkdtemp(path.join(tmpdir(), "murph-workspace-entrypoint-"));
    const previousStdIoLogSetting = process.env.MURPH_HOSTED_EXECUTION_STDIO_LOGS;
    const consoleInfo = vi.spyOn(console, "info").mockImplementation(() => undefined);
    const checkpointRequests: HostedWorkspaceCheckpointRequest[] = [];
    ...
    const shutdownDirtyWaitStarted = createDeferred<void>();
    const shutdownController = new AbortController();
    let activeDirtyWake: ((notification: { notifiedAtEpochMs: number }) => void) | null = null;
    let assistantPhaseFinished = false;
    let assistantPhaseCalls = 0;
        ...
        return new Promise((resolve, reject) => {
          ...
          if (assistantPhaseFinished) {
            ...
            if (dirtyWaitCount === 1) {
              ...
              shutdownDirtyWaitStarted.resolve();
            }
          }
          signal?.addEventListener("abort", abort, { once: true });
        });
    ...
    try {
      ...
      await withRealTimeout(
        shutdownDirtyWaitStarted.promise,
        1_000,
        () => "Dirty checkpoint wait did not re-arm after the no-work wake.",
      );
      mailboxItems.push(createMailboxItem({
        id: "mailbox_item_shutdown_stale_runtime_wake_after_checkpoint",
        laneSeq: "1",
      }));
      shutdownController.abort(
        new DOMException("Synthetic container SIGTERM.", "AbortError"),
  ...
  test("shutdown after an idle-window checkpoint trigger does not service a retained runtime wake", async () => {
    const vaultRoot = await mkdtemp(path.join(tmpdir(), "murph-workspace-entrypoint-"));
    const checkpointRequests: HostedWorkspaceCheckpointRequest[] = [];
    const events: string[] = [];
    const mailboxItems: ReturnType<typeof createMailboxItem>[] = [];
      ...
      const resultPromise = runHostedWorkspaceRuntimeJobInProcess(
        ...
        }),
        {
          async createCheckpointSnapshot(snapshotInput) {
            ...
            if (snapshotInput.idleCheckpointTrigger === "idle_window") {
              mailboxItems.push(createMailboxItem({
  ...
  test("shutdown after a consumed post-checkpoint replay import checkpoints metadata and hands off the wake", async () => {
    ...
    try {
      ...
      assert.deepEqual(
        events.filter((event) => event.startsWith("mailbox.importItem:")),
        ["mailbox.importItem:mailbox_item_shutdown_after_consumed_replay"],
      );
      assert.equal(
  ...
  test("shutdown after checkpointing returns a due assistant wake instead of servicing it", async () => {
    const vaultRoot = await mkdtemp(path.join(tmpdir(), "murph-workspace-entrypoint-"));
    const checkpointRequests: HostedWorkspaceCheckpointRequest[] = [];
    const events: string[] = [];
    const shutdownController = new AbortController();
