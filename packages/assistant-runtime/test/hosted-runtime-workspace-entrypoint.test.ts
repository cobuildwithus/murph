...
describe("hosted workspace runtime entrypoint", () => {
  ...
  test("e2e handles due legacy wearable maintenance wake as nudge without assistant work", async () => {
    const vaultRoot = await mkdtemp(path.join(tmpdir(), "murph-workspace-entrypoint-"));
    const events: string[] = [];
    const checkpointRequests: HostedWorkspaceCheckpointRequest[] = [];
    const dueWakeAt = "2026-04-26T23:59:59.000Z";
    ...
    try {
      ...
      const result = await runHostedWorkspaceRuntimeJobInProcess(
        createWorkspaceRuntimeJobInput({
          request: {
            attemptId: "attempt_synthetic_legacy_wearable_due_nudge",
            idleCheckpointDelayMs: 1,
            reason: "nudge",
            workspaceVersion: "0",
          },
        }),
        {
          async createCheckpointSnapshot(snapshotInput) {
            ...
            return {
              snapshotRef: createBundleRef({
                hash: "e".repeat(64),
                key: "users/bundles/member-synthetic/legacy-wearable-due.bundle.json",
                size: 512,
              }),
            };
          },
          async importItem() {
            throw new Error("No mailbox items should be imported for maintenance.");
          },
          platform: createPlatform({
            mailboxPort: createMailboxPort({ events, items: [] }),
            workspacePort: createWorkspacePort({
          ...
          async runAssistantPhase() {
            throw new Error("Assistant phase should not run for handled maintenance.");
          },
          vaultRoot,
        },
      );
      ...
      assert.equal(result.status, "idle");
      assert.equal(result.nextWakeAt, null);
      assert.equal(result.redactedStatus?.legacyWearableReceiptCompactionCompactedCount, 0);
    } finally {
      await removeTempRoot(vaultRoot);
    }
  });
