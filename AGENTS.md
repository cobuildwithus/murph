...
- Use `agent-docs/operations/completion-workflow.md` for mandatory completion audits. One preliminary `completion-specialists` ReviewGPT pass applies the relevant Product UX, prompt, frontend, and coverage lenses together. Every substantive specialist result and every final `FINDINGS` result pauses before remediation so the parent can report evidence-backed accept/reject dispositions, except for that section's narrowly proven behavior-preserving `Complexity Collapse` and non-production-only docs/process/PR-evidence or isolated test/fixture remediation paths. Those exceptions require the same user-visible disposition update but not a turn-ending user-resume pause; `agent-docs/operations/pr-reviewgpt-loop.md` § Finding Disposition Pause owns their exact boundaries. A validated final `ROUND_OUTCOME: PASS` also proceeds without a user-resume pause. Agents may reject speculative, unproven, or disproportionate fixes, and only accepted findings require remediation. The parent must inspect and verify any returned coverage patch after the required disposition boundary and before applying it.
