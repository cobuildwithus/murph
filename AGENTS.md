...
- Prompt-primary PRs do not run ReviewGPT; use the required local `prompt-review` pass instead unless non-prompt scope independently requires the loop. Local `deep-review` and PR-lane ReviewGPT are mutually exclusive for the same completed change: when ReviewGPT is selected, it is the sole cross-cutting gate, even for complex or sensitive work or an explicit final bug hunt. When ReviewGPT is required for a PR-lane head, start it as soon as the exact pushed head is ready and no PR-specific change is already known to be required. Run it concurrently with CI; never wait for CI to finish before starting ReviewGPT. ReviewGPT certifies the PR-specific patch, not the age of its base: round 1 is the full-patch audit and later rounds verify only remediation deltas and directly affected paths. Do not rerun merely because `main`/the base advanced, an isolated regression test or explanatory doc changed, or a clean merge/rebase updated only base history; rerun CI instead. Start a new substantive round after a PR-specific production-code, runtime-config, schema, behavior, or behavior-changing conflict-resolution change. Follow `agent-docs/operations/pr-reviewgpt-loop.md` for immutable first-head baselines, anomaly retrospectives, and retry counting.
...
- Local Codex completion verification must use the canonical `pnpm test:diff ...` and `pnpm verify:acceptance` commands so the repo dispatcher can use configured Crabbox-backed Blacksmith Testboxes. Never forward `.env`, Vercel development state, or local environment values into that lane; use the documented local-only escape hatch for an environment-bound direct scenario.
