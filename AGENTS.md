...
- When opening or updating a PR, follow `agent-docs/operations/completion-workflow.md` § PR Description. Keep the body short: outcome, Product UX result, direct evidence, changelog, and only the risk details that apply.
- Reuse an existing component from `/design?tab=components` before adding a near-duplicate, and add new shared components there. Use `/screenshots` only for difficult or reusable presentation states. Screenshot studies render the real production component with synthetic props only: no live data, no real requests, and interactive controls held `inert`. A user-facing UI diff does not need a catalog change by default, but its PR must embed at least one redacted, production-faithful screenshot in the Evidence section. Add phone and desktop screenshots when responsive behavior can differ; the submitted evidence must match the changed claim.
...
- Use `agent-docs/operations/completion-workflow.md` for mandatory completion audits. One preliminary `completion-specialists` ReviewGPT pass applies the relevant Product UX, prompt, frontend, and coverage lenses together; the parent must inspect and verify any returned coverage patch before applying it.
- When an implementation handoff intentionally uses `cobuild-review-gpt thread wake` and no explicit per-run `--poll-timeout` is already supplied, pass `--poll-timeout 180m`. Preserve any explicit caller- or user-supplied bound. This polling limit is separate from the repo's ReviewGPT response-capture timeout, which also defaults to 180 minutes.
