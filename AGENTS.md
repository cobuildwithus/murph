...
- Local Codex completion verification must use the canonical `pnpm test:diff ...` and `pnpm verify:acceptance` commands. They stay local by default; use the documented explicit Crabbox-backed Blacksmith fallback only after bounded local admission wait. Never forward `.env`, Vercel development state, or local environment values into that lane; use the documented local-only escape hatch for an environment-bound direct scenario.
