...
- For PR-lane work, start ReviewGPT as soon as the exact pushed head is ready and no PR-specific change is already known to be required. Run it concurrently with CI; never wait for CI to finish before starting ReviewGPT. Rerun after PR-specific changes, not a base-only update after a zero-finding round.
