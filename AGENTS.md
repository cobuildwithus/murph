...
- Never roll back a Cloudflare deployment without the user's explicit permission for that exact rollback. This includes `wrangler rollback`, deploying or restoring an older Worker version, and repointing production aliases, routes, or configuration to an older deployment. A request to diagnose or fix an issue, deploy a new change, or follow a documented technical rollback procedure is not rollback authorization; stop and ask before executing the rollback.
