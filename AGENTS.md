...
- Use `agent-docs/operations/completion-workflow.md` for mandatory completion audits. Exactly one review-only local specialist subagent applies every relevant Product UX, prompt, frontend, and coverage lens together; never run one subagent per lens or use ReviewGPT for this stage. After any substantive specialist result, the parent reports evidence-backed accept/reject dispositions as a progress update and may continue accepted remediation within the existing task authority without a turn-ending user-resume pause. Every final ReviewGPT `FINDINGS` result still pauses all candidate mutation, including pending specialist remediation, except for that section's narrowly proven behavior-preserving `Complexity Collapse` and non-production-only docs/process/PR-evidence or isolated test/fixture remediation paths. Those exceptions require the same user-visible disposition update but not a turn-ending user-resume pause; `agent-docs/operations/pr-reviewgpt-loop.md` § Finding Disposition Boundary owns the exact final-gate boundaries. A validated final `ROUND_OUTCOME: PASS` also proceeds without a user-resume pause. Agents may reject speculative, unproven, or disproportionate fixes, and only accepted findings require remediation.
