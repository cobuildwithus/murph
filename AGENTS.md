...
- Use `agent-docs/operations/completion-workflow.md` for mandatory completion audits. One preliminary `completion-specialists` ReviewGPT pass applies the relevant Product UX, prompt, frontend, and coverage lenses together. Every substantive specialist result and every final `FINDINGS` result pauses before remediation so the parent can report evidence-backed accept/reject dispositions, except that a sole accepted, behavior-preserving `Complexity Collapse` may be reported as a progress update and remediated without separate user permission under `agent-docs/operations/pr-reviewgpt-loop.md` § Finding Disposition Pause. A validated final `ROUND_OUTCOME: PASS` proceeds without a user-resume pause. Agents may reject speculative, unproven, or disproportionate fixes, and only accepted findings require remediation. The parent must inspect and verify any returned coverage patch after the required disposition boundary and before applying it.
