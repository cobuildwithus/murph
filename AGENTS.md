...
- Local Codex completion verification must use the canonical `pnpm test:diff ...` and `pnpm verify:acceptance` commands. They stay local by default; follow the verification guide's explicit Crabbox-backed Blacksmith decision rule for the ordinary bounded-admission fallback and the required post-landing trust-root proof. Never forward `.env`, Vercel development state, or local environment values into that lane; use the documented local-only escape hatch for an environment-bound direct scenario.
