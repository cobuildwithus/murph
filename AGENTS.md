...
- When an implementation handoff intentionally uses `cobuild-review-gpt thread wake`, pass `--poll-timeout 180m` by default unless the user supplies a different bound. This polling limit is separate from the repo's ReviewGPT response-capture timeout, which also defaults to 180 minutes.
