...
- Use `agent-docs/operations/completion-workflow.md` for mandatory completion audits. One preliminary `completion-specialists` ReviewGPT pass applies the relevant Product UX, prompt, frontend, and coverage lenses together. Every substantive ReviewGPT result pauses before remediation so the parent can report evidence-backed accept/reject dispositions; agents may reject speculative, unproven, or disproportionate fixes, and only accepted findings require remediation. The parent must inspect and verify any returned coverage patch after the pause and before applying it.
- Give every long-running ReviewGPT job one completion owner. Prefer the normal tool-owned `--wait` capture; when the review must outlive the active turn, arm detached `cobuild-review-gpt thread wake` so its watcher resumes the owning Codex session only after completion. Do not spend active agent turns repeatedly status-polling ReviewGPT. If completion notification is unavailable and a manual check is necessary, leave at least five minutes between checks. For an intentional implementation handoff, pass `--poll-interval 5m` and, unless the caller already supplied an explicit per-run bound, `--poll-timeout 240m`; preserve explicit caller- or user-supplied bounds. The wake timeout remains separate from ReviewGPT response capture, which defaults to 250 minutes. Follow `agent-docs/operations/pr-reviewgpt-loop.md` § Wait And Wake Ownership.
