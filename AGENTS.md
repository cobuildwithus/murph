...
- Inside this monorepo, app/test code must import workspace packages by package name and resolve them from source; do not add imports, aliases, TS paths, or launcher steps that point internal consumers at another package's `dist/` output. Treat `dist/` as a publish/runtime artifact only.
