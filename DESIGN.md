...
Keep the copy observational and conversational. Prefer `You slept less after cycling` over a statistical sentence. Never use `caused` or `proved`. Show the activity result and other comparable days as two labeled bars in the cell detail. Keep matched-day counts in accessible copy instead of repeating them visually. On narrow screens, use one card per factor with its illustration, compact evidence-coverage bars, and available comparisons in a two-column grid. Use serif values for changed results. Group neutral measures as small tappable names under one `No clear change` label at the bottom of the card. Group measures that still need data in one native disclosure so sparse cards remain concise. Open mobile result details in a bottom drawer with comparison bars, evidence dates, a close control, and safe-area spacing. Reuse the detail content in desktop hover popovers. Desktop keeps the sortable comparison matrix.
