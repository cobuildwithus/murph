...
Keep the copy observational and conversational. Prefer `You slept less after cycling` over a statistical sentence. Never use `caused` or `proved`. Show the activity result and other comparable days as two labeled bars in the cell detail. Keep matched-day counts out of the main result grid; expose them in supporting detail. On narrow screens, use one card per factor with its illustration, compact evidence-coverage bars, and available comparisons in a two-column grid. Use serif values for changed results. Collapse neutral measures behind one `No clear change` row at the bottom of the card; expand to a two-column list of tappable measure names. Neutral-only cards use a compact factor header with that disclosure label beneath the name. Omit measures without enough comparable data from mobile cards. Open mobile result details in a bottom drawer with comparison bars, evidence dates, a close control, and safe-area spacing. Show the number of days behind each average in mobile detail drawers. Coverage bars open their own mobile drawer explaining the factor observation count and its distinction from comparison sample sizes. Reuse the detail content in desktop hover popovers. Desktop keeps the sortable comparison matrix.
