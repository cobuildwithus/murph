...
Use one flat paper surface for repeated action-to-outcome comparisons. Put Quiver-style factor illustrations on rows and next-day outcomes on columns. Circle size and fill show evidence strength. The arrow shows direction, and the percentage shows the size of the difference. Color must not label higher values as good or lower values as bad.
