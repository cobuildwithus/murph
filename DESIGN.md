...
Post-signup step for adding Murph as a contact (`apps/web/src/components/murph/murph-contact-card-picker.tsx`, spec in `agent-docs/product-specs/murph-contact-card-picker.md`). Drawer under 768px, dialog above. Layout: contact preview (avatar circle + Fraunces "Murph" + mono kicker), then a 3-column radiogroup of avatar circles with DM Mono labels (selected = 2px sage ring with offset), then a full-width primary "Add Murph to Contacts" CTA over a ghost "Skip for now". Headshot avatars come from `public/murph-headshots/`; the logo option is the dot-grid mark on a slate circle; the no-photo option is a sand circle with a serif M initial (Expert Card avatar treatment).
